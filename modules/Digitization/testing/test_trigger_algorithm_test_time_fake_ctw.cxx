...
int main(int argc_, char **argv_) {
  ...
  if (is_help) {
    std::cerr << std::endl
              ...
              << "$ "
                 "BuildProducts/bin/"
                 "falaisedigitizationplugin-test_trigger_algorithm_test_time_fake_ctw [OPTIONS] "
                 "[ARGUMENTS]"
              << std::endl
              << std::endl
  ...
  try {
    ...
    std::clog << "Test program for class "
                 "'snemo::digitization::test_trigger_algorithm_test_time_fake_ctw' !"
              << std::endl;
    int32_t seed = 314159;
    ...
    my_e_mapping.add_preconstructed_type(
        snemo::digitization::mapping::CALO_MAIN_WALL_CATEGORY_TYPE);
    ...
    // Add fake CTW calo and geiger :
    {
      ...
      my_calo_ctw.set_header(155, my_ctw_gid,
    ...
    for (unsigned int ict = 8; ict < 19; ict++) {
      {
        ...
        geomtools::geom_id my_ctw_gid(snemo::digitization::mapping::TRACKER_CONTROL_BOARD_TYPE,
                                      snemo::digitization::mapping::GEIGER_RACK_ID, 1, 10);
        my_geiger_ctw.set_header(ict + 50, my_ctw_gid, ict);
...

    {
      for (unsigned int ict = 500; ict < 512; ict++) {
        ...
        geomtools::geom_id my_ctw_gid(snemo::digitization::mapping::TRACKER_CONTROL_BOARD_TYPE,
                                      snemo::digitization::mapping::GEIGER_RACK_ID, 1, 10);
        my_geiger_ctw.set_header(ict + 50, my_ctw_gid,
