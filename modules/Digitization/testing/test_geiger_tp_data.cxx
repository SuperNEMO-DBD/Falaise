...
int main(int argc_, char** argv_) {
  ...
  try {
    ...
    std::vector<datatools::handle<snemo::digitization::geiger_tp> >
        my_list_of_gg_tp_per_clocktick_per_crate;

    // For this example, 20 is the clocktick and 0 is the number of crate
    my_geiger_tp_data.get_list_of_gg_tp_per_clocktick_per_crate(
        20, 0, my_list_of_gg_tp_per_clocktick_per_crate);

    std::clog << "Size of my list of geiger tp for a clocktick = 20 and crate number = 0 : "
              << my_list_of_gg_tp_per_clocktick_per_crate.size() << " geiger tp(s) in the list "
              << std::endl;

    for (int i = 0; i < (int)my_list_of_gg_tp_per_clocktick_per_crate.size(); i++) {
      my_list_of_gg_tp_per_clocktick_per_crate[i].get().tree_dump(
          std::clog, "My list of geiger TP per clocktick per crate", "INFO : ");
    }
