...
int main(int argc_, char **argv_) {
  ...
  if (is_help) {
    std::cerr << std::endl
              ...
              << "This program take a file with a matrix pattern to convert it into geiger ctw "
                 "data then run the tracker algorithm "
              << std::endl
              << std::endl
              << "$ "
                 "BuildProducts/bin/"
                 "falaisedigitizationplugin-test_tracker_trigger_algorithm_load_from_file "
                 "[OPTIONS] [ARGUMENTS]"
              << std::endl
              << std::endl
              ...
              << "$ "
                 "BuildProducts/bin/"
                 "falaisedigitizationplugin-test_tracker_trigger_algorithm_load_from_file --input "
                 "${FALAISE_DIGITIZATION_TESTING_DIR}/data/100_events_tracker_matrix_output.data "
                 "-op ${FALAISE_DIGITIZATION_TESTING_DIR}/output_default"
              << " --number 5" << std::endl
              << std::endl
  ...
  try {
    ...
    bool geiger_matrix[snemo::digitization::trigger_info::NSIDES]
                      [snemo::digitization::trigger_info::NLAYERS]
                      [snemo::digitization::trigger_info::NROWS];
    ...
    while (std::getline(fin, line)) {
      ...
      if (line.empty()) {
        for (unsigned int i = 0; i < snemo::digitization::trigger_info::NSIDES; i++) {
          if (i == 0) {
            for (unsigned int j = snemo::digitization::trigger_info::NLAYERS - 1;
                 j != (unsigned)0 - 1; j--)  // Value GEIGER_LAYER_SIZE = 9
            {
        ...
        temporary_elec_id.set(snemo::digitization::mapping::RACK_INDEX,
                              snemo::digitization::mapping::GEIGER_RACK_ID);
        temporary_elec_id.set(snemo::digitization::mapping::CRATE_INDEX, 0);
        temporary_elec_id.set(snemo::digitization::mapping::BOARD_INDEX,
                              snemo::digitization::mapping::CONTROL_BOARD_ID);

        my_geiger_ctw_0.set_header(hit_id, temporary_elec_id, clocktick_800ns);

        temporary_elec_id.set(snemo::digitization::mapping::CRATE_INDEX, 1);
        my_geiger_ctw_1.set_header(hit_id, temporary_elec_id, clocktick_800ns);

        temporary_elec_id.set(snemo::digitization::mapping::CRATE_INDEX, 2);
        my_geiger_ctw_2.set_header(hit_id, temporary_elec_id, clocktick_800ns);
        ...
        for (unsigned int iblock = 0; iblock < 19; iblock++) {
          ...
          for (unsigned int jlayer = 0; jlayer < snemo::digitization::trigger_info::NLAYERS;
               jlayer++) {
            // CTW 0 :
        ...
        // std::clog << "GG CTW DATA SIZE = " << my_geiger_ctw_data.get_geiger_ctws().size() <<
        // std::endl;
        ...
        my_tracker_trigger_algo.process(my_geiger_ctw_data, my_tracker_records);
