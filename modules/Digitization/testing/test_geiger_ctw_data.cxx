...
int main(int argc_, char** argv_) {
  ...
  try {
    ...
    std::vector<datatools::handle<snemo::digitization::geiger_ctw> >
        my_list_of_gg_ctw_per_clocktick;
    ...
    std::clog << "Size of my list of geiger CTW for a clocktick = 20  : "
              << my_list_of_gg_ctw_per_clocktick.size() << " geiger CTW(s) in the list "
              << std::endl;

    for (unsigned int i = 0; i < my_list_of_gg_ctw_per_clocktick.size(); i++) {
      my_list_of_gg_ctw_per_clocktick[i].get().tree_dump(
          std::clog, "My CTW(s) in the list with a clocktick = 20 : ", "INFO : ");
    }
