...
int main(int argc_, char** argv_) {
  ...
  try {
    ...
    std::vector<datatools::handle<snemo::digitization::calo_tp> >
        my_list_of_calo_tp_per_clocktick_per_crate;

    // For this example, 20 is the clocktick and 0 is the number of crate
    my_calo_tp_data.get_list_of_tp_per_clocktick_per_crate(
        20, 0, my_list_of_calo_tp_per_clocktick_per_crate);

    std::clog << "Size of my list of calo tp for a clocktick = 20 and crate number = 0 : "
              << my_list_of_calo_tp_per_clocktick_per_crate.size() << " calo tp(s) in the list "
              << std::endl;

    for (unsigned int i = 0; i < my_list_of_calo_tp_per_clocktick_per_crate.size(); i++) {
      my_list_of_calo_tp_per_clocktick_per_crate[i].get().tree_dump(
          std::clog, "My list of calo TP per clocktick per crate", "INFO : ");
    }
