...
int main(int argc_, char** argv_) {
  ...
  if (is_help) {
    std::cerr << std::endl
              << "Usage :" << std::endl
              << std::endl
              << "$ BuildProducts/bin/falaisedigitizationplugin-test_trigger_analysis_gen_histo "
                 "[OPTIONS] [ARGUMENTS]"
              << std::endl
              << std::endl
  ...
  // Process
  try {
    std::clog << "Test program for class 'snemo::digitization::trigger_analysis_gen_histo' !"
              << std::endl;
    ...
    if (is_output_path && is_run_number) {
      ...
      output_tracker_histo_1 =
          default_path + "tracker_trigger_histo_" + default_run_number + "_1_mean.hist";
      datatools::fetch_path_with_env(output_tracker_histo_1);
      output_tracker_histo_2 =
          default_path + "tracker_trigger_histo_" + default_run_number + "_2_xy.hist";
      datatools::fetch_path_with_env(output_tracker_histo_2);
      output_tracker_histo_3 =
          default_path + "tracker_trigger_histo_" + default_run_number + "_3_yz.hist";
      datatools::fetch_path_with_env(output_tracker_histo_3);
    }
      ...
      if (ER.has(SD_bank_label) && ER.is_a<mctools::simulated_data>(SD_bank_label)) {
        ...
        if (SD.has_step_hits("gg")) {
          for (unsigned int i = 0; i < number_of_step_hits; i++) {
            ...
            if (base_step_hit.get_geom_id().get(1) == 0)
              x_y_position_hist.fill((int)-base_step_hit.get_geom_id().get(2),
                                     (int)base_step_hit.get_geom_id().get(3), weight);
            if (base_step_hit.get_geom_id().get(1) == 1)
              x_y_position_hist.fill((int)base_step_hit.get_geom_id().get(2),
                                     (int)base_step_hit.get_geom_id().get(3), weight);
