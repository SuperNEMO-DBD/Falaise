...
int main(int argc_, char** argv_) {
  ...
  if (is_help) {
    std::cerr << std::endl
              << "Usage :" << std::endl
              << std::endl
              << "$ BuildProducts/bin/falaisedigitizationplugin-test_tracker_trigger_algorithm "
                 "[OPTIONS] [ARGUMENTS]"
              << std::endl
              << std::endl
              ...
              << "-rn [ --run-number ]     set the output run number for output files (ex : -rn 10 "
                 "::  output -> tracker_trigger_algorithm_10.brio)"
              << std::endl
              ...
              << "input file           = "
                 ...
                 "Se82_0nubb-source_strips_bulk_SD_10_events.brio"
              << std::endl
              << "ouput path files     = ${FALAISE_DIGITIZATION_TESTING_DIR}/output_default/"
              << std::endl
  ...
  // Process
  try {
    std::clog << "Test program for class 'snemo::digitization::tracker_trigger_algorithm' !"
              << std::endl;
    ...
    if (is_input_file)
      ...
      // "${DATA_NEMO_PERSO_DIR}/trigger/simulated_data_brio/Se82_0nubb_500000-source_strips_bulk_SD.brio";
      // pipeline_simulated_data_filename =
      // "${DATA_NEMO_PERSO_DIR}/trigger/simulated_data_brio/Bi214_Po214_500000-source_strips_bulk_SD.brio";
      // pipeline_simulated_data_filename =
      // "${FALAISE_DIGITIZATION_TESTING_DIR}/data/Se82_0nubb-source_strips_bulk_SD_10_events.brio";
      pipeline_simulated_data_filename =
          "${DATA_NEMO_PERSO_DIR}/trigger/simulated_data_brio/"
          "Se82_0nubb_500000-source_strips_bulk_SD.brio";
      // pipeline_simulated_data_filename =
      // "/home/goliviero/software/my_falaise/outputs/Bi214_Po214_10000-source_strips_external_surface_SD.brio";
    }
      ...
      if (ER.has(SD_bank_label) && ER.is_a<mctools::simulated_data>(SD_bank_label)) {
        ...
        if (SD.has_step_hits("gg")) {
          ...
          if (signal_data.has_geiger_signals()) {
            ...
            snemo::digitization::tracker_trigger_algorithm::tracker_record
                tracker_level_one_finale_decison;

            my_tracker_algo.process(my_geiger_ctw_data, tracker_collection_records);
            // tracker_level_one_finale_decison =
            // my_tracker_algo.get_tracker_record_finale_decision_structure();
