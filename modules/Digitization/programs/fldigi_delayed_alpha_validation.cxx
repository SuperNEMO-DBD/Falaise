...
int main(int  argc_ , char ** argv_)
{
  ...
  try {
    ...
    static const int MAXIMUM_DELAYED_TIME = 6000; // in nanosecond, linked with the digi trigger and geiger drift

    // Event record :
    datatools::things ER;

    int psd_count = 0;         // Event counter
    std::string five_or_more_gg_cells_delayed_DT_no = output_path + "four_or_more_gg_cells_delayed_DT_no.brio";
    // Event writer : 
    ...
    writer_config_1.store ("files.single.filename", five_or_more_gg_cells_delayed_DT_no);
    ...
    int five_delayed_geiger_cells_count = 0;
    int six_delayed_geiger_cells_count = 0;
    int seven_delayed_geiger_cells_count = 0;
	...
	if (ER.has(SD_bank_label) && ER.is_a<mctools::simulated_data>(SD_bank_label)) 
	  {
	    ...
	    switch (number_of_delayed_gg_cells)
	      {
	      case 0:
		{
		  no_delayed_geiger_cells_count++;
		  if (debug) std::clog << "psd  count = " << psd_count << std::endl;
		  break;
		}
	      case 1:
		{
		...
		{
		  writer_1.process(ER);
		  five_delayed_geiger_cells_count++;		  
		  break;
		} 
	      case 6:
		{
		  writer_1.process(ER);
		  six_delayed_geiger_cells_count++;		  
		  break;
		}  
	      case 7:
		{
		  writer_1.process(ER);
		  seven_delayed_geiger_cells_count++;		  
		  break;
		}
	      }
	  }
    ...
    std::clog << "5 delayed GG cells : " << five_delayed_geiger_cells_count << std::endl;
    std::clog << "6 delayed GG cells : " << six_delayed_geiger_cells_count << std::endl;
    std::clog << "7 delayed GG cells : " << seven_delayed_geiger_cells_count << std::endl;
