...
int main(int argc_, char** argv_) {
  ...
  try {
    ...
    delayed_cells_tree->Branch("number_of_gg_cells",
                               &ptr_number_of_gg_cells);  //, "number_of_gg_cells/I");
    delayed_cells_tree->Branch(
        "number_of_delayed_gg_cells",
        &ptr_number_of_delayed_gg_cells);  //, "number_of_delayed_gg_cells/I");
    delayed_cells_tree->Branch(
        "number_of_not_delayed_gg_cells",
        &ptr_number_of_not_delayed_gg_cells);  //, "number_of_not_delayed_gg_cells/I");
    delayed_cells_tree->Branch("delayed_time_shift",
                               &ptr_delayed_time_shift);          //, "delayed_time_shift/I");
    ...
    while (!reader.is_terminated()) {
      ...
      if (ER.has(SD_bank_label) && ER.is_a<mctools::simulated_data>(SD_bank_label)) {
        ...
        for (Int_t delayed_time_shift = 10; delayed_time_shift <= MAXIMUM_DELAYED_TIME;
             delayed_time_shift += 10)  // Time for delayed gg cells (in ns)
        {
          ...
          if (SD.has_step_hits("gg")) {
            ...
            for (mctools::simulated_data::hit_handle_collection_type::const_iterator i =
              ...
              if (time_start > delayed_time_shift)
