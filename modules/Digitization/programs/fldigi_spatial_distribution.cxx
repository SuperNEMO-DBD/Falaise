...
int main(int argc_, char** argv_) {
  ...
  if (is_help) {
    std::cerr << std::endl
              ...
              << "Example : " << std::endl
              << std::endl
              << "$ BuildProducts/bin/fldigi_spatial_distribution -n 1000 -i "
                 "${FALAISE_DIGITIZATION_TESTING_DIR}/data/"
                 "Se82_0nubb-source_strips_bulk_SD_10_events.brio -op "
                 "${FALAISE_DIGITIZATION_TESTING_DIR}/output_default -p -v"
              << std::endl
              << std::endl;
    return 0;
  }
  ...
  // Process
  try {
    std::clog << "Little test program for analysis of spatial distribution of geiger cells hit and "
                 "vertex !"
              << std::endl;
    ...
    TH2F* geiger_cells_distribution_TH2F =
        new TH2F("Prompt Geiger cells spatial distribution",
                 Form("%s : Prompt Geiger cells spatial distribution", run_number.c_str()), 115,
                 -1.5, 114 - 0.5, 28, -5.5, 23 - 0.5);

    TH2F* vertex_distribution_TH2F =
        new TH2F("Vertex spatial distribution TH2F",
                 Form("%s : Vertex spatial distribution; x_vertex_position; y_vertex_position",
                      run_number.c_str()),
                 104, -2700.5, 2700 - 0.5, 120, -600, 600);

    TH1F* vertex_distribution_TH1F = new TH1F(
        "Vertex spatial distribution TH1F",
        Form("%s : Vertex spatial distribution; y_vertex_position; count", run_number.c_str()), 104,
        -2600.5, 2600 - 0.5);
        ...
        if (SD.has_step_hits("gg")) {
          ...
          for (mctools::simulated_data::hit_handle_collection_type::const_iterator i = BSHC.begin();
            ...
            if (BSH.get_auxiliaries().has_flag("geiger_already_hit") ||
              ...
              if (verbosity)
                std::clog << "side = " << side << " layer = " << layer << " row = " << row
                          << std::endl;
              ...
              if (verbosity)
                std::clog << "X_HISTO = " << X_histo << " Y_HISTO = " << Y_histo << std::endl
                          << std::endl;
