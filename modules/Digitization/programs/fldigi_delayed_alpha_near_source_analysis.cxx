...
int main(int argc_, char **argv_) {
  ...
  try {
    ...
    std::string input_filename =
        "${FALAISE_DIGITIZATION_TESTING_DIR}/output_default/fldigi_delayed_alpha_validation.root";
    std::string output_filename =
        "${FALAISE_DIGITIZATION_TESTING_DIR}/output_default/fldigi_delayed_alpha_analysis.root";
    ...
    input_delayed_cells_tree->SetBranchAddress("number_of_delayed_gg_cells",
                                               &vect_number_of_delayed_gg_cells);
    input_delayed_cells_tree->SetBranchAddress("number_of_not_delayed_gg_cells",
                                               &vect_number_of_not_delayed_gg_cells);
    ...
    TH2F *histo_delayed_geiger_cells_delayed_time =
        new TH2F("Delayed Geiger Cells", "Delayed GG cells", 60, 0., 60., 20, 0., 20.);

    // Begining of the analysis
    std::clog << "Number of Entries in TTree = " << input_delayed_cells_tree->GetEntries()
              << std::endl;
    ...
    for (int i = 0; i < n_entry; i++) {
      ...
      for (size_t j = 0; j < vector_size; j++) {
        ...
        histo_delayed_geiger_cells_delayed_time->Fill(delayed_time_shift,
                                                      number_of_delayed_gg_cells);
        // std::clog << "Delayed time shift              = " << delayed_time_shift << std::endl;
        // std::clog << "Number of gg cells             = " << number_of_gg_cells << std::endl;
        // std::clog << "Number of delayed gg cells     = " << number_of_delayed_gg_cells <<
        // std::endl; std::clog << "Number of not delayed gg cells = " <<
        // number_of_not_delayed_gg_cells << std::endl << std::endl;
      }
    }
