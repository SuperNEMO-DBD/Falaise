...
class trigger_algorithm_test_time {
 public:
  ...
  get_coincidence_calo_records_1600ns_vector() const;

  /// Get the vector of tracker record
  const std::vector<trigger_structures::tracker_record> get_tracker_records_vector() const;

  /// Get the vector of coincidence record
  const std::vector<trigger_structures::coincidence_event_record> get_coincidence_records_vector()
      const;

  /// Get the vector of L1 calo decision record
  const std::vector<trigger_structures::L1_calo_decision> get_L1_calo_decision_records_vector()
      const;
  ...
  /// General process
  void process(const calo_ctw_data& calo_ctw_data_, const geiger_ctw_data& geiger_ctw_data_);

 protected:
  /// Rescaling calorimeter 25 ns at 1600 ns for coincidences with tracker
  void _rescale_calo_records_at_1600ns(
  ...
  void _update_coinc_calo_record(
      const trigger_structures::calo_summary_record& a_calo_summary_record_25ns,
  ...
  void _process(const calo_ctw_data& calo_ctw_data_, const geiger_ctw_data& geiger_ctw_data_);

 private:
  typedef boost::circular_buffer<trigger_structures::previous_event_record>
      buffer_previous_event_record_type;
  ...
  unsigned int _coincidence_calorimeter_gate_size_;  //!< Size of calorimeter gate for extension of
                                                     //!< calo records during X CT 1600ns
  unsigned int _L2_decision_coincidence_gate_size_;  //!< Size of the L2 decision coincidence gate
                                                     //!< (typically 5 x 1600 ns)
  unsigned int _previous_event_circular_buffer_depth_;  //!< Depth for the previous events circular
                                                        //!< buffer (Pile of PERs)
  ...
  coincidence_trigger_algorithm_test_time
      _coinc_algo_;  //!< Coincidence trigger algorithm for matching calo and tracker trigger

  // Data :
  std::vector<trigger_structures::calo_summary_record>
      _calo_records_25ns_;  //!< Collection of calo summary record @ 25 ns
  std::vector<trigger_structures::coincidence_calo_record>
      _coincidence_calo_records_1600ns_;  //!< Collection of coincidence calo records @ 1600 ns
  std::vector<trigger_structures::tracker_record>
      _tracker_records_;  //!< Collection of tracker record @ 1600 ns
  std::vector<trigger_structures::geiger_matrix>
      _geiger_matrix_records_;  //!< Collection of Geiger matrix @ 1600 ns
  std::vector<
      std::pair<trigger_structures::coincidence_calo_record, trigger_structures::tracker_record> >
      _pair_records_;  //!< Pair of a coincidence calo record and a tracker record (at the same
                       //!< clocktick 1600 ns)
  std::vector<trigger_structures::coincidence_event_record>
      _coincidence_records_;  //!< Collection of coincidence record @ 1600 ns
  boost::scoped_ptr<buffer_previous_event_record_type>
      _previous_event_records_;  //!< Collection of previous event records @ 1600 ns
  std::vector<trigger_structures::L1_calo_decision>
      _L1_calo_decision_records_;  //!< Collection of L1 calorimeter decision @ 25 ns
  std::vector<trigger_structures::L2_decision>
      _L2_decision_records_;  //!< Collection of L2 decision (which launch the readout)
