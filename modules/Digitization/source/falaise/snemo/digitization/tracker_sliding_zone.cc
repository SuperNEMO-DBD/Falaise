...
void tracker_sliding_zone::compute_lr_proj() {
  ...
  for (unsigned int irow = 0; irow < width(szone_id); irow++) {
    ...
    if (szone_id == 0 || szone_id == 1)
      addr_row_proj.set(irow + 8 - width(szone_id), false);
    else
...
void tracker_sliding_zone::build_pattern(tracker_trigger_mem_maker::mem1_type& mem1_,
...
void tracker_sliding_zone::print(std::ostream& out_) const {
  if (szone_id == 0 || szone_id == 1)
    out_ << "Problem with display for projections due to an initial shift for these sliding zones"
         << std::endl;
...
void tracker_sliding_zone::print_projections(std::ostream& out_) const {
  ...
  // 	  if (i != 0 && i != snemo::digitization::trigger_info::SLZONE_LAYER_PROJ - 1) out_ <<
  // addr_layer_proj[i]; 	  if (i == 0) out_ << " ==> " << data_IO_proj << "\n";
  // 	}
}
