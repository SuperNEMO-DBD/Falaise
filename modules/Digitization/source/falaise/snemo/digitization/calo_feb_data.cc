...
namespace snemo {
  namespace digitization {
    ...
    void calo_feb_data::initialize(bool has_waveform_, unsigned int nb_sample_, unsigned int first_sample_)  {
    ...
    void calo_feb_data::set_data(std::vector<uint16_t> *waveform_array_ ){
      DT_THROW_IF(!is_initialized(), std::logic_error, "Current calo_feb_data not initialized ! (it should be my fellow) ");
      ...
      DT_THROW_IF(!is_ok_array_size, std::logic_error, "Input waveform array has not the expected array size ! (that's the dark side) ");
	...
	out_ << indent_;
	if(i == 0)out_ << datatools::i_tree_dumpable::skip_tag;else{out_ << datatools::i_tree_dumpable::last_skip_tag; }
