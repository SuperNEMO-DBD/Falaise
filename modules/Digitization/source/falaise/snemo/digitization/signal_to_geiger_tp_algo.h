...
class signal_to_geiger_tp_algo : boost::noncopyable {
 public:
  ...
  void add_geiger_tp(const signal_to_tp_working_data &my_wd_data_, uint32_t signal_clocktick_,
                     int32_t hit_id_, geiger_tp_data &my_geiger_tp_data_);
  ...
  void process(const signal_data &signal_data_, geiger_tp_data &my_geiger_tp_data_);
 ...
 protected:
  ...
  void _process(const signal_data &signal_data_, geiger_tp_data &my_geiger_tp_data_);
 ...
 private:
  ...
  // Data :
  bool _activated_bits_[geiger::tp::TP_SIZE];  //!< Table of booleans to see which bits were
                                               //!< activated
};
