...
bool geiger_tp_to_ctw_algo::is_initialized() const { return _initialized_; }
...
void geiger_tp_to_ctw_algo::reset() {
  DT_THROW_IF(!is_initialized(), std::logic_error,
              "Geiger tp to ctw algo is not initialized, it can't be reset ! ");
  _initialized_ = false;
  return;
}

unsigned int geiger_tp_to_ctw_algo::board_to_index_mapping(uint32_t board_id_) { return board_id_; }

void geiger_tp_to_ctw_algo::_fill_a_geiger_ctw(const geiger_tp& my_geiger_tp_,
  ...
  a_geiger_ctw_.set_header(
      my_geiger_tp_.get_hit_id(), temporary_feb_id,
...
void geiger_tp_to_ctw_algo::_process_for_a_ctw_for_a_clocktick(
    const std::vector<datatools::handle<geiger_tp> >& my_list_of_geiger_tp_,
    geiger_ctw& a_geiger_ctw_) {
...
void geiger_tp_to_ctw_algo::process(const geiger_tp_data& geiger_tp_data_,
                                    geiger_ctw_data& geiger_ctw_data_) {
  DT_THROW_IF(!is_initialized(), std::logic_error,
              "Geiger tp to ctw algo is not initialized, it can't process ! ");
  for (uint32_t i = geiger_tp_data_.get_clocktick_min(); i <= geiger_tp_data_.get_clocktick_max();
       i++) {
    for (unsigned int j = 0; j <= mapping::NUMBER_OF_CRATES; j++) {
      ...
      geiger_tp_data_.get_list_of_gg_tp_per_clocktick_per_crate(
          i, j, geiger_tp_list_per_clocktick_per_crate);
