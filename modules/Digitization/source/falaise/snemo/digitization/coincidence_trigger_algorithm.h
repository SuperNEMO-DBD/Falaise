...
class coincidence_trigger_algorithm {
 public:
  ...
  enum trigger_mode { INVALID = 0, CARACO = 1, APE = 2, DAVE = 3 };
  ...
  struct coincidence_event_record : public coincidence_base_record {
    ...
    std::bitset<trigger_info::DATA_FULL_BITSET_SIZE>
        tracker_finale_data_per_zone[trigger_info::NSIDES][trigger_info::NZONES];
  ...
  /// Previous event structure who pass the caraco trigger, store trigger informations (total = 173
  /// [bits]). Useful for searching delayed alpha pattern.
  ...
  get_coincidence_calo_records_vector() const;
 ...
 protected:
  ...
  void _preparing_calo_coincidence(
      const std::vector<calo_trigger_algorithm::calo_summary_record>& calo_records_);

  /// Update a coinc calo record 1600 from a calo summary record 25 ns
  void _update_coinc_calo_record(
      const calo_trigger_algorithm::calo_summary_record& a_calo_summary_record_25ns,
      coincidence_trigger_algorithm::coincidence_calo_record& a_coinc_calo_record_1600ns);
  ...
  void _creating_pair_per_clocktick(
      const std::vector<coincidence_trigger_algorithm::coincidence_calo_record>&
          coinc_calo_records_,
      const std::vector<tracker_trigger_algorithm::tracker_record>& tracker_records_);
  ...
  /// Build the previous event record based on CARACO process for searching delayed alpha
  /// coincidences
 ...
 private:
  typedef boost::circular_buffer<coincidence_trigger_algorithm::previous_event_record>
  ...
  unsigned int _previous_event_circular_buffer_depth_;  //!< Depth for the previous events circular
                                                        //!< buffer (Pile of PERs)
  std::vector<coincidence_trigger_algorithm::coincidence_calo_record>
      _coincidence_calo_records_;  //!< Vector of coincidence calo tracker record and delayed
                                   //!< coincidence record
  ...
  boost::scoped_ptr<buffer_previous_event_record_type>
      _previous_event_records_;         //!< Previous prompt event record
  ...
  std::vector<coincidence_trigger_algorithm::L2_coincidence_decision>
      _L2_coincidence_decison_records_;  //!< Vector of L2 decision for an event (it is possible to
                                         //!< trig several times for the same "SD" event
};
