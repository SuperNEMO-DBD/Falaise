...
void calo_tp::set_header(int32_t hit_id_, const geomtools::geom_id& electronic_id_,
...
void calo_tp::set_data(const double amplitude_, const bool xt_bit_, const bool spare_bit_) {
  // To check :
...
void calo_tp::update_data(const double amplitude_, const bool xt_bit_, const bool spare_bit_) {
...
uint32_t calo_tp::get_clocktick_25ns() const { return _clocktick_25ns_; }
...
const std::bitset<calo_tp::FULL_SIZE>& calo_tp::get_tp_bitset() const { return _tp_; }
...
void calo_tp::set_htm(const unsigned int multiplicity_) {
  DT_THROW_IF(multiplicity_ > MAX_NUMBER_OF_CHANNELS, std::logic_error,
...
bool calo_tp::is_htm() const { return get_htm() != 0; }
...
bool calo_tp::is_lto() const { return _tp_.test(LTO_BIT); }
...
bool calo_tp::is_xt() const { return _tp_.test(XT_BIT); }
...
bool calo_tp::is_spare() const { return _tp_.test(SPARE_BIT); }

bool calo_tp::is_valid() const { return has_geom_id() && has_hit_id() && has_clocktick_25ns(); }
...
void calo_tp::tree_dump(std::ostream& out_, const std::string& title_, const std::string& indent_,
                        bool inherit_) const {
  ...
  out_ << indent_ << datatools::i_tree_dumpable::inherit_tag(inherit_) << "TP (5 bits) : " << _tp_
       << std::endl;
  return;
}
