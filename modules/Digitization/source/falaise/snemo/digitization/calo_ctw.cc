...
void calo_ctw::set_header(int32_t hit_id_, const geomtools::geom_id& electronic_id_,
...
bool calo_ctw::is_main_wall() const {
  if (_layout_ == calo::ctw::LAYOUT_MAIN_WALL)
    return true;
  else
    return false;
}

calo::ctw::layout calo_ctw::get_layout() const { return _layout_; }

uint32_t calo_ctw::get_clocktick_25ns() const { return _clocktick_25ns_; }
...
void calo_ctw::set_htm_main_wall(unsigned int multiplicity_) {
  DT_THROW_IF(multiplicity_ > mapping::NUMBER_OF_FEBS_BY_CRATE, std::logic_error,
              "Multiplicity value [" << multiplicity_ << "] is not valid ! ");
  DT_THROW_IF(_layout_ == calo::ctw::LAYOUT_UNDEFINED || _layout_ == calo::ctw::LAYOUT_XWALL_GVETO,
              std::logic_error, "Layout value [" << _layout_ << "] is not valid ! ");
...
unsigned int calo_ctw::get_htm_main_wall_info() const {
  if (_ctw_.test(calo::ctw::HTM_MAIN_WALL_BIT0) == 0 &&
      _ctw_.test(calo::ctw::HTM_MAIN_WALL_BIT1) == 0) {
    return 0;
  } else if (_ctw_.test(calo::ctw::HTM_MAIN_WALL_BIT0) == 1 &&
             _ctw_.test(calo::ctw::HTM_MAIN_WALL_BIT1) == 0) {
    return 1;
  } else if (_ctw_.test(calo::ctw::HTM_MAIN_WALL_BIT0) == 0 &&
             _ctw_.test(calo::ctw::HTM_MAIN_WALL_BIT1) == 1) {
    return 2;
  }
  return 3;
}

bool calo_ctw::is_htm_main_wall() const { return get_htm_main_wall_info() != 0; }

void calo_ctw::set_htm_gveto(unsigned int multiplicity_) {
  DT_THROW_IF(multiplicity_ > mapping::NUMBER_OF_FEBS_BY_CRATE, std::logic_error,
              "Multiplicity value [" << multiplicity_ << "] is not valid ! ");
  DT_THROW_IF(_layout_ == calo::ctw::LAYOUT_UNDEFINED || _layout_ == calo::ctw::LAYOUT_MAIN_WALL,
              std::logic_error, "Layout value [" << _layout_ << "] is not valid ! ");
...
unsigned int calo_ctw::get_htm_gveto_info() const {
  if (_ctw_.test(calo::ctw::HTM_GVETO_BIT0) == 0 && _ctw_.test(calo::ctw::HTM_GVETO_BIT1) == 0) {
    return 0;
  } else if (_ctw_.test(calo::ctw::HTM_GVETO_BIT0) == 1 &&
             _ctw_.test(calo::ctw::HTM_GVETO_BIT1) == 0) {
    return 1;
  } else if (_ctw_.test(calo::ctw::HTM_GVETO_BIT0) == 0 &&
             _ctw_.test(calo::ctw::HTM_GVETO_BIT1) == 1) {
    return 2;
  }
  return 3;
}

bool calo_ctw::is_htm_gveto() const { return get_htm_gveto_info() != 0; }
...
unsigned int calo_ctw::get_htm_xwall_side_0_info() const {
  if (_ctw_.test(calo::ctw::HTM_XWALL_SIDE0_BIT0) == 0 &&
      _ctw_.test(calo::ctw::HTM_XWALL_SIDE0_BIT1) == 0) {
    return 0;
  } else if (_ctw_.test(calo::ctw::HTM_XWALL_SIDE0_BIT0) == 1 &&
             _ctw_.test(calo::ctw::HTM_XWALL_SIDE0_BIT1) == 0) {
    return 1;
  } else if (_ctw_.test(calo::ctw::HTM_XWALL_SIDE0_BIT0) == 0 &&
             _ctw_.test(calo::ctw::HTM_XWALL_SIDE0_BIT1) == 1) {
    return 2;
  }
  return 3;
}

bool calo_ctw::is_htm_xwall_side_0() const { return get_htm_xwall_side_0_info() != 0; }
...
unsigned int calo_ctw::get_htm_xwall_side_1_info() const {
  if (_ctw_.test(calo::ctw::HTM_XWALL_SIDE1_BIT0) == 0 &&
      _ctw_.test(calo::ctw::HTM_XWALL_SIDE1_BIT1) == 0) {
    return 0;
  } else if (_ctw_.test(calo::ctw::HTM_XWALL_SIDE1_BIT0) == 1 &&
             _ctw_.test(calo::ctw::HTM_XWALL_SIDE1_BIT1) == 0) {
    return 1;
  } else if (_ctw_.test(calo::ctw::HTM_XWALL_SIDE1_BIT0) == 0 &&
             _ctw_.test(calo::ctw::HTM_XWALL_SIDE1_BIT1) == 1) {
    return 2;
  }
  return 3;
}

bool calo_ctw::is_htm_xwall_side_1() const { return get_htm_xwall_side_1_info() != 0; }

void calo_ctw::get_main_zoning_word(
    std::bitset<calo::ctw::MAIN_ZONING_BITSET_SIZE>& zoning_word_) const {
  zoning_word_ = 0x0;
...
void calo_ctw::get_xwall_zoning_word(
    std::bitset<calo::ctw::XWALL_ZONING_BITSET_SIZE>& xwall_zoning_word_) const {
...
void calo_ctw::set_xwall_zoning_word(
    std::bitset<calo::ctw::XWALL_ZONING_BITSET_SIZE>& xwall_zoning_word_) {
  DT_THROW_IF(_layout_ == calo::ctw::LAYOUT_UNDEFINED || _layout_ == calo::ctw::LAYOUT_MAIN_WALL,
              std::logic_error, "Layout value [" << _layout_ << "] is not valid ! ");
...
void calo_ctw::set_zoning_bit(int bit_pos_, bool value_) {
  DT_THROW_IF(_layout_ == calo::ctw::LAYOUT_UNDEFINED, std::logic_error,
              "Layout value [" << _layout_ << "] is not valid ! ");
  DT_THROW_IF(bit_pos_ < calo::ctw::X_ZW_BIT0 && bit_pos_ > calo::ctw::X_ZW_BIT3, std::logic_error,
              " Bit position for crate number 2 is not defined ! ");
...
bool calo_ctw::is_lto_main_wall() const {
  if (_ctw_.test(calo::ctw::LTO_MAIN_WALL_BIT) == true)
    return true;
  else
    return false;
}
...
bool calo_ctw::is_lto_xwall_side_0() const {
  if (_ctw_.test(calo::ctw::LTO_XWALL_SIDE0_BIT) == true)
    return true;
  else
    return false;
}
...
bool calo_ctw::is_lto_xwall_side_1() const {
  if (_ctw_.test(calo::ctw::LTO_XWALL_SIDE1_BIT) == true)
    return true;
  else
    return false;
}
...
bool calo_ctw::is_lto_gveto() const { return _ctw_.test(calo::ctw::LTO_GVETO_BIT); }
...
bool calo_ctw::is_xt() const { return _ctw_.test(calo::ctw::XT_PC_BIT); }
...
bool calo_ctw::is_valid() const {
  if (_clocktick_25ns_ != clock_utils::INVALID_CLOCKTICK)
    return true;
  else
    return false;
}
...
void calo_ctw::tree_dump(std::ostream& out_, const std::string& title_, const std::string& indent_,
                         bool inherit_) const {
  base_hit::tree_dump(out_, title_, indent_, true);

  out_ << indent_ << datatools::i_tree_dumpable::tag << "Clock tick (25 ns)  : " << _clocktick_25ns_
       << std::endl;
