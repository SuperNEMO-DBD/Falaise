...
void signal_to_calo_tp_algo::initialize(electronic_mapping& my_electronic_mapping_) {
  DT_THROW_IF(is_initialized(), std::logic_error,
              "SD to calo tp algorithm is already initialized ! ");
  _electronic_mapping_ = &my_electronic_mapping_;
  _initialized_ = true;
  return;
}

bool signal_to_calo_tp_algo::is_initialized() const { return _initialized_; }

void signal_to_calo_tp_algo::reset() {
  DT_THROW_IF(!is_initialized(), std::logic_error,
              "SD to calo tp algorithm is not initialized, it can't be reset ! ");
  _initialized_ = false;
  _electronic_mapping_ = 0;
  _clocktick_ref_ = clock_utils::INVALID_CLOCKTICK;
  datatools::invalidate(_clocktick_shift_);
...
void signal_to_calo_tp_algo::_process(const signal_data& signal_data_,
  ...
  for (std::size_t i = 0; i < number_of_hits; i++) {
    ...
    if (calo_hit_amplitude >= calo_tp::LOW_THRESHOLD) {
      ...
      uint32_t a_calo_signal_clocktick =
          _clocktick_ref_ + clock_utils::CALO_FEB_SHIFT_CLOCKTICK_NUMBER;
      ...
      for (unsigned int j = 0; j < my_calo_tp_data_.get_calo_tps().size(); j++) {
        if (my_calo_tp_data_.get_calo_tps()[j].get().get_geom_id() == electronic_id &&
            my_calo_tp_data_.get_calo_tps()[j].get().get_clocktick_25ns() ==
      ...
      if (existing == false) {
        ...
        calo_tp.set_header(a_calo_signal.get_hit_id(), electronic_id, a_calo_signal_clocktick);
        calo_tp.set_data(calo_hit_amplitude, calo_xt_bit, calo_spare_bit);
        // calo_tp.tree_dump(std::clog, "Calo TP first creation : ", "INFO : ");
      }

      else {
        snemo::digitization::calo_tp& existing_calo_tp =
            my_calo_tp_data_.grab_calo_tps()[existing_index].grab();
        existing_calo_tp.update_data(calo_hit_amplitude, calo_xt_bit, calo_spare_bit);
