...
void tracker_trigger_mem_maker::initialize() {
  DT_THROW_IF(_mem_type_ == MEM_UNDEFINED, std::logic_error,
              "Configuration is not (fully) set, object can't be initialized ! ");
  _build();
  _initialized_ = true;
  return;
}

bool tracker_trigger_mem_maker::is_initialized() const { return _initialized_; }
  ...
  switch (_mem_type_) {
    ...
    default:
      DT_THROW(std::logic_error,
               "Memory type '" << _mem_type_ << "' is not supported, enter a good value ! ");
      break;
  };
...
void tracker_trigger_mem_maker::_build_mem3() {
  ...
  for (unsigned long addr = 0; addr < _mem3_->get_number_of_addresses(); addr++) {
    ...
    if (address.test(szB_BIT_INNER) || address.test(szC_BIT_INNER) || address.test(szD_BIT_INNER) ||
        address.test(szA_BIT_INNER))
      data.set(data_BIT_INNER);
    if (address.test(szB_BIT_OUTER) || address.test(szC_BIT_OUTER) || address.test(szD_BIT_OUTER) ||
        address.test(szA_BIT_OUTER))
      data.set(data_BIT_OUTER);
...
void tracker_trigger_mem_maker::store_to_file(const std::string& output_file_,
  ...
  DT_THROW_IF(!is_initialized(), std::logic_error,
              "Object is not initialized, store can't be done ! ");
