...
DATATOOLS_SERIALIZATION_SERIAL_TAG_IMPLEMENTATION(calo_tp_data,
                                                  "snemo::digitalization::calo_tp_data")

calo_tp_data::calo_tp_data() { return; }
...
void calo_tp_data::get_list_of_tp_per_clocktick(
    uint32_t clocktick_25ns_, calo_tp_collection_type& my_list_of_tps_per_clocktick_) const {
  if (_calo_tps_.size() == 0) {
    return;
  }
...
void calo_tp_data::get_list_of_tp_per_clocktick_per_crate(
    uint32_t clocktick_25ns_, unsigned int crate_number_,
  ...
  for (unsigned int i = 0; i < _calo_tps_.size(); i++) {
    if (_calo_tps_[i].get().get_clocktick_25ns() == clocktick_25ns_ &&
        _calo_tps_[i].get().get_geom_id().get(mapping::CRATE_INDEX) == crate_number_) {
...
calo_tp_data::calo_tp_collection_type& calo_tp_data::grab_calo_tps() { return _calo_tps_; }
...
void calo_tp_data::tree_dump(std::ostream& out_, const std::string& title_,
