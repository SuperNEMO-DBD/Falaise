...
namespace snemo {
  namespace digitization {
    ...
    bool tracker_feb_data::is_initialized() const {
    ...
    {
      DT_THROW_IF(!is_initialized(), std::logic_error, "tracker feb data not initialized, can't get anodic status ! ");
    ...
    {
      DT_THROW_IF(!is_initialized(), std::logic_error, "tracker feb data not initialized, can't get cathodic status ! ");
    ...
    bool tracker_feb_data::is_nemo3_mode(){
      DT_THROW_IF(!is_initialized(), std::logic_error, "tracker feb data not initialized, can't get tracker mode ! ");
      bool status = false;
    ...
    void tracker_feb_data::set_anodic_times(uint64_t t0_, uint64_t t1_,uint64_t t2_, uint64_t t3_, uint64_t t4_){
      set_anodic_t0(t0_);
      set_anodic_t1(t1_);
      set_anodic_t2(t2_);
      set_anodic_t3(t3_);
    ...
    void tracker_feb_data::set_data(uint64_t t0_, uint64_t t1_,uint64_t t2_, uint64_t t3_, uint64_t t4_,uint64_t t5_, uint64_t t6_){
    ...
    void tracker_feb_data::tree_dump (std::ostream & out_,
	...
	if(has_anodic_t0()) out_ << indent_<<datatools::i_tree_dumpable::skip_tag
				 << datatools::i_tree_dumpable::tag
				 <<"t0 = "  << anodic_t0 << std::endl;
	
	if(has_anodic_t1()) out_ << indent_<<datatools::i_tree_dumpable::skip_tag
				 << datatools::i_tree_dumpable::tag
				 <<"t1 = "  << anodic_t1 << std::endl;
	
	if(has_anodic_t2()) out_ << indent_<<datatools::i_tree_dumpable::skip_tag
				 << datatools::i_tree_dumpable::tag
				 <<"t2 = "  << anodic_t2 << std::endl;
	
	if(has_anodic_t3()) out_ << indent_<<datatools::i_tree_dumpable::skip_tag
				 << datatools::i_tree_dumpable::tag
				 <<"t3 = "  << anodic_t3 << std::endl;
	
	if(has_anodic_t4()) out_ << indent_<<datatools::i_tree_dumpable::skip_tag
				 <<datatools::i_tree_dumpable::inherit_tag (inherit_)
	...
	std::clog<<"INFO : tracker_feb_data::tree_dump : Current object not initialized !"<<std::endl;
      }
