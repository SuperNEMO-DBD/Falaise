...
void electronic_mapping::add_preconstructed_type(int type_) {
  DT_THROW_IF(supported_types().count(type_) == 0, std::logic_error,
              "Type [" << type_ << "] is not supported ! ");
...
bool electronic_mapping::geo_manager_is_set() const { return _geo_manager_status_; }
...
bool electronic_mapping::module_number_is_set() const { return _module_number_status_; }

void electronic_mapping::initialize() {
  DT_THROW_IF(!geo_manager_is_set() && !module_number_is_set(), std::logic_error,
              "Geo manager or module number is not set ! ");
  _initialize();
  return;
}

bool electronic_mapping::is_initialized() const { return _initialized_; }

void electronic_mapping::reset() {
  DT_THROW_IF(!is_initialized(), std::logic_error,
              "Electronic mapping is not initialized, it can't be reset ! ");
  _geo_manager_ = 0;
  _initialized_ = false;
  return;
}
...
void electronic_mapping::convert_GID_to_EID(const bool tracker_trigger_mode_,
  ...
  const_cast<electronic_mapping *>(this)->_convert_GID_to_EID(tracker_trigger_mode_, geom_id_,
                                                              electronic_id_);
  return;
}
...
void electronic_mapping::convert_EID_to_GID(const bool tracker_trigger_mode_,
  ...
  const_cast<electronic_mapping *>(this)->_convert_EID_to_GID(tracker_trigger_mode_, electronic_id_,
                                                              geom_id_);
  return;
}
...
void electronic_mapping::_initialize() {
  ...
  for (std::set<int32_t>::iterator it = _pre_constructed_types_.begin();
    ...
    if (*it == mapping::GEIGER_CATEGORY_TYPE)
      _init_geiger();
    else if (*it == mapping::CALO_MAIN_WALL_CATEGORY_TYPE)
      _init_mcalo();
  }
...
void electronic_mapping::_convert_EID_to_GID(const bool tracker_trigger_mode_,
  ...
  DT_THROW_IF(elec_id_.get_type() != mapping::FEB_CATEGORY_TYPE, std::logic_error,
              "elect_id incorrect type ! ");
  DT_THROW_IF(tracker_trigger_mode_ != mapping::THREE_WIRES_TRACKER_MODE, std::logic_error,
              " Give a correct traker trigger mode (Two wires mode is not supported yet) ! ");
  geom_id_.reset();
