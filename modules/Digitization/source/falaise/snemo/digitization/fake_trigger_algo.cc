...
fake_trigger_algo::fake_trigger_algo() { return; }
...
bool fake_trigger_algo::is_initialized() const { return _initialized_; }
...
void fake_trigger_algo::reset() {
  DT_THROW_IF(!is_initialized(), std::logic_error,
              "Fake trigger algo is not initialized, it can't be reset ! ");
  _initialized_ = false;
  return;
}
...
bool fake_trigger_algo::process(const mctools::simulated_data& SD_) {
  DT_THROW_IF(!is_initialized(), std::logic_error,
              "Fake trigger algo is not initialized, it can't process ! ");
...
bool fake_trigger_algo::_process(const mctools::simulated_data& SD_) {
  ...
  if (number_of_calorimeters >= 1 && number_of_geiger_cells >= 3)
    _status_for_a_SD_ = true;
  else
    _status_for_a_SD_ = false;
