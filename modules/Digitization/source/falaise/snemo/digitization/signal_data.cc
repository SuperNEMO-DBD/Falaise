...
signal_data::signal_data() { return; }
...
std::size_t signal_data::get_number_of_geiger_signals() const { return _geiger_signals_.size(); }
...
std::size_t signal_data::get_number_of_prompt_geiger_signals(double time_limit_) const {
  ...
  std::vector<datatools::handle<geiger_signal> >::const_iterator it_signal =
      _geiger_signals_.begin();
...
std::size_t signal_data::get_number_of_calo_signals() const { return _calo_signals_.size(); }
...
void signal_data::tree_dump(std::ostream& out_, const std::string& title_,
...
void signal_data::_check() {
  for (unsigned int i = 0; i < _geiger_signals_.size() - 1; i++) {
    ...
    for (unsigned int j = i + 1; j < _geiger_signals_.size(); j++) {
      ...
      DT_THROW_IF(geiger_signal_a.get_geom_id() == geiger_signal_b.get_geom_id(), std::logic_error,
  ...
  for (unsigned int i = 0; i < _calo_signals_.size() - 1; i++) {
    ...
    for (unsigned int j = i + 1; j < _calo_signals_.size(); j++) {
      ...
      DT_THROW_IF(calo_signal_a.get_geom_id() == calo_signal_b.get_geom_id(), std::logic_error,
