...
void geiger_ctw::set_header(int32_t hit_id_, const geomtools::geom_id& electronic_id_,
...
uint32_t geiger_ctw::get_clocktick_800ns() const { return _clocktick_800ns_; }
...
void geiger_ctw::get_100_bits_in_ctw_word(unsigned int block_index_,
                                          std::bitset<geiger::tp::FULL_SIZE>& my_bitset_) const {
  DT_THROW_IF(block_index_ > mapping::NUMBER_OF_FEBS_BY_CRATE, std::logic_error,
              "Block index out of range (should be [0;19])  ! ");
...
void geiger_ctw::set_100_bits_in_ctw_word(unsigned int block_index_,
...
void geiger_ctw::get_55_bits_in_ctw_word(unsigned int block_index_,
...
void geiger_ctw::set_55_bits_in_ctw_word(unsigned int block_index_,
...
void geiger_ctw::set_full_hardware_status(
    const std::bitset<geiger::tp::THWS_SIZE>& gg_tp_hardware_status_) {
  for (unsigned int i = geiger::tp::THWS_BEGIN; i <= CTW_BITSET_FULL_SIZE;
       i += geiger::tp::FULL_SIZE) {
...
void geiger_ctw::_set_full_board_id() {
  ...
  for (unsigned int i = geiger::tp::BOARD_ID_BIT0; i <= CTW_BITSET_FULL_SIZE;
       i += geiger::tp::FULL_SIZE) {
...
void geiger_ctw::set_full_crate_id(
    const std::bitset<geiger::tp::CRATE_ID_WORD_SIZE>& gg_tp_crate_id_) {
  for (unsigned int i = geiger::tp::CRATE_ID_BIT0; i <= CTW_BITSET_FULL_SIZE;
       i += geiger::tp::FULL_SIZE) {
...
bool geiger_ctw::is_valid() const { return _clocktick_800ns_ != clock_utils::INVALID_CLOCKTICK; }
...
void geiger_ctw::tree_dump(std::ostream& out_, const std::string& title_,
  ...
  out_ << indent_ << datatools::i_tree_dumpable::inherit_tag(inherit_) << "CTW (" << _gg_ctw_.size()
       << " bits) : " << std::endl;

  std::bitset<100> my_bitset;
  for (unsigned int i = 0; i < mapping::NUMBER_OF_FEBS_BY_CRATE; i++) {
    get_100_bits_in_ctw_word(i, my_bitset);
    out_ << indent_ << datatools::i_tree_dumpable::inherit_tag(inherit_) << my_bitset << std::endl;
  }
