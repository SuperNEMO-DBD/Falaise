...
signal_to_geiger_tp_algo::signal_to_tp_working_data::signal_to_tp_working_data() { reset(); }
...
bool signal_to_geiger_tp_algo::signal_to_tp_working_data::operator<(
    const signal_to_tp_working_data& other_) const {
...
void signal_to_geiger_tp_algo::initialize(electronic_mapping& my_electronic_mapping_) {
  DT_THROW_IF(is_initialized(), std::logic_error,
              "SD to geiger tp algorithm is already initialized ! ");
...
bool signal_to_geiger_tp_algo::is_initialized() const { return _initialized_; }

void signal_to_geiger_tp_algo::reset() {
  DT_THROW_IF(!is_initialized(), std::logic_error,
              "SD to geiger tp algorithm is not initialized, it can't be reset ! ");
  _initialized_ = false;
  _electronic_mapping_ = 0;
  _clocktick_ref_ = clock_utils::INVALID_CLOCKTICK;
  return;
...
void signal_to_geiger_tp_algo::add_geiger_tp(const signal_to_tp_working_data& my_wd_data_,
                                             uint32_t signal_clocktick_, int32_t hit_id_,
  ...
  gg_tp.set_header(hit_id_, temporary_feb_id, signal_clocktick_, mapping::THREE_WIRES_TRACKER_MODE,
                   mapping::SIDE_MODE, mapping::NUMBER_OF_CONNECTED_ROWS);
...
void signal_to_geiger_tp_algo::_prepare_working_data(const signal_data& signal_data_,
  ...
  DT_THROW_IF(!is_initialized(), std::logic_error,
              "SD to geiger TP algorithm is not initialized ! ");
  std::size_t number_of_hits = signal_data_.get_geiger_signals().size();
  double first_geiger_time_reference =
      signal_data_.get_geiger_signals()[0].get().get_anode_avalanche_time();

  for (std::size_t i = 0; i < number_of_hits; i++) {
    if (signal_data_.get_geiger_signals()[i].get().get_anode_avalanche_time() <
        first_geiger_time_reference) {
      first_geiger_time_reference =
          signal_data_.get_geiger_signals()[i].get().get_anode_avalanche_time();
    }
  }
  ...
  for (std::size_t i = 0; i < number_of_hits; i++) {
    ...
    _electronic_mapping_->convert_GID_to_EID(mapping::THREE_WIRES_TRACKER_MODE, geom_id,
                                             electronic_id);

    double relative_time = a_geiger_signal.get_anode_avalanche_time() - first_geiger_time_reference;
    uint32_t a_geiger_signal_clocktick = std::floor(first_geiger_time_reference / 800) +
                                         _clocktick_ref_ +
                                         clock_utils::TRACKER_FEB_SHIFT_CLOCKTICK_NUMBER;
...
void signal_to_geiger_tp_algo::_geiger_tp_process(
    const working_data_collection_type& wd_collection_, geiger_tp_data& my_geiger_tp_data_) {
  DT_THROW_IF(!is_initialized(), std::logic_error,
              "SD to geiger TP algorithm is not initialized ! ");
  ...
  for (unsigned int i = 0; i < wd_collection_.size(); i++) {
    ...
    for (unsigned int j = 0; j < clock_utils::ACTIVATED_GEIGER_CELLS_NUMBER; j++) {
      ...
      my_geiger_tp_data_.get_list_of_gg_tp_per_eid(wd_collection_[i].feb_id,
                                                   my_list_of_gg_tp_per_eid);
      ...
      if (my_list_of_gg_tp_per_eid.empty()) {
        ...
        while (existing_ct == false && iterator_list < my_list_of_gg_tp_per_eid.size()) {
          if (signal_clocktick ==
              my_list_of_gg_tp_per_eid[iterator_list].get().get_clocktick_800ns()) {
            existing_ct = true;
      ...
      // Eid is not existing or clocktick is different, geiger TP first creation
        ...
        add_geiger_tp(wd_collection_[i], signal_clocktick, geiger_tp_hit_id, my_geiger_tp_data_);
      }
      signal_clocktick++;
    }  // end of for (j < 10)
