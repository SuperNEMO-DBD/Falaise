...
void trigger_display_manager::initialize(const datatools::properties& config_) {
  DT_THROW_IF(is_initialized(), std::logic_error,
              "Trigger display manager is already initialized ! ");
  if (!is_calo_25ns()) {
...
bool trigger_display_manager::is_initialized() const { return _initialized_; }

void trigger_display_manager::reset() {
  DT_THROW_IF(!is_initialized(), std::logic_error,
              "Trigger display manager is not initialized, it can't be reset ! ");
  _initialized_ = false;
  _calo_25ns_ = false;
  _calo_1600ns_ = false;
  _tracker_1600ns_ = false;
...
bool trigger_display_manager::is_calo_25ns() const { return _calo_25ns_; }

bool trigger_display_manager::is_calo_1600ns() const { return _calo_1600ns_; }

bool trigger_display_manager::is_tracker_1600ns() const { return _tracker_1600ns_; }

bool trigger_display_manager::is_coinc_1600ns() const { return _coinc_1600ns_; }

bool trigger_display_manager::is_decision_trigger() const { return _decision_trigger_; }

void trigger_display_manager::fill_calo_trigger_matrix_25ns(
    std::bitset<10> zoning_word_[trigger_info::NSIDES]) {
      ...
      if (izone == 0 || izone == 9) {
        if (zoning_word_[iside][izone] == true) {
          for (uint32_t j = 0; j < NUMBER_OF_HORIZONTAL_CHAR; j++) {
            if (iside == 0 && izone == 0 && j > 3 && j < 11)
              _char_matrix_[0][j] = '*';
            else if (iside == 1 && izone == 0 && j > 3 && j < 11)
              _char_matrix_[20][j] = '*';
            else if (iside == 0 && izone == 9 && j > 107 && j < 115)
              _char_matrix_[0][j] = '*';
            else if (iside == 1 && izone == 9 && j > 107 && j < 115)
              _char_matrix_[20][j] = '*';
          }
        } else {
          for (uint32_t j = 0; j < NUMBER_OF_HORIZONTAL_CHAR; j++) {
        ...
        if (zoning_word_[iside][izone] == true) {
          for (uint32_t j = 0; j < NUMBER_OF_HORIZONTAL_CHAR; j++) {
            if (iside == 0 && j > 60 && j < 70)
              _char_matrix_[0][j] = '*';
            else if (iside == 1 && j > 60 && j < 70)
              _char_matrix_[20][j] = '*';
          }
        } else {
          for (uint32_t j = 0; j < NUMBER_OF_HORIZONTAL_CHAR; j++) {
        ...
        if (zoning_word_[iside][izone] == true) {
          for (uint32_t j = 0; j < NUMBER_OF_HORIZONTAL_CHAR; j++) {
            if (iside == 0 && izone == 1 && j > 12 && j < 23)
              _char_matrix_[0][j] = '*';
            else if (iside == 1 && izone == 1 && j > 12 && j < 23)
              _char_matrix_[20][j] = '*';
            else if (iside == 0 && izone == 2 && j > 24 && j < 35)
              _char_matrix_[0][j] = '*';
            else if (iside == 1 && izone == 2 && j > 24 && j < 35)
              _char_matrix_[20][j] = '*';
            else if (iside == 0 && izone == 3 && j > 36 && j < 47)
              _char_matrix_[0][j] = '*';
            else if (iside == 1 && izone == 3 && j > 36 && j < 47)
              _char_matrix_[20][j] = '*';
            else if (iside == 0 && izone == 4 && j > 48 && j < 59)
              _char_matrix_[0][j] = '*';
            else if (iside == 1 && izone == 4 && j > 48 && j < 59)
              _char_matrix_[20][j] = '*';
            else if (iside == 0 && izone == 6 && j > 71 && j < 82)
              _char_matrix_[0][j] = '*';
            else if (iside == 1 && izone == 6 && j > 71 && j < 82)
              _char_matrix_[20][j] = '*';
            else if (iside == 0 && izone == 7 && j > 83 && j < 94)
              _char_matrix_[0][j] = '*';
            else if (iside == 1 && izone == 7 && j > 83 && j < 94)
              _char_matrix_[20][j] = '*';
            else if (iside == 0 && izone == 8 && j > 95 && j < 106)
              _char_matrix_[0][j] = '*';
            else if (iside == 1 && izone == 8 && j > 95 && j < 106)
              _char_matrix_[20][j] = '*';
          }
        } else {
          for (uint32_t j = 0; j < NUMBER_OF_HORIZONTAL_CHAR; j++) {
...
void trigger_display_manager::fill_calo_trigger_matrix_1600ns(
    std::bitset<10> zoning_word_[trigger_info::NSIDES]) {
  for (unsigned int iside = 0; iside < trigger_info::NSIDES; iside++) {
    for (unsigned int izone = 0; izone < trigger_info::NZONES; izone++) {
      if (izone == 0 || izone == 9) {
...
void trigger_display_manager::fill_tracker_trigger_matrix_1600ns(
    bool geiger_matrix_[trigger_info::NSIDES][trigger_info::NLAYERS][trigger_info::NROWS]) {
  for (unsigned int iside = 0; iside < trigger_info::NSIDES; iside++) {
    unsigned int layer = 0;
    for (unsigned int jlayer = trigger_info::NLAYERS - 1; jlayer != (unsigned)0 - 1;
         jlayer--)  // Value GEIGER_LAYER_SIZE = 9
    {
      for (unsigned int krow = 0; krow < trigger_info::NROWS; krow++) {
        if (geiger_matrix_[iside][jlayer][krow] && iside == 0) {
          _char_matrix_[layer + 1][krow + 3] = '*';
        } else if (geiger_matrix_[iside][jlayer][krow] && iside == 1) {
...
void trigger_display_manager::fill_coincidence_trigger_matrix_1600ns(
    std::bitset<10> zoning_word_[trigger_info::NSIDES],
    bool geiger_matrix_[trigger_info::NSIDES][trigger_info::NLAYERS][trigger_info::NROWS]) {
...
void trigger_display_manager::display_calo_trigger_25ns(trigger_algorithm& a_trigger_algo_,
  ...
  DT_THROW_IF(!is_calo_25ns(), std::logic_error,
              "Boolean calo 25ns is not activated, it can't display ! ");
  ...
  for (unsigned int i = 0; i < a_trigger_algo_._calo_records_.size(); i++) {
    if (clocktick_25ns_ == a_trigger_algo_._calo_records_[i].clocktick_25ns)
      a_calo_summary_record = a_trigger_algo_._calo_records_[i];
  }
  ...
  std::clog
      ...
      << std::endl;
  std::clog
      << "********************* Display Calorimeter trigger info @ 25 ns *********************"
      << std::endl;
  std::clog << "********************* Clocktick 25 ns = " << clocktick_25ns_
            << " ********************* " << std::endl
            << std::endl;
  ...
  std::clog << "Single Side coinc : " << a_calo_summary_record.single_side_coinc
            ...
            << "  |  Trigger final decision : " << a_calo_summary_record.calo_finale_decision
            << std::endl;
...
void trigger_display_manager::display_calo_trigger_25ns(trigger_algorithm& a_trigger_algo_) {
  ...
  if (vector_size == 0) {
    ...
    for (uint32_t iclocktick = clocktick_25ns_begin; iclocktick <= clocktick_25ns_end;
         iclocktick++) {
      reset_calo_display();
...
void trigger_display_manager::display_calo_trigger_1600ns(trigger_algorithm& a_trigger_algo_,
  ...
  DT_THROW_IF(!is_calo_1600ns(), std::logic_error,
              "Boolean calo 1600ns is not activated, it can't display ! ");

  coincidence_trigger_algorithm::coincidence_calo_record a_coinc_calo_record;
  for (unsigned int i = 0; i < a_trigger_algo_._coinc_algo_._coincidence_calo_records_.size();
       i++) {
    if (clocktick_1600ns_ ==
        a_trigger_algo_._coinc_algo_._coincidence_calo_records_[i].clocktick_1600ns)
      a_coinc_calo_record = a_trigger_algo_._coinc_algo_._coincidence_calo_records_[i];
  }
  ...
  std::clog
      ...
      << std::endl;
  std::clog
      << "******************** Display Calorimeter trigger info @ 1600 ns ********************"
      << std::endl;
  std::clog << "******************** Clocktick 1600 ns = " << clocktick_1600ns_
            << " ******************** " << std::endl
            << std::endl;
  std::clog << "CT |XTS|L|HG|L|L|H1|H0| ZONING S1| ZONING S0 " << std::endl;
...
void trigger_display_manager::display_calo_trigger_1600ns(trigger_algorithm& a_trigger_algo_) {
  ...
  if (vector_size == 0) {
    ...
    uint32_t clocktick_1600ns_begin =
        a_trigger_algo_._coinc_algo_._coincidence_calo_records_[0].clocktick_1600ns;
    uint32_t clocktick_1600ns_end =
        a_trigger_algo_._coinc_algo_._coincidence_calo_records_[vector_size - 1].clocktick_1600ns;

    for (uint32_t iclocktick = clocktick_1600ns_begin; iclocktick <= clocktick_1600ns_end;
         iclocktick++) {
...
void trigger_display_manager::display_tracker_trigger_1600ns(trigger_algorithm& a_trigger_algo_,
                                                             uint32_t clocktick_1600ns_) {
  DT_THROW_IF(!is_tracker_1600ns(), std::logic_error,
              "Boolean tracker 1600ns is not activated, it can't display ! ");
  ...
  for (unsigned int i = 0; i < a_trigger_algo_._tracker_records_.size(); i++) {
    if (clocktick_1600ns_ == a_trigger_algo_._tracker_records_[i].clocktick_1600ns &&
        clocktick_1600ns_ ==
            a_trigger_algo_._tracker_algo_._geiger_matrix_records_[i].clocktick_1600ns) {
  ...
  //   std::clog <<
  //   "********************************************************************************" <<
  //   std::endl; std::clog << "******************** Display Tracker trigger info @ 1600 ns
  //   ********************" << std::endl; std::clog << "******************** Clocktick 1600 ns = "
  //   << clocktick_1600ns_ << " ******************** " << std::endl << std::endl; std::clog <<
  //   "Bitset : [NSZL NSZR L M R O I] " << std::endl; for (unsigned int iside = 0; iside <
  //   trigger_info::NSIDES; iside++)
  //     {
  //       std::clog << "Side = " << iside << " | ";
  //       for (unsigned int izone = 0; izone < trigger_info::NZONES; izone++)
  // 	{
  ...
  //       for (unsigned int ibit = 0; ibit < a_tracker_record.zoning_word_pattern[0].size();
  //       ibit++)
  // 	{
  ...
  //       for (unsigned int ibit = 0; ibit < a_tracker_record.zoning_word_near_source[0].size();
  //       ibit++)
  // 	{
  ...
  //   std::clog << "Tracker level one decision : [" << a_tracker_record.finale_decision << "]" <<
  //   std::endl << std::endl; display_matrix();
  // }

  else {
    std::clog << "********************************************************************************"
              << std::endl;
    std::clog << "******************** Display Tracker trigger info @ 1600 ns ********************"
              << std::endl;
    std::clog << "******************** Clocktick 1600 ns = " << clocktick_1600ns_
              << " ******************** " << std::endl
              << std::endl;
...
void trigger_display_manager::display_coincidence_trigger_1600ns(trigger_algorithm& a_trigger_algo_,
                                                                 uint32_t clocktick_1600ns_) {
  DT_THROW_IF(!is_coinc_1600ns(), std::logic_error,
              "Boolean coinc 1600ns is not activated, it can't display ! ");
  ...
  for (unsigned int i = 0; i < a_trigger_algo_._coincidence_records_.size(); i++) {
    if (clocktick_1600ns_ == a_trigger_algo_._coincidence_records_[i].clocktick_1600ns)
  ...
  std::clog
      ...
      << std::endl;
  std::clog
      << "******************** Display Coincidence trigger info @ 1600 ns ********************"
      << std::endl;
  std::clog << "******************** Clocktick 1600 ns = " << clocktick_1600ns_
            << " ******************** " << std::endl
            << std::endl;
  ...
  std::clog << "Coincidence Clocktick : [" << a_coincidence_record.clocktick_1600ns << "]"
            << std::endl;
  std::clog << "Coincidence zoning : S0 [" << a_coincidence_record.zoning_word[0] << "]"
            << std::endl;
  std::clog << "                     S1 [" << a_coincidence_record.zoning_word[1] << "]"
            << std::endl;

  std::clog << "Coincidence trigger mode : [" << a_coincidence_record.trigger_mode << "]"
            << std::endl;
  std::clog << "Coincidence event record decision  : [" << a_coincidence_record.decision << "]"
            << std::endl
            << std::endl;
...
void trigger_display_manager::display_coincidence_trigger_1600ns(
    trigger_algorithm& a_trigger_algo_) {
...
void trigger_display_manager::display_decision_trigger() {
  DT_THROW_IF(!is_decision_trigger(), std::logic_error,
              "Boolean decision trigger is not activated, it can't display ! ");
  return;
}
    ...
    for (unsigned int j = 0; j < NUMBER_OF_HORIZONTAL_CHAR; j++) {
      if (i == 0 || i == 20) {
        if (j == 0 || j == 1)
          ...
          _char_matrix_[i][j] = '|';
        else if (j == 3 || j == 12 || j == 24 || j == 36 || j == 48 || j == 60 || j == 71 ||
                 j == 83 || j == 95 || j == 107)
          _char_matrix_[i][j] = '[';
        else if (j == 11 || j == 23 || j == 35 || j == 47 || j == 59 || j == 70 || j == 82 ||
                 j == 94 || j == 106 || j == 115)
          _char_matrix_[i][j] = ']';
        else
          _char_matrix_[i][j] = ' ';
...
void trigger_display_manager::display_matrix() {
  std::clog << "     Zone0      Zone1       Zone2       Zone3       Zone4      Zone5       Zone6   "
               "    Zone7      Zone 8     Zone9 "
            << std::endl;
  for (unsigned int i = 0; i < NUMBER_OF_VERTICAL_CHAR; i++) {
    for (unsigned int j = 0; j < NUMBER_OF_HORIZONTAL_CHAR; j++) {
