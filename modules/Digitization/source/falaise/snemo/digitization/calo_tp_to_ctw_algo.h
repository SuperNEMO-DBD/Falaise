...
/// \brief Algorithm processing. Take a calo_tp_data object (collection of calo_tp) in input used to
/// fill a calo_ctw_data (collection of calo crate trigger word) in output.
class calo_tp_to_ctw_algo {
 public:
  enum mode_type { MODE_UNDEFINED = -1, MODE_MAIN_WALL = 0, MODE_XWALL_GVETO = 1 };
 ...
 protected:
  ...
  void _process_for_a_ctw_for_a_clocktick_for_main_wall(
  ...
  void _process_for_a_ctw_for_a_clocktick_for_xwall_gveto(
      const std::vector<datatools::handle<calo_tp> >& my_list_of_calo_tp_, calo_ctw& a_calo_ctw_);
