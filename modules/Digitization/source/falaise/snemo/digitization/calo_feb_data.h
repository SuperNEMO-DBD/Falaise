...
class calo_feb_data : public geomtools::base_hit {
 ...
 public:
  enum desc_bit_index { WAVEFORM_STATUS = 0 };

  enum treshold_bit_index { LOW_TRESHOLD = 0, HIGH_TRESHOLD = 1 };
  ...
  inline uint16_t get_time_max_per_channel(int channel_) {
    return _time_max_peak_amplitude[channel_];
  }
  ...
  inline bool has_low_treshsold_per_channel(int channel_) {
    return _treshold_status_[channel_].test(LOW_TRESHOLD);
  }
  inline bool has_high_treshsold_per_channel(int channel_) {
    return _treshold_status_[channel_].test(HIGH_TRESHOLD);
  }

  // void set_header(int32_t id_,
  // 								const geomtools::geom_id &
  // electronic_id_, 								uint64_t data_time_
  // 								);

  void set_header(int32_t id_, uint16_t rack_, uint16_t crate_, uint16_t board_, uint16_t samlong_,
                  uint64_t data_time_);

  /// Set the metadata with valid values for both channels
  void set_metadata(double *baseline_array_, double *charge_array_, double *max_array_,
                    uint16_t *time_max_array_, double *falling_array_, double *rising_array_);
 ...
 private:
  ...
  // HEADER
  // id_ + _data_timestamp + _threshold_status_ + _data_description_ + address(_rack_id,
  // _crate_id,_board_id,_samlong_id)
