...
void trigger_algorithm_test_time::set_electronic_mapping(
...
void trigger_algorithm_test_time::set_clock_manager(const clock_utils& my_clock_manager_) {
  DT_THROW_IF(is_initialized(), std::logic_error,
              "Trigger algorithm is already initialized, clock manager can't be set ! ");
...
void trigger_algorithm_test_time::set_calorimeter_gate_size(unsigned int calorimeter_gate_size_) {
  DT_THROW_IF(is_initialized(), std::logic_error,
              "Trigger algorithm is already initialized, coincidence calorimeter gate size can't "
              "be set ! ");
...
void trigger_algorithm_test_time::set_L2_decision_coincidence_gate_size(
    unsigned int L2_decision_coincidence_gate_size_) {
  DT_THROW_IF(is_initialized(), std::logic_error,
              "Trigger algorithm is already initialized, coincidence calorimeter gate size can't "
              "be set ! ");
...
void trigger_algorithm_test_time::set_previous_event_buffer_depth(
    unsigned int previous_event_circular_buffer_depth_) {
  DT_THROW_IF(
      is_initialized(), std::logic_error,
      "Trigger algorithm is already initialized, previous event buffer depth can't be set ! ");
...
void trigger_algorithm_test_time::initialize(const datatools::properties& config_) {
  ...
  if (!has_calorimeter_gate_size()) {
    if (config_.has_key("coincidence_calorimeter_gate_size")) {
      int coincidence_calorimeter_gate_size =
          config_.fetch_integer("coincidence_calorimeter_gate_size");
      DT_THROW_IF(coincidence_calorimeter_gate_size <= 0, std::domain_error,
                  "Invalid value of coincidence_calorimeter_gate_size !");
  ...
  if (!has_L2_decision_coincidence_gate_size()) {
    if (config_.has_key("L2_decision_coincidence_gate_size")) {
      int L2_decision_coincidence_gate_size =
          config_.fetch_integer("L2_decision_coincidence_gate_size");
      DT_THROW_IF(L2_decision_coincidence_gate_size <= 0, std::domain_error,
                  "Invalid value of L2_decision_coincidence_gate_size !");
...
bool trigger_algorithm_test_time::is_initialized() const { return _initialized_; }

const std::vector<trigger_structures::calo_summary_record>
trigger_algorithm_test_time::get_calo_records_25ns_vector() const {
  return _calo_records_25ns_;
}

const std::vector<trigger_structures::coincidence_calo_record>
trigger_algorithm_test_time::get_coincidence_calo_records_1600ns_vector() const {
  return _coincidence_calo_records_1600ns_;
}

const std::vector<trigger_structures::tracker_record>
trigger_algorithm_test_time::get_tracker_records_vector() const {
  return _tracker_records_;
}

const std::vector<trigger_structures::coincidence_event_record>
trigger_algorithm_test_time::get_coincidence_records_vector() const {
  return _coincidence_records_;
}

const std::vector<trigger_structures::L1_calo_decision>
trigger_algorithm_test_time::get_L1_calo_decision_records_vector() const {
  return _L1_calo_decision_records_;
}

const std::vector<trigger_structures::L2_decision>
trigger_algorithm_test_time::get_L2_decision_records_vector() const {
  return _L2_decision_records_;
}

bool trigger_algorithm_test_time::get_finale_decision() const { return _finale_trigger_decision_; }

void trigger_algorithm_test_time::_rescale_calo_records_at_1600ns(
    const std::vector<trigger_structures::calo_summary_record>& calo_records_25_ns_,
    std::vector<trigger_structures::coincidence_calo_record>& coincidence_calo_records_1600ns_) {
  std::vector<trigger_structures::calo_summary_record>::const_iterator it =
      calo_records_25_ns_.begin();
  for (; it != calo_records_25_ns_.end(); it++) {
    ...
    if (a_ctrec.calo_finale_decision == true) {
      ...
      _clock_manager_->compute_clocktick_25ns_to_1600ns(a_ctrec.clocktick_25ns,
                                                        ctrec_clocktick_1600ns);
        ...
        if (ctrec_clocktick_1600ns == coincidence_calo_records_1600ns_[i].clocktick_1600ns) {
          ...
          if (i != coincidence_calo_records_1600ns_.size() - 1) {
            for (unsigned int j = i + 1; j < coincidence_calo_records_1600ns_.size(); j++) {
              if (coincidence_calo_records_1600ns_[j].clocktick_1600ns <=
                  coincidence_calo_records_1600ns_[i].clocktick_1600ns +
                      _coincidence_calorimeter_gate_size_) {
                uint32_t clocktick_1600_before_modification =
                    coincidence_calo_records_1600ns_[j].clocktick_1600ns;
                coincidence_calo_records_1600ns_[j] = coincidence_calo_records_1600ns_[i];
                coincidence_calo_records_1600ns_[j].clocktick_1600ns =
                    clocktick_1600_before_modification;
              }
            }  // end of for j
          }
          ...
          uint32_t clocktick_max_1600 =
              coincidence_calo_records_1600ns_[index_max].clocktick_1600ns;

          for (unsigned int j = clocktick_max_1600 + 1; j <= clocktick_to_create; j++) {
            bool no_same_clocktick = true;
            for (unsigned int k = actual_index + 1; k < coincidence_calo_records_1600ns_.size();
                 k++) {
            ...
            if (no_same_clocktick == true) {
              trigger_structures::coincidence_calo_record grow_back_coincidence_calo_record =
                  coincidence_calo_records_1600ns_[index_max];
      ...
      if (coinc_calo_record_find == false) {
        ...
        for (unsigned int iclocktick = new_coincidence_calo_record.clocktick_1600ns + 1;
             iclocktick <
...
void trigger_algorithm_test_time::_update_coinc_calo_record(
    const trigger_structures::calo_summary_record& a_calo_summary_record_25ns_,
  ...
  std::bitset<trigger_info::CALO_ZONING_PER_SIDE_BITSET_SIZE> zoning_word_side_0 =
      a_coinc_calo_record_1600ns_.calo_zoning_word[0];
  std::bitset<trigger_info::CALO_ZONING_PER_SIDE_BITSET_SIZE> zoning_word_side_1 =
      a_coinc_calo_record_1600ns_.calo_zoning_word[1];
  std::bitset<trigger_info::CALO_XT_INFO_BITSET_SIZE> xt_info =
      a_coinc_calo_record_1600ns_.xt_info_bitset;
...
void trigger_algorithm_test_time::_build_previous_event_record() {
  // A PER is created or updated only if a coincidence between tracker and calorimeter is ok (so
  // CALO_TRACKER_TIME_COINC or CARACO)
  ...
  DT_THROW_IF(the_L2_decision.L2_trigger_mode != trigger_structures::L2_trigger_mode::CARACO &&
                  the_L2_decision.L2_trigger_mode !=
                      trigger_structures::L2_trigger_mode::CALO_TRACKER_TIME_COINC,
              std::logic_error,
              "Previous event record can't be build because L2 decision is not equal to CARACO or "
              "CALO_TRACKER_TIME_COINC !");

  // Construct the PER on the coincidence records pushed back (to have the maximum of calo / tracker
  // information (concatenate)) :
  ...
  for (unsigned int i = 0; i < _coincidence_records_.size(); i++) {
    ...
    // Check if the coincidence record is in the L2 gate and with the good trigger mode to construct
    // the PER :
    if (a_coincidence_record.clocktick_1600ns >= L2_decision_clocktick &&
        a_coincidence_record.clocktick_1600ns <
            (L2_decision_clocktick + _L2_decision_coincidence_gate_size_) &&
        (a_coincidence_record.trigger_mode == trigger_structures::L2_trigger_mode::CARACO ||
         a_coincidence_record.trigger_mode ==
             trigger_structures::L2_trigger_mode::CALO_TRACKER_TIME_COINC)) {
      ...
      for (unsigned int iside = 0; iside < trigger_info::NSIDES; iside++) {
        for (unsigned int ibit = 0; ibit < trigger_info::CALO_ZONING_PER_SIDE_BITSET_SIZE; ibit++) {
          if (a_previous_event_record.calo_zoning_word[iside].test(ibit) ||
              a_coincidence_record.calo_zoning_word[iside].test(ibit)) {
      ...
      unsigned int multiplicity_side_0 =
          a_previous_event_record.total_multiplicity_side_0.to_ulong();
      unsigned int multiplicity_side_1 =
          a_previous_event_record.total_multiplicity_side_1.to_ulong();
      ...
      unsigned int number_of_zone_touch_side_0 =
          a_previous_event_record.calo_zoning_word[0].count();
      unsigned int number_of_zone_touch_side_1 =
          a_previous_event_record.calo_zoning_word[1].count();
      ...
      if (multiplicity_side_0 < a_coincidence_record.total_multiplicity_side_0.to_ulong()) {
        a_previous_event_record.total_multiplicity_side_0 =
            a_coincidence_record.total_multiplicity_side_0.to_ulong();
      }
      ...
      if (multiplicity_side_1 < a_coincidence_record.total_multiplicity_side_1.to_ulong()) {
        a_previous_event_record.total_multiplicity_side_1 =
            a_coincidence_record.total_multiplicity_side_1.to_ulong();
      }
      ...
      if (multiplicity_gveto < a_coincidence_record.total_multiplicity_gveto.to_ulong()) {
        a_previous_event_record.total_multiplicity_gveto =
            a_coincidence_record.total_multiplicity_gveto.to_ulong();
      }
      ...
      if (single_side || a_coincidence_record.single_side_coinc)
        a_previous_event_record.single_side_coinc = true;
      else if (!single_side || !a_coincidence_record.single_side_coinc)
        a_previous_event_record.single_side_coinc = false;
      if (total_multiplicity_threshold || a_coincidence_record.total_multiplicity_threshold)
      ...
      for (unsigned int iside = 0; iside < trigger_info::NSIDES; iside++) {
        for (unsigned int izone = 0; izone < trigger_info::NZONES; izone++) {
          for (unsigned int ibit = 0; ibit < trigger_info::DATA_FULL_BITSET_SIZE; ibit++) {
            if (a_previous_event_record.tracker_finale_data_per_zone[iside][izone].test(ibit) ||
                a_coincidence_record.tracker_finale_data_per_zone[iside][izone].test(ibit)) {
              a_previous_event_record.tracker_finale_data_per_zone[iside][izone].set(ibit);
            }
          }  // end of ibit
          if (a_coincidence_record.coincidence_zoning_word[iside].test(izone) ||
              a_previous_event_record.coincidence_zoning_word[iside].test(izone)) {
            a_previous_event_record.coincidence_zoning_word[iside].set(izone);
          }
          if (a_coincidence_record.tracker_zoning_word_pattern[iside].test(izone) ||
              a_previous_event_record.tracker_zoning_word_pattern[iside].test(izone)) {
            a_previous_event_record.tracker_zoning_word_pattern[iside].set(izone);
          }
          if (a_coincidence_record.tracker_zoning_word_near_source[iside].test(izone) ||
              a_previous_event_record.tracker_zoning_word_near_source[iside].test(izone)) {
...
void trigger_algorithm_test_time::_process(const calo_ctw_data& calo_ctw_data_,
  ...
  for (unsigned int i = 0; i < geiger_ctw_data_1600ns.get_geiger_ctws().size(); i++) {
    ...
    if (a_gg_ctw.get_clocktick_800ns() % 2 == 1) {
      geiger_ctw_data_1600ns.grab_geiger_ctws().erase(
          geiger_ctw_data_1600ns.grab_geiger_ctws().begin() + i);
    }
  }
  ...
  // Warning Clocktick of gg ctw is not at 800 ns but at 1600 ns (only even CT 800 are kept (1 of
  // 2))
  for (unsigned int i = 0; i < geiger_ctw_data_1600ns.get_geiger_ctws().size(); i++) {
  ...
  _calo_algo_.process(calo_ctw_data_, _calo_records_25ns_);
  ...
  for (unsigned int i = 0; i < _calo_records_25ns_.size(); i++) {
    ...
    if ((last_calo_ct_25ns == clock_utils::INVALID_CLOCKTICK ||
         calo_record_ct_25ns != last_calo_ct_25ns + 1) &&
        a_calo_record_25ns.calo_finale_decision == true) {
  ...
  // Any coincidences are activated and calo only is not activated, rescaling calorimeter at 1600 ns
  // in any case
  else if (_activate_any_coincidences_ && !_activate_calorimeter_only_) {
    _rescale_calo_records_at_1600ns(_calo_records_25ns_, _coincidence_calo_records_1600ns_);
    ...
    if (calorimeter_ct_min_1600 != clock_utils::INVALID_CLOCKTICK)
      clocktick_min = calorimeter_ct_min_1600;
    else if (tracker_ct_min_1600 != clock_utils::INVALID_CLOCKTICK)
      clocktick_min = tracker_ct_min_1600;

    if ((tracker_ct_min_1600 != clock_utils::INVALID_CLOCKTICK &&
         calorimeter_ct_min_1600 != clock_utils::INVALID_CLOCKTICK) &&
        tracker_ct_min_1600 < clocktick_min)
      clocktick_min = tracker_ct_min_1600;

    uint32_t clocktick_max = clock_utils::INVALID_CLOCKTICK;
    if (tracker_ct_max_1600 != clock_utils::INVALID_CLOCKTICK)
      clocktick_max = tracker_ct_max_1600;
    else if (calorimeter_ct_max_1600 != clock_utils::INVALID_CLOCKTICK)
      clocktick_max = calorimeter_ct_max_1600;
    if ((tracker_ct_max_1600 != clock_utils::INVALID_CLOCKTICK &&
         calorimeter_ct_max_1600 != clock_utils::INVALID_CLOCKTICK) &&
        calorimeter_ct_max_1600 > clocktick_max)
      clocktick_max = calorimeter_ct_max_1600;
    ...
    if (clocktick_min != clock_utils::INVALID_CLOCKTICK &&
      ...
      // Maybe prepare tracker record outside this loop but it breaks the time implementation (close
      // to the electronics)
      ...
      for (uint32_t ict1600 = clocktick_min; ict1600 <= clocktick_max; ict1600++) {
        ...
        if (!_previous_event_records_->empty()) {
          // _previous_event_records_->back().counter_1600ns =
          // clock_utils::PREVIOUS_EVENT_RECORD_LIVING_NUMBER_OF_CLOCKTICK - ict1600 +
          // _previous_event_records_->back().previous_clocktick_1600ns; std::clog << "CT PER = " <<
          // _previous_event_records_->back().previous_clocktick_1600ns << " counter = " <<
          ...
          if (per_to_delete.first) {
            if (_previous_event_records_->size() > 1) {
              _previous_event_records_->erase(_previous_event_records_->begin() +
                                              per_to_delete.second);
          ...
          for (; it_circ != _previous_event_records_->end(); it_circ++) {
            DT_THROW_IF(it_circ->counter_1600ns >
                            clock_utils::PREVIOUS_EVENT_RECORD_LIVING_NUMBER_OF_CLOCKTICK,
                        std::logic_error,
                        "Previous Event Record counter out of bounds "
                        "[0;clock_utils::PREVIOUS_EVENT_RECORD_LIVING_NUMBER_OF_CLOCKTICK (625)] "
                        "!");
            if (it_circ->counter_1600ns <=
                clock_utils::PREVIOUS_EVENT_RECORD_LIVING_NUMBER_OF_CLOCKTICK) {
              int counter_result = clock_utils::PREVIOUS_EVENT_RECORD_LIVING_NUMBER_OF_CLOCKTICK -
                                   ict1600 + it_circ->previous_clocktick_1600ns;
              if (counter_result <= 0) counter_result = 0;
              it_circ->counter_1600ns = counter_result;
              // std::clog << "CT PER = " << it_circ->previous_clocktick_1600ns << " counter = " <<
              // it_circ->counter_1600ns << std::endl;
            }
        ...
        // std::clog << "Size of PERs = " << _previous_event_records_->size() << " Empty : " <<
        // _previous_event_records_->empty() << std::endl;
        ...
        if (geiger_ctw_data_1600ns.get_geiger_ctws().size() != 0) {
          ...
          geiger_ctw_data_1600ns.get_list_of_geiger_ctw_per_clocktick(
              ict1600, geiger_ctw_list_per_clocktick_1600);
          _tracker_algo_.process(geiger_ctw_list_per_clocktick_1600, a_tracker_record);
          ...
          trigger_structures::geiger_matrix a_geiger_matrix =
              _tracker_algo_.get_geiger_matrix_for_a_clocktick();
        ...
        std::vector<trigger_structures::coincidence_calo_record>::const_iterator it_calo =
            _coincidence_calo_records_1600ns_.begin();
        ...
        if (!a_coinc_calo_record_for_pair.is_empty() || !a_tracker_record.is_empty()) {
          std::pair<trigger_structures::coincidence_calo_record, trigger_structures::tracker_record>
          ...
          if (!_L2_decision_records_.empty() &&
              (int)_L2_decision_records_.back().L2_ct_decision >=
                  (int)((ict1600 - _L2_decision_coincidence_gate_size_)) &&
          ...
          _coinc_algo_.process(pair_for_a_clocktick, a_coincidence_event_record, a_L2_decision,
          ...
          if (a_coincidence_event_record.clocktick_1600ns != clock_utils::INVALID_CLOCKTICK &&
              ...
              a_coincidence_event_record.trigger_mode !=
                  trigger_structures::L2_trigger_mode::INVALID) {
            _coincidence_records_.push_back(a_coincidence_event_record);
          }

          // L2 decision to push back (not at each clocktick). Only if there is no other L2 decision
          // in the L2 decision gate [CTi-5; CTi]
        ...
        if (!_L2_decision_records_.empty()) {
          // std::clog << "L2 ct + L2 decision = " << _L2_decision_records_.back().L2_ct_decision +
          // _L2_decision_coincidence_gate_size_ << " ict = " << ict1600 << std::endl;

          trigger_structures::L2_decision the_L2_decision = _L2_decision_records_.back();

          if (ict1600 == (_L2_decision_records_.back().L2_ct_decision +
                          _L2_decision_coincidence_gate_size_) &&
  ...
  // boost::circular_buffer<trigger_structures::previous_event_record>::iterator it_circ =
  // _previous_event_records_->begin(); for (; it_circ != _previous_event_records_->end();
  // it_circ++)
  // 	{
  // 	  it_circ -> display();
  // 	}
  ...
  // std::clog << "Calo collection size @ 25 ns            : " << _calo_records_25ns_.size() <<
  // std::endl; std::clog << "Calo collection size @ 1600 ns          : " <<
  // _coincidence_calo_records_1600ns_.size() << std::endl; std::clog << "Tracker collection size @
  // 1600 ns       : " << _tracker_records_.size() << std::endl; std::clog << "Geiger matrix
  // collection size @ 1600 ns : " << _geiger_matrix_records_.size() << std::endl; std::clog <<
  // "Pair records collection size @ 1600 ns  : " << _pair_records_.size() << std::endl; std::clog
  // << "Coincidence collection size @ 1600 ns   : " << _coincidence_records_.size() << std::endl;
  // std::clog << "Previous event collection size          : " << _previous_event_records_->size()
  // << std::endl; std::clog << "L1 calo collection size @ 25 ns         : " <<
  // _L1_calo_decision_records_.size() << std::endl; std::clog << "L2 decision collection size @
  // 1600 ns   : " << _L2_decision_records_.size() << std::endl;
