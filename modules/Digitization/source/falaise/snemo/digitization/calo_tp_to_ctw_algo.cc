...
namespace snemo {
  ...
  namespace digitization {
    ...
    calo_algo_tp_to_ctw::calo_algo_tp_to_ctw()
    {
      return;
    }
    ...
    void calo_algo_tp_to_ctw::initialize(const calo_tp_data & my_tp_data_, calo_ctw_data & my_ctw_data_)
    {
      return;
    }

    void calo_algo_tp_to_ctw::set_ctw_clocktick(const calo_tp & my_calo_tp_, calo_ctw & my_ctw_)
    {
      my_ctw_.set_clocktick_25ns(my_calo_tp_.get_clocktick_25ns());
      return;
    }

    void calo_algo_tp_to_ctw::set_ctw_htm(const calo_tp & my_calo_tp_, calo_ctw & my_ctw_)
    {
    ...
    void calo_algo_tp_to_ctw::set_ctw_zone_bit(const calo_tp & my_calo_tp_, calo_ctw & my_ctw_)
    {     
    ...
    void calo_algo_tp_to_ctw::set_ctw_lto(const calo_tp & my_calo_tp_, calo_ctw & my_ctw_)
    {    
    ...
    void calo_algo_tp_to_ctw::process(const calo_tp_data & tp_data_,  calo_ctw_data & ctw_data_)
    { 
      for(int32_t i = tp_data_.get_clocktick_min(); i <= tp_data_.get_clocktick_max(); i++)
	{
	  for(int j = 0 ; j < 3 ; j++) //crate number
	      ...
	      if(!tp_list_per_clocktick_per_crate.empty())
		{
		  ...
		  process_for_a_ctw_for_a_clocktick(tp_list_per_clocktick_per_crate,a_ctw_);
    ...
    void calo_algo_tp_to_ctw::process_for_a_ctw_for_a_clocktick(const std::vector<datatools::handle<calo_tp> > & my_list_of_calo_tp_,  calo_ctw & a_calo_ctw_) 
    {       
      ...
      for(int i = 0; i < my_list_of_calo_tp_.size(); i++)
	{
	  ...
	  fill_a_calo_ctw(my_calo_tp, a_calo_ctw_);	  
	}
      std::clog << "DEBUG : process_for_a_ctw/clocktick number of TP : " << my_list_of_calo_tp_.size() << std::endl;
      return;
    }

    void calo_algo_tp_to_ctw::fill_a_calo_ctw(const calo_tp & my_calo_tp_, calo_ctw & a_calo_ctw_)
    {  
      std::clog << "DEBUG : set_ctw_clock : " << my_calo_tp_.get_clocktick_25ns() << std::endl;
