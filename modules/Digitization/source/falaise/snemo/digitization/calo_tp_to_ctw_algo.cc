...
bool calo_tp_to_ctw_algo::is_initialized() const { return _initialized_; }
...
void calo_tp_to_ctw_algo::reset() {
  DT_THROW_IF(!is_initialized(), std::logic_error,
              "Calo tp to ctw algo is not initialized, it can't be reset ! ");
  _initialized_ = false;
  _crate_number_ = -1;
  _mode_ = MODE_UNDEFINED;
  return;
...
void calo_tp_to_ctw_algo::set_crate_number(unsigned int crate_number_) {
  DT_THROW_IF(
      crate_number_ >= mapping::NUMBER_OF_CRATES, std::logic_error,
      "Crate number can't be set because crate number [" << _crate_number_ << "] is not valid ! ");

  _crate_number_ = crate_number_;

  if (_crate_number_ == mapping::MAIN_CALO_SIDE_0_CRATE ||
      _crate_number_ == mapping::MAIN_CALO_SIDE_1_CRATE)
    _set_mode(MODE_MAIN_WALL);
  if (_crate_number_ == mapping::XWALL_GVETO_CALO_CRATE) _set_mode(MODE_XWALL_GVETO);
  return;
}

int calo_tp_to_ctw_algo::get_crate_number() const { return _crate_number_; }
...
void calo_tp_to_ctw_algo::set_ctw_htm(const calo_tp& my_calo_tp_, calo_ctw& my_ctw_) {
  ...
  if (!my_ctw_.is_main_wall())  // Mode == XWall Gveto
  {
    ...
    if (board_id == 4 || board_id == 5 || board_id == 15 ||
        board_id == 16) {  // Filling gamma veto multiplicity
    ...
    if (board_id == 6 || board_id == 7 || board_id == 8 ||
        board_id == 9) {  // Filling xwall side 0 multiplicity
    ...
    if (board_id == 11 || board_id == 12 || board_id == 13 ||
        board_id == 14) {  // Filling xwall side 1 multiplicity
  ...
  if (my_ctw_.is_main_wall())  // Mode == Main Wall
  {
    if (my_calo_tp_.is_htm()) {
      if (my_calo_tp_.get_geom_id().get(mapping::BOARD_INDEX) > 9) {
        activated_zone_id =
            (my_calo_tp_.get_geom_id().get(mapping::BOARD_INDEX) + BOARD_SHIFT_INDEX) / 2;
      } else {
...
void calo_tp_to_ctw_algo::_fill_a_main_wall_ctw(const calo_tp& my_calo_tp_, calo_ctw& a_calo_ctw_) {
  ...
  uint32_t clocktick_with_internal_shift =
      my_calo_tp_.get_clocktick_25ns() + clock_utils::CALO_CB_SHIFT_CLOCKTICK_NUMBER;

  a_calo_ctw_.set_header(my_calo_tp_.get_hit_id(), temporary_feb_id, clocktick_with_internal_shift);
...
void calo_tp_to_ctw_algo::_process_for_a_ctw_for_a_clocktick_for_main_wall(
    const std::vector<datatools::handle<calo_tp> >& my_list_of_calo_tp_, calo_ctw& a_calo_ctw_) {
  DT_THROW_IF(_mode_ != MODE_MAIN_WALL, std::logic_error,
              "Process for main wall can't process because crate number [" << _crate_number_
                                                                           << "] is not valid ! ");
  DT_THROW_IF(!is_main_wall(), std::logic_error,
              "Process for main wall can't process because mode number is not valid ! ");
...
void calo_tp_to_ctw_algo::_process_for_a_ctw_for_a_clocktick_for_xwall_gveto(
    const std::vector<datatools::handle<calo_tp> >& my_list_of_calo_tp_, calo_ctw& a_calo_ctw_) {
  DT_THROW_IF(_mode_ != MODE_XWALL_GVETO, std::logic_error,
              "Process for main wall can't process because crate number [" << _crate_number_
                                                                           << "] is not valid ! ");
  DT_THROW_IF(!is_xwall_gveto(), std::logic_error,
              "Process for main wall can't process because mode number is not valid ! ");
  for (unsigned int i = 0; i < my_list_of_calo_tp_.size(); i++) {
    const calo_tp& my_calo_tp = my_list_of_calo_tp_[i].get();
...
void calo_tp_to_ctw_algo::process(const calo_tp_data& calo_tp_data_,
  ...
  DT_THROW_IF(!is_initialized(), std::logic_error,
              "Calo tp to ctw algo is not initialized, it can't process ! ");
  DT_THROW_IF(_mode_ == MODE_UNDEFINED, std::logic_error,
              "Mode type is not defined, check your crate number ! ");

  for (uint32_t i = calo_tp_data_.get_clocktick_min(); i <= calo_tp_data_.get_clocktick_max();
       i++) {
    // std::clog << "Actual CT = " << i << std::endl;
    if (_mode_ == MODE_MAIN_WALL) {
      std::vector<datatools::handle<calo_tp> > tp_list_per_clocktick_per_crate;
      calo_tp_data_.get_list_of_tp_per_clocktick_per_crate(i, _crate_number_,
                                                           tp_list_per_clocktick_per_crate);
