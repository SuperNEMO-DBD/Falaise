...
void sd_to_calo_signal_algo::initialize() {
  DT_THROW_IF(is_initialized(), std::logic_error,
              "SD to calo signal algorithm is already initialized ! ");
  _initialized_ = true;
  return;
}

bool sd_to_calo_signal_algo::is_initialized() const { return _initialized_; }

void sd_to_calo_signal_algo::reset() {
  DT_THROW_IF(!is_initialized(), std::logic_error,
              "SD to calo signal algorithm is not initialized, it can't be reset ! ");
  _initialized_ = false;
  _geo_manager_ = 0;
  return;
}
...
void sd_to_calo_signal_algo::process(const mctools::simulated_data& sd_,
  ...
  DT_THROW_IF(!is_initialized(), std::logic_error,
              "SD to calo signal algorithm is not initialized ! ");
  _process(sd_, signal_data_);
  return;
}

int sd_to_calo_signal_algo::_process(const mctools::simulated_data& sd_, signal_data& signal_data) {
  DT_THROW_IF(!is_initialized(), std::logic_error,
              "SD to calo signal algorithm is not initialized ! ");
      ...
      for (size_t ihit = 0; ihit < number_of_main_calo_hits; ihit++) {
        ...
        if (signal_data.get_calo_signals().size() == 0) {
          ...
          if (calo_already_hit)  // update calo signal
          {
            ...
            calo_signal& already_hit_calo_signal =
                signal_data.grab_calo_signals()[hit_calo_signal_position].grab();
            if (main_calo_hit.get_geom_id() == already_hit_calo_signal.get_geom_id()) {
              ...
              already_hit_calo_signal.set_data(max_time - calo_signal::DELAYED_PM_TIME,
                                               amplitude_sum);
            }
