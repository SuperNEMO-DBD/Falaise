...
bool ID_convertor::is_initialized() const { return _initialized_; }

void ID_convertor::initialize() {
  DT_THROW_IF(is_initialized(), std::logic_error, "Convertor is already initialized ! ");
  DT_THROW_IF(_module_number_ != mapping::DEMONSTRATOR_MODULE_NUMBER, std::logic_error,
...
datatools::logger::priority ID_convertor::get_logging() const { return _logging_; }

void ID_convertor::set_geo_manager(const geomtools::manager& mgr_) { _geo_manager_ = &mgr_; }
...
geomtools::geom_id ID_convertor::convert_GID_to_EID(const geomtools::geom_id& geom_id_) const {
  ...
  if (geom_id_.get_type() ==
      mapping::GEIGER_CATEGORY_TYPE) {  // Drift cell --> Side [0;1] Layer [0;8] Row [0;112]
    ...
    if (row_index > mapping::THREE_WIRES_CRATE_1_LIMIT &&
        row_index <= mapping::BOARD_ID_SHIFT_CRATE_2_LIMIT) {
      crate_id = 2;
  ...
  if (geom_id_.get_type() == mapping::CALO_MAIN_WALL_CATEGORY_TYPE) {  // MCALO -- Side [0;1] Column
                                                                       // [0;19] (Row[0;12] )type
                                                                       // --> 1302
    ...
    if (side_index_ == 0)
      crate_id = 0;
    else if (side_index_ == 1)
      crate_id = 1;

    int32_t shift_no_feb_10_index = 1;

    if (column_index_ < mapping::CONTROL_BOARD_ID)
      board_id = column_index_;
    else if (column_index_ >= mapping::CONTROL_BOARD_ID)
      board_id = column_index_ + shift_no_feb_10_index;
    channel_id = row_index_;
  }  // End of Calo Main Wall Category type

  if (geom_id_.get_type() ==
      mapping::CALORIMETER_X_WALL_CATEGORY_TYPE) {  // XCALO  -- Side [0;1 ] Wall [0;1] Column [0;1]
                                                    // (Row[0;15])  type --> 1232
    rack_id = mapping::CALO_RACK_ID;
  ...
  if (geom_id_.get_type() == mapping::CALORIMETER_GVETO_CATEGORY_TYPE) {  // GVETO -- Side [0;1 ]
                                                                          // Wall [0;1] (Column
                                                                          // [0;15])type --> 1252
    rack_id = mapping::CALO_RACK_ID;
    crate_id = mapping::XWALL_GVETO_CALO_CRATE;
