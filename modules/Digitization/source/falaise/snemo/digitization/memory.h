...
class memory {
 public:
  ...
  void fetch(const std::bitset<AddressSize>& address_bitset_, std::bitset<DataSize>& data_bitset_);
 ...
 private:
  typedef std::map<std::bitset<AddressSize>, std::bitset<DataSize>, bitset_compare<AddressSize> >
      memory_dict_type;
