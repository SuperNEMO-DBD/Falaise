...
bool coincidence_trigger_algorithm_test_time::is_initialized() const { return _initialized_; }
...
void coincidence_trigger_algorithm_test_time::_process_calo_tracker_coincidence(
    const std::pair<trigger_structures::coincidence_calo_record, trigger_structures::tracker_record>
...

{
  ...
  DT_THROW_IF(a_calo_record.clocktick_1600ns != a_tracker_record.clocktick_1600ns, std::logic_error,
              "Calo tracker coincidence can't process, clockticks of a calo record ["
                  << a_calo_record.clocktick_1600ns << "] and a tracker record ["
                  << a_tracker_record.clocktick_1600ns << "] are not the same ! ");
  ...
  if ((a_coincidence_record_.decision = true) &&
      _coincidence_decision_ == trigger_structures::L2_trigger_mode::CARACO) {
    ...
    for (unsigned int iside = 0; iside < mapping::NUMBER_OF_SIDES; iside++) {
      ...
      a_coincidence_record_.tracker_zoning_word_pattern[iside] =
          a_tracker_record.zoning_word_pattern[iside];
      a_coincidence_record_.tracker_zoning_word_near_source[iside] =
          a_tracker_record.zoning_word_near_source[iside];
...
void coincidence_trigger_algorithm_test_time::_process_delayed_coincidence(
    const std::pair<trigger_structures::coincidence_calo_record, trigger_structures::tracker_record>
        a_pair_for_a_clocktick_,
  ...
  for (unsigned int iside = 0; iside < trigger_info::NSIDES; iside++) {
    for (unsigned int izone = 0; izone < trigger_info::NZONES; izone++) {
      ...
      delayed_hpattern_per_zone[right] =
          a_tracker_record.finale_data_per_zone[iside][izone][right + 2];
      ...
      if (delayed_hpattern_per_zone.any()) {
        if (delayed_hpattern_per_zone.test(left) && izone == 0 &&
          ...
          a_delayed_coincidence_record_.tracker_finale_data_per_zone[iside][izone] =
              a_tracker_record.finale_data_per_zone[iside][izone];
        ...
        if (delayed_hpattern_per_zone.test(left) && (int)(izone - 1) > -1 &&
            (a_previous_event_record_.tracker_finale_data_per_zone[0][izone - 1].test(right + 2) ||
             a_previous_event_record_.tracker_finale_data_per_zone[0][izone].test(left + 2) ||
             a_previous_event_record_.tracker_finale_data_per_zone[0][izone].test(mid + 2) ||
        ...
        if (delayed_hpattern_per_zone.test(right) && izone == 10 &&
            (a_previous_event_record_.tracker_finale_data_per_zone[0][izone].test(mid + 2) ||
             a_previous_event_record_.tracker_finale_data_per_zone[0][izone].test(right + 2) ||
             a_previous_event_record_.tracker_finale_data_per_zone[1][izone].test(mid + 2) ||
             a_previous_event_record_.tracker_finale_data_per_zone[1][izone].test(right + 2))) {
  ...
  if (a_delayed_coincidence_record_.decision &&
      _coincidence_decision_ == trigger_structures::L2_trigger_mode::APE) {
    for (unsigned int iside = 0; iside < trigger_info::NSIDES; iside++) {
      a_delayed_coincidence_record_.tracker_zoning_word_pattern[iside] =
          a_tracker_record.zoning_word_pattern[iside];
      a_delayed_coincidence_record_.tracker_zoning_word_near_source[iside] =
          a_tracker_record.zoning_word_near_source[iside];
    }
    ...
    for (unsigned int iside = 0; iside < trigger_info::NSIDES; iside++) {
      for (unsigned int izone = 0; izone < trigger_info::NZONES; izone++) {
        ...
        if (delayed_near_source_per_zone.any()) {
          ...
          if (delayed_near_source_per_zone.test(near_source_right) && izone == 10 &&
              (a_previous_event_record_.tracker_finale_data_per_zone[0][izone].test(
                   near_source_left + 5) ||
               a_previous_event_record_.tracker_finale_data_per_zone[0][izone].test(
                   near_source_right + 5) ||
...
void coincidence_trigger_algorithm_test_time::process(
    const std::pair<trigger_structures::coincidence_calo_record, trigger_structures::tracker_record>
  ...
  _process(pair_for_a_clocktick_, a_coincidence_record_, a_L2_decision_record_,
           previous_event_records_);
  return;
}

void coincidence_trigger_algorithm_test_time::_process(
    const std::pair<trigger_structures::coincidence_calo_record, trigger_structures::tracker_record>
        pair_for_a_clocktick_,
    trigger_structures::coincidence_event_record& a_coincidence_record_,
    trigger_structures::L2_decision& a_L2_decision_record_,
  ...
  reset_data();
  // Process CARACO :
  _process_calo_tracker_coincidence(pair_for_a_clocktick_, a_coincidence_record_,
  ...
  if (previous_event_records_->size() != 0) {
    ...
    for (; it_circ != previous_event_records_->end(); it_circ++) {
      // If no CARACO or already previous associated event, search for an APE or DAVE delayed
      // coincidence in the PER:
      const trigger_structures::previous_event_record a_previous_event_record = *it_circ;
      if (a_previous_event_record.counter_1600ns <=
              clock_utils::PREVIOUS_EVENT_RECORD_LIVING_NUMBER_OF_CLOCKTICK &&
          a_previous_event_record.counter_1600ns > 0 && !a_L2_decision_record_.L2_decision_bool) {
        _process_delayed_coincidence(pair_for_a_clocktick_, a_coincidence_record_,
                                     a_L2_decision_record_, a_previous_event_record);
      }  // end of if counter
