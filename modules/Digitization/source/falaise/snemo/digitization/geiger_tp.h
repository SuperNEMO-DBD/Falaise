...
class geiger_tp : public geomtools::base_hit {
 public:
  ...
  /// Get the tracker row mode (return the number of connected Geiger rows to the FEB (up to 7
  /// rows))
  ...
  void extract_bitset_from_to(unsigned int BIT_BEGIN_, unsigned int BIT_END_,
                              boost::dynamic_bitset<>& my_bitset_word_) const;

  /// Extract dynamic bitset from bit begin with a given length
  void extract_bitset_from_is_length(unsigned int BIT_BEGIN_, unsigned int word_size_,
                                     boost::dynamic_bitset<>& my_bitset_word_) const;
