  ...
  if (fichier) {
    for (x = 0; x <= 200; x += 0.01) {
      if (x >= 0 && x <= borne_1) {
        ...
        result = (gauss1_constant)*std::exp(-(std::pow((x - gauss1_mean), 2)) /
                                            (2 * gauss1_sigma * gauss1_sigma));
      } else if (x > borne_1 && x <= borne_2) {
        result = (gauss2_constant)*std::exp(-(std::pow((x - gauss2_mean), 2)) /
                                            (2 * gauss2_sigma * gauss2_sigma));
      } else if (x > borne_2) {
...
int main(int argc_, char** argv_) {
  std::clog << "Litle program to calculate the calorimeter signal. \n"
            ...
            << "calo_signal_electron_700keV.out, calo_signal_electron_1000keV.out and "
               "calo_signal_electron_1300keV.out"
            << std::endl;
