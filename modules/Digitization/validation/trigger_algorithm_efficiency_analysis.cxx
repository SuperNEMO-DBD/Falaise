...
int main(int argc_, char **argv_) {
  ...
  try {
    ...
    if (debug)
      std::clog << "Test program for analysis of trigger algorithm efficiency (Real Trigger) !"
                << std::endl;
    ...
    if (is_help) {
      std::cerr << std::endl
                ...
                << "$ "
                   "BuildProducts/bin/"
                   "falaisedigitizationplugin-trigger_algorithm_efficiency_validation [OPTIONS] "
                   "[ARGUMENTS]"
                << std::endl
                << std::endl
                << "Allowed options: " << std::endl
                << "-h  [ --help ]           produce help message" << std::endl
                << "-ip [ --input-path ]     set an input path where the file is. Filename is "
                   "'trigger_validation.root' "
                << std::endl
                << "-op [ --output-path ]    set a path where files will be stored" << std::endl
                << std::endl;
      return 0;
    }
    ...
    input_tree->SetBranchAddress("total_number_of_prompt_gg_cells",
                                 &total_number_of_prompt_gg_cells);
    input_tree->SetBranchAddress("total_number_of_delayed_gg_cells",
                                 &total_number_of_delayed_gg_cells);
    ...
    mean_number_of_delayed_gg_cells =
        mean_number_of_delayed_gg_cells / nbr_of_delayed_triggered_event;
    mean_number_of_gg_cells =
        mean_number_of_gg_cells / (nbr_of_prompt_triggered_event + nbr_of_delayed_triggered_event);

    double fake_trigger_efficiency = (static_cast<double>(n_entry) / TOTAL_NUMBER_OF_EVENTS) * 100;

    double percentage_of_prompt_triggered_event_to_total =
        (static_cast<double>(nbr_of_prompt_triggered_event) / TOTAL_NUMBER_OF_EVENTS) * 100;
    double percentage_of_prompt_triggered_event_to_FT =
        (static_cast<double>(nbr_of_prompt_triggered_event) / static_cast<double>(n_entry)) * 100;

    double percentage_of_delayed_triggered_event_to_total =
        (static_cast<double>(nbr_of_delayed_triggered_event) / TOTAL_NUMBER_OF_EVENTS) * 100;
    double percentage_of_delayed_triggered_event_to_FT =
        (static_cast<double>(nbr_of_delayed_triggered_event) / static_cast<double>(n_entry)) * 100;

    if (debug)
      std::clog << "% of prompt triggered event to total  = "
                << percentage_of_prompt_triggered_event_to_total << "%" << std::endl;
    if (debug)
      std::clog << "% of prompt triggered event to FT     = "
                << percentage_of_prompt_triggered_event_to_FT << "%" << std::endl;
    if (debug)
      std::clog << "% of delayed triggered event to total = "
                << percentage_of_delayed_triggered_event_to_total << "%" << std::endl;
    if (debug)
      std::clog << "% of delayed triggered event to FT    = "
                << percentage_of_delayed_triggered_event_to_FT << "%" << std::endl;

    if (debug)
      std::clog << "Number of prompt event triggered      = " << nbr_of_prompt_triggered_event
                << std::endl;
    if (debug)
      std::clog << "Number of delayed event triggered     = " << nbr_of_delayed_triggered_event
                << std::endl;
    if (debug)
      std::clog << "Mean number of calo                   = " << mean_number_of_calo << std::endl;
    if (debug)
      std::clog << "Mean number of main calo              = " << mean_number_of_main_calo
                << std::endl;
    if (debug)
      std::clog << "Mean number of gveto                  = " << mean_number_of_gveto << std::endl;
    if (debug)
      std::clog << "Mean number of gg cells               = " << mean_number_of_gg_cells
                << std::endl;
    if (debug)
      std::clog << "Mean number of prompt gg cells        = " << mean_number_of_prompt_gg_cells
                << std::endl;
    if (debug)
      std::clog << "Mean number of delayed gg cells       = " << mean_number_of_delayed_gg_cells
                << std::endl;
    ...
    if (file) {
      ...
      file << "& % of prompt triggered event to total = "
           << percentage_of_prompt_triggered_event_to_total << std::endl;
      file << "& % of prompt triggered event to FT = " << percentage_of_prompt_triggered_event_to_FT
           << std::endl;
      file << "& % of delayed triggered event to total = "
           << percentage_of_delayed_triggered_event_to_total << std::endl;
      ...
      file << "& Mean number of delayed gg cells = " << mean_number_of_delayed_gg_cells
           << std::endl;
      file.close();
    } else {
