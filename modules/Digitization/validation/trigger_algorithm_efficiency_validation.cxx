...
int main(int argc_, char** argv_) {
  ...
  if (is_help) {
    std::cerr << std::endl
              ...
              << "$ "
                 "BuildProducts/bin/"
                 "falaisedigitizationplugin-trigger_algorithm_efficiency_validation --input "
  ...
  try {
    ...
    trigger_decision_tree->Branch("raw_trigger_prompt_decision", &raw_trigger_prompt_decision,
                                  "raw_trigger_prompt_decision/O");
    trigger_decision_tree->Branch("raw_trigger_delayed_decision", &raw_trigger_delayed_decision,
                                  "raw_trigger_delayed_decision/O");
    trigger_decision_tree->Branch("total_number_of_calo", &total_number_of_calo,
                                  "total_number_of_calo/I");
    trigger_decision_tree->Branch("total_number_of_main_calo", &total_number_of_main_calo,
                                  "total_number_of_main_calo/I");
    trigger_decision_tree->Branch("total_number_of_gveto", &total_number_of_gveto,
                                  "total_number_of_gveto/I");
    trigger_decision_tree->Branch("total_number_of_gg_cells", &total_number_of_gg_cells,
                                  "total_number_of_gg_cells/I");
    trigger_decision_tree->Branch("total_number_of_prompt_gg_cells",
                                  &total_number_of_prompt_gg_cells,
                                  "total_number_of_prompt_gg_cells/I");
    trigger_decision_tree->Branch("total_number_of_delayed_gg_cells",
                                  &total_number_of_delayed_gg_cells,
                                  "total_number_of_delayed_gg_cells/I");
    ...
    int coincidence_calorimeter_gate_size =
        4;  // Don't forget to modify at 10 CT 1600 for new trigger analysis
    ...
    my_trigger_algo.grab_tracker_tmp_file().out()
        << "Input file : " << pipeline_simulated_data_filename << std::endl;
      ...
      if (ER.has(SD_bank_label) && ER.is_a<mctools::simulated_data>(SD_bank_label)) {
        ...
        if (SD.has_step_hits("calo") || SD.has_step_hits("xcalo") || SD.has_step_hits("gveto") ||
          ...
          if (signal_data.has_calo_signals()) {
            ...
            total_number_of_main_calo = signal_data.get_number_of_main_calo_signals() +
                                        signal_data.get_number_of_xcalo_signals();
          ...
          if (signal_data.has_geiger_signals()) {
            ...
            total_number_of_prompt_gg_cells =
                signal_data.get_number_of_prompt_geiger_signals(time_limit);
            total_number_of_delayed_gg_cells =
                signal_data.get_number_of_delayed_geiger_signals(time_limit);
          }  // end of if has geiger signal

          // Creation of outputs collection structures for calo and tracker
          std::vector<snemo::digitization::calo_trigger_algorithm::calo_summary_record>
              calo_collection_records;
          std::vector<snemo::digitization::tracker_trigger_algorithm::tracker_record>
              tracker_collection_records;
          std::vector<snemo::digitization::coincidence_trigger_algorithm::coincidence_event_record>
          ...
          if (debug)
            std::clog << "Calo collection size    : " << calo_collection_records.size()
                      << std::endl;
          if (debug)
            std::clog << "Tracker collection size : " << tracker_collection_records.size()
                      << std::endl;
          if (debug)
            std::clog << "Coincidence collection size : " << coincidence_collection_records.size()
                      << std::endl;

        }  // end of if has "calo" || "xcalo" || "gveto" || "gg" step hits

        std::vector<snemo::digitization::coincidence_trigger_algorithm::coincidence_calo_record>
            coincidence_collection_calo_records =
                my_trigger_algo.get_coincidence_calo_records_vector();
        ...
        if (debug)
          std::clog << "trigger_finale_decision         [" << raw_trigger_prompt_decision << "]"
                    << std::endl;
        if (debug)
          std::clog << "delayed trigger_finale_decision [" << raw_trigger_delayed_decision << "]"
                    << std::endl;
        ...
        if (raw_trigger_prompt_decision && !raw_trigger_delayed_decision)
          writer_3.process(ER);
        else if (raw_trigger_prompt_decision && raw_trigger_delayed_decision)
          writer_4.process(ER);
