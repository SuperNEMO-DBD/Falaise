...
namespace snemo {
  ...
  namespace reconstruction {
    ...
    {
      static const std::string _s("short_alpha");
      return _s;
    }

    const std::string & alpha_finder_driver::get_id()
    {
      static const std::string _s("AFD");
      return _s;
    }
    ...
    void alpha_finder_driver::_find_short_track_(const snemo::datamodel::TrackerHitHdlCollection & hits_,
      ...
      snemo::datamodel::particle_track_data & particle_track_data_)
    {
      ...
      a_short_alpha->set_charge(snedm::particle_track::CHARGE_UNDEFINED);
      ...
      snedm::VertexHdlCollection & vertices = a_short_alpha->get_vertices();
      {
        // Vertex on wire
        snedm::VertexHdl vertex = datatools::make_handle<snedm::Vertex>();
        vertex->set_hit_id(vertices.size());
        vertex->set_category(snedm::VERTEX_CATEGORY_ON_WIRE);
        vertex->get_spot().set_blur_dimension(geomtools::blur_spot::dimension_three);
        vertex->get_spot().set_position(last_vertex);
...

      {
        ...
        snedm::vertex_category_type vertexCategory = snedm::VERTEX_CATEGORY_UNDEF;
        if (std::abs(first_vertex.x()) < epsilon) {
          vertexCategory = snedm::VERTEX_CATEGORY_ON_SOURCE_FOIL;
        } else if (std::abs(first_vertex.x() - xcalo_bd[0]) < epsilon ||
                   std::abs(first_vertex.x() - xcalo_bd[1]) < epsilon) {
          vertexCategory = snedm::VERTEX_CATEGORY_ON_MAIN_CALORIMETER;
        } else if (std::abs(first_vertex.y() - ycalo_bd[0]) < epsilon ||
                   std::abs(first_vertex.y() - ycalo_bd[1]) < epsilon) {
          vertexCategory = snedm::VERTEX_CATEGORY_ON_X_CALORIMETER;
        } else if (std::abs(first_vertex.z() - zcalo_bd[0]) < epsilon ||
                   std::abs(first_vertex.z() - zcalo_bd[1]) < epsilon) {
          vertexCategory = snedm::VERTEX_CATEGORY_ON_GAMMA_VETO;
        } else {
          vertexCategory = snedm::VERTEX_CATEGORY_ON_WIRE;
        }
        snedm::VertexHdl newVertex = datatools::make_handle<snedm::Vertex>();
        newVertex->set_hit_id(vertices.size());
        newVertex->set_category(vertexCategory);
        newVertex->get_spot().set_blur_dimension(geomtools::blur_spot::dimension_three);
        newVertex->get_spot().set_position(first_vertex);
