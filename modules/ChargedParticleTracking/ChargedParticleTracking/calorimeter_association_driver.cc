...
namespace snemo {
  ...
  namespace reconstruction {
    ...
    void calorimeter_association_driver::process(const snemo::datamodel::CalorimeterHitHdlCollection & calorimeter_hits_,
    ...
    void calorimeter_association_driver::_measure_matching_calorimeters_(const snemo::datamodel::CalorimeterHitHdlCollection & calorimeter_hits_,
                                                                         snemo::datamodel::particle_track & particle_)
    {
      ...
      snedm::VertexHdlCollection & the_vertices = particle_.get_vertices();
      // Mutable because we'd like to assign the geom ID of a calo block:
      for (snedm::VertexHdl & a_vertex : the_vertices) {
        // Do not take care of vertex other than the ones on calorimeters
        if (!a_vertex->is_on_main_calorimeter() &&
            !a_vertex->is_on_x_calorimeter() &&
            !a_vertex->is_on_gamma_veto()) {
          continue;
        }
        ...
        for (const datatools::handle<snedm::calibrated_calorimeter_hit> & a_calo_hit :
          ...
          for (const geomtools::geom_id & a_gid : gids) {
            ...
            geomtools::vector_2d spotPosXy(a_vertex->get_spot().get_position().x(),
                                           a_vertex->get_spot().get_position().y());
	    // double spotPosZ = a_vertex->get_spot().get_position().z();
            if (geomtools::mapping::check_inside(*ginfo_ptr,
                                                 a_vertex->get_spot().get_position(),
                                                 tolerance,
                                                 true)) {
              ...
              const double distance = (calo_position - a_vertex->get_spot().get_position()).mag();
