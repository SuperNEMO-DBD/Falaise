...
void charge_computation_driver::_measure_particle_charge_(
  ...
  if (a_track_pattern.get_pattern_id() == snemo::datamodel::line_trajectory_pattern::pattern_id()) {
    DT_LOG_DEBUG(get_logging_priority(),
                 "Trajectory pattern is 'line', no curvature can be extracted");
  ...
  DT_LOG_TRACE(get_logging_priority(),
               "Particle charge is " << (is_negative ? "negative" : "positive"));
  DT_LOG_TRACE(get_logging_priority(), "Exiting.");
  return;
}
