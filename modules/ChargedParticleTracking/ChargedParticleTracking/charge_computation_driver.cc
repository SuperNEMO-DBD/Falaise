...
namespace snemo {
  ...
  namespace reconstruction {
    ...
    {
      static const std::string _s("CCD");
      return _s;
    }
                                            ...
                                            snemo::datamodel::particle_track & particle_)
    {
      ...
      if (a_track_pattern.get_pattern_id() == snedm::line_trajectory_pattern::pattern_id()) {
        particle_.set_charge(snedm::particle_track::CHARGE_UNDEFINED);
        return;
      }
      ...
      if (a_charge < 0) {
        particle_.set_charge(snedm::particle_track::CHARGE_NEGATIVE);
      } else {
        particle_.set_charge(snedm::particle_track::CHARGE_POSITIVE);
      }
      return;
    }
