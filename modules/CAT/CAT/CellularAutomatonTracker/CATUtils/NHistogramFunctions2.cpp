...
TH3F* Divide3D(TH3F* num, TH3F* den) {
  ...
  for (int i = 0; i <= num->GetNbinsX(); i++) x_bins[i] = num->GetXaxis()->GetBinLowEdge(i + 1);
  for (int i = 0; i <= num->GetNbinsY(); i++) y_bins[i] = num->GetYaxis()->GetBinLowEdge(i + 1);
  for (int i = 0; i <= num->GetNbinsZ(); i++) z_bins[i] = num->GetZaxis()->GetBinLowEdge(i + 1);

  TH3F* result = new TH3F("result", "result", num->GetNbinsX(), x_bins, num->GetNbinsY(), y_bins,
                          num->GetNbinsZ(), z_bins);
    ...
    for (int j = 1; j <= num->GetNbinsY(); j++) {
      for (int k = 1; k <= num->GetNbinsZ(); k++) {
        if (den->GetBinContent(i, j, k) != 0) {
          ...
          double error =
              sqrt(std::abs((num->GetBinContent(i, j, k) *
                             (1.0 - num->GetBinContent(i, j, k) / den->GetBinContent(i, j, k))))) /
              den->GetBinContent(i, j, k);
  ...
  for (int i = 1; i <= num->GetNbinsX(); i++) {
    for (int j = 1; j <= num->GetNbinsY(); j++) {
      if (den->GetBinContent(i, j) != 0) {
        ...
        double error =
            sqrt(std::abs((num->GetBinContent(i, j) *
                           (1.0 - num->GetBinContent(i, j) / den->GetBinContent(i, j))))) /
            den->GetBinContent(i, j);
...
TH1F* Divide1D(TH1F* num, TH1F* den) {
  ...
  for (int i = 1; i <= num->GetNbinsX(); i++) {
    if (den->GetBinContent(i) != 0) {
      ...
      double error = sqrt(std::abs((num->GetBinContent(i) *
                                    (1.0 - num->GetBinContent(i) / den->GetBinContent(i))))) /
                     den->GetBinContent(i);
...
TH2F* Average3D_to_2D(TH3F* object, char* axis1, char* axis2) {
  ...
  if ((0 == strcmp(axis1, "1") && 0 == strcmp(axis2, "2")) ||
      (0 == strcmp(axis1, "2") && 0 == strcmp(axis2, "1")))
    axis3 = "3";
  if ((0 == strcmp(axis1, "1") && 0 == strcmp(axis2, "3")) ||
      (0 == strcmp(axis1, "3") && 0 == strcmp(axis2, "1")))
    axis3 = "2";
  if ((0 == strcmp(axis1, "2") && 0 == strcmp(axis2, "3")) ||
      (0 == strcmp(axis1, "3") && 0 == strcmp(axis2, "2")))
    axis3 = "1";
  ...
  if (0 == strcmp(axes, "123")) {
    ...
    for (int i = 0; i <= xbins; i++) x_bins[i] = object->GetXaxis()->GetBinLowEdge(i + 1);
    ybins = object->GetNbinsY();
    y_bins = new double[ybins + 1];
    for (int i = 0; i <= ybins; i++) y_bins[i] = object->GetYaxis()->GetBinLowEdge(i + 1);
    zbins = object->GetNbinsZ();
    z_bins = new double[zbins + 1];
    for (int i = 0; i <= zbins; i++) z_bins[i] = object->GetZaxis()->GetBinLowEdge(i + 1);
    ...
    y_bins = new double[ybins + 1];
    for (int i = 0; i <= ybins; i++) y_bins[i] = object->GetZaxis()->GetBinLowEdge(i + 1);
    zbins = object->GetNbinsY();
    z_bins = new double[zbins + 1];
    for (int i = 0; i <= zbins; i++) z_bins[i] = object->GetYaxis()->GetBinLowEdge(i + 1);
  } else if (0 == strcmp(axes, "213")) {
    xbins = object->GetNbinsY();
    x_bins = new double[xbins + 1];
    for (int i = 0; i <= xbins; i++) x_bins[i] = object->GetYaxis()->GetBinLowEdge(i + 1);
    ybins = object->GetNbinsX();
    y_bins = new double[ybins + 1];
    for (int i = 0; i <= ybins; i++) y_bins[i] = object->GetXaxis()->GetBinLowEdge(i + 1);
    zbins = object->GetNbinsZ();
    z_bins = new double[zbins + 1];
    for (int i = 0; i <= zbins; i++) z_bins[i] = object->GetZaxis()->GetBinLowEdge(i + 1);
    ...
    z_bins = new double[zbins + 1];
    for (int i = 0; i <= zbins; i++) z_bins[i] = object->GetXaxis()->GetBinLowEdge(i + 1);
  } else if (0 == strcmp(axes, "312")) {
    xbins = object->GetNbinsZ();
    x_bins = new double[xbins + 1];
    for (int i = 0; i <= xbins; i++) x_bins[i] = object->GetZaxis()->GetBinLowEdge(i + 1);
    ybins = object->GetNbinsX();
    y_bins = new double[ybins + 1];
    for (int i = 0; i <= ybins; i++) y_bins[i] = object->GetXaxis()->GetBinLowEdge(i + 1);
    zbins = object->GetNbinsY();
      ...
      if (0 == strcmp(axis1, "1")) {
        if (object->GetBinContent(i, j) != -99) {
          if (strcmp(option, "omega") == 0) {
            sum += object->GetBinContent(i, j) *
                   (cos(object->GetYaxis()->GetBinLowEdge(j)) -
                    cos(object->GetYaxis()->GetBinUpEdge(j))) *
                   mult;
            count++;
            errorSum2 += pow(object->GetBinError(i, j), 2) *
                         (cos(object->GetYaxis()->GetBinLowEdge(j)) -
        ...
        if (object->GetBinContent(j, i) != -99) {
          ...
          count++;
          errorSum2 +=
              pow(object->GetBinError(j, i), 2) * object->GetXaxis()->GetBinWidth(j) * mult;
          ;
        }
      }
    }
...
double Integrate2D(TH2F* object, double mult, char* option) {
  ...
  for (int i = 1; i <= object->GetNbinsX(); i++) {
    for (int j = 1; j <= object->GetNbinsY(); j++) {
      if (strcmp(option, "omega") == 0) {
        sum +=
            object->GetBinContent(i, j) * object->GetXaxis()->GetBinWidth(i) *
            (cos(object->GetYaxis()->GetBinLowEdge(j)) - cos(object->GetYaxis()->GetBinUpEdge(j))) *
            mult;
      } else {
        sum += object->GetBinContent(i, j) * object->GetXaxis()->GetBinWidth(i) *
