...
int main(int argc_, char* argv_[]) {
    ...
    if (token[0] == '-') {
      ...
      if ((option == "-h") || (option == "--help")) {
        cout << endl
             ...
             << "   --param <string>  :   parameter file for tracking (default: " << param_file
             << ") " << endl
             << "   -l --limits  <string> :   limits file for tracking (default: " << limits_file
             << ") " << endl
             << "   -m --modulo <integer>   :  modulo for record print  " << endl
             << "   -n <integer> :      number of events to track , by default all from the DST "
                "(recommended) "
             << endl
             << "   -f --first <integer>:      number of first event to track" << endl
             << "   -fen --first_event_number <integer>:      number of first event number to track"
             << endl
             << "   -v <integer>:      set verbosity level " << endl
             << "             0 = mute, " << endl
             << "             4 = , " << endl
             << "             5 (default) = print warnings, " << endl
             << "             6 = event display & reconstruction, " << endl
             << "             7 = tracks creation, " << endl
             << "             8 = details, " << endl
             << "             9 = , " << endl
             << "            10 = all, " << endl
             << "   --with-root-ntuple :      create root ntuple" << endl
             << "" << endl
             << "" << endl
             << endl;
  ...
  if (nevents >= 0 && first_event >= 0) nevents += first_event;
  ...
  for (int i = 0; i >= 0; i++) {
    ...
    if (event_ref.find_property("G4_evt_num")) {
      ...
      if (evn % modulo == 0) cout << " event " << i << " event number " << evn << endl;
    }
    if (event_ref.find_property("NEMOEVNUMBER")) {
      ...
      evn = i;
      if (evn % modulo == 0) cout << " event " << i << endl;
    }
  ...
  if (root_ntuple_mode) nt->finalize();
  ...
  cout << " number of selected events: " << count_selected << " ( "
       << (double)count_selected / (double)count_events << " ) " << endl;
  cout << " number of skipped events: " << count_skipped << " ( "
       << (double)count_skipped / (double)count_events << " ) " << endl;
  cout << " number of reconstructed events: " << count_selected - count_skipped << " ( "
       << (double)(count_selected - count_skipped) / (double)(count_events) << " ) " << endl;
