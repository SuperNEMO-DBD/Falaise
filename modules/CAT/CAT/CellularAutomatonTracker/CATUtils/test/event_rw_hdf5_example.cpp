...
int main(int, char* []) {
  ...
  Assert(H5T_STRING == H5Tget_class(tid1.getId()) && H5Tis_variable_str(tid1.getId()), __FILE__,
         __LINE__, internal_logic("This is not a variable lenght string type!"));
  ...
  for (size_t ievent = 1; ievent <= 20; ievent++) {
    ...
    if (!H5Tequal(native_type, tid1.getId())) cerr << "native type is not tid1!!!" << endl;
