...
class sreader : public engine {
 public:
  ...
  ~sreader() {
    if (panel_.switch_closed("file")) m_dataFile.close();
  }
  ...
  void read() {
    ...
    while (!m_dataFile.eof()) {
      ...
      try {
        Assert(test == "S" || test == "D" || test == "I" || test == "V" || test == "DV" ||
                   test == "IV" || test == "SV",
               __FILE__, __LINE__, internal_logic("must be S, D, I, V, DV,IV, SV"));
        ...
        } else if (compare_string_nocase(test, "V") || compare_string_nocase(test, "DV")) {
          size_t size;
          // double value;
          std::string value;
          dvector<double> dv;
 ...
 private:
  ...
  std::string nextEntry() {
    ...
    do {
      m_dataFile >> word;
      if (word[0] == '#') getline(m_dataFile, dummy);
