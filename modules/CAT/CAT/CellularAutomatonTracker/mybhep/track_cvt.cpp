...
track_cvt::track_cvt(const track& trk, int index) : ibconverter() {
  set_index(index);
  set_name("__track__");
  set_owner(false);
  track_ = &trk;
...
track_cvt::track_cvt(std::string def) : ibconverter() { restore(def); }

track_cvt::~track_cvt() {
  if (owner()) delete ntrack_;
}
...
std::string track_cvt::store() {
  ...
  for (size_t iray = 0; iray < rays.size(); iray++) {
    ...
    tmp2 = to_string(x[0]) + " " + to_string(x[1]) + " " + to_string(x[2]);
    to_string(tmp, tmp2);
...
const track& track_cvt::retrieve() const { return *track_; }

track& track_cvt::reference() { return *ntrack_; }

track* track_cvt::create() {
  Assert(ntrack_, __FILE__, __LINE__, internal_logic("  null pointer!!!"));
