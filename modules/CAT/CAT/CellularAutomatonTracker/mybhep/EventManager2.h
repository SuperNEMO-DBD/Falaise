...
class EventManager2 : public EventManager {
 public:
  EventManager2(const mybhep::gstore& gs, mybhep::prlevel vl = mybhep::NORMAL)
      : EventManager(gs, vl) {
    header_saved = false;
    run_properties_saved = false;
  }

  EventManager2(const mybhep::gstore& data_store, const mybhep::gstore& param_store,
  ...
  mybhep::sstore get_run_properties() { return run_properties; };

  // return dst properties as a store
  mybhep::sstore get_dst_properties() { return dst_properties; };
