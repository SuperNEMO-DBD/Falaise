...
class event : public bproperties {
 ...
 private:
  ...
  void erase_mother_memory(const particle& cpm, const particle* p, ptype type);
 ...
 public:
  ...
  //! set event number
  void set_event_number(int number) { event_number_ = number; }
  ...
  */
  ...
  void add_true_particle(particle* p) { true_particles_.push_back(p); }

  //! adds a digi (normally it means reconstructed) particle to event
  void add_digi_particle(particle* p) { digi_particles_.push_back(p); }
  ...
  void add_sparticle(sparticle* p) { sparticles_.push_back(p); }

  //! deletes a particle and all its descendants from event
  bool delete_particle(particle* pr, ptype type);
  //! return event number
  const int& event_number() const { return event_number_; }

  //! return vertex
  const mybhep::Point3D& vertex() const { return *vertex_; }

  //! return std::vector of true particles (const)
  const std::vector<particle*>& true_particles() const { return true_particles_; }

  //! return std::vector of particles
  std::vector<particle*>& true_particles() { return true_particles_; }

  //! return std::vector of digi particles (const)
  const std::vector<particle*>& digi_particles() const { return digi_particles_; }

  //! return std::vector of particles
  std::vector<particle*>& digi_particles() { return digi_particles_; }

  //! return std::vector of sparticles (const)
  const std::vector<sparticle*>& sparticles() const { return sparticles_; }

  //! return std::vector of sparticles
  std::vector<sparticle*>& sparticles() { return sparticles_; }
  ...
  bool filter(ptype type, std::string property_name, std::string property_value,
