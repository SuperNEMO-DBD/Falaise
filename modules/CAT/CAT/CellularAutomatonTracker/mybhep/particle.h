...
class particle : public particle_definition {
 ...
 public:
  ...
  //! general constructor
  particle(ptype type, std::string name, int pdg, double m, double q, double l = -1);
 ...
 public:
  ...
  void add_daughter(const particle& p_arg) { daughters_.push_back(&p_arg); }

  //! add a track to the particle
  void add_track(track* tr) { tracks_.push_back(tr); }

  //! add a mirror to the particle
  void add_mirror(mparticle* mp) { mp_.push_back(mp); }

  //! add a hit to the particle
  void add_hit(std::string detector, hit* ht) { hits_.insert(make_pair(detector, ht)); }
  ...
  //! return mother
  const particle& mother() const { return *mother_; }
  ...
  void set_type(ptype type_arg) { ptype_ = type_arg; }

  //! set description of particle state
  void set_state(pstate state_arg) { pstate_ = state_arg; }
