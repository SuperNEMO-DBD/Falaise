...
class particle_definition : public bproperties {
 public:
  //! constructor
  particle_definition(std::string name);
  particle_definition(int code, idcode scheme = GEANT3);
  particle_definition(std::string name, int pdg, double m, double q, double l = -1);
 ...
 private:
  ...
  void set_particle_name(std::string name);
...
std::ostream& operator<<(std::ostream& s, const particle_definition& p);
}  // namespace mybhep
#endif
