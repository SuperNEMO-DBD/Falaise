...
mybhep::event& EventManager::read(size_t ie) {
  ...
  if (ie) {
    mybhep::event& e = random_read(ie);
    return e;
  } else {
    mybhep::event& e = sequential_read();
    return e;
  }
}
...
mybhep::event& EventManager::sequential_read() {
  ...
  if (iext == "gz") {
    mybhep::event& ev = rgz.read_next();
    return ev;
  }
#ifdef HDF5
  else if (iext == "h5") {
    mybhep::event& ev = rh5.read_event(ievent);
    return ev;
  }
#endif
  else {
    mybhep::event& ev = rtxt.read_next();
    return ev;
  }
}
...
mybhep::event& EventManager::random_read(size_t ie) {
  ...
  if (iext == "gz") {
    mybhep::event& ev = rgz.read_event(index);
    return ev;
  }
#ifdef HDF5
  else if (iext == "h5") {
    mybhep::event& ev = rh5.read_event(index);
    return ev;
  }
#endif
  else {
    mybhep::event& ev = rtxt.read_event(index);
    return ev;
  }
}
...
size_t EventManager::getNumEventsFile() {
  ...
  if (iext == "txt") {
    max_evt = rtxt.max_events();
  } else if (iext == "gz") {
    max_evt = rgz.max_events();
  }
#ifdef HDF5
  else {
    max_evt = rh5.max_events();
  }
#endif
  return max_evt;
}
...
void EventManager::write(mybhep::event& event) {
  ...
  if (oext == "gz") {
    wgz.write(event, oevent);
  }
#ifdef HDF5
  else if (oext == "h5") {
    wh5.write(event, oevent);
  }
#endif
  else {
    wtxt.write(event, oevent);
  }
...
void EventManager::readParam() {
  ...
  if (allow_warnings)
    warnings = store.fetch_istore("warnings");
  else
    warnings = true;
}
