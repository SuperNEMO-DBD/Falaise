...
class logic_scenario : public tracking_object {
 ...
 public:
  ...
  logic_scenario(const topology::scenario &sc) {
    for (std::vector<topology::sequence>::const_iterator iseq = sc.sequences_.begin();
  ...
  //! get sequences
  const std::vector<logic_sequence> &sequences() const { return sequences_; }

  //! get helix_chi2
  const double &helix_chi2() const { return helix_chi2_; }

  //! get tangent_chi2
  const double &tangent_chi2() const { return tangent_chi2_; }

  //! get ndof
  const int32_t &ndof() const { return ndof_; }
  ...
  void calculate_n_overlaps(const std::vector<topology::cell> &cells,
    ...
    for (std::vector<logic_sequence>::iterator iseq = sequences_.begin(); iseq != sequences_.end();
         ++iseq) {
      for (std::vector<logic_cell>::iterator in = iseq->cells_.begin(); in != iseq->cells_.end();
           ++in) {
        if (in->id() >= cells.size()) {
          if (print_level() >= mybhep::VVERBOSE)
            std::clog << " problem: cell " << in->id() << " has larger id than n of cells "
                      << cells.size() << std::endl;
          continue;
        }
      ...
      if (iseq->has_tangent_vertex() && iseq->tangent_vertex_type() == "calo") {
        ...
        if (iseq->tangent_vertex_id() != iseq->calo_helix_id() &&
            ...
            iseq->tangent_vertex_id() != iseq->helix_vertex_id()) {  // avoid double counting if
                                                                     // both extrapolations point to
                                                                     // the same calo
  ...
  double helix_Prob() const { return probof(helix_chi2(), ndof()); }

  double tangent_Prob() const { return probof(tangent_chi2(), ndof()); }
  ...
  bool better_scenario_than(const logic_scenario &s) {
    ...
    if (print_level() >= mybhep::VVERBOSE) {
      std::clog << " delta n_free_families = (" << n_free_families() << " - " << s.n_free_families()
                ...
                << s.n_overlaps() << ")= " << deltanoverls << " delta prob_helix = ("
                << helix_Prob() << " - " << s.helix_Prob() << ") = " << deltaprob_helix
                << " delta prob_tangent = (" << tangent_Prob() << " - " << s.tangent_Prob()
                << ") = " << deltaprob_tangent << std::endl;
    }
