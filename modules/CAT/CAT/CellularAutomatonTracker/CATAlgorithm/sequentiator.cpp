...
//*************************************************************
bool sequentiator::initialize(const mybhep::sstore &store, const mybhep::gstore & /*gs*/,
                              mybhep::EventManager2 *emanager) {
  //*************************************************************

  m.message("CAT::sequentiator::initialize: Beginning algorithm sequentiator", mybhep::VERBOSE);
  fflush(stdout);
...
void sequentiator::initializeHistos(void) {
  ...
  /*
    ...
    hman.h1("prob_connections", "probability that chi2 is larger than observed for a good connection
    of triplets", 100, -0.05, 1.05); hman.h1("chi2_sequence", "chi2 for each sequence", 100,
    -0.05, 20.05); hman.h1("prob_sequence", "probability that chi2 is larger than observed for a
    sequence", 100, -0.05, 1.05); hman.h1("helix_chi2_points", "chi2 for each sequence point to be
    on a helix", 100, -0.05, 20.05); hman.h1("helix_chi2", "chi2 for each sequence to be a helix",
    100, -0.05, 20.05); hman.h1("helix_prob", "probability that chi2 is larger than observed for a
    sequence to be a helix", 100, -0.05, 1.05);
    ...
    hman.h2("EfficiencyVsTheta", "track efficiency vs true cos theta", 21, -1., 1., 21,
    -0.05, 1.05); hman.h2("QResVsTheta", "charge error vs true cos theta", 21, -1., 1., 5,
    -2.5, 2.5); hman.h1("GGresx", "GG residual in x direction ", 100, -30., 30.); hman.h1("GGresy",
    "GG residual in y direction ", 500, -150., 150.); hman.h1("GGresz", "GG residual in z direction
    ", 100, -30., 30.); hman.h1("GGHelixresx", "GG residual in x direction ", 100, -30., 30.);
    ...
    hman.h2("VertexResxVsTheta", "extrapolation to foil residual in x direction vs true cos theta ",
    21, -1., 1.,  100, -100., 100.); hman.h2("VertexResyVsTheta", "extrapolation to foil residual in
    y direction vs true cos theta ", 21, -1., 1., 100, -500., 500.); hman.h2("VertexReszVsTheta",
    "extrapolation to foil residual in z direction vs true cos theta ", 21, -1., 1., 100, -100.,
    100.); hman.h1("DVertexResx", "extrapolation to calorimeter residual in x direction ", 100,
    -70., 70.); hman.h1("DVertexResy", "extrapolation to calorimeter residual in y direction ", 100,
    -70., 70.); hman.h1("DVertexResz", "extrapolation to calorimeter residual in z direction ", 100,
    -70., 70.); hman.h2("DVertexResxVsTheta", "extrapolation to calorimeter residual in x direction
    vs true cos theta ", 21, -1., 1., 100, -500., 500.); hman.h2("DVertexResyVsTheta",
    "extrapolation to calorimeter residual in y direction vs true cos theta ", 21, -1., 1., 100,
    -2000., 2000.); hman.h2("DVertexReszVsTheta", "extrapolation to calorimeter residual in z
    direction vs true cos theta ", 21, -1., 1., 100, -100., 100.); hman.h1("VertexPullx",
    "extrapolation to foil pull in x direction ", 100, -100., 100.); hman.h1("VertexPully",
    "extrapolation to foil pull in y direction ", 100, -500., 500.); hman.h1("VertexPullz",
    "extrapolation to foil pull in z direction ", 100, -100., 100.); hman.h1("DVertexPullx",
    "extrapolation to calorimeter pull in x direction ", 100, -500., 500.); hman.h1("DVertexPully",
    "extrapolation to calorimeter pull in y direction ", 100, -2000., 2000.);
    ...
    hman.h2("all_point_map", "map of position wrt cell center", 100, -rad/1.9, rad/1.9, 100,
    -rad/1.9, rad/1.9); hman.h2("end_point_map", "map of end-point position wrt cell center", 100,
    -rad/1.9, rad/1.9, 100, -rad/1.9, rad/1.9);
    }
...
bool sequentiator::finalize() {
  ...
  m.message("CAT::sequentiator::finalize: Ending algorithm sequentiator", mybhep::NORMAL);
  fflush(stdout);

  m.message("CAT::sequentiator::finalize: Initial events: ", InitialEvents, mybhep::NORMAL);
  m.message("CAT::sequentiator::finalize: Skipped events: ", SkippedEvents, "(",
            100. * SkippedEvents / InitialEvents, "%)", mybhep::NORMAL);

  clock.start(" sequentiator: finalize ");

  if (PrintMode) finalizeHistos();
...
void sequentiator::readDstProper(const mybhep::sstore &global, mybhep::EventManager2 * /*eman*/) {
  ...
  if (!global.find("GEOM_MODULES")) {
    _MaxBlockSize = 1;
    m.message("CAT::sequentiator::readDstProper: no bar design, MUDULES Nr set to = ",
              _MaxBlockSize, "\n", mybhep::VERBOSE);
    fflush(stdout);
  } else {
  ...
  if (!global.find("NEMO3")) {
    SuperNemo = true;
    m.message("CAT::sequentiator::readDstProper:SuperNemo kind of data", mybhep::VERBOSE);
    fflush(stdout);
  } else {
    SuperNemo = false;
    m.message("CAT::sequentiator::readDstProper:Nemo-3 kind of data", mybhep::VERBOSE);
    fflush(stdout);
  }

  if (global.find("NEMORA_model")) {
    NemoraOutput = true;
    m.message("CAT::sequentiator::readDstProper:data already processed by Nemora", mybhep::VERBOSE);
    fflush(stdout);
  } else
    NemoraOutput = false;
  ...
  m.message("CAT::sequentiator::readDstProper: pmax", pmax, "MeV", mybhep::VERBOSE);
  fflush(stdout);
  m.message("CAT::sequentiator::readDstProper: small radius", SmallRadius, "mm", mybhep::VERBOSE);
  fflush(stdout);
  m.message("CAT::sequentiator::readDstProper: tangent phi", TangentPhi, mybhep::VERBOSE);
  fflush(stdout);
  m.message("CAT::sequentiator::readDstProper: tangent theta", TangentTheta, mybhep::VERBOSE);
  fflush(stdout);
  m.message("CAT::sequentiator::readDstProper: small number", SmallNumber, "mm", mybhep::VERBOSE);
  fflush(stdout);
  m.message("CAT::sequentiator::readDstProper: quadrant angle", QuadrantAngle, mybhep::VERBOSE);
  fflush(stdout);
  m.message("CAT::sequentiator::readDstProper: ratio", Ratio, mybhep::VERBOSE);
  fflush(stdout);
  m.message("CAT::sequentiator::readDstProper: compatibility distance", CompatibilityDistance,
            mybhep::VERBOSE);
  fflush(stdout);
  m.message("CAT::sequentiator::readDstProper: maximum chi2", MaxChi2, mybhep::VERBOSE);
  fflush(stdout);
  m.message("CAT::sequentiator::readDstProper: maximum time", MaxTime, " ms ", mybhep::NORMAL);
  fflush(stdout);
  m.message("CAT::sequentiator::readDstProper: xsize is read as", xsize, "mm", mybhep::VERBOSE);
  fflush(stdout);
  m.message("CAT::sequentiator::readDstProper: ysize is read as", ysize, "mm", mybhep::VERBOSE);
  fflush(stdout);
  m.message("CAT::sequentiator::readDstProper: zsize is read as", zsize, "mm", mybhep::VERBOSE);
  fflush(stdout);
  m.message("CAT::sequentiator::readDstProper: plasma speed is read as: ", vel, "mm/ns",
            mybhep::VERBOSE);
  fflush(stdout);
  m.message("CAT::sequentiator::readDstProper: wire length is read as: ", len, "mm",
            mybhep::VERBOSE);
  fflush(stdout);
  m.message("CAT::sequentiator::readDstProper: cell diameter is read as: ", rad, "mm",
            mybhep::VERBOSE);
  fflush(stdout);
  m.message("CAT::sequentiator::readDstProper: distance from wire to wire is read as: ",
            CellDistance, "mm", mybhep::VERBOSE);
  fflush(stdout);
  ...
  if (SuperNemo) {
    ...
    if (global.find(pname)) {
      ...
      m.message(
          "CAT::sequentiator::readDstProper: +++ NEMO3 GG ERROR, GLOBAL PROPERTY NOT FOUND IN DST",
          pname, mybhep::NORMAL);
      fflush(stdout);
...
void sequentiator::readDstProper(void) {
  ...
  if (_MaxBlockSize <= 0) {
    _MaxBlockSize = 1;
    m.message("CAT::sequentiator::readDstProper: no bar design, MODULES Nr set to = ",
              _MaxBlockSize, "\n", mybhep::NORMAL);
  }
...
bool sequentiator::sequentiate(topology::tracked_data &tracked_data_) {
  ...
  event_number++;
  m.message("CAT::sequentiator::sequentiate: local_tracking: preparing event", event_number,
            mybhep::VERBOSE);

  if (event_number < first_event_number) {
    m.message("CAT::sequentiator::sequentiate:  local_tracking: skip event", event_number,
  ...
  m.message("CAT::sequentiator::sequentiate: sequentiate... ", mybhep::VVERBOSE);
  fflush(stdout);
  ...
  for (vector<topology::cluster>::iterator icluster = the_clusters.begin();
  ...
  m.message("CAT::sequentiator::sequentiate: sequentiation done ", mybhep::VVERBOSE);
  fflush(stdout);
...
bool sequentiator::sequentiate_after_sultan(topology::tracked_data &tracked_data_,
                                            bool conserve_clustering_from_removal_of_cells) {
  //*************************************************************
  ...
  event_number++;
  m.message("CAT::sequentiator::sequentiate_after_sultan: local_tracking: preparing event",
            event_number, mybhep::VERBOSE);

  if (event_number < first_event_number) {
    m.message("CAT::sequentiator::sequentiate_after_sultan:  local_tracking: skip event",
              event_number, " first event is ", first_event_number, mybhep::VERBOSE);
    return true;
  }
  ...
  m.message("CAT::sequentiator::sequentiate_after_sultan: sequentiate ", the_clusters.size(),
            " clusters ", mybhep::VVERBOSE);
  fflush(stdout);
  ...
  interpret_physics_after_sultan(tracked_data_.get_calos(),
...
void sequentiator::plot_hard_scattering(topology::tracked_data /*__tracked_data*/) {
  ...
  /*
    ...
    for(std::vector<topology::sequence>::iterator isequence =
    __tracked_data.scenarios_[0].sequences_.begin(); isequence !=
    __tracked_data.scenarios_[0].sequences_.end(); ++isequence){

    for(std::vector<topology::node>::iterator in = isequence->nodes_.begin(); in !=
    isequence->nodes_.end(); ++in){ topology::experimental_vector dist(in->c().ep(), in->ep());
    ...
    m.message(" sequence ", isequence->name(), "has last cell ", isequence->last_node().c().id(), "
    disconnected", mybhep::NORMAL);

    for(std::vector<topology::sequence>::iterator jsequence =
    __tracked_data.scenarios_[0].sequences_.begin(); jsequence !=
    __tracked_data.scenarios_[0].sequences_.end(); ++jsequence){ if( isequence == jsequence )
    continue; if( jsequence->nodes_[0].c().id() != isequence->last_node().c().id() &&
    ...
    topology::experimental_vector dist(isequence->last_node().c().ep(),
    isequence->last_node().ep()); hman.fill("end_point_map", dist.z().value(), dist.x().value());
    }
...
void sequentiator::make_plots(topology::tracked_data /*__tracked_data*/) {
  ...
  /*
    ...
    for(std::vector<topology::sequence>::iterator isequence = sequences_.begin(); isequence !=
    sequences_.end(); ++isequence){
...
void sequentiator::sequentiate_cluster(topology::cluster &cluster_) {
  ...
  for (vector<topology::node>::iterator inode = cluster_.nodes_.begin();
...
void sequentiator::make_new_sequence(topology::node &first_node) {
  ...
  if (level >= mybhep::VERBOSE) {
    clog << "CAT::sequentiator::make_new_sequence: begin track [" << sequences_.size()
         << "] with node " << first_node.c().id() << endl;
  ...
  if (level >= mybhep::VERBOSE) print_a_sequence(newsequence);
  ...
  if (newsequence.nodes().size() != 2) {
    ...
    m.message("CAT::sequentiator::make_new_sequence:  finished track [", sequences_.size() - 1,
              "] ", mybhep::VERBOSE);
    fflush(stdout);
    clean_up_sequences();
  } else {
...
//*************************************************************
topology::joint sequentiator::find_best_matching_joint(topology::joint j,
                                                       std::vector<topology::joint> js,
                                                       topology::cell A, topology::cell B,
                                                       topology::cell C, double *chi2,
                                                       bool A_is_on_gap, bool B_is_on_gap) {
  //*************************************************************
...
void sequentiator::make_new_sequence_after_sultan() {
  ...
  if (level >= mybhep::VERBOSE) {
    ...
    for (std::vector<topology::node>::const_iterator in = local_cluster_->nodes_.begin();
         in != local_cluster_->nodes_.end(); ++in)
      std::clog << " [" << in->c().id() << "]";
    std::clog << " " << std::endl;
  }

  if (s == 0) {
    m.message(" problem: CAT::sequentiator::make_new_sequence_after_sultan: cluster size is ", s,
              mybhep::NORMAL);
    return;
  }
  ...
  if (s == 3) {
    ...
    m.message(" CAT::sequentiator::make_new_sequence_after_sultan: make sequence with size ", s,
              " and ", joints.size(), " joints ", mybhep::VERBOSE);
    for (std::vector<topology::joint>::const_iterator ij = joints.begin(); ij != joints.end();
         ++ij) {
      topology::sequence newsequence(local_cluster_->nodes(), level, probmin);
  ...
  if (build_sequences_from_ambiguous_alternatives(sets_of_bl_alternatives, &seqs)) {
    for (std::vector<topology::sequence>::iterator iseq = seqs.begin(); iseq != seqs.end();
         ++iseq) {
      make_name(*iseq);
...
//*************************************************************
bool sequentiator::increase_iterations(
    std::vector<std::vector<topology::broken_line> > sets_of_bl_alternatives,
    std::vector<size_t> *iterations, int *block_which_is_increasing, int *first_augmented_block) {
  //*************************************************************

  clock.start(" sequentiator: increase_iterations ", "cumulative");

  iterations->at(*block_which_is_increasing)++;
  if (iterations->at(*block_which_is_increasing) ==
      sets_of_bl_alternatives[*block_which_is_increasing].size()) {
    ...
    if (prev_block < *first_augmented_block) *first_augmented_block = prev_block;
  }
...
//*************************************************************
bool sequentiator::build_sequences_from_ambiguous_alternatives(
    std::vector<std::vector<topology::broken_line> > sets_of_bl_alternatives,
    std::vector<topology::sequence> *seqs) {
  //*************************************************************
  ...
  if (level >= mybhep::VERBOSE) {
    std::clog << " CAT::cluster::build_sequences_from_ambiguous_alternatives: there are "
              << sets_of_bl_alternatives.size() << " sets_of_bl_alternatives " << std::endl;
    for (std::vector<std::vector<topology::broken_line> >::const_iterator iset =
             sets_of_bl_alternatives.begin();
         iset != sets_of_bl_alternatives.end(); ++iset) {
      std::clog << " CAT::cluster::build_sequences_from_ambiguous_alternatives: set "
                << iset - sets_of_bl_alternatives.begin() << " has " << iset->size()
                << " alternatives, from " << local_cluster_->nodes_[iset->front().ifirst()].c().id()
                << " to " << local_cluster_->nodes_[iset->front().ilast()].c().id() << std::endl;
    }
  }
  ...
  for (std::vector<topology::node>::const_iterator inode = local_cluster_->nodes_.begin() + 1;
       inode != local_cluster_->nodes_.end() - 1; ++inode) {
    base_bl.eps_.push_back(inode->ccc()[0].joints()[0].epb());
  }
  base_bl.eps_.push_back(
      local_cluster_->nodes_[local_cluster_->nodes_.size() - 2].ccc()[0].joints()[0].epc());

  if (sets_of_bl_alternatives.size() == 0) {
    topology::sequence best_seq(local_cluster_->nodes(), level, probmin);
    for (std::vector<topology::node>::iterator inode = best_seq.nodes_.begin();
         inode != best_seq.nodes_.end(); ++inode) {
  ...
  while (true) {
    if (level >= mybhep::VERBOSE) {
      std::clog << " iterations: (";
      for (std::vector<size_t>::const_iterator it = iterations.begin(); it != iterations.end();
           ++it)
        std::clog << " " << *it;
      std::clog << ")" << std::endl;
    }
    ...
    for (std::vector<topology::broken_line>::const_iterator ibl = bls.begin(); ibl != bls.end();
         ++ibl) {
      size_t ifirst = ibl->ifirst();
      for (std::vector<topology::experimental_point>::const_iterator ipoint = ibl->eps_.begin();
           ipoint != ibl->eps_.end(); ++ipoint)
        local_bl.eps_[ifirst + ipoint - ibl->eps_.begin()] = *ipoint;
    }

    for (std::vector<topology::experimental_point>::const_iterator ipoint = local_bl.eps_.begin();
         ipoint != local_bl.eps_.end(); ++ipoint)
      local_seq.nodes_[ipoint - local_bl.eps_.begin()].set_ep(*ipoint);

    if (level >= mybhep::VERBOSE) {
      std::clog << " points: [";
      for (std::vector<topology::node>::iterator inode = local_seq.nodes_.begin();
           inode != local_seq.nodes_.end(); ++inode) {
    ...
    if (!local_seq.calculate_helix(Ratio, conserve_clustering_from_removal,
                                   conserve_clustering_from_reordering) &&
        !local_seq.has_kink()) {
      m.message("CAT::sequentiator::build_sequences_from_ambiguous_alternatives: not a good helix",
                mybhep::VERBOSE);
      fflush(stdout);
      if (!increase_iterations(sets_of_bl_alternatives, &iterations, &block_which_is_increasing,
                               &first_augmented_block))
        break;
      continue;
    }
...
void sequentiator::make_copy_sequence(topology::node &first_node) {
  ...
  if (local_devel) {
    ...
    clog << "DEVEL: "
         << "CAT::sequentiator::make_copy_sequence: "
         << "Entering..." << endl;
  }
  ...
  while (there_is_free_sequence_beginning_with(first_node.c(), &isequence)) {
    ...
    m.message("CAT::sequentiator::make_copy_sequence: begin, with cell", first_node.c().id(),
              ", parallel track ", sequences_.size(), " to track ", isequence, mybhep::VERBOSE);
    fflush(stdout);

    if (level >= mybhep::VVERBOSE) {
      std::clog << "CAT::sequentiator::make_copy_sequence: original sequence before copying: "
                << std::endl;
    ...
    m.message("CAT::sequentiator::make_copy_sequence: copied from sequence  ", isequence,
              mybhep::VVERBOSE);
    fflush(stdout);

    if (level >= mybhep::VVERBOSE) {
      m.message("CAT::sequentiator::make_copy_sequence: original sequence after copy ",
                mybhep::VVERBOSE);
      fflush(stdout);
      print_a_sequence(sequences_[isequence]);
      m.message("CAT::sequentiator::make_copy_sequence: new copy ", mybhep::VVERBOSE);
      fflush(stdout);
    ...
    bool updated = true;
    while (updated) updated = evolve(newcopy);
    clock.stop(" sequentiator: make copy sequence: evolve ");

    if (late()) return;

    if (level >= mybhep::VVERBOSE) print_a_sequence(newcopy);
    ...
    if (newcopy.nodes().size() == ilfn + 1) {
      m.message(
          "CAT::sequentiator::make_copy_sequence: not adding new sequence, since it couldn't "
          "evolve past lfn ",
          mybhep::VERBOSE);
      fflush(stdout);
      clean_up_sequences();
      ...
      if (newcopy.nodes().size() > ilfn + 1) {
        if (!sequences_[isequence].nodes().empty()) {
          ...
          m.message("CAT::sequentiator::make_copy_sequence: setting as used original node ", ilfn,
                    "  cc ", it1, mybhep::VVERBOSE);
          if (ilfn == 0)
      ...
      if (newcopy.contained(sequences_[isequence]) &&
          !newcopy.Free())  // new copy is contained in original
      {
        m.message("CAT::sequentiator::make_copy_sequence: not adding new sequence, contained in ",
                  isequence, "from which it was copied", mybhep::VERBOSE);
        fflush(stdout);
        clean_up_sequences();
      } else {                                           // adding: case 3
        if (sequences_[isequence].contained(newcopy)) {  // original is contained in new copy
          for (size_t k = 0; k < ilfn; k++) {
            ...
            for (vector<topology::cell_couplet>::iterator icc =
                     sequences_[isequence].nodes_[k].cc_.begin();
                 icc != sequences_[isequence].nodes_[k].cc_.end(); ++icc) {
              newcopy.nodes_[k].cc_[icc - sequences_[isequence].nodes_[k].cc_.begin()].set_free(
                  icc->free());
              newcopy.nodes_[k].cc_[icc - sequences_[isequence].nodes_[k].cc_.begin()].set_begun(
                  icc->begun());

              for (vector<topology::line>::iterator itang =
                       sequences_[isequence]
                           .nodes_[k]
                           .cc_[icc - sequences_[isequence].nodes_[k].cc_.begin()]
                           .tangents_.begin();
                   itang != sequences_[isequence]
                                .nodes_[k]
                                .cc_[icc - sequences_[isequence].nodes_[k].cc_.begin()]
                                .tangents_.end();
                   ++itang)
                newcopy.nodes_[k]
                    .cc_[icc - sequences_[isequence].nodes_[k].cc_.begin()]
                    .tangents_[itang - sequences_[isequence]
                                           .nodes_[k]
                                           .cc_[icc - sequences_[isequence].nodes_[k].cc_.begin()]
                                           .tangents_.begin()]
                    .set_used(itang->used());
            ...
            for (vector<topology::cell_triplet>::iterator iccc =
                     sequences_[isequence].nodes_[k].ccc_.begin();
                 iccc != sequences_[isequence].nodes_[k].ccc_.end(); ++iccc) {
              newcopy.nodes_[k].ccc_[iccc - sequences_[isequence].nodes_[k].ccc_.begin()].set_free(
                  iccc->free());
              newcopy.nodes_[k].ccc_[iccc - sequences_[isequence].nodes_[k].ccc_.begin()].set_begun(
                  iccc->begun());

              for (vector<topology::joint>::iterator ijoint =
                       sequences_[isequence]
                           .nodes_[k]
                           .ccc_[iccc - sequences_[isequence].nodes_[k].ccc_.begin()]
                           .joints_.begin();
                   ijoint != sequences_[isequence]
                                 .nodes_[k]
                                 .ccc_[iccc - sequences_[isequence].nodes_[k].ccc_.begin()]
                                 .joints_.end();
                   ++ijoint)
                newcopy.nodes_[k]
                    .ccc_[iccc - sequences_[isequence].nodes_[k].ccc_.begin()]
                    .joints_[ijoint - sequences_[isequence]
                                          .nodes_[k]
                                          .ccc_[iccc - sequences_[isequence].nodes_[k].ccc_.begin()]
                                          .joints_.begin()]
                    .set_used(ijoint->used());
            }
          }

          if (ilfn < 2) {
            for (vector<topology::cell_couplet>::iterator icc =
                     sequences_[isequence].nodes_[ilfn].cc_.begin();
                 icc != sequences_[isequence].nodes_[ilfn].cc_.end(); ++icc)
              if ((size_t)(icc - sequences_[isequence].nodes_[ilfn].cc_.begin()) != ilink) {
                newcopy.nodes_[ilfn]
                    .cc_[icc - sequences_[isequence].nodes_[ilfn].cc_.begin()]
                    .set_free(icc->free());
                newcopy.nodes_[ilfn]
                    .cc_[icc - sequences_[isequence].nodes_[ilfn].cc_.begin()]
                    .set_begun(icc->begun());
            ...
            for (vector<topology::cell_triplet>::iterator iccc =
                     sequences_[isequence].nodes_[ilfn].ccc_.begin();
                 iccc != sequences_[isequence].nodes_[ilfn].ccc_.end(); ++iccc)
              if ((size_t)(iccc - sequences_[isequence].nodes_[ilfn].ccc_.begin()) != ilink) {
                newcopy.nodes_[ilfn]
                    .ccc_[iccc - sequences_[isequence].nodes_[ilfn].ccc_.begin()]
                    .set_free(iccc->free());
                newcopy.nodes_[ilfn]
                    .ccc_[iccc - sequences_[isequence].nodes_[ilfn].ccc_.begin()]
                    .set_begun(iccc->begun());
          ...
          m.message("CAT::sequentiator::make_copy_sequence: erased original sequence ", isequence,
                    "contained in sequence", sequences_.size() + 1, "which was copied from it",
                    mybhep::VERBOSE);
          fflush(stdout);
          clean_up_sequences();
        }
        ...
        if (newcopy.nodes().size() != 2) {
          ...
          m.message("CAT::sequentiator::make_copy_sequence: finished track [",
                    sequences_.size() - 1, "] ", mybhep::VERBOSE);
          fflush(stdout);
          clean_up_sequences();
        } else {
          add_pair(newcopy);
...
void sequentiator::make_copy_sequence_after_sultan() {
  ...
  if (local_devel) {
    level = mybhep::VVERBOSE;
    clog << "DEVEL: "
         << "CAT::sequentiator::make_copy_sequence_after_sultan: "
         << "Entering..." << endl;
  }
  ...
  while (there_is_free_sequence_beginning_with(first_node.c(), &isequence)) {
    ...
    m.message("CAT::sequentiator::make_copy_sequence_after_sultan: begin, with cell",
              first_node.c().id(), ", parallel track ", sequences_.size(), " to track ", isequence,
              mybhep::VERBOSE);
    fflush(stdout);

    if (level >= mybhep::VVERBOSE) {
      std::clog << "CAT::sequentiator::make_copy_sequence_after_sultan: original sequence before "
                   "copying: "
                << std::endl;
      print_a_sequence(sequences_[isequence]);
    }
    ...
    m.message("CAT::sequentiator::make_copy_sequence_after_sultan: copied from sequence  ",
              isequence, mybhep::VVERBOSE);
    fflush(stdout);

    if (level >= mybhep::VVERBOSE) {
      m.message("CAT::sequentiator::make_copy_sequence_after_sultan: original sequence after copy ",
                mybhep::VVERBOSE);
      fflush(stdout);
      print_a_sequence(sequences_[isequence]);
      m.message("CAT::sequentiator::make_copy_sequence_after_sultan: new copy ", mybhep::VVERBOSE);
      fflush(stdout);
      print_a_sequence(newcopy);
    }
    ...
    if (newcopy.nodes().size() == ilfn + 1) {
      m.message(
          "CAT::sequentiator::make_copy_sequence_after_sultan: not adding new sequence, since it "
          "couldn't evolve past lfn ",
          mybhep::VERBOSE);
      fflush(stdout);
      clean_up_sequences();
    } else {
      ...
      if (newcopy.nodes().size() > ilfn + 1) {
        if (!sequences_[isequence].nodes().empty()) {
          ...
          clock.stop(" sequentiator: get link index ");
          m.message(
              "CAT::sequentiator::make_copy_sequence_after_sultan: setting as used original node ",
              ilfn, "  cc ", it1, mybhep::VVERBOSE);
          if (ilfn == 0)
            sequences_[isequence].nodes_[ilfn].cc_[it1].set_all_used();
          else
            sequences_[isequence].nodes_[ilfn].ccc_[it1].set_all_used();
      ...
      {
        m.message(
            "CAT::sequentiator::make_copy_sequence_after_sultan: not adding new sequence, "
            "contained in ",
            isequence, "from which it was copied", mybhep::VERBOSE);
        fflush(stdout);
        clean_up_sequences();
        ...
        if (sequences_[isequence].contained(newcopy)) {  // original is contained in new copy
          ...
          sequences_.erase(sequences_.begin() + isequence);
          m.message("CAT::sequentiator::make_copy_sequence_after_sultan: erased original sequence ",
                    isequence, "contained in sequence", sequences_.size() + 1,
                    "which was copied from it", mybhep::VERBOSE);
          fflush(stdout);
          clean_up_sequences();
        }
        ...
        if (newcopy.nodes().size() != 2) {
          make_name(newcopy);
          sequences_.push_back(newcopy);
          m.message("CAT::sequentiator::make_copy_sequence_after_sultan: finished track [",
                    sequences_.size() - 1, "] ", mybhep::VERBOSE);
          fflush(stdout);
          clean_up_sequences();
        } else {
...
bool sequentiator::late(void) {
  ...
  if (clock.read(" sequentiator: sequentiation ") >= MaxTime) {
    m.message("CAT::sequentiator::late: execution time ",
              clock.read(" sequentiator: sequentiation "), " ms  greater than MaxTime", MaxTime,
              " quitting! ", mybhep::NORMAL);
...
bool sequentiator::evolve(topology::sequence &sequence) {
  ...
  m.message("CAT::sequentiator::evolve: evolving sequence of size", sequence_size,
            mybhep::VVERBOSE);
  fflush(stdout);

  // protection
  if (sequence_size < 1) {
    m.message("CAT::sequentiator::evolve: problem: sequence has length ", sequence_size,
              "... stop evolving ", mybhep::NORMAL);
    fflush(stdout);
  ...
  if (level >= mybhep::VVERBOSE) print_a_sequence(sequence);

  if (sequence_size == 3) {
    clock.start(" sequentiator: get link index ", "cumulative");
    size_t it1 = sequence.get_link_index_of_cell(0, sequence.nodes()[1].c());
    if (it1 >= sequence.nodes_[0].cc_.size()) {
      m.message("CAT::sequentiator::evolve: problem: it1 ", it1, " nodes size ",
                sequence.nodes_.size(), " cc size ", sequence.nodes_[0].cc_.size(), mybhep::NORMAL);
      fflush(stdout);
      clock.stop(" sequentiator: evolve: part A ");
      clock.stop(" sequentiator: evolve ");
      return false;
  ...
  if (!there_is_link) {
    m.message("CAT::sequentiator::evolve: no links could be added... stop evolving ",
              mybhep::VERBOSE);
    fflush(stdout);
  ...
  sequence.nodes_.push_back(newnode);
  m.message("CAT::sequentiator::evolve: points have been added ", mybhep::VERBOSE);
  fflush(stdout);
...
bool sequentiator::good_first_node(topology::node &node_) {
  ...
  if (type != "VERTEX" && type != "MULTI_VERTEX" && type != "ISOLATED") {
    // clock.stop(" sequentiator: good first node ");
    m.message("CAT::sequentiator::good_first_node: not a good first node: type ", type,
              mybhep::VVERBOSE);
    fflush(stdout);
    return false;
  }
  ...
  for (vector<topology::sequence>::const_iterator iseq = sequences_.begin();
    ...
    if (iseq->has_cell(node_.c())) {
      if (type == "VERTEX") {
        m.message(
            "CAT::sequentiator::good_first_node: not a good first node: already used as vertex in "
            "seuqence ",
            iseq - sequences_.begin(), mybhep::VVERBOSE);
        fflush(stdout);
        ...
        if (iseq->nodes_.size() > 1) {
          if (iseq->nodes_[0].c().id() == node_.c().id())
            ...
            m.message("CAT::sequentiator::good_first_node: problem: multi-vertex ", node_.c().id(),
                      " belongs to sequence ", iseq->name(), " but not as first or last cell",
                      mybhep::NORMAL);
            continue;
          }
          // add to done_connections cell ids of those cells
          // that have already been connected to NODE in other sequences
          fid = std::find(done_connections.begin(), done_connections.end(),
  ...
  if (type == "MULTI_VERTEX") {
    for (size_t i = 0; i < done_connections.size(); i++) {
      ...
      if (!node_.has_couplet(done_connections[i], &cc_index))
        ...
        m.message("CAT::sequentiator::good_first_node: multi-vertex ", node_.c().id(),
                  " has already been added to a sequence connecting to cell ", done_connections[i],
                  " so couplet ", cc_index, " will be erased", mybhep::VERBOSE);
...
void sequentiator::make_families() {
  ...
  for (std::vector<topology::sequence>::const_iterator iseq = sequences_.begin();
    ...
    if (found) {
      ...
      if (!added) families_[ifam].push_back(ipart);
    } else {
      Fam.clear();
...
bool sequentiator::make_scenarios(topology::tracked_data &td, bool after_sultan) {
  ...
  if (level >= mybhep::VERBOSE) print_families();
  ...
  for (std::vector<topology::sequence>::iterator iseq = sequences_.begin();
    ...
    m.message("CAT::sequentiator::make_scenarios: begin scenario with sequence ", iseq->name(),
              mybhep::VVERBOSE);
    if (level >= mybhep::VVERBOSE) print_a_sequence(*iseq, after_sultan);
    ...
    while (can_add_family(sc, &jmin, &nfree, &Chi2, &noverlaps, &ndof, td, after_sultan)) {
      m.message("CAT::sequentiator::make_scenarios: best sequence to add is ", jmin,
                mybhep::VVERBOSE);
      if (level >= mybhep::VVERBOSE) print_a_sequence(sequences_[jmin], after_sultan);
      m.message("CAT::sequentiator::make_scenarios: nfree ", nfree, " noverls ", noverlaps,
                " Chi2 ", Chi2, mybhep::VVERBOSE);
  ...
  if (level >= mybhep::VERBOSE) print_scenarios(after_sultan);

  if (scenarios_.size() > 0) {
    size_t index_tmp = pick_best_scenario();

    if (level > mybhep::NORMAL) print_a_scenario(scenarios_[index_tmp], after_sultan);
...
size_t sequentiator::pick_best_scenario() {
  ...
  for (std::vector<topology::scenario>::iterator sc = scenarios_.begin(); sc != scenarios_.end();
       ++sc) {
    if (level >= mybhep::VVERBOSE)
      std::clog << "CAT::sequentiator::pick_best_scenario: ...scenario " << sc - scenarios_.begin()
...
//*************************************************************
bool sequentiator::can_add_family(topology::scenario &sc, size_t *jmin, size_t *nfree, double *Chi2,
                                  size_t *noverlaps, int *ndof, topology::tracked_data &td,
                                  bool after_sultan) {
  //*************************************************************
  ...
  for (std::vector<topology::sequence>::iterator iseq = sc.sequences_.begin();
       iseq != sc.sequences_.end(); ++iseq)
    scnames[iseq->name()] = iseq - sc.sequences_.begin();

  for (std::vector<topology::sequence>::iterator jseq = sequences_.begin();
       jseq != sequences_.end(); ++jseq) {
    ...
    m.message("CAT::sequentiator::can_add_family: ...try to add sequence ", jseq->name(),
              mybhep::VVERBOSE);
    if (level >= mybhep::VVERBOSE) print_a_sequence(*jseq, after_sultan);
    m.message("CAT::sequentiator::can_add_family: ...nfree ", tmp.n_free_families(), " noverls ",
              tmp.n_overlaps(), " chi2 ", tmp.helix_chi2(), " prob ", tmp.helix_Prob(),
              mybhep::VVERBOSE);
...
topology::plane sequentiator::get_foil_plane() {
  ...
  if (!SuperNemo) {
    m.message("CAT::sequentiator::get_foil_plane: problem: asking for foil plane in Nemo3! ",
              mybhep::NORMAL);
    topology::plane null;
    return null;
  }
  ...
  topology::experimental_vector sizes(xsize, ysize, 0., 0., 0., 0.);

  topology::plane pl(center, sizes, norm, level, probmin);

  std::string the_type = "Nemo3";
  if (SuperNemo) the_type = "SuperNEMO";
...
topology::circle sequentiator::get_foil_circle() {
  ...
  if (SuperNemo) {
    m.message("CAT::sequentiator::get_foil_circle: problem: asking for foil circle in SuperNemo ",
              mybhep::NORMAL);
...
void sequentiator::refine_sequences_near_walls(std::vector<topology::calorimeter_hit> &calos) {
  ...
  for (std::vector<topology::sequence>::iterator iseq = sequences_.begin();
    ...
    if (gap_number(iseq->second_last_node().c()) == 0 &&
      ...
      m.message("CAT::sequentiator::refine_sequences_near_walls: removing last node ",
                iseq->last_node().c(), " near foil of sequence ", iseq->name(),
                "(it belongs to other family and makes large kink) ", mybhep::VVERBOSE);
      fflush(stdout);
      iseq->remove_last_node();
    }

    if (iseq->nodes_.size() < 3) continue;
    if (gap_number(iseq->nodes_[1].c()) == 0 && iseq->phi_kink(1) * 180. / M_PI > 45 &&
        belongs_to_other_family(iseq->nodes_[0].c(), &(*iseq))) {
      m.message("CAT::sequentiator::refine_sequences_near_walls: removing 1st node ",
                iseq->last_node().c(), " near foil of sequence ", iseq->name(),
                "(it belongs to other family and makes large kink) ", mybhep::VVERBOSE);
      fflush(stdout);
    ...
    if (iseq->nodes_.size() < 3) continue;
    for (std::vector<topology::calorimeter_hit>::iterator ic = calos.begin(); ic != calos.end();
         ++ic) {
      if (near(iseq->nodes_[1].c(), *ic) && iseq->phi_kink(1) * 180. / M_PI > 45 &&
          belongs_to_other_family(iseq->nodes_[0].c(), &(*iseq))) {
        m.message("CAT::sequentiator::refine_sequences_near_walls: removing 1st node ",
                  iseq->last_node().c(), " near calo of sequence ", iseq->name(),
                  "(it belongs to other family and makes large kink) ", mybhep::VVERBOSE);
        fflush(stdout);
        iseq->remove_first_node();
        break;
...
void sequentiator::interpret_physics(std::vector<topology::calorimeter_hit> &calos) {
  ...
  m.message("CAT::sequentiator::interpret_physics: interpreting physics of ", sequences_.size(),
            " sequences with ", calos.size(), " calorimeter hits ", mybhep::VVERBOSE);
  fflush(stdout);
  ...
  topology::experimental_point tangent_extrapolation_from_begin,
      tangent_extrapolation_local_from_begin;
  ...
  while (iseq != sequences_.end()) {
    m.message("CAT::sequentiator::interpret_physics: ... interpreting physics of sequence ",
              iseq->name(), mybhep::VVERBOSE);
    fflush(stdout);
    ...
    if (level >= mybhep::VVERBOSE) print_a_sequence(*iseq);

    // by default, conserve_clustering_from_removal_of_whole_cluster = false and
    // conserve_clustering_from_reordering = true
    if (!iseq->calculate_helix(Ratio) && !iseq->has_kink()) {
      size_t index = iseq - sequences_.begin();
      m.message("CAT::sequentiator::interpret_physics: erased sequence ", index, "not a good helix",
                mybhep::VERBOSE);
      fflush(stdout);
      sequences_.erase(iseq);
      iseq = sequences_.begin() + index;
      if (index + 1 >= sequences_.size()) break;
      continue;
    }
    ...
    if (!calos.empty()) {
      m.message("CAT::sequentiator::interpret_physics: extrapolate decay vertex with ",
                calos.size(), " calo hits ", mybhep::VVERBOSE);
      ...
      for (std::vector<topology::calorimeter_hit>::iterator ic = calos.begin(); ic != calos.end();
           ++ic) {
        m.message("CAT::sequentiator::interpret_physics: trying to extrapolate to calo hit ",
                  ic - calos.begin(), " id ", ic->id(), " on view ", ic->pl_.view(), " energy ",
                  ic->e().value(), mybhep::VVERBOSE);
        ...
        if (!near(iseq->last_node().c(), *ic)) {
          ...
          if (!iseq->intersect_plane_from_end(ic->pl(), &helix_extrapolation_local_from_end)) {
            m.message("CAT::sequentiator::interpret_physics: no helix intersection from end ",
                      mybhep::VVERBOSE);
          } else {
            ...
            if (dist_from_end < helix_min_from_end) {
              ...
              m.message(
                  "CAT::sequentiator::interpret_physics: new helix intersection from end with "
                  "minimum distance ",
                  dist_from_end, " position: ", helix_extrapolation_from_end.x().value(),
                  helix_extrapolation_from_end.y().value(),
                  helix_extrapolation_from_end.z().value(), mybhep::VVERBOSE);
            }
          }

          if (!iseq->intersect_plane_with_tangent_from_end(ic->pl(),
                                                           &tangent_extrapolation_local_from_end)) {
            m.message("CAT::sequentiator::interpret_physics: no tangent intersection from end ",
                      mybhep::VVERBOSE);
          } else {
            ...
            if (dist_from_end < tangent_min_from_end) {
              ...
              m.message(
                  "CAT::sequentiator::interpret_physics: new tangent intersection from end with "
                  "minimum distance ",
                  dist_from_end, " position: ", tangent_extrapolation_from_end.x().value(),
                  tangent_extrapolation_from_end.y().value(),
                  tangent_extrapolation_from_end.z().value(), mybhep::VVERBOSE);
            }
          }
        }

        if (!near(iseq->nodes_[0].c(), *ic)) {
          m.message("CAT::sequentiator::interpret_physics: beginning is not near ",
                    mybhep::VVERBOSE);
        } else if (ihelix_min_from_end == ic->id() || itangent_min_from_end == ic->id()) {
          m.message(
              "CAT::sequentiator::interpret_physics: beginning is near, but end was already "
              "extrapolated to same calo ",
              mybhep::VVERBOSE);
        } else {
          if (!iseq->intersect_plane_from_begin(ic->pl(), &helix_extrapolation_local_from_begin)) {
            m.message("CAT::sequentiator::interpret_physics: no helix intersection from beginning ",
                      mybhep::VVERBOSE);
          } else {
            ...
            if (dist_from_begin < helix_min_from_begin) {
              ...
              m.message(
                  "CAT::sequentiator::interpret_physics: new helix intersection from beginning "
                  "with minimum distance ",
                  dist_from_begin, " position: ", helix_extrapolation_from_begin.x().value(),
                  helix_extrapolation_from_begin.y().value(),
                  helix_extrapolation_from_begin.z().value(), mybhep::VVERBOSE);
            }
          }

          if (!iseq->intersect_plane_with_tangent_from_begin(
                  ic->pl(), &tangent_extrapolation_local_from_begin)) {
            m.message(
                "CAT::sequentiator::interpret_physics: no tangent intersection from beginning ",
                mybhep::VVERBOSE);
          } else {
            dist_from_begin =
                tangent_extrapolation_local_from_begin.distance(ic->pl_.face()).value();
            if (dist_from_begin < tangent_min_from_begin) {
              ...
              m.message(
                  "CAT::sequentiator::interpret_physics: new tangent intersection from beginning "
                  "with minimum distance ",
                  dist_from_begin, " position: ", tangent_extrapolation_from_begin.x().value(),
                  tangent_extrapolation_from_begin.y().value(),
                  tangent_extrapolation_from_begin.z().value(), mybhep::VVERBOSE);
            }
          }
        }
      ...
      if (helix_found_from_begin) {
        if (ihelix_min_from_begin >= calos.size()) {
          m.message("CAT::sequentiator::interpret_physics: problem: calo hit of id ",
                    ihelix_min_from_begin, " but n of calo hits is ", calos.size(), mybhep::NORMAL);
        } else {
          m.message("CAT::sequentiator::interpret_physics: track extrapolated by helix to calo ",
                    ihelix_min_from_begin, mybhep::VVERBOSE);
      ...
      if (tangent_found_from_begin) {
        if (itangent_min_from_begin >= calos.size()) {
          m.message("CAT::sequentiator::interpret_physics: problem: tangent calo hit of id ",
                    itangent_min_from_begin, " but n of calo hits is ", calos.size(),
                    mybhep::NORMAL);
        } else {
          m.message("CAT::sequentiator::interpret_physics: track extrapolated by tangent to calo ",
                    itangent_min_from_begin, mybhep::VVERBOSE);
          iseq->set_tangent_vertex(tangent_extrapolation_from_begin, "calo",
                                   itangent_min_from_begin);
        }
      }
      ...
      if (helix_found_from_end) {
        if (ihelix_min_from_end >= calos.size()) {
          m.message("CAT::sequentiator::interpret_physics: problem: calo hit of id ",
                    ihelix_min_from_end, " but n of calo hits is ", calos.size(), mybhep::NORMAL);
        } else {
          m.message("CAT::sequentiator::interpret_physics: track extrapolated by helix to calo ",
                    ihelix_min_from_end, mybhep::VVERBOSE);
      ...
      if (tangent_found_from_end) {
        if (itangent_min_from_end >= calos.size()) {
          m.message("CAT::sequentiator::interpret_physics: problem: tangent calo hit of id ",
                    itangent_min_from_end, " but n of calo hits is ", calos.size(), mybhep::NORMAL);
        } else {
          m.message("CAT::sequentiator::interpret_physics: track extrapolated by tangent to calo ",
                    itangent_min_from_end, mybhep::VVERBOSE);
          iseq->set_decay_tangent_vertex(tangent_extrapolation_from_end, "calo",
                                         itangent_min_from_end);
        }
      }
    }
    ...
    if (!iseq->nodes_.empty()) {
      m.message("CAT::sequentiator::interpret_physics: extrapolate vertex on foil: supernemo ",
                SuperNemo, mybhep::VVERBOSE);

      if (gap_number(iseq->last_node().c()) != 0) {
        m.message("CAT::sequentiator::interpret_physics: end not near ", mybhep::VVERBOSE);
        fflush(stdout);
      } else {
        if (SuperNemo) {
          ...
          if (!iseq->intersect_plane_with_tangent_from_end(get_foil_plane(),
                                                           &tangent_extrapolation_from_end)) {
            m.message("CAT::sequentiator::interpret_physics: no tangent intersection from end ",
                      mybhep::VVERBOSE);
            fflush(stdout);
          } else
          ...
          if (!iseq->intersect_circle_from_end(get_foil_circle(), &helix_extrapolation_from_end)) {
            ...
            m.message(
                "CAT::sequentiator::interpret_physics: track extrapolated by helix to foil from "
                "end ",
                mybhep::VVERBOSE);
            iseq->set_decay_helix_vertex(helix_extrapolation_from_end, "foil");
          }

          if (!iseq->intersect_circle_with_tangent_from_end(get_foil_circle(),
                                                            &tangent_extrapolation_from_end)) {
            m.message("CAT::sequentiator::interpret_physics: no tangent intersection from end ",
                      mybhep::VVERBOSE);
            fflush(stdout);
            ...
            m.message(
                "CAT::sequentiator::interpret_physics: track extrapolated by tangent to foil from "
                "end",
                mybhep::VVERBOSE);
            iseq->set_decay_tangent_vertex(tangent_extrapolation_from_end, "foil");
          }
      ...
      if (gap_number(iseq->nodes_[0].c()) != 0) {
        m.message("CAT::sequentiator::interpret_physics: beginning not near ", mybhep::VVERBOSE);
        fflush(stdout);
      } else {
        if (SuperNemo) {
          if (!iseq->intersect_plane_from_begin(get_foil_plane(),
                                                &helix_extrapolation_from_begin)) {
            m.message("CAT::sequentiator::interpret_physics: no helix intersection from beginning ",
                      mybhep::VVERBOSE);
            fflush(stdout);
          } else {
          ...
          if (!iseq->intersect_plane_with_tangent_from_begin(get_foil_plane(),
                                                             &tangent_extrapolation_from_begin)) {
            m.message(
                "CAT::sequentiator::interpret_physics: no tangent intersection from beginning ",
                mybhep::VVERBOSE);
            fflush(stdout);
          ...
          if (!iseq->intersect_circle_from_begin(get_foil_circle(),
                                                 &helix_extrapolation_from_begin)) {
            m.message("CAT::sequentiator::interpret_physics: no helix intersection from beginning ",
                      mybhep::VVERBOSE);
            fflush(stdout);
          ...
          if (!iseq->intersect_circle_with_tangent_from_begin(get_foil_circle(),
                                                              &tangent_extrapolation_from_begin)) {
            m.message(
                "CAT::sequentiator::interpret_physics: no tangent intersection from beginning ",
                mybhep::VVERBOSE);
    ...
    if (level >= mybhep::VVERBOSE) {
      std::clog << "CAT::sequentiator::interpret_physics: sequence " << iseq - sequences_.begin()
                << " has: " << std::endl;
      fflush(stdout);
      std::clog << "CAT::sequentiator::interpret_physics: center ";
      iseq->center().dump();
      fflush(stdout);
      std::clog << "CAT::sequentiator::interpret_physics: radius ";
      iseq->radius().dump();
      std::clog << " " << std::endl;
      fflush(stdout);
      std::clog << "CAT::sequentiator::interpret_physics: pitch ";
      iseq->pitch().dump();
      std::clog << " " << std::endl;
      fflush(stdout);
      std::clog << "CAT::sequentiator::interpret_physics: momentum ";
      iseq->momentum().length().dump();
      std::clog << " " << std::endl;
      fflush(stdout);
      std::clog << "CAT::sequentiator::interpret_physics: charge ";
      iseq->charge().dump();
      std::clog << " " << std::endl;
      fflush(stdout);
      if (iseq->has_helix_vertex()) {
        std::clog << "CAT::sequentiator::interpret_physics: helix_vertex "
                  << iseq->helix_vertex_type() << " ";
        iseq->helix_vertex().dump();
        if (iseq->helix_vertex_type() == "calo") std::clog << " icalo " << iseq->helix_vertex_id();
        std::clog << " " << std::endl;
        fflush(stdout);
      }
      if (iseq->has_decay_helix_vertex()) {
        std::clog << "CAT::sequentiator::interpret_physics: decay helix_vertex "
                  << iseq->decay_helix_vertex_type() << " ";
        iseq->decay_helix_vertex().dump();
        if (iseq->decay_helix_vertex_type() == "calo")
          std::clog << " icalo " << iseq->calo_helix_id();
        std::clog << " " << std::endl;
        fflush(stdout);
      }
      if (iseq->has_tangent_vertex()) {
        std::clog << "CAT::sequentiator::interpret_physics: tangent_vertex "
                  << iseq->tangent_vertex_type() << " ";
        iseq->tangent_vertex().dump();
        if (iseq->tangent_vertex_type() == "calo")
          std::clog << " icalo " << iseq->tangent_vertex_id();
        std::clog << " " << std::endl;
        fflush(stdout);
      }
      if (iseq->has_decay_tangent_vertex()) {
        std::clog << "CAT::sequentiator::interpret_physics: decay tangent_vertex "
                  << iseq->decay_tangent_vertex_type() << " ";
        iseq->decay_tangent_vertex().dump();
        if (iseq->decay_tangent_vertex_type() == "calo")
          std::clog << " icalo " << iseq->calo_tangent_id();
        std::clog << " " << std::endl;
        fflush(stdout);
      }
      if (iseq->has_tangent_length()) {
        std::clog << "CAT::sequentiator::interpret_physics: tangent length ";
        iseq->tangent_length().dump();
        std::clog << " " << std::endl;
        fflush(stdout);
      }
      if (iseq->has_helix_length()) {
        std::clog << "CAT::sequentiator::interpret_physics: helix length ";
        iseq->helix_length().dump();
        std::clog << " " << std::endl;
        fflush(stdout);
      }
    }
...
//*************************************************************
void sequentiator::match_to_calorimeter(std::vector<topology::calorimeter_hit> &calos,
                                        topology::sequence *iseq) {
  //*************************************************************
  ...
  // match to calorimeter
  if (!calos.empty()) {
    m.message("CAT::sequentiator::match_to_calorimeter: extrapolate decay vertex with ",
              calos.size(), " calo hits ", mybhep::VVERBOSE);

    for (std::vector<topology::calorimeter_hit>::iterator ic = calos.begin(); ic != calos.end();
         ++ic) {
      m.message("CAT::sequentiator::match_to_calorimeter: trying to extrapolate to calo hit ",
                ic - calos.begin(), " id ", ic->id(), " on view ", ic->pl_.view(), " energy ",
                ic->e().value(), mybhep::VVERBOSE);
      ...
      if (!near(iseq->last_node().c(), *ic)) {
        ...
        if (!iseq->intersect_plane_from_end(ic->pl(), &helix_extrapolation_local_from_end)) {
          m.message("CAT::sequentiator::match_to_calorimeter: no helix intersection from end ",
                    mybhep::VVERBOSE);
        } else {
          dist_from_end = helix_extrapolation_local_from_end.distance(ic->pl_.face()).value();
          if (dist_from_end < helix_min_from_end) {
            ...
            helix_found_from_end = true;
            m.message(
                "CAT::sequentiator::match_to_calorimeter: new helix intersection from end with "
                "minimum distance ",
                dist_from_end, " position: ", helix_extrapolation_from_end.x().value(),
                helix_extrapolation_from_end.y().value(), helix_extrapolation_from_end.z().value(),
                mybhep::VVERBOSE);
          }
        }
        ...
        if (!iseq->intersect_plane_with_tangent_from_end(ic->pl(),
                                                         &tangent_extrapolation_local_from_end)) {
          m.message("CAT::sequentiator::match_to_calorimeter: no tangent intersection from end ",
                    mybhep::VVERBOSE);
        } else {
          dist_from_end = tangent_extrapolation_local_from_end.distance(ic->pl_.face()).value();
          if (dist_from_end < tangent_min_from_end) {
            ...
            tangent_found_from_end = true;
            m.message(
                "CAT::sequentiator::match_to_calorimeter: new tangent intersection from end with "
                "minimum distance ",
                dist_from_end, " position: ", tangent_extrapolation_from_end.x().value(),
                tangent_extrapolation_from_end.y().value(),
                tangent_extrapolation_from_end.z().value(), mybhep::VVERBOSE);
      ...
      if (!near(iseq->nodes_[0].c(), *ic)) {
        m.message("CAT::sequentiator::match_to_calorimeter: beginning is not near ",
                  mybhep::VVERBOSE);
      } else if (ihelix_min_from_end == ic->id() || itangent_min_from_end == ic->id()) {
        m.message(
            "CAT::sequentiator::match_to_calorimeter: beginning is near, but end was already "
            "extrapolated to same calo ",
            mybhep::VVERBOSE);
      } else {
        if (!iseq->intersect_plane_from_begin(ic->pl(), &helix_extrapolation_local_from_begin)) {
          m.message(
              "CAT::sequentiator::match_to_calorimeter: no helix intersection from beginning ",
              mybhep::VVERBOSE);
        } else {
          dist_from_begin = helix_extrapolation_local_from_begin.distance(ic->pl_.face()).value();
          if (dist_from_begin < helix_min_from_begin) {
            ...
            helix_found_from_begin = true;
            m.message(
                "CAT::sequentiator::match_to_calorimeter: new helix intersection from beginning "
                "with minimum distance ",
                dist_from_begin, " position: ", helix_extrapolation_from_begin.x().value(),
                helix_extrapolation_from_begin.y().value(),
                helix_extrapolation_from_begin.z().value(), mybhep::VVERBOSE);
        ...
        if (!iseq->intersect_plane_with_tangent_from_begin(
                ic->pl(), &tangent_extrapolation_local_from_begin)) {
          m.message(
              "CAT::sequentiator::match_to_calorimeter: no tangent intersection from beginning ",
              mybhep::VVERBOSE);
        } else {
          ...
          if (dist_from_begin < tangent_min_from_begin) {
            ...
            tangent_found_from_begin = true;
            m.message(
                "CAT::sequentiator::match_to_calorimeter: new tangent intersection from beginning "
                "with minimum distance ",
                dist_from_begin, " position: ", tangent_extrapolation_from_begin.x().value(),
                tangent_extrapolation_from_begin.y().value(),
                tangent_extrapolation_from_begin.z().value(), mybhep::VVERBOSE);
    ...
    if (helix_found_from_begin) {
      if (ihelix_min_from_begin >= calos.size()) {
        m.message("CAT::sequentiator::match_to_calorimeter: problem: calo hit of id ",
                  ihelix_min_from_begin, " but n of calo hits is ", calos.size(), mybhep::NORMAL);
      } else {
        m.message("CAT::sequentiator::match_to_calorimeter: track extrapolated by helix to calo ",
                  ihelix_min_from_begin, mybhep::VVERBOSE);
        iseq->set_helix_vertex(helix_extrapolation_from_begin, "calo", ihelix_min_from_begin);
      }
    }
    ...
    if (tangent_found_from_begin) {
      if (itangent_min_from_begin >= calos.size()) {
        m.message("CAT::sequentiator::match_to_calorimeter: problem: tangent calo hit of id ",
                  itangent_min_from_begin, " but n of calo hits is ", calos.size(), mybhep::NORMAL);
      } else {
        m.message("CAT::sequentiator::match_to_calorimeter: track extrapolated by tangent to calo ",
                  itangent_min_from_begin, mybhep::VVERBOSE);
...
void sequentiator::match_to_foil(topology::sequence *iseq) {
  ...
  if (!iseq->nodes_.empty()) {
    m.message("CAT::sequentiator::match_to_foil: extrapolate vertex on foil: supernemo ", SuperNemo,
              mybhep::VVERBOSE);

    if (gap_number(iseq->last_node().c()) != 0) {
      m.message("CAT::sequentiator::match_to_foil: end not near ", mybhep::VVERBOSE);
      fflush(stdout);
    } else {
      if (SuperNemo) {
        if (!iseq->intersect_plane_from_end(get_foil_plane(), &helix_extrapolation_from_end)) {
          m.message("CAT::sequentiator::match_to_foil: no helix intersection from end ",
                    mybhep::VVERBOSE);
          fflush(stdout);
        } else {
          iseq->set_decay_helix_vertex(helix_extrapolation_from_end, "foil");
        ...
        if (!iseq->intersect_plane_with_tangent_from_end(get_foil_plane(),
                                                         &tangent_extrapolation_from_end)) {
          m.message("CAT::sequentiator::match_to_foil: no tangent intersection from end ",
                    mybhep::VVERBOSE);
          fflush(stdout);
        } else
          iseq->set_decay_tangent_vertex(tangent_extrapolation_from_end, "foil");
        ...
        if (!iseq->intersect_circle_from_end(get_foil_circle(), &helix_extrapolation_from_end)) {
          ...
          m.message(
              "CAT::sequentiator::match_to_foil: track extrapolated by helix to foil from end ",
              mybhep::VVERBOSE);
          iseq->set_decay_helix_vertex(helix_extrapolation_from_end, "foil");
        }
        ...
        if (!iseq->intersect_circle_with_tangent_from_end(get_foil_circle(),
          ...
          m.message(
              "CAT::sequentiator::match_to_foil: track extrapolated by tangent to foil from end",
              mybhep::VVERBOSE);
          iseq->set_decay_tangent_vertex(tangent_extrapolation_from_end, "foil");
        }
      }
    ...
    if (gap_number(iseq->nodes_[0].c()) != 0) {
      m.message("CAT::sequentiator::match_to_foil: beginning not near ", mybhep::VVERBOSE);
      fflush(stdout);
    } else {
      if (SuperNemo) {
        if (!iseq->intersect_plane_from_begin(get_foil_plane(), &helix_extrapolation_from_begin)) {
          m.message("CAT::sequentiator::match_to_foil: no helix intersection from beginning ",
                    mybhep::VVERBOSE);
          fflush(stdout);
        } else {
          iseq->set_helix_vertex(helix_extrapolation_from_begin, "foil");
        ...
        if (!iseq->intersect_plane_with_tangent_from_begin(get_foil_plane(),
                                                           &tangent_extrapolation_from_begin)) {
          m.message("CAT::sequentiator::match_to_foil: no tangent intersection from beginning ",
                    mybhep::VVERBOSE);
          fflush(stdout);
        } else
          iseq->set_tangent_vertex(tangent_extrapolation_from_begin, "foil");
        ...
        if (!iseq->intersect_circle_from_begin(get_foil_circle(),
          ...
          m.message(
              "CAT::sequentiator::match_to_foil: track extrapolated by helix to foil from "
              "beginning ",
              mybhep::VVERBOSE);
          iseq->set_helix_vertex(helix_extrapolation_from_begin, "foil");
        }
        ...
        if (!iseq->intersect_circle_with_tangent_from_begin(get_foil_circle(),
          ...
          m.message(
              "CAT::sequentiator::match_to_foil: track extrapolated by tangent to foil from begin",
              mybhep::VVERBOSE);
          iseq->set_tangent_vertex(tangent_extrapolation_from_begin, "foil");
        }
      }
...
//*************************************************************
void sequentiator::interpret_physics_after_sultan(std::vector<topology::calorimeter_hit> &calos,
                                                  bool conserve_clustering_from_removal_of_cells) {
  //*************************************************************
  ...
  m.message("CAT::sequentiator::interpret_physics_after_sultan: interpreting physics of ",
            sequences_.size(), " sequences with ", calos.size(), " calorimeter hits ",
            mybhep::VVERBOSE);
  fflush(stdout);

  // after_sultan, conserve_clustering_from_removal  = true, conserve_clustering_from_reordering =
  // false, conserve_clustering_from_removal_of_cells = false (N3), true (SN) after_nemor,
  // conserve_clustering_from_removal  = true, conserve_clustering_from_reordering = false,
  // conserve_clustering_from_removal_of_cells = true
  ...
  while (iseq != sequences_.end()) {
    m.message(
        "CAT::sequentiator::interpret_physics_after_sultan: ... interpreting physics of sequence ",
        iseq->name(), mybhep::VVERBOSE);
    fflush(stdout);
    ...
    if (level >= mybhep::VVERBOSE) print_a_sequence(*iseq, true);

    if (!iseq->calculate_helix(Ratio, conserve_clustering_from_removal_of_whole_cluster,
                               conserve_clustering_from_reordering) &&
        !iseq->has_kink()) {
      size_t index = iseq - sequences_.begin();
      m.message("CAT::sequentiator::interpret_physics_after_sultan: erased sequence ", index,
                "not a good helix", mybhep::VERBOSE);
      fflush(stdout);
      sequences_.erase(iseq);
      iseq = sequences_.begin() + index;
      if (index + 1 >= sequences_.size()) break;
    ...
    if (level >= mybhep::VVERBOSE) {
      std::clog << "CAT::sequentiator::interpret_physics_after_sultan: sequence "
                << iseq - sequences_.begin() << " has: " << std::endl;
      fflush(stdout);
      std::clog << "CAT::sequentiator::interpret_physics_after_sultan: center ";
      iseq->center().dump();
      fflush(stdout);
      std::clog << "CAT::sequentiator::interpret_physics_after_sultan: radius ";
      iseq->radius().dump();
      std::clog << " " << std::endl;
      fflush(stdout);
      std::clog << "CAT::sequentiator::interpret_physics_after_sultan: pitch ";
      iseq->pitch().dump();
      std::clog << " " << std::endl;
      fflush(stdout);
      std::clog << "CAT::sequentiator::interpret_physics_after_sultan: momentum ";
      iseq->momentum().length().dump();
      std::clog << " " << std::endl;
      fflush(stdout);
      std::clog << "CAT::sequentiator::interpret_physics_after_sultan: charge ";
      iseq->charge().dump();
      std::clog << " " << std::endl;
      fflush(stdout);
      if (iseq->has_helix_vertex()) {
        std::clog << "CAT::sequentiator::interpret_physics_after_sultan: helix_vertex "
                  << iseq->helix_vertex_type() << " ";
        iseq->helix_vertex().dump();
        if (iseq->helix_vertex_type() == "calo") std::clog << " icalo " << iseq->helix_vertex_id();
        std::clog << " " << std::endl;
      ...
      if (iseq->has_decay_helix_vertex()) {
        std::clog << "CAT::sequentiator::interpret_physics_after_sultan: decay helix_vertex "
                  << iseq->decay_helix_vertex_type() << " ";
        iseq->decay_helix_vertex().dump();
        if (iseq->decay_helix_vertex_type() == "calo")
          std::clog << " icalo " << iseq->calo_helix_id();
      ...
      if (iseq->has_tangent_vertex()) {
        std::clog << "CAT::sequentiator::interpret_physics_after_sultan: tangent_vertex "
                  << iseq->tangent_vertex_type() << " ";
        iseq->tangent_vertex().dump();
        if (iseq->tangent_vertex_type() == "calo")
          std::clog << " icalo " << iseq->tangent_vertex_id();
      ...
      if (iseq->has_decay_tangent_vertex()) {
        std::clog << "CAT::sequentiator::interpret_physics_after_sultan: decay tangent_vertex "
                  << iseq->decay_tangent_vertex_type() << " ";
        iseq->decay_tangent_vertex().dump();
        if (iseq->decay_tangent_vertex_type() == "calo")
          std::clog << " icalo " << iseq->calo_tangent_id();
      ...
      if (iseq->has_tangent_length()) {
        std::clog << "CAT::sequentiator::interpret_physics_after_sultan: tangent length ";
        iseq->tangent_length().dump();
        std::clog << " " << std::endl;
        fflush(stdout);
      }
      if (iseq->has_helix_length()) {
        std::clog << "CAT::sequentiator::interpret_physics_after_sultan: helix length ";
        iseq->helix_length().dump();
        std::clog << " " << std::endl;
        fflush(stdout);
      }
...
void sequentiator::print_sequences() const {
  ...
  m.message("CAT::sequentiator::print_sequences: sequence matrix is", mybhep::NORMAL);
  fflush(stdout);
  for (vector<topology::sequence>::const_iterator iseq = sequences_.begin();
       iseq != sequences_.end(); ++iseq) {
...
void sequentiator::print_a_sequence(const topology::sequence &sequence, bool after_sultan) const {
  ...
  for (vector<topology::node>::const_iterator inode = sequence.nodes_.begin();
    ...
    if (level >= mybhep::VVERBOSE) {
      ...
      if (!after_sultan) {
        ...
        for (vector<topology::cell>::const_iterator ilink = (*inode).links_.begin();
             ilink != (*inode).links_.end(); ++ilink) {
  ...
  std::clog << " center (" << sequence.center().x().value() << ", " << sequence.center().y().value()
            << ", " << sequence.center().z().value() << ")  radius " << sequence.radius().value()
            << " pitch " << sequence.pitch().value() << " momentum "
            << sequence.momentum().length().value() << "  tangent charge "
            << sequence.charge().value() << " +- " << sequence.charge().error() << " helix charge "
            << sequence.helix_charge().value() << " +- " << sequence.helix_charge().error()
            << " detailed charge " << sequence.detailed_charge().value() << " +- "
            << sequence.detailed_charge().error() << " chi2 " << sequence.chi2() << " prob "
            << sequence.Prob() << " helix chi2 " << sequence.helix_chi2() << " helix prob "
            << sequence.helix_Prob();
  fflush(stdout);
  if (sequence.has_helix_vertex()) {
    std::clog << " helix vertex on " << sequence.helix_vertex_type();
    fflush(stdout);
    if (sequence.helix_vertex_type() == "calo")
      std::clog << " icalo " << sequence.helix_vertex_id();
  }
  if (sequence.has_decay_helix_vertex()) {
    std::clog << " helix decay vertex on " << sequence.decay_helix_vertex_type();
    fflush(stdout);
    if (sequence.decay_helix_vertex_type() == "calo")
      std::clog << " icalo " << sequence.calo_helix_id();
  }
  if (sequence.has_tangent_vertex()) {
    std::clog << " tangent vertex on " << sequence.tangent_vertex_type() << " ";
    fflush(stdout);
    if (sequence.tangent_vertex_type() == "calo")
      std::clog << " icalo " << sequence.tangent_vertex_id();
  }
  if (sequence.has_decay_tangent_vertex()) {
    std::clog << " tangent decay vertex on " << sequence.decay_tangent_vertex_type();
    fflush(stdout);
    if (sequence.decay_tangent_vertex_type() == "calo")
      std::clog << " icalo " << sequence.calo_tangent_id();
  }
  std::clog << " " << std::endl;
  fflush(stdout);
...
void sequentiator::print_scenarios(bool after_sultan) const {
  ...
  clog << "CAT::sequentiator::print_scenarios: Printing scenarios: " << scenarios_.size() << endl;
  fflush(stdout);
...
void sequentiator::add_pair(const topology::sequence &newsequence) {
  ...
  m.message("CAT::sequentiator::add_pair: Entering... ", mybhep::VVERBOSE);
  fflush(stdout);

  if (newsequence.nodes().size() != 2) {
    m.message("CAT::sequentiator::add_pair: problem: pair has size ", newsequence.nodes().size(),
              mybhep::NORMAL);
  ...
  if (!newsequence.nodes_[0].has_couplet(newsequence.nodes()[1].c(), &cc)) {
    m.message("CAT::sequentiator::add_pair: problem: node ", newsequence.nodes_[0].c().id(),
              " has no pair ", newsequence.nodes()[0].c().id(), "-",
              newsequence.nodes()[1].c().id(), mybhep::NORMAL);
    clock.stop(" sequentiator: add pair ");
    return;
  }
  ...
  m.message("CAT::sequentiator::add_pair: n of tangents: ", cc.tangents_.size(), mybhep::VVERBOSE);
  fflush(stdout);
  topology::sequence pair(nodes, level, probmin);
  for (std::vector<topology::line>::iterator itangent = cc.tangents_.begin();
       itangent != cc.tangents_.end(); ++itangent) {
    ...
    if (itangent - cc.tangents_.begin() > 0) {
      NCOPY++;
      for (std::vector<topology::cell_couplet>::iterator icc = pair.nodes_[0].cc_.begin();
           icc != pair.nodes_[0].cc_.end(); ++icc)
        icc->set_all_used();
      for (std::vector<topology::cell_triplet>::iterator iccc = pair.nodes_[1].ccc_.begin();
           iccc != pair.nodes_[1].ccc_.end(); ++iccc)
        iccc->set_all_used();
    }
    ...
    if (level >= mybhep::VVERBOSE) {
      std::clog << "CAT::sequentiator::add_pair: made track [" << sequences_.size() - 1
                << "] with cells " << na.c().id() << " [" << pair.nodes_[0].ep().x().value() << ", "
                << pair.nodes_[0].ep().z().value() << "]"
                << "and" << nb.c().id() << " [" << pair.nodes_[1].ep().x().value() << ", "
                << pair.nodes_[1].ep().z().value() << "]" << std::endl;
      fflush(stdout);
    }

    if (erased) erased = clean_up_sequences();
  }
...
bool sequentiator::clean_up_sequences() {
  ...
  while (iseq != sequences_.end()) {
    ...
    m.message("CAT::sequentiator::clean_up_sequences: should we erase last sequence [",
              sequences_.size() - 1, "] ", sequences_.back().name(), " in favour of [",
              iseq - sequences_.begin(), "] ", iseq->name(), " ? ", mybhep::VVERBOSE);
    fflush(stdout);
    if (!sequences_.back().contained(*iseq)) {
      m.message("no, it's not contained ", mybhep::VVERBOSE);
      fflush(stdout);
      //}else if( sequences_.back().Free() && !sequences_.back().contained_same_extreme_quadrants(
      //*iseq ) ){
      //      m.message("no, it's a free sequence with different extreme quadrants "
      //      ,mybhep::VVERBOSE); fflush(stdout);
    } else if (sequences_.back().Free()) {
      m.message("CAT::sequentiator::clean_up_sequences:no, it's a free sequence ",
                mybhep::VVERBOSE);
      fflush(stdout);
    } else {
      m.message("CAT::sequentiator::clean_up_sequences: erased last sequence ",
                sequences_.size() - 1, "  contained in sequence", iseq - sequences_.begin(),
                mybhep::VERBOSE);
      fflush(stdout);
      sequences_.pop_back();
      changed = true;
    ...
    m.message("CAT::sequentiator::clean_up_sequences: should we erase sequence [",
              iseq - sequences_.begin(), "] ", iseq->name(), " in favour of the last [",
              sequences_.size() - 1, "] ", sequences_.back().name(), " ? ", mybhep::VVERBOSE);
    fflush(stdout);
    if (!iseq->contained(sequences_.back())) {
      m.message("CAT::sequentiator::clean_up_sequences:no, it's not contained ", mybhep::VVERBOSE);
      fflush(stdout);
      //}else if( iseq->Free() && !iseq->contained_same_extreme_quadrants( sequences_.back() ) ){
      // m.message("no, it's a free sequence with different extreme quadrants " ,mybhep::VVERBOSE);
      // fflush(stdout);
    } else if (iseq->Free()) {
      m.message("CAT::sequentiator::clean_up_sequences:no, it's a free sequence", mybhep::VVERBOSE);
      fflush(stdout);
    } else {
      m.message("CAT::sequentiator::clean_up_sequences: erased sequence ",
                iseq - sequences_.begin(), "contained in last sequence", sequences_.size() - 1,
                mybhep::VERBOSE);
      fflush(stdout);
      sequences_.erase(iseq);
      changed = true;
    ...
    m.message("CAT::sequentiator::clean_up_sequences: should we erase sequence [",
              iseq - sequences_.begin(), "] ", iseq->name(),
              " because of connection out of range ? ", mybhep::VVERBOSE);
    fflush(stdout);
    for (std::vector<topology::node>::iterator in = iseq->nodes_.begin(); in != iseq->nodes_.end();
         in++) {
      if (changed) continue;
      ...
      if (!sequence_is_within_range(nA, nB, *iseq)) {
        m.message("CAT::sequentiator::clean_up_sequences: erased sequence ",
                  iseq - sequences_.begin(), " not in range", mybhep::VERBOSE);
        fflush(stdout);
        sequences_.erase(iseq);
        changed = true;
        iseq = sequences_.begin() + (iseq - sequences_.begin());
    ...
    while (kseq != sequences_.end()) {
      ...
      if (level >= mybhep::VVERBOSE) {
        std::clog << "CAT::sequentiator::clean_up_sequences: should we erase sequence ["
                  << kseq - sequences_.begin() << "] " << kseq->name() << " as bridge between ["
                  << iseq - sequences_.begin() << "] " << iseq->name() << " and last ["
                  << sequences_.size() - 1 << "] " << sequences_.back().name() << " ? "
                  << std::endl;
        fflush(stdout);
      }
      if (kseq->is_bridge(*iseq, sequences_.back()) &&
          (*kseq).nodes().size() < (*iseq).nodes().size() &&
          (*kseq).nodes().size() < sequences_.back().nodes().size() && !kseq->Free()) {
        if (level >= mybhep::VVERBOSE) {
          std::clog << "CAT::sequentiator::clean_up_sequences: erased sequence  ["
                    << kseq - sequences_.begin() << "] " << kseq->name() << " as bridge between ["
                    << iseq - sequences_.begin() << "] " << iseq->name() << " and last ["
                    << sequences_.size() - 1 << "] " << sequences_.back().name() << std::endl;
          fflush(stdout);
        }
      ...
      if (level >= mybhep::VVERBOSE) {
        std::clog << "CAT::sequentiator::clean_up_sequences: should we erase last ["
                  << sequences_.size() - 1 << "] " << sequences_.back().name()
      ...
      if (sequences_.back().is_bridge(*iseq, *kseq) &&
          ...
          sequences_.back().nodes().size() < (*kseq).nodes().size() && !sequences_.back().Free()) {
        if (level >= mybhep::VVERBOSE) {
          std::clog << "CAT::sequentiator::clean_up_sequences: erased  last ["
                    << sequences_.size() - 1 << "] " << sequences_.back().name()
                    << " as bridge between [" << iseq - sequences_.begin() << "] " << iseq->name()
                    << " and  [" << kseq - sequences_.begin() << "] " << kseq->name() << std::endl;
          fflush(stdout);
        }
        sequences_.pop_back();
        changed = true;
...
double sequentiator::distance_from_foil(const topology::experimental_point &ep) {
  ...
  if (SuperNemo) return std::abs(ep.z().value());
...
bool sequentiator::direct_out_of_foil(void) {
  ...
  for (std::vector<topology::sequence>::iterator iseq = sequences_.begin();
    ...
    if (distance_from_foil(iseq->nodes().front().ep()) >
      ...
      m.message("CAT::sequentiator::direct_out_of_foil: sequence ", iseq - sequences_.begin(),
                " will be directed out of foil ", mybhep::VVERBOSE);
...
bool sequentiator::direct_scenarios_out_of_foil(void) {
  ...
  for (std::vector<topology::scenario>::iterator isc = scenarios_.begin(); isc != scenarios_.end();
       ++isc) {
    for (std::vector<topology::sequence>::iterator iseq = isc->sequences_.begin();
         iseq != isc->sequences_.end(); ++iseq) {
      if (distance_from_foil(iseq->nodes().front().ep()) >
          distance_from_foil(iseq->nodes().back().ep())) {
        m.message("CAT::sequentiator::direct_scenarios_out_of_foil: sequence ",
                  iseq - sequences_.begin(), " in scenario ", isc - scenarios_.begin(),
                  " will be directed out of foil ", mybhep::VVERBOSE);
        topology::sequence is = iseq->invert();
        std::swap(*iseq, is);
      }
...
bool sequentiator::near(const topology::cell &c, topology::calorimeter_hit &ch) {
  ...
  if (pl.view() == "x") {
    m.message("CAT::sequentiator::near: matching cell ", c.id(), " with cell number ",
              c.cell_number(), " to calo on view ", pl.view(), " max cell number ", cell_max_number,
              " plane norm x ", pl.norm().x().value(), mybhep::VVERBOSE);
    ...
    if (level >= mybhep::VVERBOSE)
      std::clog << "CAT::sequentiator::near:  checking if cell " << c.id()
                << " is near plane: " << pl.center().x().value() << ", " << pl.center().y().value()
                << ", " << pl.center().z().value() << " on view " << pl.view() << " distance z "
                << distance.z().value() << " size z " << size_z << " distance x "
                << distance.x().value() << " size x " << size_x << std::endl;
    ...
    m.message("CAT::sequentiator::near:  checking if cell ", c.id(), " on gap ", g,
              " is near plane: ", pl.center().x().value(), pl.center().y().value(),
              pl.center().z().value(), mybhep::VVERBOSE);

    if (g <= 0 || std::abs(calorimeter_layer - c.layer()) > NOffLayers)
      return false;  // cell is not on a gap or is facing the foil

    if (g == 1 || std::abs(calorimeter_layer - c.layer()) <= NOffLayers) return true;

    m.message("CAT::sequentiator::near:  problem: can't match to calo on view ", pl.view(),
              mybhep::NORMAL);
    ...
    int ln = c.layer();
    int g = gap_number(c);
    m.message("CAT::sequentiator::near:  checking if cell ", c.id(), " layer and gap: ", ln, g,
              " is near plane: ", pl.center().x().value(), pl.center().y().value(),
              pl.center().z().value(), " on view ", pl.view(), mybhep::VVERBOSE);
    if (ln < 0 && (g == 3 || std::abs(ln - calorimeter_layer) <= NOffLayers)) return true;
    return false;
  } else if (pl.view() == "outer") {
    int ln = c.layer();
    int g = gap_number(c);
    m.message("CAT::sequentiator::near:  checking if cell ", c.id(), " on layer and gap: ", ln, g,
              " is near plane: ", pl.center().x().value(), pl.center().y().value(),
              pl.center().z().value(), " on view ", pl.view(), mybhep::VVERBOSE);
    if (ln > 0 && (g == 3 || std::abs(ln - calorimeter_layer) <= NOffLayers)) return true;
    return false;
    ...
    m.message("CAT::sequentiator::near:  checking if cell ", c.id(), " on gap ", g,
              " is near plane: ", pl.center().x().value(), pl.center().y().value(),
              pl.center().z().value(), " on view ", pl.view(), mybhep::VVERBOSE);
    if (ln > 0 && calorimeter_layer == 3.5 &&
        (g == 1 || std::abs(ln - calorimeter_layer) <= NOffLayers + 0.5))
      return true;
    if (ln > 0 && calorimeter_layer == 5.5 &&
        (g == 2 || std::abs(ln - calorimeter_layer) <= NOffLayers + 0.5))
      return true;
    if (ln < 0 && calorimeter_layer == -3.5 &&
        (g == 1 || std::abs(ln - calorimeter_layer) <= NOffLayers + 0.5))
      return true;
...
//*******************************************************************
void sequentiator::make_table_of_true_and_reco_sequences(
    std::vector<topology::sequence> &trueseqs) {
  //*******************************************************************
  ...
  bool first;
  size_t ncommonhits;
  for (std::vector<topology::sequence>::iterator tp = trueseqs.begin(); tp != trueseqs.end();
       ++tp) {  // loop over true particles
    ...
    for (std::vector<topology::sequence>::iterator ip = sequences_.begin(); ip != sequences_.end();
         ++ip) {  // loop over reco particles
    ...
    if (imin >= sequences_.size()) {
      m.message("CAT::sequentiator::make_table_of_true_and_reco_sequences: problem: imin", imin,
                "allparts size", sequences_.size(), mybhep::NORMAL);
      imin = 0;
    }
  ...
  if (level >= mybhep::VVERBOSE) {
    for (size_t i = 0; i < sequences_.size(); i++) {
      std::clog << "CAT::sequentiator::make_table_of_true_and_reco_sequences:  best matching for "
                   "reco track "
                << i << " is true track " << true_sequence_of_reco_[i] << " with "
                << n_common_hits_for_reco_track_[i] << " common hits " << std::endl;
      std::clog
          << "CAT::sequentiator::make_table_of_true_and_reco_sequences:  hits of reco track: ";
      for (size_t j = 0; j < sequences_[i].nodes().size(); j++) {
        std::clog << " " << sequences_[i].nodes()[j].c().id();
      }
      std::clog << " " << std::endl;
      std::clog << " hits of true track: ";
    ...
    for (size_t i = 0; i < trueseqs.size(); i++)
      std::clog << "CAT::sequentiator::make_table_of_true_and_reco_sequences:  best matching for "
                   "true track "
                << i << " is reco track " << reco_sequence_of_true_[i] << std::endl;
  }
...
void sequentiator::rec_efficiency(std::vector<topology::sequence> &trueseqs) {
  ...
  size_t ireco;
  for (std::vector<topology::sequence>::iterator itrueseq = trueseqs.begin();
       itrueseq != trueseqs.end(); ++itrueseq) {
    if (!itrueseq->primary()) continue;

    m.message("CAT::sequentiator::rec_efficiency: reconstructing efficiencies for true particle ",
              itrueseq - trueseqs.begin(), " of ", itrueseq->nodes().size(), " points, with name ",
              itrueseq->name(), mybhep::VVERBOSE);

    if (itrueseq->nodes().size() == 0) continue;

    ireco = reco_sequence_of_true_[itrueseq - trueseqs.begin()];

    if (ireco >= sequences_.size()) {
      m.message("CAT::sequentiator::rec_efficiency:  problem: ireco", ireco, " size",
                sequences_.size(), mybhep::NORMAL);
    }
    ...
    if (nhits > 0) {
      ...
      m.message("CAT::sequentiator::rec_efficiency:  ... n reco hits ", nhits, " n true hits ",
                itrueseq->nodes().size(), " n common hits ", ncommon, " TTPurity ", purity,
                " TTEfficiency ", efficiency, mybhep::VVERBOSE);
...
size_t sequentiator::getCommonHits(topology::sequence &tp, topology::sequence &dp) {
  ...
  for (std::vector<topology::node>::iterator dnode = dp.nodes_.begin(); dnode != dp.nodes_.end();
       ++dnode) {
    for (std::vector<topology::node>::iterator tnode = tp.nodes_.begin(); tnode != tp.nodes_.end();
         ++tnode) {
...
//*************************************************************
bool sequentiator::sequence_is_within_range(topology::node nodeA, topology::node nodeB,
  ...
  if (blockA == blockB && ((gnA == -1 && gnB == -1) || gnA != gnB)) {
    ...
    if (blockA != blockB) {
      ...
      if( gnA != -1 && gnB != -1 && gnA != gnB ){
        if( level >= mybhep::VVERBOSE ){
          std::clog << " connection between cells " << nodeA.c().id() << " and " << nodeB.c().id()
      << " blocks " << blockA << " and " << blockB << " gaps " << gnA << " and " << gnB << " is
      forbidden (cells face different gaps) " << std::endl;
        }
        return false;
      }
      */
      ...
      std::clog << "CAT::sequentiator::sequence_is_within_range: problem: blockA " << blockA
                << " blockB " << blockB << " gnA " << gnA << " gnB " << gnB << std::endl;
      return true;
    }
  }
  ...
  if (ep_maxr.radius().value() > rmax || ep_minr.radius().value() < rmin) {
    if (level >= mybhep::VVERBOSE) {
      std::clog << "CAT::sequentiator::sequence_is_within_range:  sequence penetrates outside of "
                   "admissible range between cells "
                << nodeA.c().id() << " layer " << nodeA.c().layer() << " r "
                << nodeA.c().ep().radius().value() << " and " << nodeB.c().id() << " layer "
                << nodeB.c().layer() << " r " << nodeB.c().ep().radius().value()
                << ": point of max radius has radius " << ep_maxr.radius().value() << " rmax "
                << rmax << " point of min radius has radius " << ep_minr.radius().value()
                << " rmin " << rmin << std::endl;
    }
    return false;
  }
  ...
  if (level >= mybhep::VVERBOSE) {
    std::clog << "CAT::sequentiator::sequence_is_within_range:  sequence blockA " << blockA
              << " blockB " << blockB << " gnA " << gnA << " gnB " << gnB << " connecting cells "
              << nodeA.c().id() << " and " << nodeB.c().id() << ": point of max radius has radius "
              << ep_maxr.radius().value() << " rmax " << rmax << " point of min radius has radius "
              << ep_minr.radius().value() << " rmin " << rmin << std::endl;
  }
...
bool sequentiator::good_first_to_be_matched(topology::sequence &seq) {
  ...
  if (seq.nodes().size() < 2) {
    m.message(
        "CAT::sequentiator::good_first_to_be_matched: SQ user: not a good first, because hits size "
        "is",
        seq.nodes().size(), mybhep::VERBOSE);
    return false;
  }
...
bool sequentiator::match_gaps(std::vector<topology::calorimeter_hit> &calos) {
  ...
  for (std::vector<topology::sequence>::iterator iseq = sequences_.begin();
    ...
    m.message("CAT::sequentiator::match_gaps: begin matching with sequence ", iseq->name(),
              mybhep::VERBOSE);
    if (level >= mybhep::VERBOSE) print_a_sequence(*iseq);
    ...
    while (can_match(newseq, &jmin, invertA, invertB, with_kink, cells_to_delete, calos)) {
      m.message("CAT::sequentiator::match_gaps:  best matching is ", sequences_[jmin].name(),
                " invertA ", invertA, " invertB ", invertB, " with kink ", with_kink,
                " delete cells ", cells_to_delete, mybhep::VERBOSE);
      if (level >= mybhep::VVERBOSE) print_a_sequence(sequences_[jmin]);

      bool ok;
      newseq =
          newseq.match(sequences_[jmin], invertA, invertB, &ok, with_kink, cells_to_delete, Ratio);
      ...
      if (level >= mybhep::VERBOSE) print_a_sequence(newseq);
      ...
      matched[ifa] = true;
      m.message("CAT::sequentiator::match_gaps: setting family ", ifa, " as used for matching",
                mybhep::VERBOSE);
    }

    if (!first) newseqs.push_back(newseq);
  }
  if (late()) return false;
  ...
  for (std::vector<topology::sequence>::iterator iseq = sequences_.begin();
       iseq != sequences_.end(); ++iseq) {
    if (!matched[mybhep::int_from_string(iseq->family())]) newseqs.push_back(*iseq);
  }
...
//*************************************************************
bool sequentiator::can_match(topology::sequence &s, size_t *jmin, bool &bestinvertA,
                             bool &bestinvertB, int &with_kink, int &cells_to_delete_best,
  ...
  m.message("CAT::sequentiator::can_match: try to match sequence", s.name(), " of chi2 = ", chi2min,
            " ndof ", ndofbest, " prob ", probmax, mybhep::VVERBOSE);
  ...
  for (std::vector<topology::sequence>::iterator jseq = sequences_.begin();
    ...
    m.message("CAT::sequentiator::can_match:  try to match sequence", s.name(), " to ",
              jseq->name(), mybhep::VVERBOSE);
    if (level >= mybhep::VVERBOSE) {
      print_a_sequence(s);
    ...
    if (s.same_families(*jseq)) {
      m.message("CAT::sequentiator::can_match:  ... forbidden, same family", jseq->family(),
                mybhep::VVERBOSE);
      continue;
    }
    ...
    if (ok_match) news = s.match(*jseq, invertA, invertB, &ok_match, with_kink, 0, Ratio);
    ...
    if (ok_match) {
      ...
      m.message("CAT::sequentiator::can_match:  ... matched to ", jseq->name(), ", chi2 =", c,
                " ndof ", n, " prob ", p, mybhep::VVERBOSE);
    }

    if (ok_match && (p > news.probmin()))
      m.message("CAT::sequentiator::can_match:  good helix match", mybhep::VVERBOSE);
    else {
      m.message("CAT::sequentiator::can_match:  ... no good helix match, try to match with kink ",
                mybhep::VVERBOSE);

      ok_kink_match = s.good_match_with_kink(*jseq, invertA, invertB, acrossGAP, limit_diagonal,
                                             NOffLayers, cells_to_delete);
      if (!ok_kink_match) {
        m.message("CAT::sequentiator::can_match:  ... obviously no good match with kink ",
                  mybhep::VVERBOSE);
      } else {
        ...
        m.message("CAT::sequentiator::can_match:  possible match with kink, across GAP", acrossGAP,
                  ", cells to delete ", cells_to_delete, ", try to extrapolate ", mybhep::VVERBOSE);
        for (std::vector<topology::calorimeter_hit>::iterator ic = calos.begin(); ic != calos.end();
             ++ic) {
          if (near(nodeA.c(), *ic) || near(nodeB.c(), *ic)) {
            ok_kink_match = false;
            m.message(
                "CAT::sequentiator::can_match:  will not match with kink because end cell is near "
                "calo ",
                ic - calos.begin(), mybhep::VVERBOSE);
            break;
          }
        }
        ...
        ok_kink_match = ok_kink_match &&
                        s.intersect_sequence(*jseq, invertA, invertB, acrossGAP, &kink_point,
                                             limit_diagonal, &with_kink, cells_to_delete, Ratio);
        ...
        if (ok_kink_match) {
          ...
          if (ok_kink_match)
            m.message("CAT::sequentiator::can_match:  good kink match ( ", kink_point.x().value(),
                      ", ", kink_point.y().value(), ", ", kink_point.z().value(), ") to sequence ",
                      jseq->name(), mybhep::VVERBOSE);

        } else {
          m.message("CAT::sequentiator::can_match:  no good kink match to sequence ", jseq->name(),
                    mybhep::VVERBOSE);
        }
      }
    }
    ...
    if (level >= mybhep::VVERBOSE) {
      std::clog << "CAT::sequentiator::can_match:  ... matched to " << jseq->name()
                << ", chi2 =" << c << " ndof " << n << " prob " << p << " with kink " << with_kink
                << " cells_to_delete " << cells_to_delete << " probmax " << probmax << " chimin "
                << chi2min << std::endl;
    }
  ...
  if (ok) {
    if (level >= mybhep::VVERBOSE) {
      std::clog << "CAT::sequentiator::can_match:  sequence " << s.name() << " can be matched to "
                << sequences_[*jmin].name() << ", chi2 =" << chi2min << " ndof " << ndofbest
                << " prob " << probmax << " cells_to_delete " << cells_to_delete << std::endl;
    }
  }
...
bool sequentiator::select_nemo_tracks(topology::tracked_data &__tracked_data) {
  ...
  m.message("CAT::sequentiator::select_nemo_track: selecting events based on nemo tracks ",
            mybhep::VVERBOSE);

  if (nemo_sequences.size() != 2) {
    m.message("CAT::sequentiator::select_nemo_track: reject: ntracks = ", nemo_sequences.size(),
              mybhep::VVERBOSE);
    return false;
  }
  m.message("CAT::sequentiator::select_nemo_track:  ntracks = ", nemo_sequences.size(),
            mybhep::VVERBOSE);

  if (!nemo_sequences[0].has_decay_helix_vertex()) {
    m.message("CAT::sequentiator::select_nemo_track: reject: 1st track has no calo ",
              mybhep::VVERBOSE);
    return false;
  }

  if (!nemo_sequences[1].has_decay_helix_vertex()) {
    m.message("CAT::sequentiator::select_nemo_track: reject: 2nd track has no calo ",
              mybhep::VVERBOSE);
    return false;
  }
  ...
  if (nemo_sequences[0].calo_helix_id() == nemo_sequences[1].calo_helix_id()) {
    m.message("CAT::sequentiator::select_nemo_track: reject: same calo ",
              nemo_sequences[0].calo_helix_id(), mybhep::VVERBOSE);
    return false;
  }
  ...
  if (caloA.e().value() < 0.2) {
    m.message("CAT::sequentiator::select_nemo_track: reject: 1st calo has energy ",
              caloA.e().value(), mybhep::VVERBOSE);
    return false;
  }

  if (caloB.e().value() < 0.2) {
    m.message("CAT::sequentiator::select_nemo_track: reject: 2nd calo has energy ",
              caloA.e().value(), mybhep::VVERBOSE);
    return false;
  }
...
size_t sequentiator::near_level(const topology::cell &c1, const topology::cell &c2) {
  ...
  topology::experimental_double distance =
      topology::experimental_vector(c1.ep(), c2.ep()).hor().length();
  ...
  if (SuperNemo) {  // use side, layer and row
    ...
    if (layer_distance == 0 && row_distance == 0) {
      if (level >= mybhep::NORMAL) {
        std::clog << "CAT::sequentiator::near_level: problem: cat asking near level of cells with "
                     "identical posiion ("
    ...
    if (std::abs(distance.value() - limit_side) < precision) return 2;

    if (std::abs(distance.value() - limit_diagonal) < precision) return 1;
...
void sequentiator::reassign_cells_based_on_helix(topology::sequence *seq) {
  ...
  inode++;
  while ((size_t)(inode - seq->nodes_.begin()) < seq->nodes_.size() - 1 &&
         seq->nodes_.size() >= 3) {
    ...
    if (level >= mybhep::VVERBOSE)
      std::clog << "CAT::sequentiator::reassign_cells_based_on_helix: sequence of "
                << seq->nodes_.size() << " nodes, node " << index << " id " << inode->c().id()
                << " distance from helix " << distance.value() << " +- " << distance.error()
                << std::endl;
