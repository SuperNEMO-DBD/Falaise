...
line::line(prlevel level, double probmin) {
  appname_ = "line: ";
  forward_axis_ =
      experimental_vector(small_neg, small_neg, small_neg, small_neg, small_neg, small_neg);
  used_ = false;
...
//! constructor
line::line(const experimental_point& epa, const experimental_point& epb, prlevel level,
           double probmin) {
  set_print_level(level);
  set_probmin(probmin);
  appname_ = "line: ";
...
/*** dump ***/
void line::dump(std::ostream& a_out, const std::string& a_title, const std::string& a_indent,
...
void line::set_epa(const experimental_point& epa) { epa_ = epa; }

//! set second experimental_point
void line::set_epb(const experimental_point& epb) { epb_ = epb; }

//! set used
void line::set_used(bool used) { used_ = used; }

//! get experimental_point a
const experimental_point& line::epa() const { return epa_; }

//! get experimental_point b
const experimental_point& line::epb() const { return epb_; }

//! get forward axis
const experimental_vector& line::forward_axis() const { return forward_axis_; }

//! get used
bool line::used() const { return used_; }

experimental_double line::phi() { return forward_axis_.phi(); }

experimental_double line::theta() { return forward_axis_.theta(); }
...
double line::chi2(const line& l, bool use_theta_kink, double* chi2_just_phi) {
  ...
  *chi2_just_phi = result;
  ...
  if (print_level() > mybhep::VERBOSE) {
    std::clog << appname_ << " calculate chi2: phi kink : ";
    (phi_kink * 180. / M_PI).dump();
    if (use_theta_kink) {
      std::clog << " theta kink : ";
      (theta_kink * 180. / M_PI).dump();
    }
