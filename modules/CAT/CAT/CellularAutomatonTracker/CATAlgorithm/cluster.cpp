...
//! set free level
void cluster::set_free(bool free) { free_ = free; }

//! get nodes
const std::vector<node> &cluster::nodes() const { return nodes_; }

//! get free level
bool cluster::Free() const { return free_; }
...
topology::node cluster::node_of_cell(const topology::cell &c) {
  ...
  if (fnode == nodes_.end()) {
    if (print_level() >= mybhep::NORMAL) {
      std::clog << " problem: requested cell " << c.id()
...
bool cluster::start_ambiguity(size_t i) {
  ...
  // - it is the 2nd node, or the previous node has 1 joint, or the previous node comes here through
  // a block
  ...
  if (nodes_[i].ccc().size() == 0) {
    if (print_level() >= mybhep::NORMAL) {
      std::clog << " problem: start ambiguity: i " << i << " triplet size "
                << nodes_[i].ccc().size() << std::endl;
      return false;
    }
  }
  ...
  if (i < nodes_.size() - 1 && nodes_[i].c().block() != nodes_[i + 1].c().block())
    return false;  // node does not connect through gap

  if (i == 1) {
    if (print_level() >= mybhep::VERBOSE) {
      std::clog << " CAT::cluster::start_ambiguity: node " << nodes_[i].c().id() << " of index "
                << i << " and " << joints.size() << " joints starts ambigous piece " << std::endl;
    }
    return true;  // second node
  }

  if (i > 0 && nodes_[i].c().block() != nodes_[i - 1].c().block()) {
    if (print_level() >= mybhep::VERBOSE) {
      std::clog << " CAT::cluster::start_ambiguity: node " << nodes_[i].c().id() << " of index "
                << i << ", block " << nodes_[i].c().block() << " and " << joints.size()
                << " joints starts ambigous piece connecting from node " << nodes_[i - 1].c().id()
                << " of block " << nodes_[i - 1].c().block() << std::endl;
    }
  ...
  if (prev_joints.size() == 1) {
    if (print_level() >= mybhep::VERBOSE) {
      std::clog << " CAT::cluster::start_ambiguity: node " << nodes_[i].c().id() << " of index "
                << i << " and " << joints.size() << " joints starts ambigous piece after node "
                << nodes_[i - 1].c().id() << " of " << prev_joints.size() << " joints "
                << std::endl;
    }
...
bool cluster::end_ambiguity(size_t i) {
  ...
  if (nodes_[i].ccc().size() == 0) {
    if (print_level() >= mybhep::NORMAL) {
      std::clog << " problem: end ambiguity: i " << i << " triplet size " << nodes_[i].ccc().size()
                << std::endl;
      return false;
    }
  }
  ...
  if (i < nodes_.size() - 1 && nodes_[i].c().block() != nodes_[i + 1].c().block()) {
    if (print_level() >= mybhep::VERBOSE) {
      std::clog << " CAT::cluster::end_ambiguity: node " << nodes_[i].c().id() << " of index " << i
                << ", block " << nodes_[i].c().block() << " ends ambigous piece connecting to node "
                << nodes_[i + 1].c().id() << " of block " << nodes_[i + 1].c().block() << std::endl;
    }
  ...
  if (next_joints.size() == 1) {
    if (print_level() >= mybhep::VERBOSE) {
      std::clog << " CAT::cluster::end_ambiguity: node " << nodes_[i].c().id() << " of index " << i
                << " and " << joints.size() << " joints ends ambigous piece before node "
                << nodes_[i + 1].c().id() << " of " << next_joints.size() << " joints "
                << std::endl;
    }
...
void cluster::solve_ambiguities(
  ...
  for (first_node = nodes_.begin() + 1; first_node != nodes_.end() - 1; ++first_node) {
    ...
    if (print_level() >= mybhep::VERBOSE) {
      ...
      for (std::vector<node>::const_iterator knode = ambiguous_piece.nodes_.begin();
           knode != ambiguous_piece.nodes_.end(); ++knode)
        std::clog << knode->c().id() << " ";
      std::clog << ")" << std::endl;
    }
    ...
    for (std::vector<node>::const_iterator knode = ambiguous_piece.nodes_.begin();
      ...
      if (knode->ccc().size() == 0) {
        if (print_level() >= mybhep::NORMAL) {
          std::clog << " problem: solve ambiguitues: node " << knode->c().id()
                    << " has no triplets " << std::endl;
          return;
        }
      }
      ...
      if (knode->ccc()[0].joints().size() < 2) {
        if (print_level() >= mybhep::NORMAL) {
          std::clog << " problem: solve ambiguitues: node " << knode->c().id() << " has "
                    << knode->ccc()[0].joints().size() << " joints " << std::endl;
          return;
        }
      }
    }
    ...
    std::vector<topology::broken_line> bls =
        solve_ambiguities_with_ends(first_node - nodes_.begin(), last_node - nodes_.begin());
...
std::vector<topology::broken_line> cluster::solve_ambiguities_with_ends__1_node(
    size_t ifirst, size_t ilast, bool first_ambiguous_is_after_gap, bool first_ambiguous_is_second,
    bool last_ambiguous_is_begore_gap, bool last_ambiguous_is_last_but_one) {
  std::vector<topology::broken_line> bls;
  ...
  if (first_ambiguous_is_after_gap) {
    if (last_ambiguous_is_last_but_one) {  // node after last ambiguous is the last:  gap - A - N |
      if (print_level() >= mybhep::VERBOSE) {
        std::clog << " CAT::cluster::solve_ambiguities_with_ends: cannot solve ambiguity: gap - A "
                     "- N | (should be decided by matching) "
                  << std::endl;
      }
    ...
    if (last_ambiguous_is_begore_gap) {  // gap - A - gap
      if (print_level() >= mybhep::VERBOSE) {
        std::clog << " CAT::cluster::solve_ambiguities_with_ends: cannot solve ambiguity: gap - A "
                     "- gap (should be decided by matching) "
                  << std::endl;
      }
      for (size_t joint_index = 0; joint_index <= 1; ++joint_index) {
        topology::broken_line bl;
    ...
    if (print_level() >= mybhep::VERBOSE) {
      std::clog << " CAT::cluster::solve_ambiguities_with_ends: cannot solve ambiguity: gap - A - "
                   "b ... with b singular (should be decided by matching) "
                << std::endl;
    }
    for (size_t joint_index = 0; joint_index <= 1; ++joint_index) {
      topology::broken_line bl;
  ...
  if (first_ambiguous_is_second) {
    if (last_ambiguous_is_begore_gap) {
      if (print_level() >= mybhep::VERBOSE) {
        std::clog << " CAT::cluster::solve_ambiguities_with_ends: cannot solve ambiguity: | 0 - A "
                     "- gap (should be decided by matching) "
                  << std::endl;
      }
      for (size_t joint_index = 0; joint_index <= 1; ++joint_index) {
    ...
    if (!last_ambiguous_is_last_but_one) {  // node after last ambiguous is not the last: 0 - A - b
      ...
      if (print_level() >= mybhep::VERBOSE) {
        std::clog << " CAT::cluster::solve_ambiguities_with_ends: cannot solve ambiguity: | 0 - A "
                     "- b ... (should be decided by matching) "
                  << std::endl;
      }
      for (size_t joint_index = 0; joint_index <= 1; ++joint_index) {
        topology::broken_line bl;
  ...
  if (last_ambiguous_is_last_but_one) {
    if (print_level() >= mybhep::VERBOSE) {
      std::clog << " CAT::cluster::solve_ambiguities_with_ends: cannot solve ambiguity: | ... a - "
                   "A - N (should be decided by matching) "
                << std::endl;
    }
    for (size_t joint_index = 0; joint_index <= 1; ++joint_index) {
      topology::broken_line bl;
  ...
  if (last_ambiguous_is_begore_gap) {  // node after last ambiguous is not the last: ... a - A - b -
    ...
    if (print_level() >= mybhep::VERBOSE) {
      std::clog << " CAT::cluster::solve_ambiguities_with_ends: cannot solve ambiguity: | ... a - "
                   "A - gap (should be decided by matching) "
                << std::endl;
    }
    for (size_t joint_index = 0; joint_index <= 1; ++joint_index) {
      topology::broken_line bl;
  ...
  if (print_level() >= mybhep::VERBOSE) {
    std::clog << " CAT::cluster::solve_ambiguities_with_ends:  ... a - A - b ... ; optimize A = "
              << nodes_[ifirst].c().id() << std::endl;
  }
...
std::vector<topology::broken_line> cluster::solve_ambiguities_with_ends__2_nodes(
    size_t ifirst, size_t ilast, bool first_ambiguous_is_after_gap, bool first_ambiguous_is_second,
    bool last_ambiguous_is_begore_gap, bool last_ambiguous_is_last_but_one) {
  std::vector<topology::broken_line> bls;
  ...
  if (first_ambiguous_is_after_gap) {
    if (last_ambiguous_is_last_but_one) {  // node after last ambiguous is the last:  gap - A - B -
                                           // N |
      if (print_level() >= mybhep::VERBOSE) {
        std::clog << " CAT::cluster::solve_ambiguities_with_ends: cannot solve ambiguity: gap - A "
                     "- B - N | (should be decided by matching)"
                  << std::endl;
      }
    ...
    if (last_ambiguous_is_begore_gap) {  // gap - A - B - gap
      if (print_level() >= mybhep::VERBOSE) {
        std::clog << " CAT::cluster::solve_ambiguities_with_ends: cannot solve ambiguity: gap - A "
                     "- B - gap (should be decided by matching) "
                  << std::endl;
      }
      for (size_t joint_index_A = 0; joint_index_A <= 1; ++joint_index_A) {
        for (size_t joint_index_B = 0; joint_index_B <= 1; ++joint_index_B) {
    ...
    // gap - A - B - c - ...
    if (print_level() >= mybhep::VERBOSE) {
      std::clog << " CAT::cluster::solve_ambiguities_with_ends: gap - A - B - c ... with c "
                   "singular : optimize B = "
                << nodes_[ilast].c().id() << std::endl;
    }
    for (size_t joint_index_A = 0; joint_index_A <= 1; ++joint_index_A) {
      topology::experimental_point pA = nodes_[ifirst].ccc()[0].joints()[joint_index_A].epb();
  ...
  if (first_ambiguous_is_second) {
    ...
    // 0 - A - B - c - ...
    if (print_level() >= mybhep::VERBOSE) {
      std::clog << " CAT::cluster::solve_ambiguities_with_ends: 0 - A - B - c optimize B = "
                << nodes_[ilast].c().id() << std::endl;
    }
    for (size_t joint_index_A = 0; joint_index_A <= 1; ++joint_index_A) {
      topology::experimental_point p0 = nodes_[ifirst].ccc()[0].joints()[joint_index_A].epa();
  ...
  if (last_ambiguous_is_last_but_one) {  // a - A - B - N
    if (print_level() >= mybhep::VERBOSE) {
      std::clog << " CAT::cluster::solve_ambiguities_with_ends: cannot solve ambiguity: ... a - A "
                   "- B - N | optimize A = "
                << nodes_[ifirst].c().id() << std::endl;
    }
    topology::experimental_point pa = nodes_[ifirst - 1].ccc()[0].joints()[0].epb();
  ...
  if (last_ambiguous_is_begore_gap) {  // a - A - B - gap
    if (print_level() >= mybhep::VERBOSE) {
      std::clog
          << " CAT::cluster::solve_ambiguities_with_ends: ... a - A - B - gap ; optimize A  = "
          << nodes_[ifirst].c().id() << std::endl;
    }
    topology::experimental_point pa = nodes_[ifirst - 1].ccc()[0].joints()[0].epb();
    for (size_t joint_index_B = 0; joint_index_B <= 1; ++joint_index_B) {
  ...
  if (print_level() >= mybhep::VERBOSE) {
    std::clog
        << " CAT::cluster::solve_ambiguities_with_ends:  ... a - A - B - b ... ; optimize A = "
        << nodes_[ifirst].c().id() << ", B = " << nodes_[ilast].c().id() << std::endl;
  }
  topology::experimental_point pa = nodes_[ifirst - 1].ccc()[0].joints()[0].epb();
  topology::experimental_point pb = nodes_[ilast + 1].ccc()[0].joints()[0].epb();
  topology::broken_line best_bl;
...
std::vector<topology::broken_line> cluster::solve_ambiguities_with_ends__3_nodes(
    size_t ifirst, size_t ilast, bool first_ambiguous_is_after_gap, bool first_ambiguous_is_second,
    bool last_ambiguous_is_begore_gap, bool last_ambiguous_is_last_but_one) {
  std::vector<topology::broken_line> bls;
  ...
  if (first_ambiguous_is_after_gap) {
    if (last_ambiguous_is_last_but_one) {  // node after last ambiguous is the last:  gap - A - B -
                                           // C - N
      if (print_level() >= mybhep::VERBOSE) {
        std::clog << " CAT::cluster::solve_ambiguities_with_ends: configuration: gap - A - B - C - "
                     "N |  optimize B = "
                  << nodes_[ifirst + 1].c().id() << std::endl;
      }
      for (size_t joint_index_A = 0; joint_index_A <= 1; ++joint_index_A) {
        for (size_t joint_index_C = 0; joint_index_C <= 1; ++joint_index_C) {
          ...
          for (size_t joint_index_B = 0; joint_index_B <= 1; ++joint_index_B) {
            ...
            topology::experimental_point pB =
                nodes_[ifirst + 1].ccc()[0].joints()[joint_index_B].epb();
            bl.eps_.push_back(pA);
            bl.eps_.push_back(pB);
            bl.eps_.push_back(pC);
            bl.eps_.push_back(pN);
    ...
    if (last_ambiguous_is_begore_gap) {  // gap - A - B - C - gap
      if (print_level() >= mybhep::VERBOSE) {
        std::clog
            << " CAT::cluster::solve_ambiguities_with_ends: gap - A - B - C - gap : optimize B = "
            << nodes_[ifirst + 1].c().id() << std::endl;
      }
      for (size_t joint_index_A = 0; joint_index_A <= 1; ++joint_index_A) {
        for (size_t joint_index_C = 0; joint_index_C <= 1; ++joint_index_C) {
    ...
    // gap - A - B - C - d ...
    if (print_level() >= mybhep::VERBOSE) {
      std::clog << " CAT::cluster::solve_ambiguities_with_ends: gap - A - B - C - d ... with d "
                   "singular; optimize B =  "
                << nodes_[ifirst + 1].c().id() << " and C = " << nodes_[ilast].c().id()
                << std::endl;
    }
    for (size_t joint_index_A = 0; joint_index_A <= 1; ++joint_index_A) {
      topology::experimental_point pA = nodes_[ifirst].ccc()[0].joints()[joint_index_A].epb();
  ...
  if (first_ambiguous_is_second) {
    if (last_ambiguous_is_begore_gap) {  // 0 - A - B - C - gap
      if (print_level() >= mybhep::VERBOSE) {
        std::clog
            << " CAT::cluster::solve_ambiguities_with_ends: | 0 - A - B - C - gap : optimize B = "
            << nodes_[ifirst + 1].c().id() << std::endl;
      }
      for (size_t joint_index_A = 0; joint_index_A <= 1; ++joint_index_A) {
        for (size_t joint_index_C = 0; joint_index_C <= 1; ++joint_index_C) {
    ...
    if (last_ambiguous_is_last_but_one) {  // node after last ambiguous is the last: 0 - A - B - C -
                                           // N
      if (print_level() >= mybhep::VERBOSE) {
        std::clog
            << " CAT::cluster::solve_ambiguities_with_ends:  | 0 - A - B - C - N | optimize B = "
            << nodes_[ifirst + 1].c().id() << std::endl;
      }
    ...
    // 0 - A - B - C - d - ...
    if (print_level() >= mybhep::VERBOSE) {
      std::clog << " CAT::cluster::solve_ambiguities_with_ends: 0 - A - B - C - d ... with d "
                   "singular; optimize B = "
                << nodes_[ifirst + 1].c().id() << ", C " << nodes_[ilast].c().id() << std::endl;
    }
  ...
  if (last_ambiguous_is_begore_gap) {  // .. - a - A - B - C - gap
    ...
    if (print_level() >= mybhep::VERBOSE) {
      std::clog
          << " CAT::cluster::solve_ambiguities_with_ends: ... - a - A - B - C - gap; optimize A = "
          << nodes_[ifirst].c().id() << ", B = " << nodes_[ifirst + 1].c().id() << std::endl;
    }
  ...
  if (last_ambiguous_is_last_but_one) {  // node after last ambiguous is the last: ... a - A - B - C
                                         // - N |
    if (print_level() >= mybhep::VERBOSE) {
      std::clog
          << " CAT::cluster::solve_ambiguities_with_ends:  ... a - A - B - C - N | optimize A = "
          << nodes_[ifirst].c().id() << ", B = " << nodes_[ifirst + 1].c().id() << std::endl;
    }
  ...
  if (print_level() >= mybhep::VERBOSE) {
    std::clog
        << " CAT::cluster::solve_ambiguities_with_ends: ... a - A - B - C - d ... ; optimize A = "
        << nodes_[ifirst].c().id() << " , B = " << nodes_[ifirst + 1].c().id()
        << ", C = " << nodes_[ilast].c().id() << std::endl;
  }
...
std::vector<topology::broken_line> cluster::solve_ambiguities_with_ends__4_nodes(
    size_t ifirst, size_t ilast, bool first_ambiguous_is_after_gap, bool first_ambiguous_is_second,
    bool last_ambiguous_is_begore_gap, bool last_ambiguous_is_last_but_one) {
  std::vector<topology::broken_line> bls;
  ...
  if (first_ambiguous_is_after_gap) {
    if (last_ambiguous_is_last_but_one) {  // node after last ambiguous is the last:  gap - A - B -
      ...
      if (print_level() >= mybhep::VERBOSE) {
        std::clog << " CAT::cluster::solve_ambiguities_with_ends: configuration: gap - A - B - C - "
                     "D - N |  optimize B = "
                  << nodes_[ifirst + 1].c().id() << " , C = " << nodes_[ilast - 1].c().id()
                  << std::endl;
      }
      for (size_t joint_index_A = 0; joint_index_A <= 1; ++joint_index_A) {
          ...
          for (size_t joint_index_B = 0; joint_index_B <= 1; ++joint_index_B) {
            for (size_t joint_index_C = 0; joint_index_C <= 1; ++joint_index_C) {
              ...
              topology::experimental_point pC =
                  nodes_[ilast - 1].ccc()[0].joints()[joint_index_C].epb();
              bl.eps_.push_back(pA);
              bl.eps_.push_back(pB);
              bl.eps_.push_back(pC);
              bl.eps_.push_back(pD);
    ...
    if (last_ambiguous_is_begore_gap) {  // gap - A - B - C - D - gap
      if (print_level() >= mybhep::VERBOSE) {
        std::clog << " CAT::cluster::solve_ambiguities_with_ends: gap - A - B - C - D - gap : "
                     "optimize B = "
                  << nodes_[ifirst + 1].c().id() << " , C = " << nodes_[ilast - 1].c().id()
                  << std::endl;
      }
    ...
    // gap - A - B - C - D - e ...
    if (print_level() >= mybhep::VERBOSE) {
      std::clog << " CAT::cluster::solve_ambiguities_with_ends: gap - A - B - C - D - e ... with d "
                   "singular; optimize B = "
                << nodes_[ifirst + 1].c().id() << ", C " << nodes_[ilast - 1].c().id()
                << ", D = " << nodes_[ilast].c().id() << std::endl;
    }
    for (size_t joint_index_A = 0; joint_index_A <= 1; ++joint_index_A) {
      topology::experimental_point pA = nodes_[ifirst].ccc()[0].joints()[joint_index_A].epb();
  ...
  if (first_ambiguous_is_second) {
    if (last_ambiguous_is_begore_gap) {  // 0 - A - B - C - D - gap
      if (print_level() >= mybhep::VERBOSE) {
        std::clog << " CAT::cluster::solve_ambiguities_with_ends: | 0 - A - B - C - D - gap : "
                     "optimize B = "
                  << nodes_[ifirst + 1].c().id() << ", C = " << nodes_[ilast - 1].c().id()
                  << std::endl;
      }
    ...
    if (last_ambiguous_is_last_but_one) {  // node after last ambiguous is the last: 0 - A - B - C -
                                           // D - N
      if (print_level() >= mybhep::VERBOSE) {
        std::clog << " CAT::cluster::solve_ambiguities_with_ends:  | 0 - A - B - C - D - N | "
                     "optimize B = "
                  << nodes_[ifirst + 1].c().id() << ", C = " << nodes_[ilast - 1].c().id()
                  << std::endl;
      }
    ...
    // 0 - A - B - C - D - e - ...
    if (print_level() >= mybhep::VERBOSE) {
      std::clog << " CAT::cluster::solve_ambiguities_with_ends: 0 - A - B - C - D - e ... with d "
                   "singular; optimize B = "
                << nodes_[ifirst + 1].c().id() << ", C = " << nodes_[ilast - 1].c().id()
                << ", D = " << nodes_[ilast].c().id() << std::endl;
    }
  ...
  if (last_ambiguous_is_begore_gap) {  // .. - a - A - B - C - D - gap
    ...
    if (print_level() >= mybhep::VERBOSE) {
      std::clog << " CAT::cluster::solve_ambiguities_with_ends: ... - a - A - B - C - D - gap; "
                   "optimize A = "
                << nodes_[ifirst].c().id() << ", B = " << nodes_[ifirst + 1].c().id()
                << ", C = " << nodes_[ilast - 1].c().id() << std::endl;
    }
  ...
  if (last_ambiguous_is_last_but_one) {  // node after last ambiguous is the last: ... a - A - B - C
                                         // - D - N |
    if (print_level() >= mybhep::VERBOSE) {
      std::clog << " CAT::cluster::solve_ambiguities_with_ends:  ... a - A - B - C - D - N | "
                   "optimize A = "
                << nodes_[ifirst].c().id() << ", B = " << nodes_[ifirst + 1].c().id()
                << " , C = " << nodes_[ilast - 1].c().id() << std::endl;
    }
  ...
  // ... a - A - B - C - D - e - ...
  if (print_level() >= mybhep::VERBOSE) {
    std::clog << " CAT::cluster::solve_ambiguities_with_ends: ... a - A - B - C - D - e ... ; "
                 "optimize A = "
              << nodes_[ifirst].c().id() << " , B = " << nodes_[ifirst + 1].c().id()
              << " , C = " << nodes_[ilast - 1].c().id() << ", D = " << nodes_[ilast].c().id()
              << std::endl;
  }
...
void cluster::solve_ambiguities_with_ends__more_than_4_nodes(topology::broken_line ACD[2][2][2],
  ...
  if (first_ambiguous_is_after_gap) {
    if (print_level() >= mybhep::VERBOSE) {
      std::clog << " CAT::cluster::solve_ambiguities_with_ends__more_than_4_nodes: configuration: "
                   "gap - A - B - C - D |  optimize B = "
                << nodes_[ifirst + 1].c().id() << std::endl;
    }
    for (size_t joint_index_A = 0; joint_index_A <= 1; ++joint_index_A) {
      for (size_t joint_index_C = 0; joint_index_C <= 1; ++joint_index_C) {
      ...
      for (size_t joint_index_A = 0; joint_index_A <= 1; ++joint_index_A) {
        for (size_t joint_index_C = 0; joint_index_C <= 1; ++joint_index_C) {
          for (size_t joint_index_D = 0; joint_index_D <= 1; ++joint_index_D) {
            ...
            std::clog << " iteration [" << joint_index_A << ", " << joint_index_C << ", "
                      << joint_index_D << "] = (" << eps[0].x().value() << ", "
                      << eps[0].z().value() << "), (" << eps[1].x().value() << ", "
                      << eps[1].z().value() << "), (" << eps[2].x().value() << ", "
                      << eps[2].z().value() << "), (" << eps[3].x().value() << ", "
                      << eps[3].z().value() << ")" << std::endl;
          }
        }
      }
    }
  ...
  // ... if first ambiguous node is second node:   0 - A - B - C - D
    ...
    if (print_level() >= mybhep::VERBOSE) {
      std::clog << " CAT::cluster::solve_ambiguities_with_ends__more_than_4_nodes: | 0 - A - B - C "
                   "- D : optimize B = "
                << nodes_[ifirst + 1].c().id() << std::endl;
    }
    for (size_t joint_index_A = 0; joint_index_A <= 1; ++joint_index_A) {
        ...
        for (size_t joint_index_C = 0; joint_index_C <= 1; ++joint_index_C) {
          for (size_t joint_index_D = 0; joint_index_D <= 1; ++joint_index_D) {
            ...
            std::clog << " iteration [" << joint_index_A << ", " << joint_index_C << ", "
                      ...
                      << eps[1].z().value() << "), (" << eps[2].x().value() << ", "
                      << eps[2].z().value() << "), (" << eps[3].x().value() << ", "
                      << eps[3].z().value() << "), (" << eps[4].x().value() << ", "
                      << eps[4].z().value() << ")" << std::endl;
          }
        }
      }
    }
  ...
  // ... a - A - B - C - D
  if (print_level() >= mybhep::VERBOSE) {
    std::clog << " CAT::cluster::solve_ambiguities_with_ends__more_than_4_nodes: ... - a - A - B - "
                 "C - D ; optimize B = "
              << nodes_[ifirst + 1].c().id() << std::endl;
  }
    ...
    for (size_t joint_index_A = 0; joint_index_A <= 1; ++joint_index_A) {
      for (size_t joint_index_C = 0; joint_index_C <= 1; ++joint_index_C) {
        for (size_t joint_index_D = 0; joint_index_D <= 1; ++joint_index_D) {
          eps = ACD[joint_index_A][joint_index_C][joint_index_D].eps();
          std::clog << " iteration [" << joint_index_A << ", " << joint_index_C << ", "
                    << joint_index_D << "] = (" << eps[0].x().value() << ", " << eps[0].z().value()
...
void cluster::solve_ambiguities_with_ends__more_than_4_nodes(topology::broken_line aACD[2][2][2][2],
                                                             size_t ifirst, size_t ilast) {
  std::vector<topology::broken_line> bls;

  // ... AN - A - B - C - D
  if (print_level() >= mybhep::VERBOSE) {
    std::clog << " CAT::cluster::solve_ambiguities_with_ends__more_than_4_nodes: ... - AN - A - B "
                 "- C - D ; optimize B = "
              << nodes_[ifirst + 1].c().id() << std::endl;
  }
      ...
      for (size_t joint_index_A = 0; joint_index_A <= 1; ++joint_index_A) {
        for (size_t joint_index_C = 0; joint_index_C <= 1; ++joint_index_C) {
          for (size_t joint_index_D = 0; joint_index_D <= 1; ++joint_index_D) {
            ...
            std::clog << " iteration [" << joint_index_AN << ", " << joint_index_A << ", "
                      << joint_index_C << ", " << joint_index_D << "] = (" << eps[0].x().value()
                      << ", " << eps[0].z().value() << "), (" << eps[1].x().value() << ", "
                      << eps[1].z().value() << "), (" << eps[2].x().value() << ", "
                      << eps[2].z().value() << "), (" << eps[3].x().value() << ", "
                      << eps[3].z().value() << ")" << std::endl;
...
void cluster::merge__more_than_4_nodes(topology::broken_line ACD[2][2][2],
                                       topology::broken_line aACD[2][2][2][2]) {
  if (print_level() >= mybhep::VERBOSE) {
    std::clog << "CAT::cluster::merge__more_than_4_nodes: merge " << ACD[0][0][0].eps().size()
              << " points with " << aACD[0][0][0][0].eps().size() << " points " << std::endl;
  }

  topology::broken_line old_ACD[2][2][2];
  for (size_t a = 0; a <= 1; ++a)
    for (size_t b = 0; b <= 1; ++b)
      for (size_t c = 0; c <= 1; ++c) old_ACD[a][b][c] = ACD[a][b][c];
      ...
      for (size_t joint_index_D = 0; joint_index_D <= 1; ++joint_index_D) {
        ...
        for (size_t joint_index_A = 0; joint_index_A <= 1; ++joint_index_A) {
          for (size_t joint_index_B = 0; joint_index_B <= 1; ++joint_index_B) {
            ...
            topology::broken_line bl2 =
                aACD[joint_index_A][joint_index_B][joint_index_C][joint_index_D];
            bl1.calculate_chi2();
            bl2.calculate_chi2();
  ...
  if (print_level() >= mybhep::VERBOSE) {
    std::clog << "CAT::cluster::merge__more_than_4_nodes: after merging there are "
              << ACD[0][0][0].eps().size() << " points " << std::endl;
  }
    ...
    for (size_t joint_index_AN = 0; joint_index_AN <= 1; ++joint_index_AN) {
      for (size_t joint_index_C = 0; joint_index_C <= 1; ++joint_index_C) {
        for (size_t joint_index_D = 0; joint_index_D <= 1; ++joint_index_D) {
          ...
          std::clog << " iteration [" << joint_index_AN << ", " << joint_index_C << ", "
                    << joint_index_D << "] =";
          for (std::vector<experimental_point>::const_iterator ip = eps.begin(); ip != eps.end();
               ++ip)
...
std::vector<topology::broken_line> cluster::finish__more_than_4_nodes(
    topology::broken_line ACD[2][2][2], size_t ifirst, size_t ipivot, size_t n_residuals) {
  size_t ilast = ipivot + n_residuals;

  if (print_level() >= mybhep::VERBOSE) {
    std::clog << " CAT::cluster::finish__more_than_4_nodes: ifirst " << nodes_[ifirst].c().id()
              << " ipivot " << nodes_[ipivot].c().id() << " ilast " << nodes_[ilast].c().id()
              << " n_residuals " << n_residuals << " npoints " << ACD[0][0][0].eps().size()
              << std::endl;
  }
    ...
    for (size_t joint_index_AN = 0; joint_index_AN <= 1; ++joint_index_AN) {
      for (size_t joint_index_Z = 0; joint_index_Z <= 1; ++joint_index_Z) {
        ...
        for (size_t joint_index_Y = 0; joint_index_Y <= 1; ++joint_index_Y) {
          topology::experimental_point pY =
              nodes_[ilast - 1].ccc()[0].joints()[joint_index_Y].epb();
    ...
    for (size_t joint_index_AN = 0; joint_index_AN <= 1; ++joint_index_AN) {
      for (size_t joint_index_Z = 0; joint_index_Z <= 1; ++joint_index_Z) {
        ...
        for (size_t joint_index_X = 0; joint_index_X <= 1; ++joint_index_X) {
          topology::experimental_point pX =
              nodes_[ilast - 2].ccc()[0].joints()[joint_index_X].epb();
          for (size_t joint_index_Y = 0; joint_index_Y <= 1; ++joint_index_Y) {
            topology::experimental_point pY =
                nodes_[ilast - 1].ccc()[0].joints()[joint_index_Y].epb();
...
std::vector<topology::broken_line> cluster::solve_ambiguities_with_ends(size_t ifirst,
                                                                        size_t ilast) {
  ////////////////////////////////////////////////////////////
  /// solve ambiguities
  ////////////////////////////////////////////////////////////
  ...
  if (ifirst < 1 || ifirst + 2 > nodes_.size()) {
    if (print_level() >= mybhep::NORMAL) {
      std::clog << " CAT::cluster::solve_ambiguities_with_ends: problem: first ambiguous node "
                << ifirst << " size " << nodes_.size() << std::endl;
    }
    return bls;
  }
  ...
  if (ilast < 1 || ilast + 2 > nodes_.size()) {
    if (print_level() >= mybhep::NORMAL) {
      std::clog << " CAT::cluster::solve_ambiguities_with_ends: problem: last ambiguous node "
                << ilast << " size " << nodes_.size() << std::endl;
    }
    return bls;
  }
  ...
  bool first_ambiguous_is_after_gap =
      (nodes_[ifirst - 1].c().block() != nodes_[ifirst].c().block());
  ...
  if (print_level() >= mybhep::VERBOSE) {
    std::clog << " CAT::cluster::solve_ambiguities_with_ends: first ambiguous node " << ifirst
              << " after_gap: " << first_ambiguous_is_after_gap
              << " is_second: " << first_ambiguous_is_second << " last ambiguous node " << ilast
              << " before_gap: " << last_ambiguous_is_begore_gap
              << " is_last_but_one: " << last_ambiguous_is_last_but_one << " n of ambiguous nodes "
              << n_ambiguous_nodes << std::endl;
  }
  ...
  if (n_ambiguous_nodes == 1) {
    bls = solve_ambiguities_with_ends__1_node(
        ifirst, ilast, first_ambiguous_is_after_gap, first_ambiguous_is_second,
        last_ambiguous_is_begore_gap, last_ambiguous_is_last_but_one);
  ...
  if (n_ambiguous_nodes == 2) {
    bls = solve_ambiguities_with_ends__2_nodes(
        ifirst, ilast, first_ambiguous_is_after_gap, first_ambiguous_is_second,
        last_ambiguous_is_begore_gap, last_ambiguous_is_last_but_one);
  ...
  if (n_ambiguous_nodes == 3) {
    bls = solve_ambiguities_with_ends__3_nodes(
        ifirst, ilast, first_ambiguous_is_after_gap, first_ambiguous_is_second,
        last_ambiguous_is_begore_gap, last_ambiguous_is_last_but_one);
  ...
  if (n_ambiguous_nodes == 4) {
    bls = solve_ambiguities_with_ends__4_nodes(
        ifirst, ilast, first_ambiguous_is_after_gap, first_ambiguous_is_second,
        last_ambiguous_is_begore_gap, last_ambiguous_is_last_but_one);
  ...
  while (n_residuals > 4) {
    ...
    if (print_level() >= mybhep::VERBOSE) {
      std::clog << " CAT::cluster::solve_ambiguities_with_ends: new first ambiguous node "
                << nodes_[new_ifirst].c().id() << " after_gap: " << first_ambiguous_is_after_gap
                << " is_second: " << first_ambiguous_is_second << " new last ambiguous node "
                << nodes_[new_ilast].c().id() << " n of ambiguous nodes " << n_ambiguous_nodes
                << " n_residuals " << n_residuals << std::endl;
    }

    if (first) {
      first = false;
      solve_ambiguities_with_ends__more_than_4_nodes(
          ACD, new_ifirst, new_ilast, first_ambiguous_is_after_gap, first_ambiguous_is_second);
    ...
    if (print_level() >= mybhep::VERBOSE) {
      std::clog << " CAT::cluster::solve_ambiguities_with_ends: broken_line ACD[0][0][0] has "
                << ACD[0][0][0].eps().size() << " points, first = " << nodes_[ifirst].c().id()
                << ", last = " << nodes_[new_ilast].c().id() << " n_residuals " << n_residuals
                << std::endl;
    }
  }
