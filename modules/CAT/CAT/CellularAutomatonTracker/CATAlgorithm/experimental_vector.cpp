...
experimental_vector::experimental_vector(const experimental_double& x, const experimental_double& y,
...
experimental_vector::experimental_vector(double x, double y, double z, double ex, double ey,
                                         double ez) {
  appname_ = "experimental_vector: ";
  x_.set_value(x);
  y_.set_value(y);
...
experimental_vector::experimental_vector(const experimental_point& epa,
...
/*** dump ***/
void experimental_vector::dump(std::ostream& a_out, const std::string& a_title,
...
//! set all coordinates
void experimental_vector::coordinates(const experimental_double& x, const experimental_double& y,
                                      const experimental_double& z) {
  x_ = x;
  y_ = y;
  z_ = z;
...
//! set all coordinates
void experimental_vector::set(const experimental_double& x, const experimental_double& y,
                              const experimental_double& z) {
  coordinates(x, y, z);
}
...
//! set x
void experimental_vector::set_x(const experimental_double& x) { x_ = x; }

//! set y
void experimental_vector::set_y(const experimental_double& y) { y_ = y; }

//! set z
void experimental_vector::set_z(const experimental_double& z) { z_ = z; }
...
//! get x
const experimental_double& experimental_vector::x() const { return x_; }
//! read x
experimental_double& experimental_vector::x() { return x_; }

//! get y
const experimental_double& experimental_vector::y() const { return y_; }
//! read y
experimental_double& experimental_vector::y() { return y_; }
//! get z
...
experimental_vector& experimental_vector::operator+=(const experimental_vector& p2) {
  ...
  for (size_t i = 0; i < 3; i++) p1(i) += p2(i);
  return p1;
}
...
experimental_vector& experimental_vector::operator*=(double a) {
  ...
  for (size_t i = 0; i < 3; i++) p1(i) = p1(i) * a;
  return p1;
}
...
experimental_double experimental_vector::distance(const experimental_vector& p2) const {
  ...
  result.set_error(std::sqrt(mybhep::square(x_.value() * x_.error()) +
                             mybhep::square(p2.x().value() * p2.x().error()) +
                             mybhep::square(y_.value() * y_.error()) +
                             mybhep::square(p2.y().value() * p2.y().error()) +
                             mybhep::square(z_.value() * z_.error()) +
                             mybhep::square(p2.z().value() * p2.z().error())) /
                   result.value());
...
experimental_double experimental_vector::length() const {
  ...
  result.set_value(std::sqrt(mybhep::square(x_.value()) + mybhep::square(y_.value()) +
...
experimental_double experimental_vector::length2() const {
  experimental_double result =
      experimental_square(x_) + experimental_square(y_) + experimental_square(z_);

  return result;
}

experimental_double experimental_vector::phi() const { return experimental_atan2(z_, x_); }

experimental_double experimental_vector::tan_phi() {
  if (x_.value() == 0.) return experimental_double(mybhep::plus_infinity, 0.);
...
experimental_double experimental_vector::kink_phi(const experimental_vector& v) {
  ...
  result.set_error(std::sqrt(mybhep::square(p1.error()) + mybhep::square(p2.error())));
...
experimental_double experimental_vector::kink_theta(const experimental_vector& v) {
  ...
  result.set_error(std::sqrt(mybhep::square(t1.error()) + mybhep::square(t2.error())));
