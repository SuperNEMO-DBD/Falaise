...
class handle {
  ...
  */
   ...
   *
   */
  handle(T* a_t_ptr = 0) : sp_(a_t_ptr) { return; }
   ...
   */
  handle(const boost::shared_ptr<T>& a_sp) : sp_(a_sp) { return; }
  ...
  bool unique() const { return sp_.unique(); }
  ...
  /*
  //! Reset the internal shared pointer with some new dynamically allocated instance created using
  its default constructor. void reset ()
  {
  sp_.reset (0);
  //      sp_.reset (new T);
  return;
...
struct handle_predicate : public i_predicate<handle<T> > {
  //! The default constructor.
  handle_predicate(const i_predicate<T>& a_predicate, bool a_no_data_means_false = true)
      : predicate_(a_predicate), no_data_means_false_(a_no_data_means_false) {
    return;
  }
  ...
  bool no_data_means_false_;        /*!< A flag to indicate the behaviour in case of NULL pointer :
                                       predicate returns false (default) or throws an exception). */
};
