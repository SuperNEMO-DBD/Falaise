...
// Ourselves:
#include <falaise/snemo/reconstruction/sultan_then_cat_driver.h>
...
namespace snemo {
  ...
  namespace reconstruction {
    ...
    double sultan_then_cat_driver::get_magfield() const
    {
      return _magfield_;
    }
	...
	{
	  _SULTAN_setup_.assign_helices_to_clusters = setup_.fetch_boolean ("SULTAN.assign_helices_to_clusters");
	}
    ...
    void sultan_then_cat_driver::reset ()
    {
      DT_THROW_IF(! is_initialized(), std::logic_error,
                  "SULTAN THEN CAT driver is not initialized !");
    ...
    CAT::topology::cell sultan_then_cat_driver::fill_CAT_hit_from_SULTAN_hit(SULTAN::topology::cell sc) {
    ...
    CAT::topology::calorimeter_hit sultan_then_cat_driver::fill_CAT_calo_hit_from_SULTAN_calo_hit(SULTAN::topology::calorimeter_hit sc) {
      ...
      CAT::topology::experimental_vector cnorm(sc.pl().norm().y().value(),sc.pl().norm().z().value(),sc.pl().norm().x().value(),
					       sc.pl().norm().y().error(),sc.pl().norm().z().error(),sc.pl().norm().x().error());
      CAT::topology::experimental_vector csizes(sc.pl().sizes().y().value(), sc.pl().sizes().z().value(), sc.pl().sizes().x().value(),
						sc.pl().sizes().y().error(), sc.pl().sizes().z().error(), sc.pl().sizes().x().error());
      CAT::topology::experimental_point ccenter(sc.pl().center().y().value(), sc.pl().center().z().value(), sc.pl().center().x().value(),
    ...
    CAT::topology::cluster sultan_then_cat_driver::fill_CAT_cluster_from_SULTAN_sequence(SULTAN::topology::sequence ss){
      
      std::vector<CAT::topology::node> nodes;
      
      std::vector<SULTAN::topology::node> snodes = ss.nodes();
      
      for( std::vector<SULTAN::topology::node>::const_iterator inode=snodes.begin(); inode!=snodes.end(); ++inode){
    ...
    void sultan_then_cat_driver::convert_sultan_data_to_cat_data(){
      ...
      for(std::vector<SULTAN::topology::cell>::const_iterator sc =  _SULTAN_input_.cells.begin(); sc !=  _SULTAN_input_.cells.end(); ++sc){
	ct::cell & c = _CAT_input_.add_cell ();
	c = fill_CAT_hit_from_SULTAN_hit(*sc);
      }
      
      for(std::vector<SULTAN::topology::calorimeter_hit>::const_iterator sc =  _SULTAN_input_.calo_cells.begin(); sc !=  _SULTAN_input_.calo_cells.end(); ++sc){
	...
	for(std::vector<SULTAN::topology::sequence>::const_iterator iseq=sequences.begin(); iseq!=sequences.end(); ++iseq){
    ...
    SULTAN::topology::sequence sultan_then_cat_driver::fill_SULTAN_sequence_from_CAT_sequence(CAT::topology::sequence cs){
      
      std::vector<SULTAN::topology::node> nodes;
      
      std::vector<CAT::topology::node> snodes = cs.nodes();
      
      for( std::vector<CAT::topology::node>::const_iterator inode=snodes.begin(); inode!=snodes.end(); ++inode){
    ...
    SULTAN::topology::cell sultan_then_cat_driver::fill_SULTAN_hit_from_CAT_hit(CAT::topology::cell cc) {
    ...
    SULTAN::topology::calorimeter_hit sultan_then_cat_driver::fill_SULTAN_calo_hit_from_CAT_calo_hit(CAT::topology::calorimeter_hit cc) {
      ...
      SULTAN::topology::experimental_double x (cc.pl().center().z().value(), cc.pl().center().z().error());
      SULTAN::topology::experimental_double y (cc.pl().center().x().value(), cc.pl().center().x().error());
      SULTAN::topology::experimental_double z (cc.pl().center().y().value(), cc.pl().center().y().error());
      ...
      SULTAN::topology::experimental_double sx (cc.pl().sizes().z().value(), cc.pl().sizes().z().error());
      SULTAN::topology::experimental_double sy (cc.pl().sizes().x().value(), cc.pl().sizes().x().error());
      SULTAN::topology::experimental_double sz (cc.pl().sizes().y().value(), cc.pl().sizes().y().error());
    ...
    SULTAN::topology::scenario sultan_then_cat_driver::fill_SULTAN_scenario_from_CAT_scenario(CAT::topology::scenario cs){
      
      std::vector<SULTAN::topology::sequence> sseqs;
      std::vector<CAT::topology::sequence> seqs = cs.sequences();
      for(  std::vector<CAT::topology::sequence>::const_iterator iseq = seqs.begin(); iseq!=seqs.end(); ++iseq){
    ...
    SULTAN::topology::cluster sultan_then_cat_driver::fill_SULTAN_cluster_from_CAT_sequence(CAT::topology::sequence cs){
    ...
    void sultan_then_cat_driver::convert_cat_data_to_sultan_data(){
      ...
      for(std::vector<CAT::topology::cell>::const_iterator cc =  _CAT_input_.cells.begin(); cc !=  _CAT_input_.cells.end(); ++cc){
	st::cell & c = _SULTAN_input_.add_cell ();
	c = fill_SULTAN_hit_from_CAT_hit(*cc);
      }
      
      for(std::vector<CAT::topology::calorimeter_hit>::const_iterator cc =  _CAT_input_.calo_cells.begin(); cc !=  _CAT_input_.calo_cells.end(); ++cc){
	...
	c = fill_SULTAN_calo_hit_from_CAT_calo_hit(*cc);
      ...
      for( std::vector<CAT::topology::scenario>::const_iterator iscen = cat_scenarios.begin(); iscen!=cat_scenarios.end(); ++iscen){
	...
	for(std::vector<CAT::topology::sequence>::const_iterator iseq=sequences.begin(); iseq!=sequences.end(); ++iseq){
    ...
    int sultan_then_cat_driver::_process_algo(const base_tracker_clusterizer::hit_collection_type & gg_hits_,
	  ...
	  if (gg_hit_type != _gg_cell_geom_type_)
	    {
	      ...
	      message << "snemo::processing::sultan_then_cat_driver::_process_algo: "
	...
	BOOST_FOREACH (const sdm::calibrated_data::calorimeter_hit_handle_type & calo_handle,
		       calo_hits_)
	  {
	    ...
	    st::experimental_double energy(sncore_calo_hit.get_energy(), sncore_calo_hit.get_sigma_energy());
	    st::experimental_double time(sncore_calo_hit.get_time(), sncore_calo_hit.get_sigma_time());
	    //size_t id = sncore_calo_hit.get_hit_id ();
	    st::experimental_point center(block_position.x(), block_position.y(), block_position.z(), 0.,0.,0.); 
      ...
      _CAT_sequentiator_.sequentiate_after_sultan (_CAT_output_.tracked_data, conserve_clustering_from_removal_of_cells);
      ...
      std::vector<SULTAN::topology::scenario> sultan_scenarios = _SULTAN_output_.tracked_data.get_scenarios();
      if( sultan_scenarios.size() ){
	sultan_sequences = sultan_scenarios[0].sequences();
      }

      for (std::vector<ct::scenario>::const_iterator iscenario = tss.begin ();
	   iscenario != tss.end ();
	   ++iscenario)
	{
	  ...
	  if( the_sequences.size() != sultan_sequences.size() ){
	    std::clog << " snreconstruction::sultan_then_cat_driver:: problem: cat_sequences.size() " << the_sequences.size() << " sultan_sequences.size() " << sultan_sequences.size() << std::endl;
	    exit(0);
	  }
	  ...
	  for (std::vector<ct::sequence>::const_iterator isequence = the_sequences.begin ();
	      ...
	      else
		{
		  ...
		  const SULTAN::topology::experimental_helix & sultan_helix = sultan_sequences[index_of_sultan_sequence].get_helix();
		  ...
		  const SULTAN::topology::experimental_vector & CAT_sultan_helix_momentum = sultan_sequences[index_of_sultan_sequence].helix_momentum();
		  ...
		  cluster_handle.grab().grab_auxiliaries ().update ("CAT_helix_x0", seq_helix.center().z().value());
		  cluster_handle.grab().grab_auxiliaries ().update ("CAT_helix_y0", seq_helix.center().x().value());
		  cluster_handle.grab().grab_auxiliaries ().update ("CAT_helix_z0", seq_helix.center().y().value());
		  ...
		  cluster_handle.grab().grab_auxiliaries ().update ("sultan_center_of_curvature_x",sultan_x0.value());
		  cluster_handle.grab().grab_auxiliaries ().update ("sultan_center_of_curvature_y",sultan_y0.value());
		  cluster_handle.grab().grab_auxiliaries ().update ("sultan_center_of_curvature_z",sultan_z0.value());
		  ...
		  cluster_handle.grab().grab_auxiliaries ().update ("sultan_helix_momentum_x",CAT_sultan_helix_momentum.x().value());
		  cluster_handle.grab().grab_auxiliaries ().update ("sultan_helix_momentum_y", CAT_sultan_helix_momentum.y().value());
		  cluster_handle.grab().grab_auxiliaries ().update ("sultan_helix_momentum_z", CAT_sultan_helix_momentum.z().value());
		  cluster_handle.grab().grab_auxiliaries ().update ("sultan_helix_momentum_x_error",CAT_sultan_helix_momentum.x().error());
		  cluster_handle.grab().grab_auxiliaries ().update ("sultan_helix_momentum_y_error",CAT_sultan_helix_momentum.y().error());
		  cluster_handle.grab().grab_auxiliaries ().update ("sultan_helix_momentum_z_error", CAT_sultan_helix_momentum.z().error());
...
DOCD_CLASS_SYSTEM_REGISTRATION(snemo::reconstruction::sultan_then_cat_driver, "snemo::reconstruction::sultan_then_cat_driver")
