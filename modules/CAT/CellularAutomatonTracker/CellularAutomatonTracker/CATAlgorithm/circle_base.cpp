...
//! constructor
circle::circle(const experimental_point &center, const experimental_double &radius, prlevel level,
               double probmin) {
  set_print_level(level);
  set_probmin(probmin);
  appname_ = "circle: ";
...
/*** dump ***/
void circle::dump(std::ostream &a_out, const std::string &a_title, const std::string &a_indent,
                  bool /*a_inherit*/) const {
  {
    ...
    /*
      ...
      a_out << indent << " .. theta " << theta.value()*180./M_PI << " x " <<
      position(theta).x().value() << " , z " << position(theta).z().value() << std::endl;
      }
    */
...
//! set center
void circle::set_center(const experimental_point &center) { center_ = center; }

//! set radius
void circle::set_radius(const experimental_double &radius) { radius_ = radius; }

//! get center
const experimental_point &circle::center() const { return center_; }

//! get radius
const experimental_double &circle::radius() const { return radius_; }

//! get curvature
experimental_double circle::curvature() const { return 1. / radius_; }
...
experimental_point circle::position(const experimental_double &phi) const {
  ...
  return (experimental_vector(center()) + experimental_vector(deltax, center().y(), deltaz))
      .point_from_vector();
}

// get the position at the theta of point p
experimental_point circle::position(const experimental_point &ep) const { return position(ep, 0.); }
...
double circle::chi2(experimental_point &ep) { return chi2(ep, 0.); }
// get the chi2 with point p
...
void circle::best_fit_pitch(std::vector<experimental_point> &ps, experimental_double *_pitch,
                            experimental_double *_center) {
  if (ps.size() == 0) {
    std::clog << " problem: asking for best fit pitch for p std::vector of size " << ps.size()
              << std::endl;
    return;
  }
  ...
  double errce = ce * sqrt(one_over_pi_err / square(one_over_pi) +
                           min_ce_over_pi_err / square(min_ce_over_pi));

  *_pitch = experimental_double(pi, errpi);
  *_center = experimental_double(ce, errce);

  if (print_level() >= mybhep::VVERBOSE) {
    std::clog << " average y " << average(ys).value() << " average phi " << average(phis).value()
              << " 1/p " << one_over_pi << " -y0/p " << min_ce_over_pi << " center y " << ce
              << " pitch " << pi << " " << std::endl;
...
bool circle::intersect_plane(plane pl, experimental_point *ep, experimental_double _phi) {
  ...
  if (diff == 0.) {  // the circle is tangent to the plane face
    *ep = (center() - the_norm * radius().value()).point_from_vector();
    ...
    if (print_level() >= mybhep::VVERBOSE) {
      std::clog << " intersecting circle with center " << center().x().value() << " "
                << center().z().value() << " with plain with face " << pl.face().x().value() << " "
                << pl.face().z().value() << " phi " << _phi.value() * 180. / acos(-1.) << std::endl;
    }

    if (pl.view() == "x") {
      double sign = 1.;
      if (sin(_phi.value()) < 0.) sign = -1.;

      ep->set_x(pl.face().x());
      ep->set_z(center().z() +
                experimental_sqrt(experimental_square(radius()) - ntp.length2()) * sign);
      ep->set_y(center().y());
    }

    else if (pl.view() == "z") {
      double sign = 1.;
      if (cos(_phi.value()) < 0.) sign = -1.;
      ...
      experimental_double transverse_dist =
          experimental_sqrt(experimental_square(radius()) - ntp.length2());
      ...
      if (print_level() >= mybhep::VVERBOSE) {
        std::clog
            << " normal to plane (" << the_norm.x().value() << ", " << the_norm.y().value() << ", "
            << the_norm.z().value()
            << ") normal vector from face of plane to parallel plane through center of circle ( "
            << ntp.x().value() << ", " << ntp.y().value() << ", " << ntp.z().value() << ") diff "
            << diff << " foot on face of plane in front of circle center: ( " << foot.x().value()
            << ", " << foot.y().value() << ", " << foot.z().value()
            << ") transverse dist: " << transverse_dist.value() << " angle "
            << angle.value() * 180. / acos(-1.) << std::endl;
      }
      ...
      std::clog << " problem: cannot intersect circle with plane of view " << pl.view()
                << std::endl;
      return false;
    }
  }

  if (ep->x().value() == small_neg || ep->y().value() == small_neg || ep->z().value() == small_neg)
    return false;

  if (std::isnan(ep->x().value()) || std::isnan(ep->y().value()) || std::isnan(ep->z().value()))
    return false;

  // vector from center of plane face to extrapolated point
  experimental_vector dist = experimental_vector(pl.face(), *ep).hor();
  if (print_level() >= mybhep::VVERBOSE) {
    std::clog << " extrapolated point: (" << ep->x().value() << ", " << ep->y().value() << ", "
              << ep->z().value()
              << "), circle distance from extrapolation to plane face: " << dist.x().value() << ", "
              << dist.y().value() << ", " << dist.z().value()
              << " plane sizes: " << pl.sizes().x().value() << " " << pl.sizes().y().value() << " "
              << pl.sizes().z().value() << std::endl;
  }
  if (pl.view() == "x") {
    if (std::abs(dist.z().value()) > pl.sizes().z().value() / 2.) return false;
    return true;
  }
  if (pl.view() == "z") {
    if (std::abs(dist.x().value()) > pl.sizes().x().value() / 2.) return false;
    return true;
  }
  if (pl.view() == "inner" || pl.view() == "outer") {
    experimental_vector transverse_dist = (dist) ^ (the_norm.hor());

    if (transverse_dist.length().value() > pl.sizes().z().value() / 2.) return false;
    return true;
  }
...
bool circle::intersect_circle(circle c, experimental_point *ep, experimental_double _phi) {
  ...
  // hence
  // 2ad = R0^2 - R1^2 + d^2
  experimental_double a = (experimental_square(radius()) - experimental_square(c.radius()) +
                           experimental_square(dist)) /
                          (dist * 2.);
  experimental_point middle = (center() + (c.center() - center()) * a / dist).point_from_vector();
  experimental_double h =
      experimental_sqrt(experimental_fabs(experimental_square(radius()) - square(a)));
  ...
  if (ep->x().value() == small_neg || ep->y().value() == small_neg ||
  ...
  if (print_level() >= mybhep::VVERBOSE) {
    ...
    clog << " intersection 1: ";
    p1.dump();
    clog << " phi1 " << phi1 << " initial phi " << initial_phi1 << " dphi1 " << dphi1 << endl;
    clog << " intersection 2: ";
    p2.dump();
    clog << " phi2 " << phi2 << " initial phi " << initial_phi2 << " dphi2 " << dphi2 << endl;
    clog << " dist ";
    dist.dump();
    std::clog << " rsum ";
    rsum.dump();
...
void circle::point_of_max_min_radius(experimental_point epa, experimental_point epb,
  ...
  if (print_level() >= mybhep::VVERBOSE) {
    std::clog << " along the circle between points (" << epa.x().value() << ", " << epa.z().value()
              << ") and (" << epb.x().value() << ", " << epb.z().value()
              << ") the point of max radius is (" << epmax->x().value() << ", "
              << epmax->z().value() << "), that of min radius is (" << epmin->x().value() << ", "
              << epmin->z().value() << ")" << std::endl;
  }
...
circle three_points_circle(const experimental_point &epa, const experimental_point &epb,
  ...
  experimental_double Xc =
      (ma * mb * (epa.z() - epc.z()) + mb * (epa.x() + epb.x()) - ma * (epb.x() + epc.x())) /
      ((mb - ma) * 2);
  ...
  experimental_double _radius =
      experimental_sqrt(experimental_square(Xc - epa.x()) + experimental_square(Zc - epa.z()));

  if (std::isnan(_radius.value())) _radius.set_value(small_neg);
...
circle best_fit_circle(std::vector<experimental_double> &xs, std::vector<experimental_double> &zs) {
  ...
  if (xs.size() == 0) {
    std::clog << " problem: asking for best fit radius for x std::vector of size " << xs.size()
              << std::endl;
    return h;
  }

  if (zs.size() != xs.size()) {
    std::clog << " problem: asking for best fit radius for z std::vector of size " << zs.size()
              << " x std::vector of size " << xs.size() << std::endl;
    return h;
  }
