...
experimental_double& experimental_double::operator*=(experimental_double a) {
  ...
  double err =
      std::sqrt(mybhep::square(a.value() * p1.error()) + mybhep::square(p1.value() * a.error()));
...
experimental_double& experimental_double::operator/=(experimental_double a) {
  ...
  if (a.value() == 0) {
    std::clog << "CAT::experimental_double::operator/=: problem: division by experimental_double "
                 "with value "
              << a.value() << std::endl;
  }

  double val = p1.value() / a.value();
  double err = std::sqrt(mybhep::square(p1.error() / a.value()) +
                         mybhep::square(p1.value() * a.error() / mybhep::square(a.value())));
  p1.set_value(val);
  p1.set_error(err);
  return p1;
}
...
experimental_double& experimental_double::operator/=(double a) {
  ...
  if (a == 0) {
    std::clog << "CAT::experimental_double::operator/=: problem: division by double with value "
              << a << std::endl;
  }
...
experimental_double experimental_atan2(const experimental_double& v1,
  ...
  if (v2.value() == 0.) {  // if angle = 90 degrees,
    ...
    double num = mybhep::square(v2.error() / v1.value()) +
                 mybhep::square(v2.value() * v1.error() / mybhep::square(v1.value()));
    ...
    double num = mybhep::square(v1.error() / v2.value()) +
                 mybhep::square(v1.value() * v2.error() / mybhep::square(v2.value()));
    v.set_error(std::sqrt(num) / den);
  }
  return v;
}
...
experimental_double weighted_average(const std::vector<experimental_double> vs) {
  ...
  for (std::vector<experimental_double>::const_iterator iv = vs.begin(); iv != vs.end(); ++iv) {
    if (iv->error()) {
      ...
      std::clog
          << "CAT::weighted_average: problem: double has error zero; switch to normal average "
          << std::endl;
      return average(vs);
    }
  }
