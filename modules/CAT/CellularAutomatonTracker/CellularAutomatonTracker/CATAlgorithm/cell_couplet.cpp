...
cell_couplet::cell_couplet(const cell &ca, const cell &cb, const std::string & /*just*/,
...
void cell_couplet::set_forward_axis() {
  if (forward_axis_calculated_) return;
...
void cell_couplet::dump(std::ostream &a_out, const std::string &a_title,
  ...
  for (std::vector<line>::const_iterator itang = tangents_.begin(); itang != tangents_.end();
       ++itang)
...
void cell_couplet::set_transverse_axis() {
  if (transverse_axis_calculated_) return;
...
void cell_couplet::obtain_tangents_between_circle_and_circle() {
  ...
  for (size_t i = 0; i < 2; i++) {
    ...
    epa = ca_.build_from_cell(forward_axis_.hor().unit(), transverse_axis(), cosa,
                              sign_parallel_crossed[0] * sign_up_down[i], false, 0.);
    epb = cb_.build_from_cell(forward_axis_.hor().unit(), transverse_axis(), cosb,
                              sign_parallel_crossed[0] * sign_up_down[i], false, 0.);

    set_first_error_in_build_from_cell(sina.value(), sign_parallel_crossed[0], sign_up_down[i],
                                       &epa);
    set_second_error_in_build_from_cell(sina.value(), sign_parallel_crossed[0], sign_up_down[i],
                                        &epb);
  ...
  if (intersecting()) {
    ...
    for (size_t i = 0; i < 2; i++) {
      cosa =
          (ca().r() - cb().r() * sign_parallel_crossed[1]) / distance_hor();  // parallel tangents
      cosb = cosa * sign_parallel_crossed[1];
      sina = experimental_sin(experimental_acos(cosa)) * sign_parallel_crossed[1] * sign_up_down[i];

      epa = ca_.build_from_cell(forward_axis_.hor().unit(), transverse_axis(), cosa,
                                sign_parallel_crossed[1] * sign_up_down[i], false, 0.);
      epb = cb_.build_from_cell(forward_axis_.hor().unit(), transverse_axis(), cosb,
                                sign_up_down[i], false, 0.);

      set_first_error_in_build_from_cell(sina.value(), sign_parallel_crossed[1], sign_up_down[i],
                                         &epa);
      set_second_error_in_build_from_cell(sina.value(), sign_parallel_crossed[1], sign_up_down[i],
                                          &epb);
...
void cell_couplet::obtain_tangents_between_circle_and_point(const cell &c, experimental_point &ep) {
  ...
  for (size_t i = 0; i < 2; i++) {
    epa = c.build_from_cell(forward_axis_.hor().unit(), transverse_axis(), cos, sign_up_down[i],
                            false, 0.);
...
void cell_couplet::obtain_tangents_between_point_and_circle(experimental_point &ep, const cell &c) {
  ...
  for (size_t i = 0; i < 2; i++) {
    epb = c.build_from_cell(forward_axis_.hor().unit(), transverse_axis(), cos, sign_up_down[i],
                            false, 0.);
    sin = experimental_sin(experimental_acos(cos)) * sign_up_down[i];
...
void cell_couplet::obtain_tangents_between_point_and_point(experimental_point &epa,
                                                           experimental_point &epb) {
  line l(epa, epb, prlevel(), probmin());
...
void cell_couplet::set_first_error_in_build_from_cell(double sin, int sign_parallel_crossed,
                                                      int sign_up_down,
  ...
  experimental_vector faa = (*epa - ca_.ep()) / ca_.r().value() +
                            (forward / distance_hor().value() +
                             transverse_axis() * sign_up_down *
                                 ((sign_parallel_crossed * cb_.r().value() - ca_.r().value()) /
                                  (sin * pow(distance_hor().value(), 2)))) *
                                ca_.r().value();
  experimental_vector fab = (forward * sign_parallel_crossed / (-distance_hor().value()) +
                             transverse_axis() * sign_up_down *
                                 ((sign_parallel_crossed * ca_.r().value() - cb_.r().value()) /
                                  (sin * pow(distance_hor().value(), 2)))) *
                            ca_.r().value();

  double errax =
      sqrt(pow(ca_.r().error() * faa.x().value(), 2) + pow(cb_.r().error() * fab.x().value(), 2));
  double erraz =
      sqrt(pow(ca_.r().error() * faa.z().value(), 2) + pow(cb_.r().error() * fab.z().value(), 2));
  epa->set_ex(errax);
  epa->set_ez(erraz);
...
void cell_couplet::set_second_error_in_build_from_cell(double sin, int sign_parallel_crossed,
                                                       int sign_up_down,
  ...
  experimental_vector fba = (forward * sign_parallel_crossed / distance_hor().value() -
                             transverse_axis() * sign_up_down *
                                 ((sign_parallel_crossed * ca_.r().value() - cb_.r().value()) /
                                  (sin * pow(distance_hor().value(), 2)))) *
                            cb_.r().value();
  experimental_vector fbb = (*epb - cb_.ep()) / cb_.r().value() +
                            (forward / (-distance_hor().value()) -
                             transverse_axis() * sign_up_down *
                                 ((sign_parallel_crossed * cb_.r().value() - ca_.r().value()) /
                                  (sin * pow(distance_hor().value(), 2)))) *
  ...
  double errbx =
      sqrt(pow(ca_.r().error() * fba.x().value(), 2) + pow(cb_.r().error() * fbb.x().value(), 2));
  double errbz =
      sqrt(pow(ca_.r().error() * fba.z().value(), 2) + pow(cb_.r().error() * fbb.z().value(), 2));
  epb->set_ex(errbx);
  epb->set_ez(errbz);
...
//! set free level
void cell_couplet::set_free(bool free) { free_ = free; }

//! set begun level
void cell_couplet::set_begun(bool begun) { begun_ = begun; }

//! set tangents
void cell_couplet::set_tangents(const std::vector<line> &tangents) { tangents_ = tangents; }

//! set fwd axis
void cell_couplet::set_a_forward_axis(const experimental_vector &v) { forward_axis_ = v; }

//! set trv axis
void cell_couplet::set_a_transverse_axis(const experimental_vector &v) { transverse_axis_ = v; }

//! set distance
void cell_couplet::set_a_distance(const experimental_double &d) { distance_ = d; }

//! set hor distance
void cell_couplet::set_a_hor_distance(const experimental_double &d) { distance_hor_ = d; }

//! get first cell
const cell &cell_couplet::ca() const { return ca_; }

//! get second cell
const cell &cell_couplet::cb() const { return cb_; }

//! get tangents
const std::vector<line> &cell_couplet::tangents() const { return tangents_; }

//! get forward axis
const experimental_vector &cell_couplet::forward_axis() const { return forward_axis_; }

//! get transverse axis
const experimental_vector &cell_couplet::transverse_axis() const { return transverse_axis_; }

//! get distance
const experimental_double &cell_couplet::distance() const { return distance_; }

//! get horizontal distance
const experimental_double &cell_couplet::distance_hor() const { return distance_hor_; }

//! get free level
bool cell_couplet::free() const { return free_; }

//! get begun level
bool cell_couplet::begun() const { return begun_; }

size_t cell_couplet::iteration() const {
  for (std::vector<line>::const_iterator itang = tangents_.begin(); itang != tangents_.end();
       ++itang)
    if (!itang->used()) return itang - tangents_.begin();
...
bool cell_couplet::intersecting() const { return ca_.intersect(cb_); }
