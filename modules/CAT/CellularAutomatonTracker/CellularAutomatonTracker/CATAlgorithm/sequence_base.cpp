...
sequence::sequence() {
  ...
  decay_tangent_vertex_ =
      experimental_point(mybhep::small_neg, mybhep::small_neg, mybhep::small_neg, mybhep::small_neg,
                         mybhep::small_neg, mybhep::small_neg);
...
void sequence::dump(std::ostream &a_out, const std::string &a_title, const std::string &a_indent,
  ...
  a_out << indent << names_[0] << ". Number of nodes: " << nodes().size() << " free: " << Free()
        << " chi2 " << chi2() << " helix chi2 " << helix_chi2() << " prob " << Prob()
        << " helix prob " << helix_Prob() << " momentum " << momentum().length().value()
  ...
  if (has_helix_vertex()) {
    a_out << indent << " helix_vertex type: " << helix_vertex_type() << " helix_vertex: ";
    helix_vertex_.dump();
    if (helix_vertex_type() == "calo") a_out << " icalo " << helix_vertex_id();
  }
  if (has_decay_helix_vertex()) {
    a_out << indent << " decay helix_vertex type " << decay_helix_vertex_type()
          << " decay_helix_vertex : ";
    decay_helix_vertex_.dump();
    if (decay_helix_vertex_type() == "calo") a_out << " icalo " << calo_helix_id();
  }
  if (has_tangent_vertex()) {
    a_out << indent << " tangent_vertex type: " << tangent_vertex_type() << " tangent_vertex: ";
    tangent_vertex_.dump();
    if (tangent_vertex_type() == "calo") a_out << " icalo " << tangent_vertex_id();
  }
  if (has_decay_tangent_vertex()) {
    a_out << indent << " decay tangent_vertex type " << decay_tangent_vertex_type()
          << " decay_tangent_vertex : ";
    decay_tangent_vertex_.dump();
...
//! set free level
void sequence::set_free(bool free) { free_ = free; }
...
//! add name
void sequence::add_name(const std::string &name) { names_.push_back(name); }

//! set helix_vertex
void sequence::set_helix_vertex(const experimental_point &v, const std::string &type,
                                size_t calo_helix_id) {
  has_helix_vertex_ = true;
  helix_vertex_ = v;
  helix_vertex_type_ = type;
  if (type == "calo") helix_vertex_id_ = calo_helix_id;
}

//! set decay_helix_vertex
void sequence::set_decay_helix_vertex(const experimental_point &v, const std::string &type,
                                      size_t calo_helix_id) {
  has_decay_helix_vertex_ = true;
  decay_helix_vertex_ = v;
  decay_helix_vertex_type_ = type;
  if (type == "calo") calo_helix_id_ = calo_helix_id;
}

//! set tangent_vertex
void sequence::set_tangent_vertex(const experimental_point &v, const std::string &type,
                                  size_t tangent_vertex_id) {
  has_tangent_vertex_ = true;
  tangent_vertex_ = v;
  tangent_vertex_type_ = type;
  if (type == "calo") tangent_vertex_id_ = tangent_vertex_id;
}

//! set decay_tangent_vertex
void sequence::set_decay_tangent_vertex(const experimental_point &v, const std::string &type,
                                        size_t calo_tangent_id) {
  has_decay_tangent_vertex_ = true;
  decay_tangent_vertex_ = v;
  decay_tangent_vertex_type_ = type;
  if (type == "calo") calo_tangent_id_ = calo_tangent_id;
}

//! set chi2 list
void sequence::set_chi2s(const std::vector<double> &chi2s) { chi2s_all_ = chi2s; }
...
//! set prob list
void sequence::set_probs(const std::vector<double> &probs) { probs_all_ = probs; }
...
//! set primary
void sequence::set_primary(bool primary) { primary_ = primary; }

//! set helix
void sequence::set_helix(const helix &h) { helix_ = h; }

//! get nodes
const std::vector<node> &sequence::nodes() const { return nodes_; }

//! get free level
bool sequence::Free() const { return free_; }

//! get name
const std::string &sequence::name() const { return names_[0]; }

//! get names
const std::vector<std::string> &sequence::names() const { return names_; }

//! get primary
bool sequence::primary() const { return primary_; }

//! get list of all chi2
const std::vector<double> &sequence::chi2s_all() const { return chi2s_all_; }

//! get list of prob
const std::vector<double> &sequence::probs_all() const { return probs_all_; }
...
const std::vector<double> &sequence::helix_chi2s() const { return helix_chi2s_; }
...
double sequence::Prob() const { return probof(chi2(), ndof()); }
...
double sequence::helix_Prob() const {
  ...
  std::vector<double> cs = helix_chi2s();
  int32_t ndof = 0;
  if (cs.size() > 2) ndof += 3 * (cs.size() - 2);

  return probof(c, ndof);
}

//! has momentum
bool sequence::has_momentum() const { return has_momentum_; }

//! has helix
bool sequence::has_helix() const { return has_helix_; }

//! has charge
bool sequence::has_charge() const { return has_charge_; }

//! has helix_charge
bool sequence::has_helix_charge() const { return has_helix_charge_; }

//! has detailed_charge
bool sequence::has_detailed_charge() const { return has_detailed_charge_; }

//! has tangent length
bool sequence::has_tangent_length() const { return has_tangent_length_; }

//! has helix length
bool sequence::has_helix_length() const { return has_helix_length_; }

//! has helix_vertex
bool sequence::has_helix_vertex() const { return has_helix_vertex_; }

//! get helix_vertex
const experimental_point &sequence::helix_vertex() const { return helix_vertex_; }

//! get helix_vertex type
const std::string &sequence::helix_vertex_type() const { return helix_vertex_type_; }

//! get helix_vertex id
size_t sequence::helix_vertex_id() const { return helix_vertex_id_; }

//! has decay_helix_vertex
bool sequence::has_decay_helix_vertex() const { return has_decay_helix_vertex_; }

//! get decay_helix_vertex
const experimental_point &sequence::decay_helix_vertex() const { return decay_helix_vertex_; }

//! get calo id
size_t sequence::calo_helix_id() const { return calo_helix_id_; }

//! get decay helix_vertex type
const std::string &sequence::decay_helix_vertex_type() const { return decay_helix_vertex_type_; }

//! has tangent_vertex
bool sequence::has_tangent_vertex() const { return has_tangent_vertex_; }

//! get tangent_vertex
const experimental_point &sequence::tangent_vertex() const { return tangent_vertex_; }

//! get tangent_vertex type
const std::string &sequence::tangent_vertex_type() const { return tangent_vertex_type_; }

//! get tangent_vertex id
size_t sequence::tangent_vertex_id() const { return tangent_vertex_id_; }

//! has decay_tangent_vertex
bool sequence::has_decay_tangent_vertex() const { return has_decay_tangent_vertex_; }

//! get decay_tangent_vertex
const experimental_point &sequence::decay_tangent_vertex() const { return decay_tangent_vertex_; }

//! get calo id
size_t sequence::calo_tangent_id() const { return calo_tangent_id_; }
...
//! get charge
const experimental_double &sequence::charge() const { return charge_; }

//! get helix_charge
const experimental_double &sequence::helix_charge() const { return helix_charge_; }

//! get detailed_charge
const experimental_double &sequence::detailed_charge() const { return detailed_charge_; }

//! get momentum
const experimental_vector &sequence::momentum() const { return momentum_; }

//! get helix_momentum
const experimental_vector &sequence::helix_momentum() const { return helix_momentum_; }

//! get tangent_length
const experimental_double &sequence::tangent_length() const { return tangent_length_; }

//! get helix_length
const experimental_double &sequence::helix_length() const { return helix_length_; }

//! get helix
const helix &sequence::get_helix() const { return helix_; }

//! get fast
bool sequence::fast() const {
  if (!nodes_.empty()) return nodes_[0].c().fast();
...
sequence sequence::invert() {
  ...
  if (has_momentum()) {
    ...
    experimental_vector hnewmom(-helix_momentum().x(), -helix_momentum().y(),
                                -helix_momentum().z());
...
void sequence::set_free_level() {
  ...
  for (std::vector<node>::iterator inode = nodes_.begin(); inode != nodes_.end(); ++inode) {
    ...
    for (std::vector<cell>::iterator ilink = (*inode).links_.begin();
      ...
      if (inode - nodes_.begin() < 1) {
        ...
        for (std::vector<line>::const_iterator itang = (*inode).cc_[iccc].tangents_.begin();
             itang != (*inode).cc_[iccc].tangents_.end(); ++itang) {
          if (!itang->used()) {
            ilink->set_free(true);
            inode->set_free(true);
            set_free(true);
            if (print_level() >= mybhep::VVERBOSE) {
              std::clog << " sequence is free, for cell " << inode->c().id()
                        << " is free, for link to cell " << ilink->id() << " is free, for tangent "
                        << itang - (*inode).cc_[iccc].tangents_.begin() << " is unused "
                        << std::endl;
            }
          }
        }
      } else {
        if (has_cell((*inode).ccc_[iccc].ca()) && has_cell((*inode).ccc_[iccc].cc())) {
          ilink->set_begun(true);
        }

        for (std::vector<joint>::const_iterator ijoint = (*inode).ccc_[iccc].joints_.begin();
             ijoint != (*inode).ccc_[iccc].joints_.end(); ++ijoint) {
          if (!ijoint->used()) {
            ilink->set_free(true);
            inode->set_free(true);
            set_free(true);
            if (print_level() >= mybhep::VVERBOSE) {
              std::clog << " sequence is free, for cell " << (*inode).c().id()
                        << " is free, for triplet with cells " << (*inode).ccc_[iccc].ca().id()
                        << "<->" << (*inode).ccc_[iccc].cc().id() << " is free, for joint "
                        << ijoint - (*inode).ccc_[iccc].joints_.begin() << " is unused "
                        << std::endl;
            }
          }
        }
...
void sequence::last_free_node(size_t *lfn, size_t *link) {
  ...
  for (std::vector<node>::iterator inode = nodes_.begin(); inode != nodes_.end(); ++inode)
    if (inode->free()) {
      ...
      found = false;
      for (std::vector<cell>::iterator itlink = (*inode).links_.begin();
           itlink != (*inode).links_.end(); ++itlink)
        if (itlink->free()) {
  ...
  if (!found) {
    if (print_level() >= mybhep::NORMAL)
      std::clog << " problem: sequence has no last free node. free status is " << Free()
                << std::endl;
    *lfn = 0;
    *link = 0;
    return;
...
topology::node sequence::node_of_cell(const topology::cell &c) {
  ...
  if (fnode == nodes_.end()) {
    if (print_level() >= mybhep::NORMAL)
      std::clog << " problem: requested cell " << c.id() << " has no node in sequence "
                << std::endl;
    ;
...
bool sequence::contained(const topology::sequence &big) const {
  ...
  if (nodes().size() > big.nodes().size()) return false;

  for (std::vector<node>::const_iterator inode = nodes_.begin(); inode != nodes_.end(); ++inode)
    if (!big.has_cell(inode->c())) return false;

  if (nodes().size() == big.nodes().size()) return contained_same_size_and_cells(big);
...
bool sequence::contained_same_extreme_quadrants(const topology::sequence &big) const {
  ...
  if (nodes().front().c().id() == big.nodes().front().c().id()) {
    if (!nodes_[0].c_.same_quadrant(nodes().front().ep(), big.nodes().front().ep())) return false;
    if (!nodes_[s - 1].c_.same_quadrant(nodes().back().ep(), big.nodes().back().ep())) return false;
  } else if (nodes().front().c().id() == big.nodes().back().c().id()) {
    if (!nodes_[0].c_.same_quadrant(nodes().front().ep(), big.nodes().back().ep())) return false;
    if (!nodes_[s - 1].c_.same_quadrant(nodes().back().ep(), big.nodes().front().ep()))
      return false;
  } else {
    if (print_level() >= mybhep::NORMAL) {
      std::clog << " problem: sequences with same size and cells have different beginnings or "
                   "endings: 1st sequence from "
                << nodes().front().c().id() << " to " << nodes().back().c().id() << ", 2nd from "
                << big.nodes().front().c().id() << " to " << big.nodes().back().c().id()
                << std::endl;
    }
  }
...
bool sequence::is_bridge(const topology::sequence &first, const topology::sequence &second) const {
  ...
  if (first.has_cell(nodes()[0].c()) && second.has_cell(nodes()[s - 1].c())) return true;

  if (second.has_cell(nodes()[0].c()) && first.has_cell(nodes()[s - 1].c())) return true;
...
void sequence::increase_iteration(size_t inode, size_t ilink) {
  if (inode >= nodes().size()) {
    if (print_level() >= mybhep::NORMAL)
      std::clog << " problem: increasing iteration of node " << inode << " but size is "
                << nodes().size() << std::endl;
    return;
  }
  ...
  if (inode < 1) {
    if (ilink >= nodes()[inode].cc().size()) {
      if (print_level() >= mybhep::NORMAL)
        std::clog << " problem: increasing iteration of link " << ilink << " but couplet size is "
                  << nodes()[inode].cc().size() << std::endl;
      return;
    }

    for (std::vector<line>::iterator itang = nodes_[inode].cc_[ilink].tangents_.begin();
         itang != nodes_[inode].cc_[ilink].tangents_.end(); ++itang) {
      if (!itang->used()) {
        itang->set_used(true);
        return;
      }
  ...
  if (ilink >= nodes()[inode].ccc().size()) {
    if (print_level() >= mybhep::NORMAL)
      std::clog << " problem: increasing iteration of link " << ilink << " but triplet size is "
                << nodes()[inode].ccc().size() << std::endl;
    return;
  }

  for (std::vector<joint>::iterator ijoint = nodes_[inode].ccc_[ilink].joints_.begin();
       ijoint != nodes_[inode].ccc_[ilink].joints_.end(); ++ijoint) {
    if (!ijoint->used()) {
...
sequence sequence::copy_to_last_free_node(size_t *ilfn, size_t *ilink) {
  ...
  *ilink = link;
  ...
  if (lfn < 1)
    ...
    if (nodes()[lfn - 1].c().id() == linkid) linkid = nodes_[lfn].ccc()[link].cc().id();
  }

  if (print_level() >= mybhep::VVERBOSE)
    std::clog << " ilfn: " << lfn << " id: " << nodes_[lfn].c().id() << " ilink " << link
              << " id: " << linkid << std::endl;
  ...
  for (size_t i = 0; i < lfn + 1; i++) {
    ...
    while (itlink != newsequence.nodes_[i].links_.end()) {
      if (itlink - newsequence.nodes_[i].links_.begin() >=
          (int)newsequence.nodes_[i].links_.size()) {
        break;
      }
      ...
      if (i == lfn && (size_t)(itlink - newsequence.nodes_[i].links_.begin()) > link) {
        if (print_level() >= mybhep::VVERBOSE) {
          std::clog << " removing from node " << newsequence.nodes_[i].c().id() << "  link "
                    << itlink - newsequence.nodes_[i].links_.begin() << " id " << itlink->id()
                    << " larger than link " << link << " from copied sequence " << std::endl;
        }
  ...
  if ((lfn < 1 && !nodes()[lfn].cc()[link].begun()) ||
    ...
    if (lfn < 1 &&
        nodes_[lfn].cc_[link].iteration() + 1 == nodes()[lfn].cc()[link].tangents().size()) {
      nodes_[lfn].cc_[link].set_free(false);
      //      nodes()[lfn].cc_[link].iteration = 0;
    } else if (lfn >= 1 &&
               nodes_[lfn].ccc_[link].iteration() + 1 == nodes()[lfn].ccc()[link].joints().size()) {
      nodes_[lfn].ccc_[link].set_free(false);
    }
  }
...
void sequence::fill_links_of_node(size_t inode, cluster local_sequence) {
  ...
  if (inode >= nodes().size()) {
    if (print_level() >= mybhep::NORMAL)
      std::clog << " problem: asking for links for node with index " << inode
                << " but nodes size is " << nodes().size() << std::endl;
    return;
  }
  ...
  if (inode == 0) {
    ...
    for (std::vector<cell_couplet>::iterator icc = nodes_[0].cc_.begin();
         icc != nodes_[0].cc_.end(); ++icc) {
  ...
  for (std::vector<cell_triplet>::iterator iccc = nodes_[inode].ccc_.begin();
       iccc != nodes_[inode].ccc_.end(); ++iccc) {
...
bool sequence::pick_new_cell(size_t *ilink, experimental_point *newp, cluster local_cluster) {
  ...
  if (nodes_[s - 1].links().empty()) fill_links_of_node(s - 1, local_cluster);

  if (print_level() >= mybhep::VVERBOSE) {
    std::clog << " possible links: ";
    for (std::vector<cell>::iterator itlink = nodes_[s - 1].links_.begin();
         itlink != nodes_[s - 1].links_.end(); ++itlink) {
      std::clog << " " << itlink->id();
    }
    std::clog << " " << std::endl;
  }
      ...
      *ilink = (size_t)(itlink - nodes_[s - 1].links_.begin());
      ...
      if (s == 1) {
        ...
        if (iteration >= nodes_[s - 1].ccc_[iccc].joints().size()) {
          if (print_level() >= mybhep::NORMAL) {
            std::clog << "problem: iteration" << iteration << " joints size "
                      << nodes_[s - 1].ccc_[iccc].joints_.size() << "... stop evolving"
                      << std::endl;
          }
        ...
        if (iteration == 0) {
          ...
          if (print_level() >= mybhep::VVERBOSE) {
            std::clog << " initially there are " << nodes_[s - 1].ccc_[iccc].joints_.size()
                      << " possible joints to go from cell " << nodes_[s - 1].c().id() << " to "
                      << itlink->id() << std::endl;
          }
          ...
          while (ijoint != nodes_[s - 1].ccc_[iccc].joints_.end()) {
            ...
            if (!compatible(&(*ijoint), *itlink)) {
              if (print_level() >= mybhep::VVERBOSE) {
                std::clog << " erase joint " << ijoint - nodes_[s - 1].ccc_[iccc].joints_.begin()
                          << std::endl;
              }
          ...
          if (print_level() >= mybhep::VVERBOSE) {
            std::clog << " after cleaning there are " << nodes_[s - 1].ccc_[iccc].joints_.size()
                      << " possible joints to go from cell " << nodes_[s - 1].c().id() << " to "
                      << itlink->id() << std::endl;
          }
        }
        ...
        if (nodes_[s - 1].ccc_[iccc].joints_.empty()) {
          if (print_level() >= mybhep::VVERBOSE) {
            std::clog << " no joints to connect cell " << nodes_[s - 1].c().id() << " to new cell "
                      << itlink->id() << std::endl;
          }
          continue;
        }
        ...
        if (print_level() >= mybhep::VVERBOSE) {
          std::clog << " connecting cell " << nodes_[s - 1].c().id() << " to new cell "
                    << itlink->id() << " with iteration " << iteration << " of "
                    << nodes_[s - 1].ccc_[iccc].joints_.size() << std::endl;
        }
        ...
        if (s >= 3) {  // recalculate chi2 of node A
          ...
          get_chi2_change_for_changing_end_of_sequence(j.epa(), j.epb(), &chi2_change_A,
                                                       &chi2_change_alpha);
          if (print_level() >= mybhep::VVERBOSE) {
            std::clog << " will change for cell A " << nodes_[s - 2].c().id() << " chi2 from "
                      << nodes_[s - 2].chi2() << " to " << nodes_[s - 2].chi2() + chi2_change_A
                      << std::endl;
            fflush(stdout);
          }
          ...
          if (s >= 4) {  // recalculate chi2 of node alpha
            if (print_level() >= mybhep::VVERBOSE) {
              std::clog << " will change for cell alpha " << nodes_[s - 3].c().id() << " chi2 from "
                        << nodes_[s - 3].chi2() << " to "
                        << nodes_[s - 3].chi2() + chi2_change_alpha << std::endl;
              fflush(stdout);
            }
...
bool sequence::compatible(joint *j, cell cc) {
  ...
  if (!ca.small() && !ca.same_quadrant(second_last_node().ep(), j->epa())) {
    if (print_level() >= mybhep::VVERBOSE) {
      std::clog << " connecting cell " << last_node().c().id()
                << " with this joint is incompatible: points on cell " << ca.id()
                << " on different quadrants " << std::endl;
    }
    return false;
  }
  ...
  if (!cb.small() && !cb.same_quadrant(last_node().ep(), j->epb())) {
    if (print_level() >= mybhep::VVERBOSE) {
      std::clog << " connecting cell " << last_node().c().id()
                << " with this joint is incompatible: points on cell " << cb.id()
                << " on different quadrants " << std::endl;
    }
    return false;
  }
  ...
  if (s > 2) {
    if (!ca.small() && !ca.intersect(cb)) {
      ...
      if (print_level() >= mybhep::VVERBOSE) {
        std::clog << " separation chi2 for cell " << ca.id() << " is " << chi2_separation_a
                  << std::endl;
        fflush(stdout);
      }
      ndof++;
    ...
    if (!cb.small() && !cb.intersect(ca) && !cb.intersect(cc)) {
      ...
      if (print_level() >= mybhep::VVERBOSE) {
        std::clog << " separation chi2 for cell " << cb.id() << " is " << chi2_separation_b
                  << std::endl;
        fflush(stdout);
      }
      ndof++;
    ...
    if (std::abs(phi_kink.value()) * 180. / M_PI > 90.) {
      if (print_level() >= mybhep::VVERBOSE)
        std::clog << " incompatible connection: kink between alpha_A and B_C is "
                  << phi_kink.value() * 180. / M_PI << std::endl;
      return false;
    }

    if (s >= 3) {  // we are changing points A and B, affecting the chi2 of connections
                   // alpha0-alpha-A and alpha-A-B
      get_chi2_change_for_changing_end_of_sequence(pa, pb, &chi2_change_A, &chi2_change_alpha);
      if (print_level() >= mybhep::VVERBOSE) {
        std::clog << " connecting cell " << cb.id() << " to " << cc.id()
                  << " changes chi2 of cell A, i.e. : " << nodes_[s - 2].c().id() << " by "
                  << chi2_change_A << std::endl;
        fflush(stdout);
        if (s >= 4) {
          std::clog << " connecting cell " << cb.id() << " to " << cc.id()
                    << " changes chi2 of cell alpha, i.e." << nodes_[s - 3].c().id() << " by "
                    << chi2_change_alpha << std::endl;
          fflush(stdout);
        }
      }
    }
  ...
  if (net_local_prob > probmin() && prob_just_phi > probmin()) {
    if (print_level() >= mybhep::VVERBOSE) {
      std::clog << " connecting cell " << last_node().c().id() << " is compatible with chi2 "
                << chi2 << " prob " << local_prob << " net prob " << net_local_prob << std::endl;
      fflush(stdout);
    }
  ...
  if (print_level() >= mybhep::VVERBOSE) {
    std::clog << " connecting cell " << last_node().c().id()
              << " with this joint is incompatible: ";
    if (s > 2) {
      std::clog << " separation a: " << local_separation_a.value() << " +- "
                << local_separation_a.error();
      std::clog << " separation b: " << local_separation_b.value() << " +- "
                << local_separation_b.error() << " total connection chi2: " << chi2 << " prob "
                << local_prob << std::endl;
      fflush(stdout);
    }
  }
...
void sequence::get_chi2_change_for_changing_end_of_sequence(
    const topology::experimental_point &new_pa, const topology::experimental_point &new_pb,
    double *delta_chi_A, double *delta_chi_alpha) {
  // sequence: [ ... alpha0 alpha A B ]
  size_t s = nodes_.size();
  if (s < 3) {
    if (print_level() >= mybhep::NORMAL)
      std::clog << " problem: ask prob change for second last node but nnodes is " << s
                << std::endl;
    return;
  }
  ...
  bool use_theta_kink_alpha_A_B =
      !(nodes_[s - 3].c().unknown_vertical() || nodes_[s - 2].c().unknown_vertical() ||
        nodes_[s - 1].c().unknown_vertical());
  ...
  if (old_chi2 > old_chi2_check) {
    if (std::abs(old_chi2 - old_chi2_check) > tolerance) {
      if (print_level() >= mybhep::NORMAL) {
        std::clog << " problem: cell A " << second_last_node().c().id() << " has old chi2 "
                  << old_chi2_check << " but just kink component is " << old_chi2 << " delta "
                  << old_chi2 - old_chi2_check << std::endl;
      }
    }
  ...
  *delta_chi_A = new_chi2 - old_chi2;
  if (*delta_chi_A + old_chi2 <= 0) {
    if (print_level() >= mybhep::NORMAL) {
      std::clog << " problem: node A has old chi2 " << old_chi2 << " new " << new_chi2 << " delta "
                << *delta_chi_A << std::endl;
    }
    *delta_chi_A = 0.;
  }

  if (s >= 4) {
    bool use_theta_kink_alpha0_alpha_A =
        !(nodes_[s - 4].c().unknown_vertical() || nodes_[s - 3].c().unknown_vertical() ||
          nodes_[s - 2].c().unknown_vertical());
    ...
    if (old_chi2 > old_chi2_check) {
      if (std::abs(old_chi2 - old_chi2_check) > tolerance) {
        if (print_level() >= mybhep::NORMAL) {
          std::clog << " problem: cell alpha " << nodes_[s - 3].c().id() << " has old chi2 "
                    << old_chi2_check << " but just kink component is " << old_chi2 << " delta "
                    << old_chi2 - old_chi2_check << std::endl;
        }
      }
    ...
    *delta_chi_alpha = new_chi2 - old_chi2;
    if (old_chi2 + *delta_chi_alpha <= 0.) {
      if (print_level() >= mybhep::NORMAL)
        std::clog << " problem: node alpha has old chi2 " << old_chi2 << " new " << new_chi2
                  << " delta " << *delta_chi_alpha << std::endl;
      *delta_chi_alpha = 0.;
    }
  }
...
int sequence::get_link_index_of_cell(size_t inode, const cell &link) const {
  ...
  if (inode >= nodes().size()) {
    if (print_level() >= mybhep::NORMAL)
      std::clog << " problem: getting link index for node index " << inode << " but size is "
                << nodes().size() << std::endl;
    return 0;
  }
  ...
  */
  ...
  if (inode < 1) {
    ...
    if (!nodes()[inode].has_couplet(link, &index)) {
      if (print_level() >= mybhep::NORMAL)
        std::clog << " problem: looking for couplet from cell index " << inode << " to "
                  << link.id() << " but it's not there " << std::endl;
      return 0;
    }
  ...
  if (!nodes()[inode].has_triplet(link, nodes_[inode - 1].c(), &index)) {
    if (print_level() >= mybhep::NORMAL) {
      std::clog << " problem: looking for triplet from cell index " << inode << " id "
                << nodes()[inode].c().id() << " to cell id " << link.id() << " but it's not there "
                << std::endl;
      fflush(stdout);
    }
    return 0;
...
void sequence::circle_order(double /*Ratio*/) {
  ...
  for (std::vector<node>::iterator inode = nodes_.begin(); inode != nodes_.end(); ++inode) {
    ...
    if (inode->c().id() != original_ids[index]) {
      ...
      for(std::vector<topology::cell_couplet>::const_iterator icc = cc.begin(); icc!=cc.end();
      ++icc){ std::vector<line> tangents = icc->tangents(); for( std::vector<line>::const_iterator
      il=tangents.begin(); il!=tangents.end(); ++il){ local_diff =
      il->epa().distance(helix_point).value(); if( local_diff < diff ){ diff = local_diff;
            inode->set_ep(il->epa());
          }
        }
      }
...
bool sequence::calculate_helix(double Ratio, bool conserve_clustering_from_removal,
                               bool conserve_clustering_from_reordering) {
  // after_sultan, conserve_clustering_from_removal  = true, conserve_clustering_from_reordering =
  // false after_nemor, conserve_clustering_from_removal  = true,
  // conserve_clustering_from_reordering = false by default, cat was used for the clustering, so
  // conserve_clustering_from_removal = false and conserve_clustering_from_reordering = true
  ...
  switch (method) {
    case 1: {
      ...
      for (std::vector<node>::iterator inode = nodes_.begin(); inode != nodes_.end(); ++inode) {
        ...
        ++cnode;
        if (print_level() >= mybhep::VVERBOSE) {
          std::clog << " calculate helix for three points " << anode->c().id() << " , "
                    << inode->c().id() << " , " << cnode->c().id() << std::endl;
        }
      ...
      experimental_double Yc =
          middle_node().ep().y() -
          helix_.pitch() * experimental_vector(_center, middle_node().ep()).phi();
      _center.set_y(Yc);
    ...
    case 2: {
      circle ci =
          three_points_circle(nodes().front().ep(), middle_node().ep(), nodes().back().ep());
    ...
    case 3: {
      ...
      // if a clustering is given from nemor or sultan, accept it as good although it does not look
      // like a circle otherwise the cluster will not be made into a track
...
const experimental_double &sequence::radius() const { return helix_.radius(); }

const experimental_double &sequence::pitch() const { return helix_.pitch(); }

const experimental_point &sequence::center() const { return helix_.center(); }

void sequence::calculate_momentum(double bfield, bool SuperNEMO, double ref_value) {
  experimental_double mom =
      experimental_sqrt(experimental_square(radius()) + experimental_square(pitch())) * 0.3 *
      bfield;
  ...
  if (!std::isnan(momentum_.x().value()) && !std::isnan(momentum_.y().value()) &&
      !std::isnan(momentum_.z().value())) {
    has_momentum_ = true;
  }

  if (print_level() >= mybhep::VVERBOSE) {
    std::clog << " sequence radius " << radius().value() << " pitch " << pitch().value()
              << " bfield " << bfield << " mom " << mom.value() << " dir ("
              << initial_dir(SuperNEMO, ref_value).x().value() << ", "
              << initial_dir(SuperNEMO, ref_value).y().value() << ", "
              << initial_dir(SuperNEMO, ref_value).z().value() << ")" << std::endl;
  }
}
...
void sequence::calculate_charge(void) {
  ...
  if (print_level() >= mybhep::VVERBOSE) {
    std::clog << " calculate tangent charge: initial phi " << phi1 * 180. / acos(-1.)
              << " final phi " << phi2 * 180. / acos(-1.) << " deltaphi " << deltaphi.value()
              << " charge " << charge_.value() << " +- " << charge_.error() << std::endl;
  }
  ...
  if (print_level() >= mybhep::VVERBOSE) {
    std::clog << " calculate helix charge: initial helix phi " << helix_phi1 * 180. / acos(-1.)
              << " final phi " << helix_phi2 * 180. / acos(-1.) << " deltaphi "
              << deltahelix_phi.value() << " charge " << helix_charge_.value() << " +- "
              << helix_charge_.error() << std::endl;
  ...
  for (size_t i = 0; i < nodes().size() - 2; i++) {
    ...
    if (deltaphi.value()) {
      ...
      if (print_level() >= mybhep::VVERBOSE) {
        std::clog << " triplet: " << i << " q " << angles.back().value() << " +- "
                  << angles.back().error() << std::endl;
      }
    }
  }
  ...
  if (print_level() >= mybhep::VVERBOSE) {
    std::clog << " detailed charge " << detailed_charge_.value() << " +- "
              << detailed_charge_.error() << std::endl;
  }
...
bool sequence::intersect_plane_with_tangent_from_begin(const plane &pl,
                                                       experimental_point *ep) const {
  // need 2 nodes to build the tangent line
  if (nodes().size() < 2) return false;
...
bool sequence::intersect_circle_with_tangent_from_end(const circle &c,
                                                      experimental_point *ep) const {
  // need 2 nodes to build the tangent line
  if (nodes().size() < 2) return false;
...
bool sequence::intersect_circle_with_tangent_from_begin(const circle &c,
                                                        experimental_point *ep) const {
  // need 2 nodes to build the tangent line
  if (nodes().size() < 2) return false;
...
bool sequence::intersect_circle_from_begin_minus_one(const circle &c,
                                                     experimental_point *ep) const {
  if (nodes().size() < 2) return false;
...
bool sequence::intersect_sequence(sequence &seq, bool invertA, bool invertB, bool acrossGAP,
  ...
  *with_kink = 0;
  ...
  if (!acrossGAP) {
    ...
    if (print_level() >= mybhep::VVERBOSE)
      std::clog << "ep (" << ep->x().value() << ", " << ep->y().value() << ", " << ep->z().value()
                << ") ... distanceA " << distanceA << " distanceB " << distanceB << " limit "
                << limit_distance << std::endl;
  ...
  if (result_tangent_B) {
    ...
    if (print_level() >= mybhep::VVERBOSE)
      std::clog << " ... extrapolation through tangent B, pB (" << pB.x().value() << ", "
                << pB.y().value() << ", " << pB.z().value() << ") distance "
                << local_distance.value() << " +- " << local_distance.error() << std::endl;
  ...
  if (result_tangent_A) {
    ...
    if (print_level() >= mybhep::VVERBOSE)
      std::clog << " ... extrapolation through tangent A, pA (" << pA.x().value() << ", "
                << pA.y().value() << ", " << pA.z().value() << ") distance "
                << local_distance.value() << " +- " << local_distance.error() << std::endl;
  ...
  if (print_level() >= mybhep::VVERBOSE)
    std::clog << " ... resultA " << resultA << " ... resultB " << resultB << " distanceA "
              << distanceA << " distanceB " << distanceB << " distance " << distance << " +- "
              << distance_error << " limit " << 2. * limit_distance << std::endl;

  if ((distance != distance) || (distance_error != distance_error)) {
    throw std::logic_error(
        "CAT::sequence::intersect_sequence: Could not compute 'distance' or 'distance_error' !");
  }
...
std::vector<size_t> sequence::families() const {
  ...
  size_t i1, i2;
  for (std::vector<std::string>::const_iterator iname = names_.begin(); iname != names_.end();
       ++iname) {
    i1 = iname->find("_");
...
bool sequence::good_match(const sequence &seq, bool &invertA, bool &invertB,
  ...
  if (distLF <= distFF && distLF <= distFL && distLF <= distLL) {  // last to first  FL --> FL
    ...
    if (has_decay_helix_vertex() && decay_helix_vertex_type() == "calo") {
      if (print_level() >= mybhep::VVERBOSE)
        std::clog << " ... forbidden, because 1st track already has decay helix_vertex on calo "
                  << std::endl;
      return false;
    }
    if (seq.has_helix_vertex() && seq.helix_vertex_type() == "calo") {
      if (print_level() >= mybhep::VVERBOSE)
        std::clog << " ... forbidden, because 2nd track already has helix_vertex on calo "
                  << std::endl;
      return false;
    }
    ...
    if (seq.has_decay_helix_vertex() && seq.decay_helix_vertex_type() == "calo") {
      if (print_level() >= mybhep::VVERBOSE)
        std::clog << " ... forbidden, because 2nd track already has decay helix_vertex on calo "
                  << std::endl;
      return false;
    }
    ...
    if (has_helix_vertex() && helix_vertex_type() == "calo") {
      if (print_level() >= mybhep::VVERBOSE)
        std::clog << " ... forbidden, because 1st track already has helix_vertex on calo "
                  << std::endl;
      return false;
    }
    if (seq.has_decay_helix_vertex() && seq.decay_helix_vertex_type() == "calo") {
  ...
  if (static_cast<size_t>(std::abs(layer_distance)) > 1 + NOffLayers) {
    if (print_level() >= mybhep::VVERBOSE)
      std::clog << " ... forbidden, because layers are far away by " << layer_distance << " planes "
                << std::endl;
    return false;
  }
  ...
  if (block_distance == 0 && static_cast<size_t>(std::abs(cell_number_distance)) > 1 + NOffLayers) {
    if (print_level() >= mybhep::VVERBOSE)
      std::clog << " ... forbidden, because block is the same and cell numbers are far away by "
                << cell_number_distance << " cells " << std::endl;
    return false;
  }

  if (print_level() >= mybhep::VVERBOSE) {
    std::clog << " ... good match, distances: FF " << distFF << " FL " << distFL << " LF " << distLF
              << " LL " << distLL << " so invertA " << invertA << " invertB " << invertB
              << " layerdistance " << layer_distance << std::endl;
  }
...
sequence sequence::match(sequence &seq, bool invertA, bool invertB, bool *ok, int with_kink,
                         int cells_to_delete, double Ratio) {
  ...
  if (cells_to_delete == 1 || cells_to_delete == 2) new_second_sequence.remove_first_node();
  if (cells_to_delete == 2) new_first_sequence.remove_last_node();
  ...
  for (size_t i = 0; i < new_second_sequence.nodes_.size(); i++) {
    ...
    if (i == new_second_sequence.nodes_.size() - 1) last = true;
    ...
    if (i == 0) {  // 1st added cell must get a new triplet
      ...
      cell_triplet ctA(new_first_sequence.nodes_[s - 2].c(), new_first_sequence.nodes_[s - 1].c(),
                       in.c());
      //          new_first_sequence.nodes_[s-1].ccc_.push_back(ctA);
      new_first_sequence.nodes_[s - 1].add_triplet(ctA);

      if (with_kink == 2) in.set_is_kink(true);
    }

    if (!last) {
      cell_triplet ct(new_first_sequence.last_node().c(), in.c(),
                      new_second_sequence.nodes_[next_index].c());
      std::vector<cell_triplet> ccc;
      ccc.push_back(ct);
      in.set_ccc(ccc);
      std::vector<cell> ll;
  ...
  if (new_second_sequence.has_decay_helix_vertex()) {
    if (new_second_sequence.decay_helix_vertex_type() == "foil" ||
        new_second_sequence.decay_helix_vertex_type() == "kink")
      new_first_sequence.set_decay_helix_vertex(new_second_sequence.decay_helix_vertex(),
                                                new_second_sequence.decay_helix_vertex_type());
    else if (new_second_sequence.decay_helix_vertex_type() == "calo")
      new_first_sequence.set_decay_helix_vertex(new_second_sequence.decay_helix_vertex(),
                                                new_second_sequence.decay_helix_vertex_type(),
                                                new_second_sequence.calo_helix_id());
  }

  if (new_second_sequence.has_decay_tangent_vertex()) {
    if (new_second_sequence.decay_tangent_vertex_type() == "foil" ||
        new_second_sequence.decay_tangent_vertex_type() == "kink")
      new_first_sequence.set_decay_tangent_vertex(new_second_sequence.decay_tangent_vertex(),
                                                  new_second_sequence.decay_tangent_vertex_type());
    else if (new_second_sequence.decay_tangent_vertex_type() == "calo")
      new_first_sequence.set_decay_tangent_vertex(new_second_sequence.decay_tangent_vertex(),
                                                  new_second_sequence.decay_tangent_vertex_type(),
                                                  new_second_sequence.calo_tangent_id());
  }
...
void sequence::calculate_length(void) {
  if (nodes_.size() >= 3) {
    ...
    if (has_helix_vertex()) fp = helix_vertex();
    if (has_decay_helix_vertex()) lp = decay_helix_vertex();

    experimental_double deltaphi = helix_.delta_phi(fp, lp);
    helix_length_ = experimental_sqrt(experimental_square(helix_.radius()) +
                                      experimental_square(helix_.pitch())) *
                    experimental_fabs(deltaphi);
    has_helix_length_ = true;
  }
...
bool sequence::good_match_with_kink(const sequence &seq, bool &invertA, bool &invertB,
                                    bool &acrossGAP, double limit_distance, size_t NOffLayers,
  ...
  if (nodes_.size() < 2 || seq.nodes().size() < 2) {
    if (print_level() >= mybhep::VVERBOSE)
      std::clog << " ... forbidden, hits sizes are " << nodes_.size() << " and "
                << seq.nodes().size() << std::endl;
    return false;
  }
  ...
  if (distLF <= distFF && distLF <= distFL && distLF <= distLL) {  // last to first  FL --> FL
    ...
    if (has_decay_helix_vertex() &&
        (decay_helix_vertex_type() == "calo" || decay_helix_vertex_type() == "foil")) {
      if (print_level() >= mybhep::VVERBOSE)
        std::clog
            << " ... forbidden, because 1st track already has decay helix_vertex on calo or foil "
            << std::endl;
      return false;
    }
    if (seq.has_helix_vertex() &&
        (seq.helix_vertex_type() == "calo" || seq.helix_vertex_type() == "foil")) {
      if (print_level() >= mybhep::VVERBOSE)
        std::clog << " ... forbidden, because 2nd track already has helix_vertex on calo or foil "
                  << std::endl;
      return false;
    }
    ...
    if (std::abs(block_distance) == 1)
      ...
      if (print_level() >= mybhep::VVERBOSE)
        std::clog << " ... forbidden, because distance " << distLF << " is larger than limit "
                  << limit_distance * (cells_to_delete + 1) << std::endl;
      return false;
    }
    ...
    if (seq.has_decay_helix_vertex() &&
        (seq.decay_helix_vertex_type() == "calo" || seq.decay_helix_vertex_type() == "foil")) {
      if (print_level() >= mybhep::VVERBOSE)
        std::clog
            << " ... forbidden, because 2nd track already has decay helix_vertex on calo or foil "
            << std::endl;
      return false;
    }
    ...
    if (last_node().c().id() == seq.last_node().c().id()) {
      ...
      cell_number_distance =
          last_node().c().cell_number() - seq.second_last_node().c().cell_number();
      ...
      cell_number_distance =
          second_last_node().c().cell_number() - seq.second_last_node().c().cell_number();
    } else {
      layer_distance = last_node().c().layer() - seq.last_node().c().layer();
      cell_number_distance = last_node().c().cell_number() - seq.last_node().c().cell_number();
    }
    ...
    if (std::abs(block_distance) == 1)
      ...
      if (print_level() >= mybhep::VVERBOSE)
        std::clog << " ... forbidden, because distance " << distLL << " is larger than limit "
                  << limit_distance * (cells_to_delete + 1) << std::endl;
      return false;
    }
    ...
    if (has_helix_vertex() && (helix_vertex_type() == "calo" || helix_vertex_type() == "foil")) {
      if (print_level() >= mybhep::VVERBOSE)
        std::clog << " ... forbidden, because 1st track already has helix_vertex on calo or foil "
                  << std::endl;
      return false;
    }
    if (seq.has_decay_helix_vertex() &&
    ...
    if (std::abs(block_distance) == 1)
      ...
      if (print_level() >= mybhep::VVERBOSE)
        std::clog << " ... forbidden, because distance " << distFL << " is larger than limit "
                  << limit_distance * (cells_to_delete + 1) << std::endl;
      return false;
    }
    ...
    if (std::abs(block_distance) == 1)
      ...
      if (print_level() >= mybhep::VVERBOSE)
        std::clog << " ... forbidden, because distance " << distFF << " is larger than limit "
                  << limit_distance * (cells_to_delete + 1) << std::endl;
      return false;
    }
  }
  ...
  if (print_level() >= mybhep::VVERBOSE) {
    std::clog << " ... good kink match, distances: FF " << distFF << " FL " << distFL << " LF "
              << distLF << " LL " << distLL << " so invertA " << invertA << " invertB " << invertB
              << " across gap " << acrossGAP << " cells_to_delete " << cells_to_delete << std::endl;
  }
...
bool sequence::one_side() const {
  ...
  for (std::vector<node>::const_iterator inode = nodes_.begin(); inode != nodes_.end(); ++inode) {
    ...
    if (inode->c().block() * old_block < 0) {
      if (print_level() >= mybhep::VVERBOSE)
        std::clog << " old block: " << old_block << " new block: " << inode->c().block()
                  << " the track is not all on one side " << std::endl;
      return false;
    }
    old_block = inode->c().block();
  }
...
experimental_vector sequence::initial_dir(bool SuperNEMO, double ref_value) const {
  ...
  if (s < 1) {
    if (print_level() >= mybhep::NORMAL) {
      std::clog << " problem: asking for initial dir of sequence with " << s << " nodes "
                << std::endl;
  ...
  if (has_helix_vertex()) {
    ...
    if (dist_first < dist_last) return experimental_vector(helix_vertex_, p_first).unit();
  ...
  if (SuperNEMO) {
    ...
    if (dist_first < dist_last) return experimental_vector(p_first, nodes_[1].ep()).unit();
...
experimental_vector sequence::initial_helix_dir(bool SuperNEMO, double ref_value) const {
  ...
  if (s < 1) {
    if (print_level() >= mybhep::NORMAL) {
      std::clog << " problem: asking for initial helix dir of sequence with " << s << " nodes "
                << std::endl;
      return experimental_vector();
    }
  }
  ...
  if (SuperNEMO) {
    dist_first = std::abs(p_first.z().value() - ref_value);
    dist_last = std::abs(p_last.z().value() - ref_value);
    if (dist_first < dist_last) return helix_.direction_at(p_first);
...
experimental_vector sequence::final_dir() const {
  if (has_decay_helix_vertex()) {
    if (nodes_.size() < 1) {
      if (print_level() >= mybhep::NORMAL) {
        std::clog << " problem: asking for final dir of sequence with " << nodes_.size()
                  << " nodes " << std::endl;
        return experimental_vector();
      }
    }
  ...
  if (nodes_.size() < 2) {
    if (print_level() >= mybhep::NORMAL) {
      std::clog << " problem: asking for final dir of sequence with " << nodes_.size() << " nodes "
                << std::endl;
      return experimental_vector();
    }
  }
...
double sequence::phi_kink(size_t inode) const {
  ...
  double phi = std::abs((joint(nodes_[inode - 1].ep(), nodes_[inode].ep(), nodes_[inode + 1].ep()))
                            .kink_phi()
                            .value());
...
bool sequence::helix_out_of_range(double lim) {
  ...
  for (std::vector<node>::const_iterator in = nodes_.begin(); in != nodes_.end(); ++in) {
    ...
    if (p.y().value() < -lim || p.y().value() > lim) {
...
void sequence::point_of_max_min_radius(experimental_point epa, experimental_point epb,
...
bool sequence::common_vertex_on_foil(const sequence *seqB, double *the_distance) const {
  ...
  for (std::vector<experimental_point>::const_iterator vA = foil_vertex_A.begin();
       vA != foil_vertex_A.end(); vA++)
    for (std::vector<experimental_point>::const_iterator vB = foil_vertex_B.begin();
         vB != foil_vertex_B.end(); vB++) {
  ...
  if (found) {
    *the_distance = min_distance;
    if (print_level() >= mybhep::VVERBOSE) {
      std::clog << " sequences " << this->name() << " and " << seqB->name()
                << " have vertex on foil with distance " << min_distance << std::endl;
    }
  }
