...
class cell : public tracking_object {
 ...
 public:
  ...
  cell(mybhep::hit hit, size_t id, bool SuperNemo, mybhep::prlevel level = mybhep::NORMAL,
  ...
  virtual void dump(std::ostream& a_out = std::clog, const std::string& a_title = "",
                    const std::string& a_indent = "", bool /*a_inherit */ = false) const {
    {
      ...
      a_out << indent << "id : " << this->id() << " layer " << this->layer() << " block "
            ...
            << this->fast() << " small " << this->small() << " unknown vertical "
            << this->unknown_vertical() << std::endl;
  ...
  //! set small radius
  void set_small_radius(double small_radius) { small_radius_ = small_radius; }
  ...
  //! set begun level
  void set_begun(bool begun) { begun_ = begun; }
  ...
  bool unknown_vertical() const {
    ...
    if (ep().y().value() == 0. && ep().y().error() > 1000.) uv = true;
  ...
  const experimental_double& r0() const { return r0_; }
 ...
 public:
  ...
  experimental_point angular_average(experimental_point epa, experimental_point epb,
                                     experimental_double* angle);
  experimental_point build_from_cell(experimental_vector forward, experimental_vector transverse,
                                     experimental_double cos, int sign, bool replace_r,
                                     double maxr) const;
