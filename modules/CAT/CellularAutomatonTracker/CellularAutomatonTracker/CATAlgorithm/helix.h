...
class helix : public tracking_object {
 ...
 public:
  ...
  //! constructor
  helix(const experimental_point &center, const experimental_double &radius,
        const experimental_double &pitch, mybhep::prlevel level = mybhep::NORMAL,
  ...
  helix(const circle &c, const experimental_double &pitch, mybhep::prlevel level = mybhep::NORMAL,
        double probmin = 1.e-200) {
    set_print_level(level);
    set_probmin(probmin);
    appname_ = "helix: ";
  ...
  void set(const experimental_point &center, const experimental_double &radius,
           const experimental_double &pitch) {
    center_ = center;
    radius_ = radius;
    pitch_ = pitch;
  }
  ...
  //! set center
  void set_center(const experimental_point &center) { center_ = center; }

  //! set radius
  void set_radius(const experimental_double &radius) { radius_ = radius; }

  //! set pitch
  void set_pitch(const experimental_double &pitch) { pitch_ = pitch; }
  ...
  //! get pitch
  const experimental_double &pitch() const { return pitch_; }

  //! get curvature
  experimental_double curvature() const { return 1. / radius_; }

  //! get circle
  circle get_circle() const { return circle(center(), radius(), print_level(), probmin()); }
  ...
  experimental_point position(const experimental_point &ep) const { return position(ep, 0.); }
  ...
  experimental_point position(const experimental_point &ep, double phi_ref, double y_ref) const {
    ...
    if (distup < dist && distup < distdown) newy = newyup;
    if (distdown < dist && distdown < distup) newy = newydown;
  ...
  double chi2(const experimental_point &ep) const { return chi2(ep, 0.); }
  ...
  double chi2(const experimental_point &ep, double phi_ref) const {
    ...
    */
    ...
    double res2 = mybhep::square(residual.x().value()) + mybhep::square(residual.y().value()) +
                  mybhep::square(residual.z().value());

    if (print_level() >= mybhep::VVERBOSE) {
      std::clog << " input point: ( ";
      ep.x().dump();
      std::clog << " , ";
      ep.y().dump();
      std::clog << " , ";
      ...
      std::clog << " , ";
      predicted.z().dump();
      std::clog << " ) local chi2: " << res2 << " " << std::endl;
    }
  ...
  bool intersect_plane(const plane &pl, experimental_point *ep, const experimental_double &_phi,
                       const double y_ref) const {
    if (pl.view() == "x" || pl.view() == "z" || pl.view() == "inner" || pl.view() == "outer") {
      bool result = get_circle().intersect_plane(pl, ep, _phi);
      if (print_level() >= mybhep::VVERBOSE) {
        std::clog << " helix intersected point y " << ep->y().value() << " _phi " << _phi.value()
                  << " y_ref " << y_ref << " -> y "
                  << position(*ep, _phi.value(), y_ref).y().value() << std::endl;
      }
    ...
    if (pl.view() == "y" || pl.view() == "top" || pl.view() == "bottom") {
      ...
      *ep = position((pl.face().y() - center().y()) / pitch());
      ...
      if (print_level() >= mybhep::VVERBOSE) {
        std::clog << " helix distance from extrapolation to plane face: " << dist.x().value()
  ...
  bool intersect_circle(const circle &c, experimental_point *ep,
                        const experimental_double &_phi) const {
  ...
  bool intersect_circle_with_tangent(const circle &c, const experimental_point &start,
    ...
    *ep = c.position(phi0 + sign * alpha);
    ...
    if (print_level() >= mybhep::VVERBOSE) {
      std::clog << " center (" << c.center().x().value() << ", " << c.center().y().value() << ", "
                << c.center().z().value() << ") r " << c.radius().value() << " +- "
                << c.radius().error() << " start (" << start.x().value() << ", "
                << start.y().value() << ", " << start.z().value() << ") cts ("
                << center_to_start.x().value() << ", " << center_to_start.y().value() << ", "
                << center_to_start.z().value() << ") length " << center_to_start.length().value()
                << " +- " << center_to_start.length().error() << " beta "
                << beta.value() * 180. / acos(-1.) << " +- " << beta.error() * 180. / acos(-1.)
                << " phi0 " << phi0.value() * 180. / acos(-1.) << " angle " << angle.value()
                << " +- " << angle.error() << " alpha " << alpha.value() * 180. / acos(-1.)
                << " sign " << sign.value() << " newangle "
                << (phi0 + sign * alpha).value() * 180. / acos(-1.) << " pos: (" << ep->x().value()
                << ", " << ep->y().value() << ", " << ep->z().value() << ") " << std::endl;
    }
  ...
  experimental_vector direction_at(const experimental_point &ep) const {
    ...
    experimental_vector dir(-radius() * experimental_sin(phi), pitch(),
...
inline helix three_points_helix(const experimental_point &epa, const experimental_point &epb,
