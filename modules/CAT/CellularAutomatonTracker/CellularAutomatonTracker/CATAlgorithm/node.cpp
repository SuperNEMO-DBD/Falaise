...
//! constructor
node::node(const cell &c, const std::vector<cell_couplet> &cc,
...
node::node(const mybhep::hit &truehit, size_t id, bool SuperNemo, prlevel level, double probmin) {
  ...
  experimental_point center(cellpos[0], cellpos[1], cellpos[2], 0., 0., 0.);
  ...
  is_kink_ = false;
  {
    experimental_point tmp_ep(truehit.x().x(), truehit.x().y(), truehit.x().z(), 0., 0., 0.);
    ep_ = tmp_ep;
  }
  circle_phi_ = mybhep::small_neg;
  return;
...
void node::dump(ostream &a_out, const std::string &a_title, const std::string &a_indent,
  ...
  a_out << indent << " main cell "
        << " free " << free() << " chi2 " << chi2() << " ndof " << ndof() << std::endl;
...
//! set cells
void node::set(const cell &c, const std::vector<cell_couplet> &cc,
               const std::vector<cell_triplet> &ccc) {
  c_ = c;
  cc_ = cc;
  ccc_ = ccc;
...
//! set cell triplets
void node::set_ccc(const std::vector<cell_triplet> &ccc) { ccc_ = ccc; }

//! set links
void node::set_links(const std::vector<cell> &links) { links_ = links; }

//! set free level
void node::set_free(bool free) { free_ = free; }

//! set is_kink
void node::set_is_kink(bool is_kink) { is_kink_ = is_kink; }

//! set chi2
void node::set_chi2(double chi2) { chi2_ = chi2; }

//! set ndof
void node::set_ndof(int32_t ndof) { ndof_ = ndof; }
...
//! get cell couplets
const std::vector<cell_couplet> &node::cc() const { return cc_; }

//! get cell triplets
const std::vector<cell_triplet> &node::ccc() const { return ccc_; }

//! get links
const std::vector<cell> &node::links() const { return links_; }

//! get free level
bool node::free() const { return free_; }

//! get is_kink
bool node::is_kink() const { return is_kink_; }

//! get chi2
double node::chi2() const { return chi2_; }

//! get ndof
int32_t node::ndof() const { return ndof_; }

//! get prob
double node::Prob() const { return probof(chi2(), ndof()); }

//! get fitted experimental_point
const experimental_point &node::ep() const { return ep_; }

void node::calculate_triplets(double Ratio, double separation_limit, double phi_limit,
                              double theta_limit) {
  if (cc_.size() < 2) return;
  for (std::vector<cell_couplet>::const_iterator icc = cc_.begin(); icc != cc_.end(); ++icc) {
    cell c1 = icc->cb();
    for (std::vector<cell_couplet>::const_iterator jcc = cc_.begin() + (size_t)(icc - cc_.begin());
         jcc != cc_.end(); ++jcc) {
      cell c2 = jcc->cb();
      if (c1.id() == c2.id()) continue;
      cell_triplet ccc(c1, c_, c2, print_level(), probmin());
      if (print_level() >= mybhep::VVERBOSE) {
        std::clog << appname_ << " calculate triplets for three cells: " << ccc.ca().id() << "  "
                  << ccc.cb().id() << "  " << ccc.cc().id() << std::endl;
      }
      ...
      if (ccc.joints().size() > 0) {
        if (print_level() >= mybhep::VVERBOSE) {
          ...
          for (std::vector<joint>::iterator ijoint = ccc.joints_.begin();
               ijoint != ccc.joints_.end(); ++ijoint)
            std::clog
                << " joint " << ijoint - ccc.joints_.begin() << " phia: "
                << experimental_vector(ccc.ca().ep(), ijoint->epa()).phi().value() * 180. / M_PI
                << " phib: "
                << experimental_vector(ccc.cb().ep(), ijoint->epb()).phi().value() * 180. / M_PI
                << " phic: "
                << experimental_vector(ccc.cc().ep(), ijoint->epc()).phi().value() * 180. / M_PI
                << " chi2 " << ijoint->chi2() << std::endl;
        }
        add_triplet(ccc);
      }
    }
...
void node::calculate_triplets_after_sultan(double Ratio) {
  ...
  if (c1.id() == c2.id()) {
    if (print_level() >= mybhep::NORMAL) {
      std::clog << appname_ << " problem: calculate triplets after sultan: cc id1 " << c1.id()
                << " id2 " << c2.id() << std::endl;
    }
    return;
  }
  ...
  cell_triplet ccc(c1, c_, c2, print_level(), probmin());
  if (print_level() >= mybhep::VVERBOSE) {
    std::clog << appname_ << " calculate joints for three cells: " << ccc.ca().id() << "  "
              << ccc.cb().id() << "  " << ccc.cc().id() << std::endl;
  }
  ...
  if (ccc.joints().size() > 0) {
    if (print_level() >= mybhep::VVERBOSE) {
      std::clog << appname_ << " adding joints " << std::endl;
      for (std::vector<joint>::iterator ijoint = ccc.joints_.begin(); ijoint != ccc.joints_.end();
           ++ijoint)
        std::clog << " joint " << ijoint - ccc.joints_.begin() << " phia: "
                  << experimental_vector(ccc.ca().ep(), ijoint->epa()).phi().value() * 180. / M_PI
                  << " phib: "
                  << experimental_vector(ccc.cb().ep(), ijoint->epb()).phi().value() * 180. / M_PI
                  << " phic: "
...
//! grab cc index map
std::map<size_t, size_t> node::cc_index() const { return cc_index_; }

//! grab ccc cb index map
std::map<size_t, size_t> node::ccc_ca_index() const { return ccc_ca_index_; }

//! grab ccc cc index map
std::map<size_t, size_t> node::ccc_cc_index() const { return ccc_cc_index_; }
...
bool node::has_couplet(const cell &a, cell_couplet *ct) const {
  ...
  if (index >= cc_.size()) {
    std::clog << " problem: cc index " << index << " for cell a of id " << a.id()
              << " is larger than cc size " << cc_.size() << endl;
    dump();
    return false;
  }
  *ct = cc_.at(index);
...
bool node::has_couplet(const cell &a, size_t *index) const {
  ...
  *index = cc_index()[a.id()];
  if (*index >= cc_.size()) {
    std::clog << " problem: cc index " << *index << " for cell of id " << a.id()
              << " is larger than cc size " << cc_.size() << endl;
    dump();
    return false;
  }
...
bool node::has_couplet(size_t idd, size_t *index) const {
  ...
  *index = cc_index()[idd];
  if (*index >= cc_.size()) {
    std::clog << " problem: cc index " << *index << " for id " << idd << " is larger than cc size "
              << cc_.size() << endl;
    dump();
    return false;
  }
...
bool node::has_triplet(const cell &a, const cell &c, size_t *index) const {
#if 1
  cell null;
  std::vector<cell_triplet>::const_iterator ftriplet =
  ...
  if (indexa >= ccc_ca_index_.size()) {
    std::clog << " problem: ccc ca index " << indexa << " is larger than ccc size " << ccc_.size()
              << endl;
    return false;
  }
  size_t indexc = ccc_cc_index()[c.id()];
  if (indexc >= ccc_cc_index_.size()) {
    std::clog << " problem: ccc cc index " << indexc << " is larger than ccc size " << ccc_.size()
              << endl;
    return false;
  }
...
#if 1
  cell null;
  if (std::find(ccc().begin(), ccc().end(), cell_triplet(a, null, c)) != ccc().end()) return true;
  return false;
#else
