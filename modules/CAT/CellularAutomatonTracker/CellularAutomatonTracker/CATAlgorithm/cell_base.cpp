...
experimental_point cell::build_from_cell(experimental_vector forward,
                                         experimental_vector transverse, experimental_double cos,
                                         int sign, bool replace_r, double maxr) const {
  ...
  experimental_point p =
      (experimental_vector(ep()) + radius * (forward * cos + transverse * sin)).point_from_vector();

  return p;
}

experimental_point cell::angular_average(experimental_point epa, experimental_point epb,
                                         experimental_double* angle) {
  if (print_level() >= mybhep::VVERBOSE)
    std::clog << "CAT::cell::angular_average: calculating angular average for cell " << id()
              << std::endl;
  ...
  *angle = phi1 - phi2;
  ...
  if (print_level() >= mybhep::VVERBOSE) {
    std::clog << "CAT::cell::angular_average: averaging phi1: ";
    (phi1 * 180. / M_PI).dump();
    std::clog << " and phi2: ";
    (phi2 * 180. / M_PI).dump();
    std::clog << " to phi_ave: ";
    (ave_phi * 180. / M_PI).dump();
    std::clog << " " << std::endl;
  }
...
bool cell::intersect(topology::cell c) const {
  ...
  if (rsum.value() > dist * fraction_limit) {
    if (print_level() >= mybhep::VVERBOSE) {
      std::clog << "CAT::cell::intersect: cells " << id() << " and " << c.id()
                << " intersect: dist " << dist << " radii " << r().value() << " and "
                << c.r().value() << " rsum " << rsum.value() << std::endl;
    }
    return true;
  }
  ...
  if( test )
    if( print_level() >= mybhep::VVERBOSE ){
      std::clog <<" cells " << id() << " and " <<c.id() << " intersect: dist " << dist << " radii "
  << r().value() << " and " <<c.r().value()<< " rsum " << rsum.value() << " +- " << rsum.error() <<
  " prob " << probof(chi2, 1) << " chi2 " << chi2 << " problimit " << probmin() << std::endl;
    }
  */
