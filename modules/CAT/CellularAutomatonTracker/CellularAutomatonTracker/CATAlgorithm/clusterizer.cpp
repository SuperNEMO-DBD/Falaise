...
//*************************************************************
bool clusterizer::initialize(const mybhep::sstore& store, const mybhep::gstore& /* gs */,
...
void clusterizer::initializeHistos(void) {
  ...
  //  hman.h1("prob_triplet", "probability that chi2 is larger than observed for a good triplet",
  //  100, -0.05, 1.05); hman.h1("largest_true_kink", "largest kink in a true particle", 100, 0.,
  //  180.); hman.h2("largest_true_kink_position", "position of largest kink in a true particle",
  //  100, -rad/1.9, rad/1.9, 100, -rad/1.9, rad/1.9);
...
bool clusterizer::finalize() {
  ...
  m.message("CAT::clusterizer::finalize: Skipped events: ", SkippedEvents, "(",
...
void clusterizer::readDstProper(const mybhep::sstore& global, mybhep::EventManager2* /*eman */) {
  ...
  */
  ...
  m.message("CAT::clusterizer::readDstProper: compatibility distance", CompatibilityDistance,
            mybhep::NORMAL);
  ...
  m.message("CAT::clusterizer::readDstProper: plasma speed is read as: ", vel, "mm/ns",
            mybhep::NORMAL);
  m.message("CAT::clusterizer::readDstProper: wire length is read as: ", len, "mm", mybhep::NORMAL);
  m.message("CAT::clusterizer::readDstProper: cell diameter is read as: ", rad, "mm",
            mybhep::NORMAL);
  m.message("CAT::clusterizer::readDstProper: distance from wire to wire is read as: ",
            CellDistance, "mm", mybhep::NORMAL);
  ...
  if (SuperNemo) {
    ...
    if (global.find(pname)) {
      ...
      m.message(
          "CAT::clusterizer::readDstProper: +++ NEMO3 GG ERROR, GLOBAL PROPERTY NOT FOUND IN DST",
          pname, mybhep::NORMAL);
      exit(1);
    }
...
void clusterizer::readDstProper() {
  ...
  */
  ...
  m.message("CAT::clusterizer::readDstProper: first event number", first_event_number,
            mybhep::NORMAL);
  ...
  if (SuperNemo && SuperNemoChannel) {
    ...
    m.message("CAT::clusterizer::readDstProper: distance from wire to wire is: ", CellDistance,
              "mm", mybhep::NORMAL);
  }
  if (!SuperNemo) {
...
void clusterizer::FillYPositions(mybhep::event& evt) {
  ...
  for (size_t i = 0; i < evt.digi_particles().size(); i++) {
    if (evt.digi_particles()[i]->find_property("SUNAMI")) FillYPositions(evt.digi_particles()[i]);
  }
...
void clusterizer::FillYPosition(mybhep::hit* h) {
  ...
  if (SuperNemo) {
    ...
    if (std::isnan(tup) || std::isinf(tup)) tup = -1.;
    if (std::isnan(tdown) || std::isinf(tdown)) tdown = -1.;
    if (std::isnan(tdelay) || std::isinf(tdelay)) tdelay = -1.;

    std::vector<float> cellpos;
    mybhep::vector_from_string(h->fetch_property("CELL_POS"), cellpos);

    if (cellpos.size() != 3) {
      m.message("CAT::clusterizer::FillYPosition: problem: cell_pos size is ", cellpos.size(),
                mybhep::MUTE);
      return;
    }
    ...
    if (std::isnan(erry) || std::isinf(erry)) erry = -1.;

    if (!h->find_property("ERRY")) h->add_property("ERRY", mybhep::to_string(erry));
  } else {
...
void clusterizer::GenerateWires(void) {
  ...
  if (SuperNemo) {
    ...
    // TRACKING GG BLOCKS
    double theta = M_PI / 8.;
    m.message("CAT::clusterizer::GenerateWires: rad=", rad, mybhep::NORMAL);
    if (std::isnan(GG_CELL_pitch)) {
      GG_CELL_pitch = rad * cos(theta);
    }
    m.message("CAT::clusterizer::GenerateWires: GG_CELL_pitch=", GG_CELL_pitch, mybhep::NORMAL);
    m.message("CAT::clusterizer::GenerateWires: GG_GRND_diam=", GG_GRND_diam, mybhep::NORMAL);
    m.message("CAT::clusterizer::GenerateWires: CHAMBER_X=", CHAMBER_X, mybhep::NORMAL);
    if (num_cells_per_plane <= 0) {
      num_cells_per_plane = (int)((CHAMBER_X - GG_GRND_diam) / GG_CELL_pitch);
    }
    m.message("CAT::clusterizer::GenerateWires: num_cells_per_plane=", num_cells_per_plane,
              mybhep::NORMAL);
    GG_BLOCK_X = num_cells_per_plane * GG_CELL_pitch + GG_GRND_diam;
    m.message("CAT::clusterizer::GenerateWires: GG_BLOCK_X=", GG_BLOCK_X, mybhep::NORMAL);
    ...
    m.message("CAT::clusterizer::GenerateWires: SOURCE_thick=", SOURCE_thick, mybhep::NORMAL);
...
size_t clusterizer::get_true_hit_index(mybhep::hit& hit, bool print) {
  ...
  for (std::vector<topology::cell>::iterator ic = cells_.begin(); ic != cells_.end(); ++ic) {
    if (ic->same_cell(tn.c())) return ic->id();
  }

  if (print)
    m.message(
        "CAT::clusterizer::get_true_hit_index: warning: can't find corresponding reco hit for true "
        "hit (",
        tn.c().ep().x().value(), ", ", tn.c().ep().y().value(), ", ", tn.c().ep().z().value(),
        ") layer", tn.c().layer(), mybhep::VVERBOSE);
...
size_t clusterizer::get_nemo_hit_index(mybhep::hit& hit, bool print) {
  ...
  if (print)
    m.message(
        "CAT::clusterizer::get_nemo_hit_index: warning: can't find corresponding reco hit for nemo "
        "hit (",
        tn.c().ep().x().value(), ", ", tn.c().ep().y().value(), ", ", tn.c().ep().z().value(),
        ") layer", tn.c().layer(), mybhep::VVERBOSE);
...
size_t clusterizer::get_calo_hit_index(const topology::calorimeter_hit& c) {
  ...
  for (std::vector<topology::calorimeter_hit>::iterator ic = calorimeter_hits_.begin();
       ic != calorimeter_hits_.end(); ++ic) {
    if (ic->same_calo(c)) return ic->id();
  }

  m.message(
      "CAT::clusterizer::get_calo_hit_index: warning: can't find corresponding calo hit for nemo "
      "calo hit (",
      c.pl().center().x().value(), ", ", c.pl().center().y().value(), ", ",
      c.pl().center().z().value(), ") layer", c.layer(), mybhep::VVERBOSE);
  ...
  if (bhep_input) {
    ...
    if (!SuperNemo && first_event) {
      ...
      if (event_ref.find_property("DATA"))
        if (mybhep::int_from_string(event_ref.fetch_property("DATA")) == 1) N3_MC = false;  // data
    ...
    if (digi_parts[0]->find_property("mID"))  // BAR design??
    {
      for (size_t pnr = 0; pnr < digi_parts.size(); pnr++)
        if (digi_parts[pnr]->fetch_property("mID") == _moduleNR) parts.push_back(digi_parts[pnr]);
    ...
    if (level >= mybhep::VVERBOSE) print_calos();

    read_true_sequences(event_ref);

    if (!SuperNemo) read_nemo_sequences(event_ref);
  }
...
bool clusterizer::prepare_event(topology::tracked_data& tracked_data_) {
  ...
  event_number++;
  m.message("CAT::clusterizer::prepare_event: local_tracking: preparing event", event_number,
            mybhep::VERBOSE);

  if (event_number < first_event_number) {
    m.message("CAT::clusterizer::prepare_event: local_tracking: skip event", event_number,
...
void clusterizer::read_true_sequences(mybhep::event& event_ref) {
  ...
  m.message("CAT::clusterizer::read_true_sequences: number of true particles is", truep.size(),
            mybhep::VVERBOSE);
  if (truep.size() != 0) {
    for (size_t ipart = 0; ipart < truep.size(); ipart++) {
      ...
      if (tp.find_property("length"))
        trueseq.set_helix_length(topology::experimental_double(
            mybhep::double_from_string(tp.fetch_property("length")), 0.));

      trueseq.set_helix_vertex(topology::experimental_point(tp.vertex().x(), tp.vertex().y(),
                                                            tp.vertex().z(), 0., 0., 0.),
                               "true");
      ...
      if (!chits.empty()) {
        ...
        trueseq.set_decay_helix_vertex(
            topology::experimental_point(tp.decay_vertex().x(), tp.decay_vertex().y(),
                                         tp.decay_vertex().z(), 0., 0., 0.),
            "calo", cindex);
      }

      trueseq.set_name(tp.name());

      trueseq.set_momentum(
          topology::experimental_vector(tp.p3().x(), tp.p3().y(), tp.p3().z(), 0., 0., 0.));
...
void clusterizer::read_nemo_sequences(mybhep::event& event_ref) {
  ...
  if (!nemo_parts.empty()) {
    m.message("CAT::clusterizer::read_nemo_sequences: number of nemo particles is",
              nemo_parts.size(), mybhep::VVERBOSE);
    ...
    for (size_t ipart = 0; ipart < nemo_parts.size(); ipart++) {
      ...
      if (tp.find_property("length"))
        nemoseq.set_helix_length(topology::experimental_double(
            mybhep::double_from_string(tp.fetch_property("length")), 0.));

      nemoseq.set_helix_vertex(topology::experimental_point(tp.vertex().x(), tp.vertex().y(),
                                                            tp.vertex().z(), 0., 0., 0.),
                               "nemo");
      ...
      if (!chits.empty()) {
        topology::calorimeter_hit ch = make_calo_hit(*chits[0], 0);
        cindex = get_calo_hit_index(ch);
        nemoseq.set_decay_helix_vertex(
            topology::experimental_point(tp.decay_vertex().x(), tp.decay_vertex().y(),
                                         tp.decay_vertex().z(), 0., 0., 0.),
            "calo", cindex);
      }
...
void clusterizer::print_calos(void) const {
  ...
  for (std::vector<topology::calorimeter_hit>::const_iterator icalo = calorimeter_hits_.begin();
...
void clusterizer::print_true_sequences(void) const {
  ...
  for (std::vector<topology::sequence>::const_iterator iseq = true_sequences_.begin();
       iseq != true_sequences_.end(); ++iseq) {
    iseq->dump();
  }
...
void clusterizer::print_nemo_sequences(void) const {
  ...
  for (std::vector<topology::sequence>::const_iterator iseq = nemo_sequences_.begin();
       iseq != nemo_sequences_.end(); ++iseq) {
    iseq->dump();
  }
...
void clusterizer::clusterize(topology::tracked_data& tracked_data_) {
  ...
  for (size_t ip = 0; ip < 2; ip++)  // loop on two sides of the foil
  {
    for (size_t iq = 0; iq < 2; iq++)  // loop on fast and slow hits
    {
      ...
      for (std::vector<topology::cell>::const_iterator icell = cells_.begin();
        ...
        m.message("CAT::clusterizer::clusterize: begin new cluster with cell ", c.id(),
                  mybhep::VERBOSE);
        ...
        for (size_t i = 0; i < cells_connected_to_c.size(); i++) {  // loop on connected cells
          ...
          m.message("CAT::clusterizer::clusterize: cluster ", clusters_.size(), " starts with ",
          ...
          for (std::vector<topology::cell>::const_iterator icnc = cells_near_iconn.begin();
            ...
            m.message("CAT::clusterizer::clusterize: ... creating couplet ", cconn.id(), " -> ",
                      cnc.id(), mybhep::VERBOSE);
          ...
          m.message("CAT::clusterizer::clusterize: cluster started with ", c.id(),
  ...
  m.message("CAT::clusterizer::clusterize: there are ", clusters_.size(), " clusters of cells ",
            mybhep::VVERBOSE);

  if (PrintMode) make_plots(tracked_data_);
...
void clusterizer::clusterize_after_sultan(topology::tracked_data& tracked_data_) {
  ...
  m.message("CAT::clusterizer::clusterize_after_sultan: local_tracking: fill clusters ",
            mybhep::VERBOSE);

  if (cells_.empty()) return;

  if (!select_true_tracks(tracked_data_)) {
    m.message("CAT::clusterizer::clusterize_after_sultan: event is not selected at true level ",
              mybhep::NORMAL);
    tracked_data_.set_selected(false);
    return;
  }

  m.message("CAT::clusterizer::clusterize_after_sultan: sultan has made ",
            tracked_data_.clusters_.size(), " clusters of cells ", mybhep::VVERBOSE);

  // loop on sultan clusters
  for (std::vector<topology::cluster>::iterator iclu = tracked_data_.clusters_.begin();
       iclu != tracked_data_.clusters_.end(); ++iclu) {
    m.message("CAT::clusterizer::clusterize_after_sultan: cluster [",
              iclu - tracked_data_.clusters_.begin(), "] has ", iclu->nodes_.size(), " cells ",
              mybhep::VVERBOSE);
    ...
    for (std::vector<topology::node>::iterator inode = iclu->nodes_.begin();
      ...
      if (inode - iclu->nodes_.begin() > 0) {
        ...
        m.message("CAT::clusterizer::clusterize_after_sultan: adding couplet", c.id(), "-", pc.id(),
                  mybhep::VVERBOSE);
        cc.push_back(pcc);
      }
      if ((size_t)(inode - iclu->nodes_.begin() + 1) < iclu->nodes_.size()) {
        ...
        m.message("CAT::clusterizer::clusterize_after_sultan: adding couplet", c.id(), "-", nc.id(),
                  mybhep::VVERBOSE);
        cc.push_back(ncc);
        links.push_back(nc);
      }

      m.message("CAT::clusterizer::clusterize_after_sultan: node [", inode->c().id(), "] has ",
                cc.size(), " couplets ", mybhep::VVERBOSE);
...
void clusterizer::make_plots(topology::tracked_data& /* tracked_data_ */) {
  ...
  /*
    if( PrintMode ){
    for(std::vector<topology::cluster>::iterator icluster = clusters_.begin();
    icluster!=clusters_.end(); ++icluster){ for(std::vector<topology::node>::iterator inode =
    (*icluster).nodes_.begin(); inode != (*icluster).nodes_.end(); ++inode){
    for(std::vector<topology::cell_triplet>::iterator iccc = (*inode).ccc_.begin(); iccc !=
    (*inode).ccc_.end(); ++iccc){ topology::cell_triplet ccc = *iccc;
    for(std::vector<double>::const_iterator ichi = ccc.chi2s().begin(); ichi != ccc.chi2s().end();
    ++ichi){ hman.fill("chi2_triplet", *ichi);
    }
    for(std::vector<double>::const_iterator iprob = ccc.probs().begin(); iprob != ccc.probs().end();
    ++iprob){ hman.fill("prob_triplet", *iprob);
    }
    }
    }
    ...
    for(std::vector<topology::sequence>::iterator iseq=true_sequences.begin(); iseq !=
    true_sequences.end(); ++iseq){ topology::node n; double phi; if( iseq->largest_kink_node(n,
    phi)){ phi *= 180./M_PI; std::clog << " largest kink on true sequence " << iseq -
    true_sequences.begin() << " is " << phi << " on cell " << n.c().id() << std::endl;
...
//*************************************************************
bool clusterizer::is_good_couplet(topology::cell* mainc, const topology::cell& candidatec,
  ...
  for (std::vector<topology::cell>::const_iterator icell = nearmain.begin();
       icell != nearmain.end(); ++icell) {
    ...
    m.message("CAT::clusterizer::is_good_couplet: ... ... check if near node ", b.id(),
              " has triplet ", a.id(), " <-> ", candidatec.id(), mybhep::VERBOSE);
    ...
    if (ccc.joints().size() > 0) {
      m.message("CAT::clusterizer::is_good_couplet: ... ... yes it does: so couplet ", a.id(),
                " and ", candidatec.id(), " is not good", mybhep::VERBOSE);
...
void clusterizer::fill_fast_information(mybhep::hit* h) {
  ...
  if (radius != 0. && tdelay == 0.)  // fast hit
    ...
    m.message("CAT::clusterizer::fill_fast_information: Problem: hit has radius", radius,
              "delay time", tdelay, mybhep::NORMAL);
  }
...
int clusterizer::cell_side(const topology::cell& c) {
  ...
  if (SuperNemo) {
    if (c.ep().z().value() > 0.) return 1;

    return -1;
  }

  if (c.ep().radius().value() > FoilRadius) return 1;
...
size_t clusterizer::near_level(const topology::cell& c1, const topology::cell& c2) {
  ...
  if (SuperNemo) {  // use side, layer and row
    ...
    if (layer_distance == 0 && row_distance == 0) {
      if (level >= mybhep::NORMAL) {
        std::clog << "CAT::clusterizer::near_level: problem: cat asking near level of cells with "
                     "identical posiion ("
...
std::vector<topology::cell> clusterizer::get_near_cells(const topology::cell& c) {
  ...
  m.message("CAT::clusterizer::get_near_cells: filling list of cells near cell ", c.id(), " fast ",
            c.fast(), " side ", cell_side(c), mybhep::VVERBOSE);

  std::vector<topology::cell> cells;

  for (std::vector<topology::cell>::iterator kcell = cells_.begin(); kcell != cells_.end();
       ++kcell) {
...
void clusterizer::setup_clusters() {
  ...
  for (std::vector<topology::cluster>::iterator icl = clusters_.begin(); icl != clusters_.end();
    ...
    for (std::vector<topology::node>::iterator inode = (*icl).nodes_.begin();
      ...
      for (std::vector<topology::cell_couplet>::iterator icc = (*inode).cc_.begin();
           icc != (*inode).cc_.end(); ++icc) {
        icc->set_print_level(level);
        icc->set_probmin(probmin);
      }

      for (std::vector<topology::cell_triplet>::iterator iccc = (*inode).ccc_.begin();
           iccc != (*inode).ccc_.end(); ++iccc) {
...
topology::calorimeter_hit clusterizer::make_calo_hit(const mybhep::hit& ahit, size_t _id) {
  ...
  if (SuperNemo) {
    ...
    if (plane == "CALO_WRAP+X") {
      ...
      m.message("CAT::clusterizer::make_calo_hit: problem: calo block not recognized", plane,
                mybhep::MUTE);
      exit(1);
    }
  } else {
    ...
    if (planeid == 0 || planeid == 1) {
      ...
      topology::experimental_vector mysizes(local_calo_X, calo_Y, calo_Z, 0., 0., 0.);
      ...
      if (id == 0)
        ...
        m.message("CAT::clusterizer::make_calo_hit: CAL wall not recognized", block_type,
                  mybhep::NORMAL);
        exit(1);
      }
    } else {
  ...
  topology::experimental_vector sizes(local_calo_X, calo_Y, calo_Z, 0., 0., 0.);
...
bool clusterizer::select_true_tracks(topology::tracked_data& tracked_data_) {
  ...
  m.message("CAT::clusterizer::select_true_tracks: selecting events based on true tracks ",
            mybhep::VVERBOSE);
  ...
  for (std::vector<topology::sequence>::iterator iseq = true_sequences.begin();
       iseq != true_sequences.end(); ++iseq) {
    if (!iseq->primary()) continue;
    if (counter == 0) sa = *iseq;
    if (counter == 1) sb = *iseq;
    counter++;
  }

  if (counter != n_tracks_) {
    m.message("CAT::clusterizer::select_true_tracks: reject: n primary tracks = ", counter,
              mybhep::NORMAL);
    return false;
  }
  ...
  if (!sa.one_side()) {
    m.message("CAT::clusterizer::select_true_tracks: reject: 1st track crosses the foil ",
              mybhep::NORMAL);
    return false;
  }
  ...
  if (n_tracks_ == 2) {
    if (!sb.one_side()) {
      m.message("CAT::clusterizer::select_true_tracks: reject: 2nd track crosses the foil ",
                mybhep::NORMAL);
      return false;
    }
  }
  ...
  if (!sa.has_decay_helix_vertex()) {
    m.message("CAT::clusterizer::select_true_tracks: reject: 1st track has no calo ",
              mybhep::NORMAL);
    return false;
  }
  ...
  if (n_tracks_ == 2) {
    if (!sb.has_decay_helix_vertex()) {
      m.message("CAT::clusterizer::select_true_tracks: reject: 2nd track has no calo ",
                mybhep::NORMAL);
      return false;
    }
  }
  ...
  if (n_tracks_ == 2) {
    if (sa.calo_helix_id() == sb.calo_helix_id()) {
      m.message("CAT::clusterizer::select_true_tracks: reject: same calo ", sa.calo_helix_id(),
                mybhep::NORMAL);
      return false;
    }
  }
  ...
  if (caloA.e().value() < emin) {
    m.message("CAT::clusterizer::select_true_tracks: reject: 1st calo has energy ",
              caloA.e().value(), mybhep::NORMAL);
    return false;
  }
  ...
  if (n_tracks_ == 2) {
    ...
    if (caloB.e().value() < emin) {
      m.message("CAT::clusterizer::select_true_tracks: reject: 2nd calo has energy ",
                caloA.e().value(), mybhep::NORMAL);
      return false;
    }
  }
  ...
  */
  ...
  for (std::vector<topology::calorimeter_hit>::iterator iseq = calos.begin(); iseq != calos.end();
       ++iseq) {
    calo_counter++;
  ...
  if (calo_counter != n_tracks_) {
    m.message("CAT::clusterizer::select_true_tracks: reject: n calos is ", calo_counter,
              mybhep::NORMAL);
    return false;
  }

  if (calo_counter_thr != n_tracks_) {
    m.message("CAT::clusterizer::select_true_tracks: reject: n calos with E > ", emin, " is ",
              calo_counter_thr, mybhep::NORMAL);
    return false;
  }
...
void clusterizer::set_planes_per_block(int block, int nplanes) {
  if (block < 0 || block >= (int)planes_per_block.size()) {
    throw std::range_error(
        "CAT::clusterizer::set_planes_per_block: Invalid GG layer block index !");
  }
  if (nplanes > 0) {
    planes_per_block.at(block) = nplanes;
  } else {
    throw std::range_error(
        "CAT::clusterizer::set_planes_per_block: Invalid number of GG layers in block !");
  }
  return;
}
...
// What is it ?
int clusterizer::get_module_nr(void) { return _MaxBlockSize; }
