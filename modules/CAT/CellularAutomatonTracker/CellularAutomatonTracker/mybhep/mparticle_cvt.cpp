...
mparticle_cvt::mparticle_cvt(const mparticle& par, int index_arg) : ibconverter() {
  set_index(index_arg);
  set_name("__mirror__");
  set_owner(false);
  particle_ = &par;
...
mparticle_cvt::mparticle_cvt(std::string def) : ibconverter() { restore(def); }
...
std::string mparticle_cvt::store() {
  ...
  for (I i = dmap.begin(); i != dmap.end(); ++i) {
    std::string tmp2 = to_string(i->first) + " " + to_string(i->second);
    to_string(tmp, tmp2);
  }
...
void mparticle_cvt::complete_restore() {
  ...
  try {
    particle_cvt& pcvt = *(dynamic_cast<particle_cvt*>(&csvc.converter(imirror_)));
...
const mparticle& mparticle_cvt::retrieve() const { return *particle_; }

mparticle& mparticle_cvt::reference() { return *nparticle_; }
