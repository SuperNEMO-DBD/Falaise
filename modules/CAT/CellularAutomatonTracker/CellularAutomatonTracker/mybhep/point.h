...
class point {
 ...
 public:
  ...
  //! set all coordinates
  void set(double x_arg, double y_arg, double z_arg) { coordinates(x_arg, y_arg, z_arg); }

  //! set  coordinates in a view
  void set(double xt, double z_arg, std::string view_arg) { coordinates(xt, z_arg, view_arg); }

  //! set  coordinates in a view
  void coordinates(double xt, double z_arg, std::string view_arg) {
    view_ = to_upper(view_arg);
    Assert(view_ == "XZ" || view_ == "YZ", __FILE__, __LINE__,
  ...
  //! set x
  void set_x(double x_arg) { x_ = x_arg; }

  //! set y
  void set_y(double y_arg) { y_ = y_arg; }

  //! set z
  void set_z(double z_arg) { z_ = z_arg; }
  ...
  const double& operator()(size_t i) const {
    Assert(i < 3, __FILE__, __LINE__, bad_index("index i must be < 3"));
    if (i == 0) return x();
    if (i == 1) return y();
    return z_;
  }
  ...
  //! operador *=
  point& operator/=(double a) {
    Assert(a != 0, __FILE__, __LINE__, divide_by_zero("error, divide by zero"));
  ...
  double distance(const point& p2) const {
    ...
    if (view_ == "XZ")
      ...
      return sqrt(square(x_ - p2.x()) + square(y_ - p2.y()) + square(z_ - p2.z()));
    } else
      Assert(1, __FILE__, __LINE__, internal_logic("error, wrong view"));
    return -9999;
  }
  //! module
  double module() const {
