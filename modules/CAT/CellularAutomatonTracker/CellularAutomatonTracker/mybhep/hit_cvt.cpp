...
hit_cvt::hit_cvt(const hit& ht, int index) : ibconverter() {
  set_index(index);
  set_name("__hit__");
  set_owner(false);
  hit_ = &ht;
...
hit_cvt::hit_cvt(std::string def) : ibconverter() { restore(def); }

hit_cvt::~hit_cvt() {
  if (owner()) delete nhit_;
}
...
std::string hit_cvt::store() {
  ...
  std::string tmp2 =
      to_string(hit_->x()[0]) + " " + to_string(hit_->x()[1]) + " " + to_string(hit_->x()[2]);
  to_string(tmp, tmp2);
...
void hit_cvt::complete_restore() {
  ...
  try {
    ...
    std::cerr << "could not resolver pointer to hit mother particle" << endl;
    std::cerr << "event is likely corrupted" << endl;
  }
}

const hit& hit_cvt::retrieve() const { return *hit_; }

hit& hit_cvt::reference() { return *nhit_; }

hit* hit_cvt::create() {
  Assert(nhit_, __FILE__, __LINE__, internal_logic("  null pointer!!!"));
