...
// default constructor
sparticle::sparticle() { sp_ = new particle(DIGI, "unknown"); }
...
//! destructor
sparticle::~sparticle() { delete sp_; }
...
void sparticle::add_particle(const particle& p, double q) {
  Assert(subp_.size() > 0, __FILE__, __LINE__, internal_logic("seed particle not set"));
...
void sparticle::add_quality(double q) { q_.push_back(q); }

std::ostream& operator<<(std::ostream& out, const sparticle& ip) {
  out << std::endl;

  out << " sparticle info: particle = " << ip.seed_particle() << " " << std::endl;
  ...
  out << " List of matching quantities " << print_vector(ip.quality()) << std::endl;
