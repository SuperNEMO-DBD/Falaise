...
class tstore : public bprint {
 ...
 public:
  ...
  const std::map<std::string, T>& store_map() const { return m_store; }
  ...
  void sstore(std::string name, T& object) { m_store[name] = object; }

  //! strong-store an object (rewrite name if necessary)
  void sstore(std::string name, const T& object) { m_store[name] = object; }
  ...
  */
  bool erase(std::string name) {
    ...
    if (count == 1)
      ...
      std::cerr << " warning, name = " << name << "  found " << count << " times in store"
                << std::endl;
      return true;
    }
  }
  //! operator []
  T& operator[](std::string name) { return fetch(name); }

  //! operator []
  const T& operator[](std::string name) const { return fetch(name); }

  //! fetch an object from its name
  const T& fetch(std::string name) const {
    Assert(find(name), __FILE__, __LINE__,
           internal_logic("name not found in --tstore::fetch(), name =" + name));
