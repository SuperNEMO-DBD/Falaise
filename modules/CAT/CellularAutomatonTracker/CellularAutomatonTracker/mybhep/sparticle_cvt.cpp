...
sparticle_cvt::sparticle_cvt(const sparticle& trk, int index) : ibconverter() {
  set_index(index);
...
void sparticle_cvt::complete_restore() {
  ...
  try {
    particle_cvt& pcvt = *(dynamic_cast<particle_cvt*>(&csvc.converter(iseed_)));

    particle& pp = pcvt.reference();
    nsparticle_->set_seed_particle(pp);
  } catch (bad_index&) {
    std::cerr << "could not resolver pointer to sparticle seed particle" << endl;
  ...
  for (size_t ipar = 0; ipar < isubp_.size(); ipar++) {
    ...
    try {
      particle_cvt& pcvt = *(dynamic_cast<particle_cvt*>(&csvc.converter(index)));
...
const sparticle& sparticle_cvt::retrieve() const { return *sparticle_; }

sparticle& sparticle_cvt::reference() { return *nsparticle_; }

sparticle* sparticle_cvt::create() {
  Assert(nsparticle_, __FILE__, __LINE__, internal_logic("  null pointer!!!"));
