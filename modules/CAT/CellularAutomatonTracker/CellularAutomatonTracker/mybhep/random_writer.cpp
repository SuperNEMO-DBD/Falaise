...
random_writer::random_writer() : brw() {
  path_ = "/DST";
  join_ = "/";
}

//! set path
void random_writer::set_path(std::string path_arg) { path_ = path_arg; }

//! return path
std::string random_writer::path() { return path_; }

//! write event
void random_writer::write(event& evt, size_t ievent, std::string key) {
  event_cvt cvt(evt, ievent);
  std::string record = cvt.store();
  if (key == "") key = to_string(ievent);
