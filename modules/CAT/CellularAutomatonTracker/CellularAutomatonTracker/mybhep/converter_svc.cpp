...
namespace mybhep {
//! destructor
converter_svc::~converter_svc() { clear(); }
...
void converter_svc::clear() { container_algorithm::destroy(converters_); }
...
ibconverter& converter_svc::converter(int index) {
  ...
  Assert(found, __FILE__, __LINE__, bad_index(" index not found"));

  return *converters_[indx];
}

void converter_svc::add_converter(ibconverter* ib) { converters_.push_back(ib); }
