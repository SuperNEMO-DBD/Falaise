...
namespace mybhep {
  ...
  */
  ...
  private:
    ...
    vector<const particle*> daughters_;
  ...
  public:
    ...
    particle(ptype type, string name, const ray& r);
    ...
    //! general constructor
    particle(ptype type, string name, int pdg, double m, 
	     double q, double l=-1);
  ...
  public:
    ...
    const vector<const particle*>&  daughters() const {return daughters_;}
  
    //! return vector of daughters (const)
    vector<const particle*>&  daughters()  {return daughters_;}
    ...
    const vector<mparticle*>&  mparticles() const {return mp_;}

    //! return vector of mparticles
    vector<mparticle*>&  mparticles()  {return mp_;}

    //! return vector of hits associated to a detector
    vector<hit*>  hits(string detector) const;

    //! return multimap of hits 
    const multimap<string,hit*>& hit_map() const {return hits_;}
