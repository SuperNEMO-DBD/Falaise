...
event::event() : event_number_(0) {
  vertex_ = new mybhep::Point3D(small_neg, small_neg, small_neg);
}

// constructor
event::event(int number, const mybhep::Point3D& vertex_arg) : event_number_(number) {
  vertex_ = new mybhep::Point3D(vertex_arg);
}

// constructor
event::event(int number) : event_number_(number) {
  vertex_ = new mybhep::Point3D(small_neg, small_neg, small_neg);
}
...
// set vertex
void event::set_vertex(const mybhep::Point3D& p) { vertex_->set(p.x(), p.y(), p.z()); }
...
bool event::filter(ptype type_arg, std::string property_name, std::string property_value,
...
int event::find_tree(particle* p, ptype type_arg, std::vector<particle*>& daut) {
  ...
  while (test == true) {
    ...
    for (size_t jp = 0; jp < dau.size(); jp++) {
      ...
      if (test2) container_algorithm::append(dau0, dau1);
    }
...
bool event::delete_particle(particle* pr, ptype type_arg) {
  ...
  if (test2) {
    ...
    if (cpm.has_mother()) erase_mother_memory(cpm, pr, type_arg);
...
void event::erase_mother_memory(const particle& cpm, const particle* p, ptype type_arg) {
  std::vector<particle*> prt;
  if (type_arg == TRUTH)
    prt = true_particles();
  else
...
std::ostream& operator<<(std::ostream& out, const event& ip) {
  ...
  out << " event number= " << ip.event_number() << std::endl;

  out << " event vertex (cm)= " << ip.vertex() / cm << std::endl;
  ...
  out << " Event has " << ip.true_particles().size() << " true particles" << std::endl;
  out << " Event has " << ip.digi_particles().size() << " digi particles" << std::endl;
  out << " Event has " << ip.sparticles().size() << " super particles" << std::endl;
