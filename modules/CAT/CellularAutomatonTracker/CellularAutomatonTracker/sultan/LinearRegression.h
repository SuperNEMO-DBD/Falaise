...
namespace SULTAN {
  ...
  public:
    ...
    //! constructor
    LinearRegression(std::vector<double> &xi, std::vector<double> &yi, mybhep::prlevel level=mybhep::NORMAL, double probmin=1.e-200){
      set_print_level(level);
      set_probmin(probmin);
      appname_= "LinearRegression: ";
      xi_ = xi;
    ...
    virtual void dump (std::ostream & a_out         = std::clog,
                       ...
                       bool /*a_inherit*/          = false)
      {
        ...
        for(std::vector<double>::iterator it=xi_.begin(); it != xi_.end(); ++it){
          a_out << indent << " .. x " << *it << " y " << yi_[it - xi_.begin()] << " predicted " << position(*it) << std::endl;
        }
