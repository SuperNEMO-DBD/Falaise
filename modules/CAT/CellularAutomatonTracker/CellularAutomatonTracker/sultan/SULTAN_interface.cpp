...
const std::string &setup_data::get_error_message() const { return _error_message; }
...
void clusterizer_configure(clusterizer &czer_, const setup_data &setup_) {
  if (!setup_.check()) {
    std::cerr << "ERROR: SULTAN::clusterizer_configure: " << setup_.get_error_message()
              << std::endl;
...
void sultan_configure(sultan &stor_, const setup_data &setup_) {
  if (!setup_.check()) {
    std::cerr << "ERROR: SULTAN::sultan_configure: " << setup_.get_error_message() << std::endl;
...
bool input_data::check() const { return gg_check() && calo_check(); }
...
bool input_data::gg_check() const {
  ...
  for (int i = 0; i < (int)cells.size(); i++) {
    ...
    if (cell_id < 0 || cell_id > 10000) {
      std::cerr << "ERROR: SULTAN::input_data::check: "
                << "Out of range cell ID '" << cell_id << "' !" << std::endl;
      return false;
    }
    if (mids.find(cell_id) != mids.end()) {
      std::cerr << "ERROR: SULTAN::input_data::check: "
                << "Duplicate cell ID '" << cell_id << "' !" << std::endl;
      return false;
    }
    mids[cell_id] = true;
  }
  ...
  for (int i = 0; i < (int)cells.size(); i++) {
    ...
    if ((cell_id < 0) || (cell_id >= (int)cells.size())) {
      std::cerr << "ERROR: SULTAN::input_data::check: "
                << "Invalid cell ID '" << cell_id << "' !" << std::endl;
      return false;
    }
    if (ids[cell_id]) {
      std::cerr << "ERROR: SULTAN::input_data::check: "
...
bool input_data::calo_check() const {
  ...
  for (int i = 0; i < (int)calo_cells.size(); i++) {
    ...
    if (calo_cell_id < 0 || calo_cell_id > 10000) {
      std::cerr << "ERROR: SULTAN::input_data::calo_check: "
                << "Out of range calo_cell ID '" << calo_cell_id << "' !" << std::endl;
      return false;
    }
    if (mids.find(calo_cell_id) != mids.end()) {
      std::cerr << "ERROR: SULTAN::input_data::check: "
                << "Duplicate calo_cell ID '" << calo_cell_id << "' !" << std::endl;
      return false;
    }
  ...
  for (int i = 0; i < (int)calo_cells.size(); i++) {
    ...
    if ((calo_cell_id < 0) || (calo_cell_id >= (int)calo_cells.size())) {
      std::cerr << "ERROR: SULTAN::input_data::check: "
                << "Invalid calo_cell ID '" << calo_cell_id << "' !" << std::endl;
      return false;
    }
    if (ids[calo_cell_id]) {
      std::cerr << "ERROR: SULTAN::input_data::check: "
...
input_data::input_data() { return; }

/***********************************************************/

output_data::output_data() { return; }
