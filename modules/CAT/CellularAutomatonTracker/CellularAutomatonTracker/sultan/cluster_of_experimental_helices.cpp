...
bool cluster_of_experimental_helices::can_merge__optimist(cluster_of_experimental_helices cluster,
                                                          double nsigmas) const {
  //
...
bool cluster_of_experimental_helices::different_cells(
    topology::cluster_of_experimental_helices b) const {
  std::vector<size_t> bids = b.ids();
  for (std::vector<size_t>::const_iterator id = ids_.begin(); id != ids_.end(); ++id) {
    if (std::find(bids.begin(), bids.end(), *id) == bids.end()) return true;
  }
...
bool cluster_of_experimental_helices::add_helix(experimental_helix a) {
  ...
  for (std::vector<size_t>::const_iterator id = ids.begin(); id != ids.end(); ++id) add_id(*id);
...
cluster_of_experimental_helices cluster_of_experimental_helices::merge(
    cluster_of_experimental_helices b) const {
  // create min for new cluster
  experimental_helix mina = this->helix_min();
  experimental_helix minb = b.helix_min();

  experimental_helix newmin(
      experimental_min(mina.x0(), minb.x0()), experimental_min(mina.y0(), minb.y0()),
      experimental_min(mina.z0(), minb.z0()), experimental_min(mina.R(), minb.R()),
  ...
  experimental_helix newmax(
      experimental_max(maxa.x0(), maxb.x0()), experimental_max(maxa.y0(), maxb.y0()),
      experimental_max(maxa.z0(), maxb.z0()), experimental_max(maxa.R(), maxb.R()),
  ...
  for (std::vector<size_t>::const_iterator id = ids.begin(); id != ids.end(); ++id) b.add_id(*id);
...
void cluster_of_experimental_helices::add_assigned_id(size_t id) {
  std::vector<size_t>::const_iterator fid =
      std::find(assigned_ids_.begin(), assigned_ids_.end(), id);
...
experimental_helix cluster_of_experimental_helices::average_helix() const {
  ...
  for (std::vector<experimental_helix>::const_iterator ih = helices_.begin(); ih != helices_.end();
       ++ih) {
    x0s.push_back(ih->x0());
    y0s.push_back(ih->y0());
    z0s.push_back(ih->z0());
