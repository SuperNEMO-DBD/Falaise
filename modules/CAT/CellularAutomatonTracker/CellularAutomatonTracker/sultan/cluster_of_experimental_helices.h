...
class cluster_of_experimental_helices : public tracking_object {
 ...
 public:
  //! Default constructor
  cluster_of_experimental_helices(mybhep::prlevel level = mybhep::NORMAL,
  ...
  virtual void dump(std::ostream& a_out = std::clog, const std::string& a_title = "",
    ...
    a_out << indent << appname_ << " helix min: ";
    helix_min_.dump();
    a_out << " helix max: ";
    helix_max_.dump();
    a_out << helices_.size() << " helices: ";
    for (std::vector<experimental_helix>::const_iterator ih = helices_.begin();
         ih != helices_.end(); ++ih)
      ih->dump();
    a_out << " ids: ( ";
  ...
  void reset_assigned_ids() { assigned_ids_.clear(); }
