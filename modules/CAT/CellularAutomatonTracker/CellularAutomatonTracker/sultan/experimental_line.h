...
namespace SULTAN {
  namespace topology{
	...
	epb_ = experimental_point();
      ...
      //! constructor
      experimental_line(experimental_point epa, experimental_point epb, mybhep::prlevel level=mybhep::NORMAL, double probmin=1.e-200){
        set_print_level(level);
        set_probmin(probmin);
        appname_= "experimental_line: ";
        epa_ = epa;
	...
	calculate_dir();
      ...
      virtual void dump (std::ostream & a_out         = std::clog,
                         ...
                         bool /*a_inherit*/          = false)const{
        {
          ...
          a_out << indent << appname_ << " pa ["; epa_.dump(); a_out << "] pb ["; epb_.dump(); a_out << " ]" << std::endl;
	...
	calculate_dir();
      ...
      double distance_from_cell_measurement(topology::cell c, topology::experimental_double *DR, topology::experimental_double *DH)const;

      double distance_from_cell_center(topology::cell c, topology::experimental_double *DR, topology::experimental_double *DH)const;
