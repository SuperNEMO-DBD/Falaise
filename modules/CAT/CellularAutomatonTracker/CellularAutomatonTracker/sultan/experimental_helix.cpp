...
void experimental_helix::add_ids(std::vector<size_t> ids) {
  for (std::vector<size_t>::const_iterator fid = ids.begin(); fid != ids.end(); ++fid) add_id(*fid);
}

void experimental_helix::distance_from_cell_measurement(topology::cell c, experimental_double *DR,
  ...
  experimental_double dr =
      experimental_min(experimental_fabs(delta_hor - dr1), experimental_fabs(delta_hor - dr2));
...
void experimental_helix::distance_from_cell_center(topology::cell c, experimental_double *DR,
                                                   experimental_double *DH) const {
  //////////////////////////////////////////////////////////////////////////
  //   center_of_helix              radius_of_helix    center_of_cell
  //          O                            )(                ep
...
void experimental_helix::get_phi_of_point(topology::experimental_point input_p,
                                          topology::experimental_point *p, double *angle) {
  *p = this->position(input_p);
  // angle of cell center wrt circle center
  *angle =
      atan2(input_p.y().value() - this->y0().value(), input_p.x().value() - this->x0().value());
  return;
}
...
bool experimental_helix::different_cells(topology::experimental_helix b) const {
  ...
  for (std::vector<size_t>::const_iterator id = ids_.begin(); id != ids_.end(); ++id) {
    if (std::find(bids.begin(), bids.end(), *id) == bids.end()) return true;
  }

  for (std::vector<size_t>::const_iterator id = bids.begin(); id != bids.end(); ++id) {
    if (std::find(ids_.begin(), ids_.end(), *id) == ids_.end()) return true;
  }

  return false;
}

bool experimental_helix::is_less_than__optimist(const topology::experimental_helix a,
...
bool experimental_helix::is_more_than__optimist(const topology::experimental_helix a,
                                                double nsigmas = 1) const {
  //
  // is this more than a?
...
bool experimental_helix::isnan() const {
  return (this->x0().experimental_isnan() || this->y0().experimental_isnan() ||
          this->z0().experimental_isnan() || this->R().experimental_isnan() ||
          this->H().experimental_isnan());
}

bool experimental_helix::isinf() const {
  return (this->x0().experimental_isinf() || this->y0().experimental_isinf() ||
          this->z0().experimental_isinf() || this->R().experimental_isinf() ||
...
experimental_vector experimental_helix::direction_at(const experimental_point &ep) const {
  ...
  experimental_vector dir(-R() * experimental_sin(phi), R() * experimental_cos(phi), H());
