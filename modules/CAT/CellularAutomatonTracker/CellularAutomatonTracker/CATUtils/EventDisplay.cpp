...
void EventDisplay::readDstProper() {
  ...
  m.message("distance from wire to wire is read as: ", CellDistance, "mm", mybhep::NORMAL);
  fflush(stdout);
  if (!SuperNemo) m.message("foil radius: ", FoilRadius, "mm", mybhep::NORMAL);
  fflush(stdout);
  m.message("print level:", level, mybhep::NORMAL);
  fflush(stdout);
  m.message("", mybhep::NORMAL);
  fflush(stdout);
...
void EventDisplay::initialize() {
  ...
  mapzx = new TH2F("mapzx", "", 100, -zsize / 2. - calo_Z, zsize / 2. + calo_Z, 100,
                   -xsize / 2. - calo_X, xsize / 2. + calo_X);
  mapzy = new TH2F("mapzy", "", 100, -zsize / 2. - calo_Z, zsize / 2. + calo_Z, 100,
                   -ysize / 2. - calo_Y, ysize / 2. + calo_Y);
  ...
  if (PlotSideView) sv++;
  size_t ncanvas = 0;
  if (!SuperNemo && PlotNemoTracks) ncanvas++;
  if (PlotInitialHits) ncanvas++;
  if (PlotTrueTracks) ncanvas++;
  if (PlotCats || PlotHelices) ncanvas++;
  if (PlotSultan) ncanvas++;
...
//*************************************************************
bool EventDisplay::InitializeDisplayEvent(mybhep::sstore store, mybhep::gstore gs,
...
void EventDisplay::GetPlotLimit(std::vector<CAT::topology::calorimeter_hit> calos) {
  ...
  for (size_t i = 0; i < cells_.size(); i++) {
    ...
    if (c.ep().x().value() - c.r().value() < XMin) XMin = c.ep().x().value() - c.r().value();
    if (c.ep().y().value() < YMin) YMin = c.ep().y().value();
    if (c.ep().z().value() - c.r().value() < ZMin) ZMin = c.ep().z().value() - c.r().value();

    if (c.ep().x().value() + c.r().value() > XMax) XMax = c.ep().x().value() + c.r().value();
    if (c.ep().y().value() > YMax) YMax = c.ep().y().value();
    if (c.ep().z().value() + c.r().value() > ZMax) ZMax = c.ep().z().value() + c.r().value();
  }
...
void EventDisplay::DrawDetectorXZ(void) {
  ...
  for (size_t j = 0; j < DriftWires.size(); j++) {
    ...
    if (SuperNemo) {
      FirstStarXY = new TMarker(DriftWires[j].z, DriftWires[j].x, 7);
    } else {
      FirstStarXY = new TMarker(DriftWires[j].x, DriftWires[j].z, 7);
    }
...
float EventDisplay::FastHit(POSITION position) {
  ...
  if (position.circle.radius != 0 &&  // fast hit
    ...
    m.message("Problem: hit has radius", position.circle.radius, "delay time", position.tdelay,
              mybhep::NORMAL);
    fflush(stdout);
    return -1.;
  }
...
void EventDisplay::Nemo3EventDisplayXZ(void) {
  ...
  if (!SuperNemo) {
    ...
    FirstStarXY_used = true;
    FirstStarXY = new TMarker(n3event.vertex[2], n3event.vertex[0], 5);
...
void EventDisplay::Nemo3EventDisplayYZ(void) {
  ...
  if (!SuperNemo) {
    ...
    FirstStarXY_used = true;
    FirstStarXY = new TMarker(n3event.vertex[2], n3event.vertex[1], 5);
    ...
    for (size_t j = 0; j < (nnodes + cnodes); j++) {
      ...
      if (TrkHit) {
        ...
        if (ahit->find_property("BLK_POS")) {
          ...
          m.message(" problem: calo hit without calo position", mybhep::NORMAL);
          fflush(stdout);
        }
        xt[j + offset] = x;
        zt[j + offset] = z;
...
void EventDisplay::fill_nemo3_event(mybhep::event& evt) {
  ...
  if (evt.find_property("DATE")) n3event.date = mybhep::int_from_string(evt.fetch_property("DATE"));
  if (evt.find_property("RUN")) n3event.run = mybhep::int_from_string(evt.fetch_property("RUN"));
  if (evt.find_property("TIME")) n3event.run = mybhep::int_from_string(evt.fetch_property("TIME"));
...
void EventDisplay::Finalize(void) {
  ...
  if (FirstStarXY_used) delete FirstStarXY;
  if (PlotLegend) {
    delete leg_xz;
    delete leg_yz;
    if (PlotTrueTracks) {
...
//*************************************************************
void EventDisplay::execute(size_t ievent, CAT::topology::tracked_data& __CAT_tracked_data,
  ...
  if (!PlotInitialHits && !PlotTrueTracks && !PlotCats && !PlotHelices && !PlotNemoTracks &&
      !PlotSultan) {
  ...
  if (PlotInitialHits) {
    if (PlotTopView) event_display_xz("InitialHits", __CAT_tracked_data, __SULTAN_tracked_data);
    if (PlotSideView) event_display_yz("InitialHits", __CAT_tracked_data, __SULTAN_tracked_data);
  }

  if (PlotTrueTracks) {
    if (PlotTopView) event_display_xz("true", __CAT_tracked_data, __SULTAN_tracked_data);
    if (PlotSideView) event_display_yz("true", __CAT_tracked_data, __SULTAN_tracked_data);
  }

  if (PlotNemoTracks) {
    if (PlotTopView) event_display_xz("nemo", __CAT_tracked_data, __SULTAN_tracked_data);
    if (PlotSideView) event_display_yz("nemo", __CAT_tracked_data, __SULTAN_tracked_data);
  }
  ...
  if (CAT_sc.size() == 0) {
    ...
    if (PlotCats || PlotHelices) {
      if (PlotTopView) event_display_xz("cats", __CAT_tracked_data, __SULTAN_tracked_data);
      if (PlotSideView) event_display_yz("cats", __CAT_tracked_data, __SULTAN_tracked_data);
    }
  ...
  if (SULTAN_sc.size() == 0) {
    ...
    if (PlotSultan) {
      if (PlotTopView) event_display_xz("sultan", __CAT_tracked_data, __SULTAN_tracked_data);
      if (PlotSideView) event_display_yz("sultan", __CAT_tracked_data, __SULTAN_tracked_data);
    }
  }
  ...
  if (PlotCellChi2s && PlutsMode) {
    ...
    for (size_t i = 0; i < CAT_sequences_.size(); i++) {
      ...
      for (size_t j = 0; j < nnodes; j++) {
        ...
        std::string title = (std::string)("cell " + mybhep::to_string(cid) + " chi2 " +
                                          mybhep::to_string_precision(helix_chi2s[j], "2"));
...
//*************************************************************
void EventDisplay::event_display_xz(std::string mode, CAT::topology::tracked_data CAT_td,
  ...
  if (PlotLegend) {
    if (mode == "cats" || mode == "sultan") leg_xz->Clear();
    if (PlotTrueTracks && mode == "true") leg_xz_true->Clear();
  ...
  if (PlotCalos) draw_calos_xz(CAT_td.get_calos());

  if (mode == "InitialHits" && PlotTangents) draw_tangents_xz();
  if (mode == "InitialHits" && PlotTriplets) draw_triplets_xz();

  if (mode == "nemo" && PlotNemoTracks) draw_helices_xz(mode);

  if (mode == "cats" && PlotCats) draw_cats_xz(mode, CAT_td.get_true_sequences());

  if (mode == "cats" && PlotHelices) draw_helices_xz(mode);

  if (mode == "sultan" && PlotSultan) draw_sultan_xz();
  ...
  if (PlotLegend) {
    locate_legend_xz();
    if (mode == "cats" || mode == "sultan") leg_xz->Draw("same");
    if (PlotTrueTracks && mode == "true ") leg_xz_true->Draw("same");
  }
...
//*************************************************************
void EventDisplay::draw_circle_xz(double x0, double z0, double radius, size_t color,
                                  size_t thickness = 1, double phi1 = 0., double phi2 = 2. * M_PI) {
  //*************************************************************
...
//*************************************************************
void EventDisplay::draw_sine_yz(double y0, double z0, double radius, double pitch, size_t color,
                                size_t thickness = 1, double phi1 = 0., double phi2 = 2. * M_PI) {
  //*************************************************************
...
void EventDisplay::draw_initial_hits_xz(void) {
  ...
  for (size_t i = 0; i < npoints; i++)  // loop on hits
  {
    ...
    if (PlotCellIds) {
      TPaveText* pt = new TPaveText(get_z_ndc(z[index]) - 0.01, get_x_ndc(x[index]) - 0.01,
                                    get_z_ndc(z[index]) + 0.01, get_x_ndc(x[index]) + 0.01, "NDC");
...
void EventDisplay::draw_calos_xz(std::vector<CAT::topology::calorimeter_hit> calos) {
  ...
  for (size_t i = 0; i < npoints; i++)  // plotting calorimeter hits
  {
    ...
    FirstStarXY = new TMarker(h.pl().center().z().value(), h.pl().center().x().value(), 3);
    FirstStarXY->Draw();
    ...
    if (SuperNemo) {
      ...
      zcal[0] = h.pl().center().z().value() + offset * sin(theta) - local_calo_x / 2. * sin(theta) +
                local_calo_z / 2. * cos(theta);
      zcal[1] = h.pl().center().z().value() + offset * sin(theta) + local_calo_x / 2. * sin(theta) +
                local_calo_z / 2. * cos(theta);
      zcal[2] = h.pl().center().z().value() + offset * sin(theta) + local_calo_x / 2. * sin(theta) -
                local_calo_z / 2. * cos(theta);
      zcal[3] = h.pl().center().z().value() + offset * sin(theta) - local_calo_x / 2. * sin(theta) -
                local_calo_z / 2. * cos(theta);
      zcal[4] = h.pl().center().z().value() + offset * sin(theta) - local_calo_x / 2. * sin(theta) +
                local_calo_z / 2. * cos(theta);

      xcal[0] = h.pl().center().x().value() + offset * cos(theta) - local_calo_x / 2. * cos(theta) -
                local_calo_z / 2. * sin(theta);
      xcal[1] = h.pl().center().x().value() + offset * cos(theta) + local_calo_x / 2. * cos(theta) -
                local_calo_z / 2. * sin(theta);
      xcal[2] = h.pl().center().x().value() + offset * cos(theta) + local_calo_x / 2. * cos(theta) +
                local_calo_z / 2. * sin(theta);
      xcal[3] = h.pl().center().x().value() + offset * cos(theta) - local_calo_x / 2. * cos(theta) +
                local_calo_z / 2. * sin(theta);
      xcal[4] = h.pl().center().x().value() + offset * cos(theta) - local_calo_x / 2. * cos(theta) -
                local_calo_z / 2. * sin(theta);
    }
    ...
    if (PlotCaloEnergy) {
      TPaveText* pt = new TPaveText(get_z_ndc((zcal[0] + zcal[2]) / 2.) - 0.01,
                                    get_x_ndc((xcal[0] + xcal[2]) / 2.) - 0.05,
                                    get_z_ndc((zcal[0] + zcal[2]) / 2.) + 0.01,
                                    get_x_ndc((xcal[0] + xcal[2]) / 2.) + 0.05, "NDC");
      pt->SetFillColor(4000);
...
void EventDisplay::draw_calos_yz(std::vector<CAT::topology::calorimeter_hit> calos) {
  ...
  for (size_t i = 0; i < npoints; i++)  // plotting calorimeter hits
  {
    ...
    FirstStarXY = new TMarker(h.pl().center().z().value(), h.pl().center().y().value(), 3);
    FirstStarXY->Draw();
    ...
    if (SuperNemo) {
      ...
      zcal[0] = h.pl().center().z().value() + offset * sin(theta) -
                local_calo_x / 2. * std::abs(sin(theta)) - local_calo_z / 2. * std::abs(cos(theta));
      zcal[1] = h.pl().center().z().value() + offset * sin(theta) +
                local_calo_x / 2. * std::abs(sin(theta)) + local_calo_z / 2. * std::abs(cos(theta));
      zcal[2] = h.pl().center().z().value() + offset * sin(theta) +
                local_calo_x / 2. * std::abs(sin(theta)) + local_calo_z / 2. * std::abs(cos(theta));
      zcal[3] = h.pl().center().z().value() + offset * sin(theta) -
                local_calo_x / 2. * std::abs(sin(theta)) - local_calo_z / 2. * std::abs(cos(theta));
      zcal[4] = h.pl().center().z().value() + offset * sin(theta) -
                local_calo_x / 2. * std::abs(sin(theta)) - local_calo_z / 2. * std::abs(cos(theta));
    }
    ...
    if (PlotCaloEnergy) {
      TPaveText* pt = new TPaveText(get_z_ndc((zcal[0] + zcal[2]) / 2.) - 0.01,
                                    ...
                                    get_z_ndc((zcal[0] + zcal[2]) / 2.) + 0.01,
                                    get_y_ndc((ycal[0] + ycal[2]) / 2.) + 0.05, "NDC");
      pt->SetFillColor(4000);
      pt->SetTextColor(color_calos);
      pt->SetTextSize(0.03);
      //      pt->SetMargin(0.3);
...
void EventDisplay::draw_helices_xz(std::string mode) {
  ...
  if (mode == "cats") {
    for (std::vector<CAT::topology::sequence>::iterator iclu = CAT_sequences_.begin();
         iclu != CAT_sequences_.end(); ++iclu) {
      ...
      if (iclu->has_decay_helix_vertex()) pf = iclu->decay_helix_vertex();
    ...
    for (std::vector<CAT::topology::sequence>::iterator iclu = nemo_sequences_.begin();
         iclu != nemo_sequences_.end(); ++iclu) {
      if (iclu->nodes_.size() <= 2) {
        continue;
      }
...
void EventDisplay::draw_helices_yz(std::string mode) {
  ...
  if (mode == "cats") {
    for (std::vector<CAT::topology::sequence>::iterator iclu = CAT_sequences_.begin();
      ...
      CAT::topology::experimental_point pi = iclu->nodes().front().ep();
      if (iclu->has_helix_vertex()) pi = iclu->helix_vertex();
      CAT::topology::experimental_point pf = iclu->nodes().back().ep();
      if (iclu->has_decay_helix_vertex()) pf = iclu->decay_helix_vertex();
...
void EventDisplay::draw_cats_xz(std::string mode, std::vector<CAT::topology::sequence> true_seqs) {
  ...
  if (mode == "true") {
    for (size_t i = 0; i < true_seqs.size(); i++) {
      ...
      graphlist.push_back(graph);
      if (PlotLegend) leg_xz_true->AddEntry(graph, s.name().c_str(), "l");
      free(xt);
      free(zt);
    }
    ...
    for (size_t i = 0; i < CAT_sequences_.size(); i++) {
      ...
      graph->SetLineColor(color + i);
      graph->SetLineWidth(2);
      if (PlotLegend) leg_xz->AddEntry(graph, s.name().c_str(), "l");
      graphlist.push_back(graph);
    ...
    for (size_t i = 0; i < nemo_sequences_.size(); i++) {
      ...
      if (s.has_helix_vertex() && !(std::abs(s.helix_vertex().x().value()) < 0.1 &&
                                    std::abs(s.helix_vertex().z().value() < 0.1)))
...
//*************************************************************
void EventDisplay::event_display_yz(std::string mode, CAT::topology::tracked_data CAT_td,
                                    SULTAN::topology::tracked_data SULTAN_td) {
  //*************************************************************
  ...
  if (PlotLegend) {
    init_quadrant_counters();
    if (mode == "cats" || mode == "sultan") leg_yz->Clear();
    if (mode == "true" && PlotTrueTracks) leg_yz_true->Clear();
  }
  ...
  if (PlotCalos) draw_calos_yz(CAT_td.get_calos());

  if (mode == "InitialHits" && PlotTangents) draw_tangents_yz();
  if (mode == "InitialHits" && PlotTriplets) draw_triplets_yz();

  if (mode == "cats" && PlotHelices) draw_helices_yz(mode);

  if (mode == "nemo" && PlotNemoTracks) draw_helices_yz(mode);

  if (mode == "cats" && PlotCats) draw_cats_yz(mode, CAT_td.get_true_sequences());

  if (mode == "sultan" && PlotSultan) draw_sultan_yz();

  if (mode == "true" && PlotTrueTracks) draw_cats_yz(mode, CAT_td.get_true_sequences());

  if (PlotLegend) {
    locate_legend_yz();
    if (mode == "cats" || mode == "sultan") leg_yz->Draw("same");
    if (mode == "true" && PlotTrueTracks) leg_yz_true->Draw("same");
  }
  canvas->Update();
...
void EventDisplay::draw_initial_hits_yz(void) {
  ...
  for (size_t i = 0; i < npoints; i++)  // loop on hits
  {
    ...
    if (PlotCellIds) {
      TPaveText* pt =
          new TPaveText(get_z_ndc(c.ep().z().value()) - 0.01, get_y_ndc(y[0]) - 0.01,
                        get_z_ndc(c.ep().z().value()) + 0.01, get_y_ndc(y[0]) + 0.01, "NDC");
      pt->SetFillColor(4000);
      pt->SetTextSize(0.02);
      //       pt->SetTextAlign(12);
...
void EventDisplay::draw_cats_yz(std::string mode, std::vector<CAT::topology::sequence> true_seqs) {
  ...
  if (mode == "true") {
    for (size_t i = 0; i < true_seqs.size(); i++) {
      ...
      graph->SetLineWidth(2);
      graphlist.push_back(graph);
      if (PlotLegend) leg_yz_true->AddEntry(graph, s.name().c_str(), "l");
    ...
    for (size_t i = 0; i < CAT_sequences_.size(); i++) {
      ...
      graph->SetLineColor(color + i);
      graph->SetLineWidth(2);
      if (PlotLegend) leg_yz->AddEntry(graph, s.name().c_str(), "l");
      graphlist.push_back(graph);
