...
int main(int argc_, char* argv_[]){
  ...
  CAT::topology::cell ca(pa, a+b, 0);  ca.set_probmin(1.e-200);
  CAT::topology::cell cb(pb, d+e, 1);  cb.set_probmin(1.e-200);
  CAT::topology::cell cc(pc, g+h, 2);  cc.set_probmin(1.e-200);

  CAT::topology::line l(pa, pb); l.set_probmin(1.e-200);
  cout << " l: "; l.dump(); cout << " -l: "; l.invert().dump(); cout << " " << endl;
  CAT::topology::line l2(pb, pc); l.set_probmin(1.e-200);
  CAT::topology::line l3(pa, pc); l.set_probmin(1.e-200);
  ...
  std::vector<CAT::topology::cell_couplet> cl1; cl1.push_back(cca);
  std::vector<CAT::topology::cell_couplet> cl2; cl2.push_back(ccb);
  std::vector<CAT::topology::cell_couplet> cl3; cl3.push_back(ccc);

  std::vector<CAT::topology::cell_triplet> ctl1; ctl1.clear();
  std::vector<CAT::topology::cell_triplet> ctl2; ctl2.clear();
  std::vector<CAT::topology::cell_triplet> ctl3; ctl3.clear();

  CAT::topology::cell_triplet cccc(ca, cb, cc); cccc.set_probmin(1.e-200);
  double Ratio = 3.;
  cccc.calculate_joints(Ratio);
  cout << " cccc: "; cccc.dump(); cout << " -cccc: "; cccc.invert().dump(); cout << " " << endl;

  CAT::topology::node na(ca, cl1, ctl1); na.set_probmin(1.e-200); na.calculate_triplets(Ratio);

  cout << " na: "; na.dump(); cout << " -na: "; na.invert().dump(); cout << " " << endl;
  CAT::topology::node nb(cb, cl2, ctl2); nb.set_probmin(1.e-200); nb.calculate_triplets(Ratio);
  CAT::topology::node nc(cc, cl3, ctl3); nc.set_probmin(1.e-200); nc.calculate_triplets(Ratio);
  ...
  CAT::topology::cluster cu(nodes); cu.set_probmin(1.e-200);
