...
namespace snemo {
  ...
  namespace visualization {
    ...
    void toy_display_driver::generate_data_files_ ()
    {
      ...
      if (get_current_event_record_ ().has (snemo::datamodel::data_info::SIMULATED_DATA_LABEL)) {
        ...
        // Draw the vertex:
        {
          ...
          if (geomtools::is_valid(vertex)) {
            temp_draw_file->out () << "# 'SD.vertex' @ index " << index << ": " << std::endl;
        ...
        if (setup_label == snemo_demo_label()) {
          ...
          if (the_sd.has_step_hits (hit_category) && the_sd.get_step_hits (hit_category).size ()) {
            ...
            for (mctools::simulated_data::hit_handle_collection_type::const_iterator i
              ...
              if (count == 0) {
                temp_draw_file->out () << "# 'SD." << hit_category << "' step hits @ index " << index << ": " << std::endl;
              }
        ...
        if (scin_hits.size () > 0) {
          ...
          for (std::map<geomtools::geom_id, bool>::const_iterator i = scin_hits.begin ();
            ...
            if (count == 0) {
              temp_draw_file->out () << "# 'SD.scin_hit' @ index " << index << ": " << std::endl;
            }
            if (shape.has_user_draw ()) {
              ...
              (*user_draw_function) (temp_draw_file->out (),
                                     ...
                                     shape,
                                     0);
              temp_draw_file->out () << "# test 1 " << std::endl;
              ...
              //temp_draw_file->out () << "# test 2 " << std::endl;
            }
            count++;
          }
          if (count) {
        ...
        if (gg_hits.size () > 0) {
          ...
          for (std::map<geomtools::geom_id, geomtools::vector_3d>::const_iterator i = gg_hits.begin ();
            ...
            if (count == 0) {
              temp_draw_file->out () << "# 'SD.gg_hit' @ index " << index << ": " << std::endl;
            }
          ...
          if (the_sd.has_step_hits (hit_category) && the_sd.get_step_hits (hit_category).size ()) {
            ...
            for (mctools::simulated_data::hit_handle_collection_type::const_iterator i
              ...
              if (BSH.get_auxiliaries ().has_key (snemo::datamodel::gg_track::minimum_approach_position_key())) {
                ...
                if (count == 0) {
                  temp_draw_file->out () << "# 'SD." << hit_category << ".MAP' @ index " << index << ": " << std::endl;
                }
          ...
          if (the_sd.has_step_hits (hit_category) && the_sd.get_step_hits (hit_category).size ()) {
            ...
            for (std::map<std::string, std::ostringstream *>::iterator i = VP_streams.begin ();
              ...
              if (VP_counters[pname] > 0) {
                temp_draw_file->out () << "# 'SD." << hit_category
      ...
      // Calibrated data :
      if (get_current_event_record_ ().has (snemo::datamodel::data_info::CALIBRATED_DATA_LABEL)) {
        DT_LOG_DEBUG (logging_priority, "has CALIBRATED_DATA_LABEL !");
        //std::cerr << "********* DEVEL: has CALIBRATED_DATA_LABEL" << std::endl;
          ...
          if (CCHC.size ()) {
            ...
            for (snemo::datamodel::calibrated_data::calorimeter_hit_collection_type::const_iterator i
              ...
              if (scin_block_shape.has_user_draw ()) {
                ...
                //                 <<  scin_block_gid << "' !");
                geomtools::gnuplot_draw::draw (temp_draw_file->out (), scin_block_world_plcmt, scin_block_shape);
              }
              ++hit_count;
            }
            if (hit_count > 0) {
      ...
      if (get_current_event_record_ ().has (snemo::datamodel::data_info::TRACKER_CLUSTERING_DATA_LABEL))  {
    ...
    void toy_display_driver::display_ ()
    {
      ...
      if (display_title) {
        ...
        title_ss << "SuperNEMO event display (Gnuplot version)";
