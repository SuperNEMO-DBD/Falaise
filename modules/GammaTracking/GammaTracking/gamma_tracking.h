...
namespace gt {
  ...
  public:
    ...
    /*!<
      ...
      \param exclude_ is the list_type to exclude from the calculations, and so, from the final
      result. \param deathless_starts_ means that the starts can be used in all gt. Usefull if starts
      represents vertexes instead of PM numbers.

      \attribute gamma_tracking::_extern_ allow only gamma tracked with starts, no elsewhere\n
      gamma_tracking::_starts_ will be merged with starts_, and will be taken in care with extern and
      other
