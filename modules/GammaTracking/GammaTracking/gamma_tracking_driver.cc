                                             ...
                                             snemo::datamodel::particle_track_data & ptd_)
    {
      ...
      for (const auto& a_list : gamma_tracks) {
        ...
        for (const int calo_id : a_list) {
          ...
          hNewVertex->set_hit_id(calo_id);
          hNewVertex->set_geom_id((*found)->get_geom_id());
          const gt::event::calorimeter_collection_type & the_gamma_calos =
            gtAlgo_.get_event().get_calorimeters();
          hNewVertex->set_category(snemo::datamodel::string_to_vertex_category(the_gamma_calos.at(calo_id).label));
          hNewVertex->get_spot().set_blur_dimension(geomtools::blur_spot::dimension_three);
          hNewVertex->get_spot().set_position(the_gamma_calos.at(calo_id).position);
