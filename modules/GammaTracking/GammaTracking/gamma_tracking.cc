...
namespace gt {
  ...
  bool gamma_tracking::sort_reflect(const list_type & ref1_,
                                    const list_type & ref2_)
  {
  ...
  {
    const event::calorimeter_collection_type & the_gamma_calos
      = _event_.get_calorimeters();

    if (the_gamma_calos.size() == 1) {
      add(the_gamma_calos.begin()->first);
    } else {
      for (auto icalo = the_gamma_calos.begin();
           icalo != the_gamma_calos.end(); ++icalo) {
        for (auto jcalo = boost::next(icalo);
