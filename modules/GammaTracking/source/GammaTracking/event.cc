...
const event::calorimeter_collection_type& event::get_calorimeters() const { return _calos_; }

event::calorimeter_collection_type& event::grab_calorimeters() { return _calos_; }
...
std::ostream& operator<<(std::ostream& out_, const event& event_) {
  ...
  for (event::calorimeter_collection_type::const_iterator icalo = the_calos.begin();
       icalo != the_calos.end(); ++icalo) {
    out_ << icalo->second;
  }
  return out_;
}
