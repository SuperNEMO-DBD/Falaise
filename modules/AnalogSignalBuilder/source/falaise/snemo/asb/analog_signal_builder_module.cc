...
struct analog_signal_builder_module::driver_entry {
 public:
  driver_entry(analog_signal_builder_module &parent_, const std::string &name_,
               const std::string &type_id_, const datatools::properties &config_);
  ~driver_entry();
...
void analog_signal_builder_module::driver_entry::_initialize_() {
  if (!_handle_.has_data()) {
    DT_THROW_IF(_type_id_.empty(), std::logic_error,
                "Missing type identifier for driver '" << _name_ << "'!");
    ...
    DT_THROW_IF(!sys_factory_register.has(_type_id_), std::logic_error,
                "Unknown signal generator driver class with id = '" << _type_id_ << "' for driver '"
                                                                    << _name_ << "'!");
...
const base_signal_generator_driver &analog_signal_builder_module::driver_entry::get_driver() const {
...
base_signal_generator_driver &analog_signal_builder_module::driver_entry::grab_driver() {
  _initialize_();
...
void analog_signal_builder_module::initialize(const datatools::properties &config_,
  ...
  /*if (_db_manager_ == nullptr) */ {
    ...
    // const snemo::XXX::::database_service & Db =
    // service_manager_.get<snemo::XXX::database_service>(_Db_label_);
...
void analog_signal_builder_module::_init_drivers_(
    const datatools::properties &config_, datatools::service_manager & /*service_manager_ */) {
...
const std::string &analog_signal_builder_module::get_sd_label() const { return _SD_label_; }
...
const std::string &analog_signal_builder_module::get_ssd_label() const { return _SSD_label_; }
...
const std::string &analog_signal_builder_module::get_geo_label() const { return _Geo_label_; }
...
const std::string &analog_signal_builder_module::get_db_label() const { return _Db_label_; }
...
void analog_signal_builder_module::add_driver(const std::string &name_, const std::string &type_id_,
                                              const datatools::properties &config_) {
  DT_THROW_IF(is_initialized(), std::logic_error,
              "Module '" << get_name() << "' is already initialized !");
...
// Processing :
dpp::base_module::process_status analog_signal_builder_module::process(
...
void analog_signal_builder_module::_process_(const mctools::simulated_data &sim_data_,
  ...
  for (driver_dict_type::iterator idriver = _drivers_.begin(); idriver != _drivers_.end();
       idriver++) {
