...
int main( int  argc_ , char **argv_  )
{
  ...
  try {
    ...
    std::string calo_signal_generator_config_filename = "@falaise:modules/AnalogSignalBuilder/calo_signal_generator.conf";
    datatools::fetch_path_with_env(calo_signal_generator_config_filename);
    std::string calo_signal_shape_builder_config_filename = "@falaise:modules/AnalogSignalBuilder/calo_signal_shape_builder.conf";
	...
	if (ER.has(SD_bank_label) && ER.is_a<mctools::simulated_data>(SD_bank_label)) 
	  {
		...
		for (unsigned int isignal = 0; isignal < SSD.get_number_of_signals("calo"); isignal++)
		  {
		    ...
		    my_signal.get_auxiliaries().export_and_rename_starting_with(signal_shape_properties, mctools::signal::base_signal::shape_parameter_prefix(), "");
		    
		    std::string unique_signal_key = "Event_" + std::to_string(psd_count) + "_Signal_" + std::to_string(isignal);
		    // ssb_1.create_signal_shape(unique_signal_key, "mctools::signal::triangle_signal_shape", signal_shape_properties);
		    ssb_2.create_signal_shape(unique_signal_key, "mctools::signal::triangle_signal_shape", signal_shape_properties);
		...
		for (const auto & fkey : fkeys) {
		  tmp_file.out() << "#" << fkey << ":\n";
		  ssb_2.get_functor(fkey).write_ascii_with_units(tmp_file.out(),
