...
geomtools::helix_3d generate_helix_geiger_hits(mygsl::rng& random_, TrackFit::gg_hits_col& hits_,
                                               size_t nhits_, double rcell_,
                                               TrackFit::default_drift_time_calibration& dtc_,
                                               bool draw_ = false, bool no_z_ = false,
                                               bool bad_ = false);
...
int main(int argc_, char** argv_) {
  ...
  try {
    ...
    if (devel) std::clog << "DEVEL: rmax = " << dtc.rmax / CLHEP::mm << " mm" << std::endl;

    // generate fake Geiger hits:
    TrackFit::gg_hits_col hits;
    geomtools::helix_3d trajectory =
        generate_helix_geiger_hits(random, hits, nhits, dtc.rmax, dtc, draw, no_z, bad);
    ...
    std::vector<TrackFit::helix_fit_params> guess(
        TrackFit::helix_fit_mgr::guess_utils::NUMBER_OF_GUESS);
    ...
    if (do_fit) {
      ...
      for (int iguess = 0; iguess < max_guess; iguess++) {
        ...
        if (HFM.get_solution().ok) {
          ...
          std::clog << "NOTICE:   Chi     = " << HFM.get_solution().chi << std::endl;
          std::clog << "NOTICE:   N(dof)  = " << HFM.get_solution().ndof << std::endl;
          std::clog << "NOTICE:   Prob(P) = " << HFM.get_solution().probability_p() << std::endl;
          std::clog << "NOTICE:   Prob(Q) = " << HFM.get_solution().probability_q() << std::endl;
    ...
    geomtools::vector_3d center(best_solution.x0, best_solution.y0, best_solution.z0);
    ...
    if (draw) {
      ...
      cmd_oss << " '" << ftmp.get_filename()
              << "' index 0 using 1:2 title \"Best solution\" with lines , ";
...
geomtools::helix_3d generate_helix_geiger_hits(mygsl::rng& random_, TrackFit::gg_hits_col& hits_,
                                               size_t nhits_, double rcell_,
                                               TrackFit::default_drift_time_calibration& dtc_,
                                               bool draw_, bool no_z_, bool bad_) {
  bool devel = false;
  bool draw = draw_;
