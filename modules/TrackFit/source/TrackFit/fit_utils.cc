...
namespace TrackFit {
                                             ...
                                             const geomtools::vector_3d & top_pos_)
  {
    ...
    for (gg_hits_col::const_iterator it_hit = hits_.begin();
      ...
      if (distance > it_hit->get_rmax() && distance < min_distance) {
        DT_LOG_TRACE(local_priority, "Candidate hit is in the vicinity of the X-Y reference position !");
        ...
        if (bottom_current_distance < top_current_distance)      near_bottom_counter++;
        else if (top_current_distance < bottom_current_distance) near_top_counter++;
      }
    }

    DT_LOG_TRACE(local_priority, "near_bottom_counter= " << near_bottom_counter << " near_top_counter= " << near_top_counter);
  ...
  double fit_utils::get_vicinity_factor() const
  {
    return _vicinity_factor_;
  }
  ...
  {
    if (hit == 0) {
      out_ << "trackfit::gg_hit_info::dump: " << "No hit !" << std::endl;
      return;
    }
    out_ << "trackfit::gg_hit_info::dump: " << "Hit #" << hit->get_id() << std::endl;
    out_ << "trackfit::gg_hit_info::dump: " << "Closest hits [" << closest_hits_map.size() << "] : "
         << std::endl;
    for (std::map<double,const gg_hit *>::const_iterator i = closest_hits_map.begin();
         i !=  closest_hits_map.end();
         i++) {
