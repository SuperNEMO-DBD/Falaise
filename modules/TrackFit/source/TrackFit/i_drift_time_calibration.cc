...
bool i_drift_time_calibration::radius_is_valid(double radius_) const { return radius_ >= 0.0; }
...
double f_time_radius::operator()(double u_) const {
  ...
  if (mode == DRIFT_TIME_TO_RADIUS) {
    const double t = u_;
    DT_THROW_IF(t < x0, std::logic_error,
...
double default_drift_time_calibration::get_max_cell_radius() const { return rmax; }

void default_drift_time_calibration::drift_time_to_radius(double time_, double& radius_,
...
double new_drift_time_calibration::get_max_cell_radius() const { return rmax; }

void new_drift_time_calibration::drift_time_to_radius(double time_, double& radius_,
                                                      double& sigma_radius_) const {
  const double time = time_;
  radius_ = td(time);
  sigma_radius_ = sigma_r;
