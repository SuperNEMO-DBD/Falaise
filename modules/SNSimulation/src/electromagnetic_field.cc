...
void electromagnetic_field::set_name(const std::string& name_) { _name_ = name_; }

auto electromagnetic_field::get_name() const -> const std::string& { return _name_; }

void electromagnetic_field::set_field_check_pos_time(bool c_) { _field_check_pos_time_ = c_; }

auto electromagnetic_field::is_field_check_pos_time() const -> bool {
  return _field_check_pos_time_;
}

auto electromagnetic_field::is_initialized() const -> bool { return _initialized_; }

auto electromagnetic_field::has_field() const -> bool { return _field_ != nullptr; }

void electromagnetic_field::set_field(const emfield::base_electromagnetic_field& f_) {
  _field_ = &f_;
}

auto electromagnetic_field::get_field() const -> const emfield::base_electromagnetic_field& {
  DT_THROW_IF(!has_field(), std::logic_error, "No electromagnetic field !");
  return *_field_;
}

auto electromagnetic_field::is_active() const -> bool { return has_field(); }
        ...
        if (mode_str == "standalone_electric_field") {
          ...
          DT_THROW(std::logic_error,
                   "Invalid value '" << mode_str
                                     << "' for 'mode' property in Geant4 electromagnetic field '"
                                     << get_name() << "' !");
        }
      } else {
        DT_THROW(std::logic_error, "Missing 'mode' property in Geant4 electromagnetic field '"
                                       << get_name() << "' !");
      }
        ...
        if (!geomtools::is_valid(_standalone_constant_electric_field_)) {
          if (config_.has_key("standalone_constant_electric_field")) {
            ...
            DT_THROW_IF(seg.size() != 3, std::logic_error,
                        "Invalid dimension for vector of constant electric field coordinates in "
...
// G4 interface:
void electromagnetic_field::GetFieldValue(const double position_[4], double* em_field_) const {
  ...
  if (_field_ != nullptr) {
    ...
    if (_field_->is_magnetic_field()) {
      ...
      DT_LOG_TRACE(_logprio(), "Geant4 magnetic field '"
                                   << get_name() << "' is : " << the_magnetic_field / CLHEP::gauss
                                   << " gauss");
    ...
    if (_field_->is_electric_field()) {
      ...
      DT_THROW_IF(status != 0, std::logic_error,
                  "Electric field named '" << _name_ << "' cannot compute electric field value at "
                                           << pos / CLHEP::mm << " [mm] / "
                                           << time / CLHEP::nanosecond << " [ns] !");
      DT_LOG_TRACE(_logprio(), "Geant4 electric field '"
                                   << get_name() << "' is : "
