...
loggable_support::~loggable_support() = default;

loggable_support::loggable_support(datatools::logger::priority p) { _logging_priority = p; }

auto loggable_support::_logprio() const -> datatools::logger::priority { return _logging_priority; }
...
void loggable_support::set_logging_priority(const std::string& priority_label_) {
  ...
  DT_THROW_IF(p == datatools::logger::PRIO_UNDEFINED, std::logic_error,
