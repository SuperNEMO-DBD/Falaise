...
void magnetic_field::set_name(const std::string& name_) { _name_ = name_; }

auto magnetic_field::get_name() const -> const std::string& { return _name_; }

void magnetic_field::set_field_check_pos_time(bool c_) { _field_check_pos_time_ = c_; }

auto magnetic_field::is_field_check_pos_time() const -> bool { return _field_check_pos_time_; }

auto magnetic_field::is_initialized() const -> bool { return _initialized_; }

auto magnetic_field::has_field() const -> bool { return _field_ != nullptr; }

auto magnetic_field::has_mag_field() const -> bool { return _field_ != nullptr; }

void magnetic_field::set_mag_field(const emfield::base_electromagnetic_field& mf_) {
  _field_ = &mf_;
}

void magnetic_field::set_field(const emfield::base_electromagnetic_field& mf_) { _field_ = &mf_; }

auto magnetic_field::get_field() const -> const emfield::base_electromagnetic_field& {
  DT_THROW_IF(!has_field(), std::logic_error, "No field !");
  return *_field_;
}

auto magnetic_field::get_mag_field() const -> const emfield::base_electromagnetic_field& {
  DT_THROW_IF(!has_mag_field(), std::logic_error, "No magnetic field !");
  return *_field_;
}

auto magnetic_field::is_active() const -> bool { return has_mag_field(); }
...
// G4 interface:
void magnetic_field::GetFieldValue(const double position_[4], double* b_field_) const {
  DT_LOG_TRACE(_logprio(),
               "Entering GetFieldValue for Geant4 magnetic field '" << get_name() << "'...");
  // DT_LOG_TRACE(datatools::logger::PRIO_ALWAYS, "Entering GetFieldValue for Geant4 magnetic field
  // '" << get_name() << "'...");
  DT_THROW_IF(!_initialized_, std::logic_error,
              "Geant4 magnetic field '" << get_name() << "' is not initialized !");
  ...
  if (_field_ != nullptr) {
    ...
    if (_field_check_pos_time_) {
      DT_THROW_IF(!_field_->position_and_time_are_valid(pos, time), std::logic_error,
    ...
    DT_THROW_IF(status != 0, std::logic_error,
                "Magnetic field named '" << _name_ << "' cannot compute magnetic field value at "
                                         << pos / CLHEP::mm << " [mm] / "
                                         << time / CLHEP::nanosecond << " [ns] !");
    DT_LOG_TRACE(_logprio(), "Geant4 magnetic field '" << get_name() << "' is : "
                                                       << the_b_field / CLHEP::gauss << " gauss");
