...
auto manager::is_initialized() const -> bool { return _initialized_; }

auto manager::is_automatic() const -> bool { return is_batch() && _g4_macro_.empty(); }

//----------------------------------------------------------------------
// Simulation Initializations (Geometry/Physics, possibly via services)
//----------------------------------------------------------------------
auto manager::has_service_manager() const -> bool { return _service_manager_ != nullptr; }
...
auto manager::grab_service_manager() -> datatools::service_manager& {
  DT_THROW_IF(_service_manager_ == nullptr, std::logic_error, "No service manager is available !");
...
void manager::set_external_geom_manager(const geomtools::manager& gm) {
  ...
  DT_THROW_IF(!gm.is_initialized(), std::logic_error,
...
auto manager::get_geom_manager() const -> const geomtools::manager& {
  if (has_external_geom_manager()) {
    DT_THROW_IF(!_external_geom_manager_->is_initialized(), std::logic_error,
                "External geometry manager is not initialized !");
...
auto manager::get_eg_manager() const -> const genbb::manager& { return _eg_manager_; }

auto manager::grab_eg_manager() -> genbb::manager& { return _eg_manager_; }

auto manager::has_event_generator() const -> bool { return _event_generator_ != nullptr; }

auto manager::get_event_generator() const -> const genbb::i_genbb& {
  DT_THROW_IF(!this->has_event_generator(), std::logic_error, "No event generator is available !");
...
auto manager::get_vg_manager() const -> const genvtx::manager& { return _vg_manager_; }

auto manager::grab_vg_manager() -> genvtx::manager& { return _vg_manager_; }

auto manager::has_vertex_generator() const -> bool { return _vertex_generator_ != nullptr; }

auto manager::get_vertex_generator() const -> const genvtx::i_vertex_generator& {
  DT_THROW_IF(!this->has_vertex_generator(), std::logic_error,
...
auto manager::get_number_of_events() const -> uint32_t { return _number_of_events_; }

void manager::set_number_of_events(uint32_t n) {
  DT_THROW_IF(_initialized_, std::logic_error, "Operation prohibited ! Manager is locked !");
  DT_THROW_IF(n < NUMBER_OF_EVENTS_LOWER_LIMIT, std::domain_error,
              "Invalid 'zero' number of events !");
  DT_THROW_IF(n > NUMBER_OF_EVENTS_UPPER_LIMIT, std::domain_error,
...
auto manager::get_number_of_events_modulo() const -> int { return _number_of_events_modulo_; }
...
auto manager::using_run_header_footer() const -> bool { return _use_run_header_footer_; }

auto manager::is_batch() const -> bool { return !is_interactive(); }

auto manager::is_interactive() const -> bool { return _interactive_; }
...
auto manager::has_g4_macro() const -> bool { return !_g4_macro_.empty(); }

auto manager::get_g4_macro() const -> const std::string& { return _g4_macro_; }
...
void manager::set_use_track_history(const bool use) { _use_track_history_ = use; }

auto manager::has_track_history() const -> bool { return _use_track_history_; }

auto manager::get_track_history() const -> const track_history& { return _track_history_; }

auto manager::grab_track_history() -> track_history& { return _track_history_; }

auto manager::forbids_private_hits() const -> bool { return _forbid_private_hits_; }
...
//----------------------------------------------------------------------
auto manager::has_simulation_ctrl() const -> bool { return _simulation_ctrl_ != nullptr; }

void manager::set_simulation_ctrl(simulation_ctrl& a_simulation_ctrl) {
  DT_THROW_IF(_initialized_, std::logic_error, "Operation prohibited ! Manager is locked !");
  DT_THROW_IF(has_simulation_ctrl(), std::logic_error,
...
//----------------------------------------------------------------------
auto manager::grab_vg_prng() -> mygsl::rng& { return _vg_prng_; }

auto manager::grab_eg_prng() -> mygsl::rng& { return _eg_prng_; }

auto manager::grab_shpf_prng() -> mygsl::rng& { return _shpf_prng_; }

auto manager::get_mgr_prng() const -> const mygsl::rng& { return _mgr_prng_; }

auto manager::grab_mgr_prng() -> mygsl::rng& { return _mgr_prng_; }

auto manager::get_g4_prng() const -> const g4_prng& { return _g4_prng_; }

auto manager::grab_g4_prng() -> g4_prng& { return _g4_prng_; }

auto manager::get_seed_manager() const -> const mygsl::seed_manager& { return _seed_manager_; }

auto manager::grab_seed_manager() -> mygsl::seed_manager& { return _seed_manager_; }
...
auto manager::get_prng_state_save_modulo() const -> int { return _prng_state_save_modulo_; }

auto manager::has_prng_state_save_modulo() const -> bool { return _prng_state_save_modulo_ > 0; }

void manager::reset_output_prng_seeds_file() { _output_prng_seeds_file_ = ""; }

void manager::reset_input_prng_seeds_file() { _input_prng_seeds_file_ = ""; }

auto manager::has_input_prng_seeds_file() const -> bool { return !_input_prng_seeds_file_.empty(); }

auto manager::get_input_prng_seeds_file() const -> const std::string& {
  return _input_prng_seeds_file_;
}

void manager::set_input_prng_seeds_file(const std::string& fn_) { _input_prng_seeds_file_ = fn_; }
...
void manager::set_output_prng_seeds_file(const std::string& fn_) { _output_prng_seeds_file_ = fn_; }

void manager::reset_output_prng_states_file() { _output_prng_states_file_ = ""; }
...
void manager::reset_input_prng_states_file() { _input_prng_states_file_ = ""; }
...
void manager::set_input_prng_states_file(const std::string& fn_) { _input_prng_states_file_ = fn_; }
...
auto manager::get_mgr_prng_seed() const -> int { return _mgr_prng_seed_; }

auto manager::has_init_seed_method() const -> bool { return !_init_seed_method_.empty(); }

void manager::set_init_seed_method(const std::string& method_) { _init_seed_method_ = method_; }

auto manager::get_init_seed_method() const -> const std::string& { return _init_seed_method_; }
...
void manager::set_output_data_file(const std::string& fn_) { _output_data_file_ = fn_; }

auto manager::using_time_stat() const -> bool { return _use_time_stat_; }
...
auto manager::get_CT_map() const -> const manager::CT_map& { return _CTs_; }

auto manager::grab_CT_map() -> manager::CT_map& { return _CTs_; }
...
void manager::add_supported_output_profile(const std::string& profile_id_,
  ...
  DT_THROW_IF(profile_id.find('+') != profile_id.npos, std::logic_error,
...
void manager::activate_output_profile(const std::string& id) {
  DT_THROW_IF(_initialized_, std::logic_error, "Operation prohibited ! Manager is locked !");
  DT_THROW_IF(!has_supported_output_profile(id), std::logic_error,
...
void manager::dump_base(std::ostream& os, const std::string& title,
  ...
  os << indent << "|-- G4 PRNG name: '"
     << (get_mgr_prng().is_initialized() ? get_mgr_prng().name() : "[none]") << "' " << std::endl;
  os << indent << "|-- PRNG seeds input file:   '" << _input_prng_seeds_file_ << "' " << std::endl;
  os << indent << "|-- PRNG seeds output file:  '" << _output_prng_seeds_file_ << "' " << std::endl;
  os << indent << "|-- PRNG states input file:  '" << _input_prng_states_file_ << "' " << std::endl;
  ...
  for (auto i = _supported_output_profile_ids_.begin(); i != _supported_output_profile_ids_.end();
       ++i) {
    auto j = i;
    j++;
    os << "|   ";
  ...
  for (auto i = _activated_output_profile_ids_.begin(); i != _activated_output_profile_ids_.end();
       ++i) {
    auto j = i;
    j++;
    os << "|   ";
      ...
      */
      ...
      // shoot events:
      {
        ...
        */
        ...
        DT_THROW_IF((effective_number_of_events == 0) || (effective_number_of_events > NO_LIMIT),
                    std::logic_error, "Invalid number of events for Geant4 !");
...
void manager::_init_core() {
  ...
  if (has_input_prng_seeds_file()) {
    ...
    if (has_output_prng_seeds_file()) {
      ...
      DT_THROW_IF(input_prng_seeds_file == test_output_prng_seeds_file, std::logic_error,
    ...
    if (boost::filesystem::exists(input_prng_seeds_file)) {
      ...
      DT_THROW_IF(!seeds_file, std::runtime_error,
      ...
      DT_THROW(std::runtime_error, "No PRNG seed dictionary input file '" << input_prng_seeds_file
                                                                          << "' to be loaded.");
    }
  }
  ...
  if (get_init_seed_method() == "current_time") {
    ...
    DT_THROW(std::logic_error, "Initialization method '" << get_init_seed_method()
                                                         << "' is unknown or not supported !");
  }
  DT_LOG_DEBUG(_logprio(), "Initialization of PRNG seeds is based on the '"
                               << get_init_seed_method() << "' method");
...
void manager::_init_geometry() {
  ...
  if (has_external_geom_manager()) {
    ...
    const datatools::properties& geometry_config = _multi_config_->get("geometry").get_properties();
    if (is_debug()) {
      DT_LOG_DEBUG(_logprio(), "Geometry configuration : ");
      geometry_config.tree_dump(std::clog);
    }
    DT_THROW_IF(!geometry_config.has_key("manager.config"), std::logic_error,
...
void manager::_init_vertex_generator() {
  ...
  if (_multi_config_->has_section("vertex_generator")) {
    ...
    if (vertex_generator_config.has_key("manager.config")) {
      ...
      std::string vtx_gtor_prop_filename = vertex_generator_config.fetch_string("manager.config");
    ...
    DT_THROW_IF(!_vg_manager_.is_initialized(), std::logic_error,
    ...
    DT_THROW_IF(!_vg_manager_.has_generator(vg_name), std::logic_error,
...
void manager::_init_event_generator() {
  ...
  DT_THROW_IF(!_multi_config_->has_section("event_generator"), std::logic_error,
  ...
  if (primary_generator_config.has_key("manager.config")) {
    // Using an external configuration file:
    std::string event_gtor_prop_filename = primary_generator_config.fetch_string("manager.config");
  ...
  DT_THROW_IF(!_eg_manager_.is_initialized(), std::logic_error,
  ...
  DT_THROW_IF(!_eg_manager_.has(eg_name), std::logic_error,
...
void manager::_init_detector_construction() {
  ...
  DT_THROW_IF(!_multi_config_->has_section("detector_construction"), std::logic_error,
...
void manager::_init_physics_list() {
  ...
  DT_THROW_IF(!_multi_config_->has_section("physics_list"), std::logic_error,
...
void manager::_init_run_action() {
  ...
  DT_THROW_IF(!_multi_config_->has_section("run_action"), std::logic_error,
...
void manager::_init_event_action() {
  ...
  DT_THROW_IF(!_multi_config_->has_section("event_action"), std::logic_error,
...
void manager::_init_primary_generator_action() {
  ...
  DT_THROW_IF(!_multi_config_->has_section("primary_generator_action"), std::logic_error,
...
void manager::_init_seeds() {
  ...
  // Checks:
  DT_THROW_IF(!mygsl::seed_manager::seed_is_valid(_vg_prng_seed_), std::logic_error,
              "Invalid vertex generator seed value !");
  DT_THROW_IF(!mygsl::seed_manager::seed_is_valid(_eg_prng_seed_), std::logic_error,
              "Invalid event generator seed value !");
  DT_THROW_IF(!mygsl::seed_manager::seed_is_valid(_mgr_prng_seed_), std::logic_error,
              "Invalid G4 generator seed value !");
  DT_THROW_IF(!mygsl::seed_manager::seed_is_valid(_shpf_prng_seed_), std::logic_error,
  ...
  if (has_output_prng_seeds_file()) {
    ...
    if (boost::filesystem::exists(seeds_filename)) {
      ...
      DT_LOG_NOTICE(_logprio(), "Backuping the former Random Number Generators seeds file '"
                                    << seeds_filename << "' to '" << backup_seeds_filename
                                    << "'...");
      if (boost::filesystem::exists(backup_seeds_filename)) {
        DT_LOG_NOTICE(_logprio(), "Deleting old backup Random Number Generators seeds file '"
...
void manager::_init_prngs_states() {
  ...
  if (!_prng_state_manager_.has_state(vertex_generator_label())) {
    _prng_state_manager_.add_state(vertex_generator_label(), _vg_prng_.get_internal_state_size());
  }
  // Event generator PRNG :
  if (!_prng_state_manager_.has_state(event_generator_label())) {
    _prng_state_manager_.add_state(event_generator_label(), _eg_prng_.get_internal_state_size());
  }
...
DOCD_CLASS_IMPLEMENT_LOAD_BEGIN(snsim::manager, ocd_) {
  ...
  ocd_.set_class_documentation(
      "The Geant4 simulation manager class embedes \n"
...

  {
    ...
    cpd.set_name_pattern("logging.priority")
        ...
        .add_example(
            "Explicitly set the logging threshold: ::  \n"
            "                                          \n"
            "  [name=\"manager\"]                      \n"
...

  {
    ...
    cpd.set_name_pattern("number_of_events")
        ...
        .set_mandatory(false)
        .set_long_description(
            "Allowed value: from ``1`` to ``100000000``                              \n"
            "                                                                        \n"
  ...
  ocd_.set_configuration_hints(
      "The simulation manager can be parameterized and initialized           \n"
