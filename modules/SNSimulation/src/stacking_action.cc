...
stacking_action::~stacking_action() { _g4_navigator_ = nullptr; }
...
auto stacking_action::ClassifyNewTrack(const G4Track* a_track) -> G4ClassificationOfNewTrack {
  ...
  if ((!_killer_volume_names_.empty()) && (_killer_volumes_.empty())) {
    ...
    for (const auto& _killer_volume_name : _killer_volume_names_) {
      ...
      DT_THROW_IF(g4_volume_log == nullptr, std::logic_error,
  ...
  if ((!_killer_material_names_.empty()) && (_killer_materials_.empty())) {
    ...
    for (const auto& _killer_material_name : _killer_material_names_) {
      ...
      boost::algorithm::replace_all(stmp, ":",
                                    "_");  // a trick for GDML does not support ':' in (XML) names.
      G4String g4_material_name = stmp.c_str();
      G4Material* g4_material = G4Material::GetMaterial(g4_material_name, false);
      DT_THROW_IF(g4_material == nullptr, std::logic_error,
...
DOCD_CLASS_IMPLEMENT_LOAD_BEGIN(snsim::stacking_action, ocd_) {
...

  {
    ...
    cpd.set_name_pattern("kill_particles.materials")
        .set_terse_description(
            "The list of materials of which logical volumes are made and where secondary particle "
            "killing is activated")
        .set_traits(datatools::TYPE_STRING, configuration_property_description::ARRAY)
        .set_mandatory(false)
        .set_triggered_by_flag("kill_particles")
        .set_long_description(
