...
auto primary_generator::is_initialized() const -> bool { return _initialized_; }

void primary_generator::set_run_action(snsim::run_action& run_action_) {
  DT_THROW_IF(_initialized_, std::logic_error, "Operation prohibited ! Manager is locked !");
  _run_action_ = &run_action_;
}

void primary_generator::set_event_action(snsim::event_action& event_action_) {
  DT_THROW_IF(_initialized_, std::logic_error, "Operation prohibited ! Manager is locked !");
...
auto primary_generator::get_event_counter() const -> size_t { return _event_counter_; }

void primary_generator::reset_event_counter() { _event_counter_ = 0; }

void primary_generator::_set_defaults() { reset_event_counter(); }
...
void primary_generator::_check() {
  if (has_vertex_generator()) {
    if (!_vertex_generator_->is_initialized()) {
      G4Exception("snsim::primary_generator::_check", "InitializationError", RunMustBeAborted,
...
void primary_generator::initialize(const datatools::properties& config_) {
  if (is_initialized()) {
    G4Exception("snsim::primary_generator::initialize", "InitializationError", RunMustBeAborted,
  ...
  if (config_.has_key("particle_names_map")) {
    ...
    for (const auto& mapping_entry : names) {
      ...
      DT_THROW_IF(
          genbb_particle_name.empty(), std::logic_error,
          "Invalid genbb particle name extracted from mapping entry '" << mapping_entry << "' !");
      DT_THROW_IF(
          g4_particle_name.empty(), std::logic_error,
...
void primary_generator::reset() {
  if (!is_initialized()) {
    G4Exception("snsim::primary_generator::reset", "InitializationError", FatalException,
...
void primary_generator::GeneratePrimaries(G4Event* g4_event_) {
  if (!is_initialized()) {
    G4Exception("snsim::primary_generator::GeneratePrimaries", "InitializationError",
                RunMustBeAborted, "Primary_Generator is not initialized!");
  }

  // THIS IS NOW INVALIDATED: 2011-04-19 FM: moved these bits to 'event_action::BeginOfEventAction'
  // : Clear event data: FIX BUG: 2011-05-10  FM+XG : DO CLEAR THE EVENT DATA MODEL HERE ! clog <<
  // "DEVEL: primary_generator::GeneratePrimaries: "
...
void primary_generator::_generate_vertex() {
  ...
  if (!_vertex_generator_->has_next_vertex()) {
    DT_LOG_ERROR(_logprio(), "No more generated vertex from the vertex generator after "
  ...
  if (_vertex_generator_->is_time_generator()) {
    ...
    _vertex_generator_->shoot_vertex_and_time(mgr.grab_vg_prng(), _current_vertex_, _current_time_);
  } else {
    // Vertex generator:
...
void primary_generator::_generate_event(G4Event* g4_event_) {
  if (!_event_generator_->has_next()) {
    DT_LOG_ERROR(_logprio(), "No more generated event from the event generator after "
                                 << _event_counter_ << " generated events !");
    G4RunManager::GetRunManager()->AbortRun();
    _event_action_->set_aborted_event(true);
    return;
  ...
  for (auto i = current_generated_event.get_particles().begin();
       i != current_generated_event.get_particles().end(); i++, particle_counter++) {
    ...
    DT_LOG_DEBUG(get_logging_priority(), "Primary particle [#" << particle_counter << "] label = '"
    ...
    if (genbb_particle.has_pdg_code()) {
      DT_LOG_DEBUG(get_logging_priority(), "  Attempting to use primary particle PDG code ["
      ...
      if (g4_particle == nullptr) {
        ...
        G4Exception("snsim::primary_generator::_generate_event", "InvalidArgument",
      ...
      if (genbb_particle.is_nucleus()) {
        ...
        */
        ...
        bool nucleus_parsing = genbb::primary_particle::label_to_nucleus(genbb_particle_label, Z, A,
                                                                         excitation_energy);
        if (!nucleus_parsing) {
          std::ostringstream message;
        ...
        bool ion_parsing = genbb::primary_particle::label_to_ion(genbb_particle_label, Z, A,
                                                                 excitation_energy, ion_charge);
        if (!ion_parsing) {
          std::ostringstream message;
        ...
        DT_LOG_DEBUG(_logprio(), "  Assign dynamic charge [" << (ion_charge > 0 ? "+" : "")
                                                             << ion_charge
                                                             << "e] to the Geant4 ion...");
        _particle_gun_->SetParticleCharge(ion_charge * CLHEP::eplus);

        // std::cerr.precision(15);
        DT_LOG_DEBUG(_logprio(), "Found an ion: "
                                     << g4_particle_name
                                     << " with G4 PDG mass = " << std::setprecision(15)
                                     << g4_particle->GetPDGMass() / CLHEP::MeV << " MeV "
        ...
        if (g4_particle == nullptr) {
          std::ostringstream message;
          message << "snsim::primary_generator::_generate_event: "
        ...
        _particle_gun_->SetParticleDefinition(g4_particle);
        g4_particle_name = g4_particle->GetParticleName();
        DT_LOG_TRACE(_logprio(), "Found the Geant4 '"
    ...
    if (!datatools::is_valid(kinetic_energy)) {
      ...
      G4Exception("snsim::primary_generator::_generate_event", "InvalidArgument", RunMustBeAborted,
                  message.str().c_str());
    }
    ...
    G4ThreeVector momentum(genbb_particle.get_momentum().x(), genbb_particle.get_momentum().y(),
    ...
    if (_logprio() == datatools::logger::PRIO_TRACE) {
      ...
      std::clog << tag2 << tag << "Time       = " << _particle_gun_->GetParticleTime() / CLHEP::ns
                << " ns" << std::endl;
...
auto primary_generator::get_g4_particle_name_from_genbb_particle(
    const ::genbb::primary_particle& p_) const -> std::string {
  if (p_.is_gamma()) {
    return "gamma";
  }
  if (p_.is_positron()) {
  ...
  auto found = _particle_names_map_.find(part_label);
 ...
 */
DOCD_CLASS_IMPLEMENT_LOAD_BEGIN(snsim::primary_generator, ocd_) {
  // The class name :
  ocd_.set_class_name("snsim::primary_generator");
...

  {
    ...
    cpd.set_name_pattern("particle_names_map")
        ...
        .set_mandatory(false)
        .set_long_description(
            "Genbb to Geant4 particle names lookup table. This table\n"
...
// Registration macro :
DOCD_CLASS_SYSTEM_REGISTRATION(snsim::primary_generator, "snsim::primary_generator")
