...
namespace mctools {
  ...
  namespace g4 {
    ...
    auto
    sensitive_hit_collection::get_hits () const -> const sensitive_hit_collection::hit_collection_type &
    {
      return _hits;
    }

    auto
    sensitive_hit_collection::grab_hits () -> sensitive_hit_collection::hit_collection_type &
    {
      return _hits;
    }

    auto sensitive_hit_collection::GetHit(size_t a_index) const -> G4VHit *
    {
      return _hits[a_index];
    }

    auto sensitive_hit_collection::GetSize() const -> size_t
    {
      return _hits.size ();
    }
