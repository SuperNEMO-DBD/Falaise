...
void sensitive_detector::set_track_gamma(bool track_gamma_) { _track_gamma_ = track_gamma_; }
...
auto sensitive_detector::has_manager() const -> bool { return _manager_ != nullptr; }

void sensitive_detector::set_manager(manager& manager_) { _manager_ = &manager_; }

auto sensitive_detector::get_auxiliaries() const -> const datatools::properties& { return _aux_; }

auto sensitive_detector::grab_auxiliaries() -> datatools::properties& { return _aux_; }

void sensitive_detector::set_record_delta_ray_from_alpha(bool flag_) {
  _record_delta_ray_from_alpha_ = flag_;
}

void sensitive_detector::set_record_track_id(bool record_) { _record_track_id_ = record_; }

void sensitive_detector::set_record_step_length(bool record_) { _record_step_length_ = record_; }

void sensitive_detector::set_record_primary_particle(bool record_) {
  _record_primary_particle_ = record_;
}

void sensitive_detector::set_record_major_track(bool record_) { _record_major_track_ = record_; }
...
void sensitive_detector::set_record_material(bool record_) { _record_material_ = record_; }

void sensitive_detector::set_record_sensitive_category(bool record_) {
  _record_sensitive_category_ = record_;
}

void sensitive_detector::set_record_momentum(bool record_) { _record_momentum_ = record_; }

void sensitive_detector::set_record_kinetic_energy(bool record_) {
  _record_kinetic_energy_ = record_;
}

void sensitive_detector::set_record_boundaries(bool record_) { _record_boundaries_ = record_; }

auto sensitive_detector::get_HCID() const -> int { return _HCID_; }

void sensitive_detector::set_HCID(int new_value_) { _HCID_ = new_value_; }

auto sensitive_detector::grab_hit_processors() -> sensitive_detector::hit_processor_dict_type& {
  return _hit_processors_;
}

auto sensitive_detector::get_hit_processors() const
    -> const sensitive_detector::hit_processor_dict_type& {
  return _hit_processors_;
}

auto sensitive_detector::grab_hit_processor(const std::string& name_)
    -> mctools::base_step_hit_processor& {
  auto found = _hit_processors_.find(name_);
  DT_THROW_IF(found == _hit_processors_.end(), std::logic_error,
...
void sensitive_detector::add_hit_processor(const std::string& name_,
  ...
  DT_THROW_IF(_hit_processors_.find(name_) != _hit_processors_.end(), std::logic_error,
              "This sensitive detector '" << _sensitive_category_ << "' already has a '" << name_
                                          << "' hit processor !");
...
void sensitive_detector::attach_logical_volume(const std::string& log_volume_name_) {
  if (find(_attached_logical_volumes_.begin(), _attached_logical_volumes_.end(),
...
auto sensitive_detector::grab_attached_logical_volumes() -> std::list<std::string>& {
...
void sensitive_detector::Initialize(G4HCofThisEvent* /*some_hit_collections_*/) {
  ...
  if (_hits_buffer_.empty()) {
    ...
    DT_LOG_NOTICE(_logprio(), "Setup the buffer of hits for detector '"
                                  << _sensitive_category_ << "' "
...
void sensitive_detector::EndOfEvent(G4HCofThisEvent* some_hit_collections_) {
  ...
  if (_used_hits_count_ > 0) {
    ...
    if (_hits_collection_ == nullptr) {
      _hits_collection_ = new sensitive_hit_collection(SensitiveDetectorName, collectionName[0]);
...
auto sensitive_detector::ProcessHits(G4Step* step_, G4TouchableHistory * /*touchable_history_*/)
    -> G4bool {
  ...
  */
  ...
  if (use_track_info) {
    ...
    if (_record_creator_process_ && !_track_info_ptr_->get_creator_process_name().empty()) {
      new_hit->grab_hit_data().set_creator_process_name(
...
void sensitive_detector::tree_dump(std::ostream& out_, const std::string& title_,
  ...
  // Logical volumes
  {
    ...
    for (auto ilog = _attached_logical_volumes_.begin(); ilog != _attached_logical_volumes_.end();
         ++ilog) {
  ...
  {
    out_ << indent << datatools::i_tree_dumpable::tag
         << "Drop zero energy deposit    : " << (_drop_zero_energy_deposit_steps_ ? "Yes" : "No")
         << std::endl;
  ...
  // Associated step hit processor
  {
    ...
    if (_hit_processors_.empty()) {
      ...
      for (auto ihp = _hit_processors_.begin(); ihp != _hit_processors_.end(); ++ihp) {
        ...
        out_ << "Processor[" << std::distance(_hit_processors_.begin(), ihp) << "]: '" << hp_name
             << "' @ " << hp << " with hit category '" << hp->get_hit_category() << "'"
             << std::endl;
      }
    }
  }
...
DOCD_CLASS_IMPLEMENT_LOAD_BEGIN(mctools::g4::sensitive_detector, ocd_) {
  ...
  ocd_.set_class_documentation(
      "This is the Geant4 virtual sensitive detector.        \n"
...

  {
    ...
    cpd.set_name_pattern("sensitive.drop_zero_energy_deposit_steps")
        .set_terse_description(
            "Do not record *zero energy deposit* hits from particles other than photons and "
            "neutrons")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_mandatory(false)
        .set_long_description(
...

  {
    ...
    cpd.set_name_pattern("sensitive.record_alpha_quenching")
        .set_terse_description(
            "Record the 'track.delta_ray_from_alpha' flag for secondary ionization electrons from "
            "an alpha particle")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_mandatory(false)
        .set_long_description(
...

  {
    ...
    cpd.set_name_pattern("sensitive.record_major_track")
        ...
        .set_mandatory(false)
        .set_long_description(
            "A *major* track is defined either :                    \n"
            "                                                       \n"
...

  {
    ...
    cpd.set_name_pattern("sensitive.major_track_minimum_energy")
        ...
        .set_long_description(
            "Default value:: 10 keV (above the energy of most delta-rays)\n"
            "                                                       \n"
            "Example::                                              \n"
            "                                                       \n"
            "  sensitive.record_major_track : boolean = 1           \n"
...

  {
    ...
    cpd.set_name_pattern("sensitive.record_kinetic_energy")
        .set_terse_description(
            "Record the 'track.start_kinetic_energy' and 'track.stop_kinetic_energy' real "
            "properties")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_mandatory(false)
        .set_long_description(
...

  {
    ...
    cpd.set_name_pattern("sensitive.hits_buffer_capacity")
        ...
        .set_mandatory(false)
        .set_long_description(
            "This property pre-allocate a buffer of hits from which \n"
