...
void manager_parameters::reset() { set_defaults(); }

void manager_parameters::tree_dump(std::ostream& out_, const std::string& title_,
  ...
  out_ << indent << datatools::i_tree_dumpable::tag << "using_time_stat     = " << using_time_stat
       << std::endl;
  out_ << indent << datatools::i_tree_dumpable::tag
       << "forbid_private_hits = " << forbid_private_hits << std::endl;
  out_ << indent << datatools::i_tree_dumpable::tag
       << "dont_save_no_sensitive_hit_events = " << dont_save_no_sensitive_hit_events << std::endl;
...
void manager_parameters::setup(const manager_parameters& a_params, manager& a_manager) {
  // Setup:
  ...
  if (a_params.input_prng_seeds_file.empty()) {
    DT_THROW_IF(!mygsl::seed_manager::seed_is_valid(a_params.vg_seed), std::logic_error,
                "Invalid vertex generator seed value !");
    DT_THROW_IF(!mygsl::seed_manager::seed_is_valid(a_params.eg_seed), std::logic_error,
                "Invalid event generator seed value !");
    DT_THROW_IF(!mygsl::seed_manager::seed_is_valid(a_params.mgr_seed), std::logic_error,
                "Invalid G4 generator seed value !");
    DT_THROW_IF(!mygsl::seed_manager::seed_is_valid(a_params.shpf_seed), std::logic_error,
  ...
  DT_THROW_IF(mlogprio == datatools::logger::PRIO_UNDEFINED, std::logic_error,
  ...
  DT_THROW_IF(a_params.manager_config_filename.empty(), std::logic_error,
  ...
  DT_THROW_IF(!boost::filesystem::exists(manager_config_filename), std::runtime_error,
