  ...
  try
    {
      ...
      additional_opts.add_options ()
        ("nevents",       po::value<int>    (&nevents),         "set the maximal number of events to draw")
      ...
      while (my_event_server.next_event ())
        {
          if (nevents > 0 && my_event_server.get_current_event_number () >= nevents)
            break;
          ...
          BOOST_FOREACH (const string & view_label, views)
            {
              ...
              if (view_label == "3d") a_canvas = my_tiny_viewer_3d.get_canvas ();
              else if (view_label == "top")   my_tiny_viewer_2d.set_view_type (TOP_VIEW);
              else if (view_label == "front") my_tiny_viewer_2d.set_view_type (FRONT_VIEW);
              else if (view_label == "side")  my_tiny_viewer_2d.set_view_type (SIDE_VIEW);
              else
                {
                  ostringstream message;
              ...
              filename << scratch_dir << "/"
                       << view_label << "_view_event_" << setw (3) << setfill ('0')
                       << my_event_server.get_current_event_number ()
                       << ".eps";
              ...
              if (!root_utilities::save_view_as (a_canvas, filename.str ()))
                {
                  cerr << datatools::utils::io::error
                       << appname_prefix
                       << "Can not save '" << view_label << "' view!" << endl;
                }
            }
