...
namespace FLVisualize {
  ...
  void FLVisualizeArgs::print(std::ostream & out_) const
  {
    ...
    out_ << last_tag
         << "inputMetadata        = [" << inputMetadata.size() << ']' << std::endl;
    return;
  }
  ...
  void do_postprocess_input_metadata(FLVisualizeArgs & flVisParameters)
  {
    ...
    if (!iMeta.experimentalSetupUrn.empty()) {
      DT_LOG_NOTICE(flVisParameters.logLevel, "Input metadata from the experimental setup identifier (URN) is '" << iMeta.experimentalSetupUrn << "'.");
    } else {
      DT_LOG_NOTICE(flVisParameters.logLevel, "No experimental setup identifier (URN) is set from input metadata.");
    }
    ...
    // Checks:
    {
      ...
      if (! flVisParameters.experimentalSetupUrn.empty()) {
        DT_THROW_IF(!iMeta.experimentalSetupUrn.empty() &&
                    ...
                    std::logic_error,
                    "Experimental setup URN='" << flVisParameters.experimentalSetupUrn << "' conflicts with experimental setup URN='"
    ...
    // Settings:
    {
      if (flVisParameters.experimentalSetupUrn.empty()) {
        ...
        DT_LOG_NOTICE(flVisParameters.logLevel, "The experimental setup identifier (URN) is set to '" << iMeta.experimentalSetupUrn << "' from input metadata.");
      }
    } // End of settings.
  ...
  void FLVisualizeArgs::do_postprocess(FLVisualizeArgs & flVisParameters)
  {
    ...
    if (!flVisParameters.inputFile.empty()) {
      ...
      DT_LOG_NOTICE(flVisParameters.logLevel, "Add input data file '" << flVisParameters.inputFile << "'");
    ...
    if (flVisParameters.experimentalSetupUrn.empty()) {
      ...
      DT_LOG_NOTICE(flVisParameters.logLevel, "Using default experimental setup URN='" << flVisParameters.experimentalSetupUrn << "'...");
    }

    if (!flVisParameters.experimentalSetupUrn.empty()) {
      DT_LOG_NOTICE(flVisParameters.logLevel, "Using experimental setup URN='" << flVisParameters.experimentalSetupUrn << "'...");
    ...
    if (expSetupUrnInfo.is_valid()) {
      ...
      if (!flVisParameters.servicesConfigUrn.empty()) {
        DT_THROW_IF(flVisParameters.servicesConfigUrn != servicesConfigUrn,
                    std::logic_error,
                    "Services config URN='" << flVisParameters.servicesConfigUrn << "' does not match "
                    << "the automatically resolved URN='" << servicesConfigUrn << "' from experimental setup '"
    ...
    if (!flVisParameters.servicesConfigUrn.empty()) {
      DT_LOG_NOTICE(flVisParameters.logLevel, "Services configuration URN='" << flVisParameters.servicesConfigUrn << "'...");
      // Check URN registration from the system URN query service:
      {
        std::string conf_category = "service";
        DT_THROW_IF(!dtkUrnQuery.check_urn_info(servicesConfigUrn, conf_category),
                    std::logic_error,
      ...
      DT_THROW_IF(!dtkUrnQuery.resolve_urn_to_path(servicesConfigUrnInfo.get_urn(),
                                                   conf_services_category,
                                                   conf_services_mime,
                                                   servicesConfigPath),
                  std::logic_error,
                  "Cannot resolve URN='" << servicesConfigUrnInfo.get_urn() << "'!");
      if (flVisParameters.servicesConfig.empty()) {
        flVisParameters.servicesConfig = servicesConfigPath;
      } else {
        DT_THROW_IF(flVisParameters.servicesConfig != servicesConfigPath,
                    std::logic_error,
                    "Services config path='" << flVisParameters.servicesConfig << "' does not match "
                    << "the automatically resolved path='" << servicesConfigPath << "' from services config URN='"
    ...
    if (servicesConfigUrnInfo.is_valid()) {
      ...
      if (servicesConfigUrnInfo.has_topic("geometry") &&
        ...
        // Check URN registration from the system URN query service:
        {
          ...
          DT_THROW_IF(!dtkUrnQuery.check_urn_info(geometryConfigUrn, conf_category),
                      std::logic_error,
                      "Cannot query URN='" << geometryConfigUrn << "'!");
        }
        geometryConfigUrnInfo = dtkUrnQuery.get_urn_info(geometryConfigUrn);
        DT_LOG_NOTICE(flVisParameters.logLevel, "Geometry configuration URN='" << geometryConfigUrn << "'...");
      }
    }
    ...
    if (geometryConfigUrnInfo.is_valid()) {
      ...
      DT_THROW_IF(!dtkUrnQuery.resolve_urn_to_path(geometryConfigUrnInfo.get_urn(),
                                                   conf_geometry_category,
                                                   conf_geometry_mime,
                                                   geometryConfigPath),
                  std::logic_error,
                  "Cannot resolve URN='" << geometryConfigUrnInfo.get_urn() << "'!");
      DT_LOG_NOTICE(flVisParameters.logLevel, "Resolved geometry configuration path='" << geometryConfigPath << "'...");
    }
      ...
      if (options_mgr.get_detector_config_file().empty()) {
        ...
        if (options_mgr.get_detector_config_file() != geometryConfigUrn &&
          ...
          DT_THROW(std::logic_error,
                   "Geometry config path='" << options_mgr.get_detector_config_file() << "' does not match "
                   << "the automatically resolved path='" << geometryConfigPath << "'!");
        }
      }
      DT_LOG_NOTICE(flVisParameters.logLevel, "Geometry configuration path='" << geometryConfigPath << "'...");
    }
    ...
    if (expSetupUrnInfo.is_valid()) {
      ...
      if (!flVisParameters.variantConfigUrn.empty()) {
        DT_THROW_IF(flVisParameters.variantConfigUrn != variantConfigUrn,
                    std::logic_error,
    ...
    if (!flVisParameters.variantConfigUrn.empty()) {
      DT_LOG_NOTICE(flVisParameters.logLevel, "Variant configuration URN='" << variantConfigUrn << "'...");
      // Check URN registration from the system URN query service:
      {
      ...
      DT_LOG_NOTICE(flVisParameters.logLevel,
                    "Resolving variant configuration URN='" << flVisParameters.variantConfigUrn << "'...");
      // Resolve variant configuration file:
      std::string conf_variants_category = "configuration";
      std::string conf_variants_mime;
      DT_THROW_IF(!dtkUrnQuery.resolve_urn_to_path(flVisParameters.variantConfigUrn,
                                                   conf_variants_category,
                                                   conf_variants_mime,
                                                   variantConfigPath),
                  std::logic_error,
                  "Cannot resolve URN='" << flVisParameters.variantConfigUrn << "'!");
      if (flVisParameters.variants.config_filename.empty()) {
        flVisParameters.variants.config_filename = variantConfigPath;
      } else {
        DT_THROW_IF(flVisParameters.variants.config_filename != variantConfigPath,
                    std::logic_error,
                    "Variant config path='" << flVisParameters.variants.config_filename << "' does not match "
                    << "the automatically resolved path='" << variantConfigPath << "' from variant config URN='"
                    << flVisParameters.variantConfigUrn << "'!");
      }
      DT_LOG_NOTICE(flVisParameters.logLevel,
                    "Variant configuration path='" << flVisParameters.variants.config_filename << "'...");
    }

    // Variant profile:
    if (!flVisParameters.variants.profile_load.empty()) {
      // Force the variant profile path:
      DT_THROW_IF(!flVisParameters.variantProfileUrn.empty(),
                  std::logic_error,
                  "Required variants profile URN='" << flVisParameters.variantProfileUrn << "' "
      ...
      if (flVisParameters.variantProfileUrn.empty()) {
        ...
        if (variantConfigUrnInfo.is_valid()) {
          ...
          if (variantConfigUrnInfo.has_topic("__default_profile__") &&
            ...
            flVisParameters.variantProfileUrn = variantConfigUrnInfo.get_component("__default_profile__");
            DT_LOG_NOTICE(flVisParameters.logLevel,
                         "Using the default variant profile '" << flVisParameters.variantProfileUrn << "'"
      ...
      if (!flVisParameters.variantProfileUrn.empty()) {
        ...
        DT_THROW_IF(!dtkUrnQuery.resolve_urn_to_path(flVisParameters.variantProfileUrn,
