...
namespace snemo {
  ...
  namespace visualization {
    ...
    namespace view {
      ...
      TObjArray * snemo_draw_manager::get_objects()
      {
        return _objects_;
      }

      TObjArray * snemo_draw_manager::get_text_objects()
      {
      ...
      void snemo_draw_manager::update()
      {
        ...
        if (_server_->get_event().has(io::TCD_LABEL)) this->_add_tracker_clustering_data();
        else {
          DT_LOG_DEBUG(options_manager::get_instance().get_logging_priority(),
        ...
        if (_server_->get_event().has(io::TTD_LABEL)) this->_add_tracker_trajectory_data();
        else {
          DT_LOG_DEBUG(options_manager::get_instance().get_logging_priority(),
        ...
        // Add 'tracker_trajectory_data' objects:
        if (_server_->get_event().has(io::PTD_LABEL)) this->_add_particle_track_data();
        else {
          DT_LOG_DEBUG(options_manager::get_instance().get_logging_priority(),
