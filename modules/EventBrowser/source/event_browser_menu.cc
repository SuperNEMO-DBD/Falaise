...
namespace snemo {
  ...
  namespace visualization {
    ...
    namespace view {
      ...
      void TGMenuEntryPlus::SetLabel(TGHotString * label_)
      {
        fLabel = label_;
      }
      void TGMenuEntryPlus::SetPicture(const TGPicture * pic_)
      {
        fPic = pic_;
      }
      void TGMenuEntryPlus::SetType(const EMenuEntryType type_)
      {
        fType = type_;
      }
      void TGMenuEntryPlus::SetEntryId(const int id_)
      {
        fEntryId = id_;
      }
      void TGMenuEntryPlus::SetPopup(TGPopupMenu * popup_)
      {
        fPopup = popup_;
      }
      void TGMenuEntryPlus::SetStatus(const int status_)
      {
        fStatus = status_;
      }
      void TGMenuEntryPlus::SetEx(const unsigned int ex_)
      {
        fEx = ex_;
      }
      void TGMenuEntryPlus::SetEy(const unsigned int ey_)
      {
        fEy = ey_;
      }
      void TGMenuEntryPlus::SetEh(const unsigned int eh_)
      {
        fEh = eh_;
      }
      void TGMenuEntryPlus::SetEw(const unsigned int ew_)
      {
        fEw = ew_;
      }
      void TGMenuEntryPlus::SetUserData(void * user_data_)
      {
        fUserData = user_data_;
      }
      //________________________________________________________________________

      TGPopupMenuPlus::TGPopupMenuPlus(const TGWindow * p) :
        TGPopupMenu(p)
      {
        return;
      }

      TGPopupMenuPlus::~TGPopupMenuPlus()
      {
        return;
      }
      ...
      event_browser_menu::event_browser_menu(TGCompositeFrame* main_)
      {
        ...
        menu_bar->AddPopup("File", menu_file,
                           new TGLayoutHints(kLHintsTop | kLHintsLeft, 0, 4, 0, 0));
        menu_bar->AddPopup("View", menu_view,
                           new TGLayoutHints(kLHintsTop | kLHintsLeft, 0, 4, 0, 0));
        menu_bar->AddPopup("Options", menu_options,
                           new TGLayoutHints(kLHintsTop | kLHintsLeft, 0, 4, 0, 0));
        menu_bar->AddPopup("Misc.", menu_misc,
                           new TGLayoutHints(kLHintsTop | kLHintsLeft, 0, 4, 0, 0));
        menu_bar->AddPopup("Help", menu_help,
      ...
      void event_browser_menu::update(const button_signals_type signal_)
      {
        ...
        if (flag) uncheck_option(signal_);
        else check_option(signal_);
      ...
      void event_browser_menu::set_default_option(const io::event_server & server_)
      {
        ...
        for (std::map<button_signals_type, bool>::const_iterator i = option_dict.begin();
          ...
          if (flag) check_option(id);
          else uncheck_option(id);
        }
      ...
      {
        std::map<button_signals_type, TGPopupMenuPlus*>::const_iterator
          found = _popup_dict_.find(signal_);
      ...
      bool event_browser_menu::rcheck_option(const button_signals_type signal_,
                                             const int status_)
      {
        std::list<std::pair<button_signals_type, button_signals_type> > signal_intervals;
        signal_intervals.push_back(std::make_pair(DUMP_INTO_TOOLTIP,
                                                  DUMP_INTO_WINDOW));
        signal_intervals.push_back(std::make_pair(SHOW_TRACKER_CLUSTERED_BOX,
                                                  SHOW_TRACKER_CLUSTERED_CIRCLE));

        for (std::list<std::pair<button_signals_type, button_signals_type> >::const_iterator
               i = signal_intervals.begin();
      ...
      void event_browser_menu::change_option(const button_signals_type signal_,
                                             const int status_)
      {
        if (!has_option(signal_)) return;
