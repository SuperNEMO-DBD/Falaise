...
namespace snemo {
  ...
  namespace visualization {
    ...
    namespace view {
      ...
      // ctor:
      display_2d::display_2d (TGCompositeFrame * main_,
                              io::event_server * server_,
                              const bool switch_mode) :
        _server_    (server_),
        _2d_drawer_ (0),
        _2d_viewer_ (0)
      {
        ...
        if (style_manager::get_instance ().use_opengl ())
          {
            _2d_viewer_ = new opengl_embedded_viewer ("2DPlot", main_,
                                                      width, height,
          ...
          _2d_viewer_ = new pad_embedded_viewer ("2DPlot", main_,
                                                 width, height,
                                                 i_embedded_viewer::VIEW_2D);

        main_->AddFrame (_2d_viewer_->get_frame (), new TGLayoutHints (kLHintsExpandX |
                                                                       kLHintsExpandY));
        ...
        if (switch_mode)
          {
            ...
            const std::string view_str[n_view] = { " top view " ,
                                                   " side view ",
                                                   " front view " };
            ...
            for (unsigned int i_view = 0; i_view < n_view; ++i_view)
              {
                ...
                TGRadioButton* view
                  = new TGRadioButton (views, view_str[i_view].c_str(), view_id);

                if (view_id == _2d_viewer_->get_view_type ()) view->SetState (kButtonDown);
                else                                          view->SetState (kButtonUp);

                view->Connect ("Clicked()", "snemo::visualization::view::display_2d",
                               this, "process_option_buttons()");
              }
