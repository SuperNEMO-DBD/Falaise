...
namespace snemo {
  ...
  namespace visualization {
    ...
    namespace view {
      ...
      // ctor:
      signal_handling::signal_handling(TGMainFrame* main_) :
        _browser_(0)
      {
        ...
        case SAVE_EVENT_AS:
          {
            const std::string dir =
              std::string(gSystem->ExpandPathName("$(PWD)"));
              TString directory(dir.c_str());
              TGFileInfo file_info;
              const char * config_file_types[] = { "BRIO (Boost to ROOT) archives", "*.brio",
                                                   "Binary archives",               "*.data",
                                                   "Text archives",                 "*.txt",
                                                   "XML archives",                  "*.xml",
                                                   "All files",                     "*",
                                                   0,                               0 };
        ...
        case LOAD_STYLE:
          {
            ...
            TString directory(dir.c_str());
            TGFileInfo file_info;
            const char * style_file_types [] = { "Style files" , "*.sty" ,
                                                 "All files"   , "*",
                                                 0             , 0 };
