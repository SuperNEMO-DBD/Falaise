...
namespace snemo {
  ...
  namespace visualization {
    ...
    namespace view {
      ...
      {
      public:
        TGListTreeItemStdPlus(const char      * name_      = 0,
                              browser_tracks  * bt_        = 0,
                              const TGPicture * open_pic_  = 0,
                              const TGPicture * close_pic_ = 0,
      ...
      TGListTreeItemStdPlus::TGListTreeItemStdPlus(const char      * name_,
                                                   browser_tracks  * bt_,
                                                   const TGPicture * open_pic_,
                                                   const TGPicture * close_pic_,
                                                   const bool checked_) :
        TGListTreeItemStd(gClient, name_, open_pic_, close_pic_, checked_),
        fBrowser(bt_)
      {
        return;
      }

      TGListTreeItemStdPlus::~TGListTreeItemStdPlus()
      {
        return;
      }
          ...
          if (state){
            if (!item->IsChecked()) {
              ...
              if (id < 0 && fBrowser) {
                ...
                if (properties->has_flag("default")) item->CheckItem(true);
                else properties->update(browser_tracks::CHECKED_FLAG, false);
              } else {
                item->CheckItem();
              }
            }
      ...
      bool browser_tracks::is_initialized() const
      {
        return _initialized_;
      }

      // ctor:
      browser_tracks::browser_tracks(TGCompositeFrame * main_,
      ...
      browser_tracks::~browser_tracks()
      {
        ...
        for (std::map<std::string, const TGPicture*>::iterator i_pict
               = _icons_.begin(); i_pict != _icons_.end(); ++i_pict)
      ...
      void browser_tracks::_at_construct_()
      {
        ...
        _main_->AddFrame(canvas,
                         new TGLayoutHints(kLHintsExpandX |
                                           kLHintsExpandY |
                                           kLHintsBottom));

        _tracks_list_box_->Connect("Checked(TObject *,Bool_t)",
                                   "snemo::visualization::view::browser_tracks",
                                   this, "item_checked(TObject *,bool)");

        _tracks_list_box_->Connect("DoubleClicked(TGListTreeItem *,Int_t)",
                                   "snemo::visualization::view::browser_tracks",
                                   this, "item_selected(TGListTreeItem *,int)");
      ...
      {
        for (std::vector<TGListTreeItem*>::iterator item
               = _item_list_.begin(); item != _item_list_.end(); ++item)
      ...
      void browser_tracks::_update_event_header()
      {
        ...
        _top_item_ = _tracks_list_box_->AddItem(0, label.str().c_str(),
      ...
      void browser_tracks::_update_simulated_data()
      {
        ...
        if (!options_mgr.get_option_flag(SHOW_MC_VERTEX) &&
            !options_mgr.get_option_flag(SHOW_MC_HITS)) return;
        ...
        if (options_mgr.get_option_flag(SHOW_MC_VERTEX)) {
          ...
          for (genbb::primary_event::particles_col_type::const_iterator ip = particles.begin();
            ...
            const long pixel = TColor::Number2Pixel(kViolet);
            const std::string hex_str = TColor::PixelAsHexString(pixel);
            TGListTreeItem * item_particle
              = _tracks_list_box_->AddItem(item_primary_event,
                                           label.str().c_str(),
        ...
        if (options_mgr.get_option_flag(SHOW_MC_TRACKS)) {
          if (sd.has_step_hits("__visu.tracks")) {
            ...
            if (!hit_collection.empty()) {
              TGListTreeItem * item_mc_step_hits
                = _tracks_list_box_->AddItem(item_simulated_data,
                                             "Highlighted simulated step hits",
                                             _get_colored_icon_("ofolder"),
                                             _get_colored_icon_("folder"));
              item_mc_step_hits->SetCheckBox(false);
              item_mc_step_hits->SetUserData((void*)(intptr_t)++icheck_id);
              ...
              for (mctools::simulated_data::hit_handle_collection_type::iterator
                ...
                if (!a_step.get_auxiliaries().has_flag(mctools::hit_utils::HIT_VISU_HIGHLIGHTED_KEY)) continue;
                ...
                TGListTreeItem * item_hit
                  = _tracks_list_box_->AddItem(item_mc_step_hits,
                                               label_hit.str().c_str());
                item_hit->SetColor(kRed);
                item_hit->SetUserData((void*)(intptr_t)-(++icheck_id));
                _base_hit_dictionnary_[-icheck_id] = &(a_step);
                item_hit->SetPictures(_get_colored_icon_("step", "", true),
                                      _get_colored_icon_("step"));
              ...
              oss << item_mc_step_hits->GetText();
              ihit == 0 ? oss << " - no hits" : oss << " - " << ihit << " hits";
              item_mc_step_hits->SetText(oss.str().c_str());
            }
          }
        }
          ...
          if (options_mgr.get_option_flag(SHOW_MC_TRACKER_HITS)) {
            if (sd.has_step_hits("gg")) {
              ...
              for (mctools::simulated_data::hit_handle_collection_type::iterator
                ...
                // Add subsubitem:
                std::ostringstream label_hit;
                label_hit << "Geiger hit #" << std::setw(2) << std::setfill('0')
                          << a_step.get_hit_id() << " - "
                          << "geom_id = "   << a_step.get_geom_id();

                TGListTreeItem * item_hit
                  = _tracks_list_box_->AddItem(item_mc_tracker,
                                               label_hit.str().c_str());
                item_hit->SetUserData((void*)(intptr_t)-(++icheck_id));
                _base_hit_dictionnary_[-icheck_id] = &(a_step);
      ...
      void browser_tracks::_update_calibrated_data()
      {
        ...
        // Start with calorimeter info:
        {
          TGListTreeItem * item_calorimeter
            = _tracks_list_box_->AddItem(item_calibrated_data,
        ...
        // Continue with tracker info:
        {
          ...
          for (snemo::datamodel::calibrated_data::tracker_hit_collection_type::iterator
            ...
            label_hit << "Geiger hit #" << std::setw(2) << std::setfill('0')
                      << a_hit.get_id() << " (r, z) = ("
                      << a_hit.get_r() / CLHEP::cm << ", "
                      << a_hit.get_z() / CLHEP::cm << ") cm";

            TGListTreeItem * item_hit
              = _tracks_list_box_->AddItem(item_tracker,
                                           label_hit.str().c_str(),
      ...
      void browser_tracks::_update_tracker_clustering_data()
      {
        ...
        TGListTreeItem * item_tracker_cluster
          = _tracks_list_box_->AddItem(_top_item_,
                                       data_bank_name.c_str(),
                                       _get_colored_icon_("ofolder"),
                                       _get_colored_icon_("folder"));
        ...
        snemo::datamodel::tracker_clustering_data::solution_col_type & cluster_solutions
          = tcd.grab_solutions();
        for (snemo::datamodel::tracker_clustering_data::solution_col_type::iterator
          ...
          TGListTreeItem * item_solution
            = _tracks_list_box_->AddItem(item_tracker_cluster,
          ...
          for (snemo::datamodel::tracker_clustering_solution::cluster_col_type::iterator
            ...
            for (snemo::datamodel::calibrated_tracker_hit::collection_type::iterator
                   igg = hits.begin();
                 igg != hits.end(); ++igg) {
              ...
              label_hit << "Geiger hit #" << std::setw(2) << std::setfill('0')
                        << a_gg_hit.get_id() << " (r, z) = ("
                        << a_gg_hit.get_r() / CLHEP::cm << ", "
                        << a_gg_hit.get_z() / CLHEP::cm << ") cm";

              TGListTreeItem * item_hit
                = _tracks_list_box_->AddItem(item_cluster,
                                             label_hit.str().c_str(),
                                             _get_colored_icon_("geiger", hex_str, true),
                                             _get_colored_icon_("geiger", hex_str));
              item_hit->SetUserData((void*)(intptr_t)-(++icheck_id));
        ...
        for (snemo::datamodel::tracker_trajectory_data::solution_col_type::iterator
          ...
          for (snemo::datamodel::tracker_trajectory_solution::trajectory_col_type::iterator
            ...
            if (properties.has_key("chi2") && properties.has_key("ndof")) {
              ...
              label_trajectory << " - chi2/ndf = "
                               << std::setprecision(2) << std::fixed
                               << chi2 << "/" << std::setprecision(0) << ndof
                               << std::setprecision(2) << std::fixed
            ...
            if (a_trajectory.get_pattern().get_pattern_id() ==
              ...
              item_trajectory
                = _tracks_list_box_->AddItem(item_helix_solution,
                                             std::string("helix" + label_trajectory.str()).c_str(),
                                             _get_colored_icon_("helix", hex_str, true),
                                             _get_colored_icon_("helix", hex_str));
            } else if (a_trajectory.get_pattern().get_pattern_id() ==
                       snemo::datamodel::line_trajectory_pattern::pattern_id()) {
              // First time instantiate it
              if (!item_line_solution) {
                const bool checked = true;
                item_line_solution = new TGListTreeItemStdPlus("Line trajectories",
                                                               this,
                                                               _get_colored_icon_("ofolder"),
                                                               _get_colored_icon_("folder"),
                                                               checked);
              ...
              item_trajectory
                = _tracks_list_box_->AddItem(item_line_solution,
                                             std::string("line" + label_trajectory.str()).c_str(),
                                             _get_colored_icon_("line", hex_str, true),
                                             _get_colored_icon_("line", hex_str));
            }
      ...
      void browser_tracks::_update_particle_track_data()
      {
        ...
        if (ptd.has_non_associated_calorimeters()) {
          ...
          for (snemo::datamodel::calibrated_data::calorimeter_hit_collection_type::iterator
                 it_hit = cc_collection.begin(); it_hit != cc_collection.end(); ++it_hit) {
            snemo::datamodel::calibrated_calorimeter_hit & a_hit = it_hit->grab();
        ...
        for (snemo::datamodel::particle_track_data::particle_collection_type::iterator
          ...
          const long pixel = TColor::Number2Pixel(item_color);
          std::string hex_str = TColor::PixelAsHexString(pixel);
          if (a_auxiliaries.has_key(browser_tracks::COLOR_FLAG)) {
            a_auxiliaries.fetch(browser_tracks::COLOR_FLAG, hex_str);
            //const size_t color = TColor::GetColor(hex_str.c_str());
            //item_cluster->SetColor(color);
          ...
          if (a_particle.has_vertices()) {
            ...
            for (snemo::datamodel::particle_track::vertex_collection_type::iterator
                   ivtx = vts.begin(); ivtx != vts.end(); ++ivtx) {
      ...
      datatools::properties * browser_tracks::get_item_properties(const int id_)
      {
        ...
        datatools::properties * properties = 0;
        {
          ...
          std::map<int, datatools::properties *>::iterator found
            = _properties_dictionnary_.find(id_);
...

        {
          ...
          std::map<int, geomtools::base_hit *>::iterator found
            = _base_hit_dictionnary_.find(id_);
      ...
      void browser_tracks::item_selected(TGListTreeItem * item_, int /*button_*/)
      {
        ...
        properties->update(HIGHLIGHT_FLAG,
                           !properties->has_flag(HIGHLIGHT_FLAG));
