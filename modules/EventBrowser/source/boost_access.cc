...
std::string boost_access::get_file_type_as_string() const { return "BOOST/Serialization"; }
...
bool boost_access::is_valid(const std::vector<std::string>& filenames_) const {
  ...
  filenames_.size() > 1 ? message << " (and others) are boost archives"
...
bool boost_access::retrieve_event(event_record& event_, const size_t event_number_) {
  if (is_sequential()) {
    if (!_reader_->has_record_tag()) {
