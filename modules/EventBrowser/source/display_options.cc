...
namespace snemo {
  ...
  namespace visualization {
    ...
    namespace view {
      ...
      bool display_options::is_initialized () const
      {
      ...
      void display_options::_at_construct_ ()
      {
        ...
        TGGroupFrame* group
          = new TGGroupFrame (_main_, "Automatic event reading", kHorizontalFrame);
        _main_->AddFrame (group, new TGLayoutHints (kLHintsTop |
                                                    kLHintsLeft, 3, 3, 3, 3));
        ...
        options_mgr.is_automatic_event_reading_mode () ?
          auto_read->SetState (kButtonDown) :
          auto_read->SetState (kButtonUp);
        auto_read->Connect ("Clicked ()",
                            "snemo::visualization::view::display_options",
                            this, "process_auto_reading ()");
        ...
        slider->Connect ("PositionChanged (Int_t)",
                         "snemo::visualization::view::display_options",
                         this, "do_slider (const int)");
        ...
        _main_->AddFrame (process_style_buttons, new TGLayoutHints (kLHintsBottom |
        ...
        reload_style_file->Connect ("Clicked ()",
                                    "snemo::visualization::view::display_options",
                                    this, "reload_style_settings ()");
        ...
        save_style_into_file->Connect ("Clicked ()",
                                       "snemo::visualization::view::display_options",
      ...
      void display_options::_build_volume_buttons_ ()
      {
        ...
        TGLayoutHints* layout = new TGLayoutHints (kLHintsTop |
                                                   kLHintsLeft, 3, 3, 3, 3);
        TGGroupFrame* volume_group
          = new TGGroupFrame (_main_, "Volume Settings", kVerticalFrame);
        ...
        for (std::map<std::string, style_manager::volume_properties>::const_iterator
             ...
             it_volume != volumes.end (); ++it_volume)
          {
            ...
            if (volume_group_name.find (".category") != std::string::npos)
              {
                volume_group_name.replace (volume_group_name.find (".category"),
                                           volume_group_name.length (),"");
              }
            ...
            visibility_check->Connect ("Clicked()",
                                       "snemo::visualization::view::display_options",
                                       this, "process_volume_settings ()");
            cframe->AddFrame (visibility_check, new TGLayoutHints (kLHintsTop | kLHintsExpandX,
                                                                   2, 2, 2, 2));
            _volumes_widgets_[volume_name]._tg_visibility_ = visibility_check;
            _button_dictionnary_[id_button++] = volume_name;

            // Add color selector
            TGColorSelect * color_selector
              = new TGColorSelect (cframe,
                                   TColor::Number2Pixel (style_mgr.get_volume_color (volume_name)),
                                   id_button);
      ...
      void display_options::_build_particle_buttons_ ()
      {
        ...
        TGLayoutHints* layout = new TGLayoutHints (kLHintsTop |
                                                   kLHintsLeft, 3, 3, 3, 3);
        TGGroupFrame* particle_group
          = new TGGroupFrame (_main_, "Simulation Settings", kVerticalFrame);
        ...
        for (std::map<std::string, style_manager::particle_properties>::const_iterator
             ...
             it_particle != particles.end (); ++it_particle)
          {
            ...
            // Add visibility widget and check visibility state
            TGCheckButton * visibility_check
              = new TGCheckButton (cframe, particle_name.c_str (), id_button);
            visibility_check->Connect ("Clicked()",
                                       "snemo::visualization::view::display_options",
                                       this, "process_particle_settings ()");
            cframe->AddFrame (visibility_check, new TGLayoutHints (kLHintsTop | kLHintsExpandX,
                                                                   2, 2, 2, 2));
            ...
            TGColorSelect * color_selector
              = new TGColorSelect (cframe,
                                   TColor::Number2Pixel (style_mgr.get_particle_color (particle_name)),
                                   id_button);
            color_selector->Connect ("ColorSelected (Pixel_t)",
                                     "snemo::visualization::view::display_options",
                                     this, "process_particle_settings ()");
      ...
      void display_options::_build_misc_buttons_ ()
      {
        ...
        TGLayoutHints* layout = new TGLayoutHints (kLHintsTop |
                                                   kLHintsLeft, 3, 3, 3, 3);
        TGGroupFrame* misc_group
          = new TGGroupFrame (_main_, "Miscellaneous", kVerticalFrame);
...

        {
          ...
          _background_color_
            = new TGColorSelect (cframe,
                                 TColor::Number2Pixel (style_mgr.get_background_color ()),
                                 COLOR_BACKGROUND);
          _background_color_->Connect ("ColorSelected (Pixel_t)",
                                       "snemo::visualization::view::display_options",
                                       this, "process_button_misc_settings ()");
...

        {
          ...
          misc_group->AddFrame (cframe);
          _line_style_
            = new TGLineStyleComboBox (cframe, LINE_STYLE);
          _line_style_->Resize (90, 22);
          _line_style_->Connect ("Selected (int)",
                                 "snemo::visualization::view::display_options",
                                 this, "process_combo_misc_settings ()");
          cframe->AddFrame (_line_style_,
...

        {
          ...
          misc_group->AddFrame (cframe);
          _line_width_
            = new TGLineWidthComboBox (cframe, LINE_WIDTH);
          _line_width_->Resize (90, 22);
          _line_width_->Connect ("Selected(Int_t)",
                                 "snemo::visualization::view::display_options",
                                 this, "process_combo_misc_settings ()");
          cframe->AddFrame (_line_width_,
                            new TGLayoutHints (kLHintsRight, 1, 1, 2, 2));
        }
      ...
      void display_options::_set_button_values_ ()
      {
        ...
        // Volume settings
        for (std::map<std::string, volume_widgets>::iterator
        ...
        for (std::map<std::string, particle_widgets>::iterator
               it_widget = _particles_widgets_.begin ();
             it_widget != _particles_widgets_.end (); ++it_widget)
          {
            const std::string particle_name = it_widget->first;

            TGCheckButton * visibility_check = it_widget->second._tg_visibility_;
            style_mgr.get_particle_visibility (particle_name) ?
              visibility_check->SetState (kButtonDown) :
              visibility_check->SetState (kButtonUp);
      ...
      void display_options::process_volume_settings ()
      {
        ...
        unsigned int color
          = TColor::GetColor (_volumes_widgets_[volume_name]._tg_color_->GetColor ());
            ...
            if (options_mgr.is_automatic_event_reading_mode ())
              {
                ...
                TTimer::SingleShot (nbr_seconds,
                                    "snemo::visualization::view::event_browser",
                                    _browser_, "change_event ()");
              }
          }
      ...
      void display_options::save_style_settings ()
      {
        ...
        const char * save_as_types[nbr_file_format] =
          {
            "Style File", "*.sty",
            0,            0
          };
        ...
        std::list<std::string>::iterator it
          = find (file_format.begin (), file_format.end (),
                  std::string ("*.sty"));
      ...
      void display_options::test_add_frame ()
      {
        ...
        TGGroupFrame* volume_group
          = new TGGroupFrame (_main_, "toto", kHorizontalFrame);
        volume_group->SetTitlePos (TGGroupFrame::kLeft);
        _main_->AddFrame (volume_group);

        TGCheckButton * test
          = new TGCheckButton (volume_group, "test toto", 10000);
        volume_group->AddFrame (test,  new TGLayoutHints (kLHintsTop &
                                                          kLHintsLeft, 2, 2, 2, 2));
