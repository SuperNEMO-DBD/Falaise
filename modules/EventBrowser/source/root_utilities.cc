...
namespace snemo {
  ...
  namespace visualization {
    ...
    namespace utils {
      ...
      root_utilities::TLatex3D::TLatex3D() : TLatex()
      {
        _x_ = 0.0;
        _y_ = 0.0;
        _z_ = 0.0;
      ...
      void root_utilities::TLatex3D::SetPadCoordinates(const double x_,
                                                       const double y_)
      {
      ...
      bool root_utilities::TLatex3D::IsEqual(const TObject * object_) const
      {
        ...
        if (this == object_) return false;
        if (this->GetX() != latex->GetX()) return false;
        if (this->GetY() != latex->GetY()) return false;
        if (this->GetZ() != latex->GetZ()) return false;
      ...
      unsigned int root_utilities::get_color_value(const std::string & color_)
      {
        ...
        if (color_[0] == '#') {
          ...
          std::string token;
          while (iss >> token) {
            if (token == "kWhite")        color += 0;
            else if (token == "kBlack")   color += 1;
            else if (token == "kGray")    color += 920;
            else if (token == "kRed")     color += 632;
            else if (token == "kGreen")   color += 416;
            else if (token == "kBlue")    color += 600;
            else if (token == "kYellow")  color += 400;
            else if (token == "kMagenta") color += 616;
            else if (token == "kCyan")    color += 432;
            else if (token == "kOrange")  color += 800;
            else if (token == "kSpring")  color += 820;
            else if (token == "kTeal")    color += 840;
            else if (token == "kAzure")   color += 860;
            else if (token == "kViolet")  color += 880;
            else if (token == "kPink")    color += 900;
            else if (token == "+") {
      ...
      bool root_utilities::save_view_as(TCanvas * canvas_,
                                        const std::string & filename_)
      {
        ...
        std::list<std::string>::iterator it
          = std::find(file_format.begin(), file_format.end(),
      ...
      TGeoShape * root_utilities::get_geo_shape(const geomtools::i_shape_3d & shape_3d_)
      {
        ...
        if (shape_3d_.is_composite()) {
          const geomtools::i_composite_shape_3d & sub_shape
            = dynamic_cast<const geomtools::i_composite_shape_3d &>(shape_3d_);
          ...
          ct1->RegisterYourself();
          {
            ...
            ct1->SetTranslation(g_translation.x(),
                                g_translation.y(),
                                g_translation.z());

            DT_LOG_TRACE(local_priority, "geo_shape_name = " << geo_shape1->GetName());
            DT_LOG_TRACE(local_priority, "(x, y, z) = (" << g_translation.x() << ", "
                         << g_translation.y() << ", " << g_translation.z() << ")");
            DT_LOG_TRACE(local_priority, "(phi, theta, psi) = (" << g_placement.get_phi() << ", "
                         << g_placement.get_theta() << ", " << g_placement.get_delta() << ")");
          }
          ...
          ct2->RegisterYourself();
          {
            ...
            const geomtools::vector_3d & g_translation = g_placement.get_translation();
            ct2->SetTranslation(g_translation.x(),
                                g_translation.y(),
                                g_translation.z());
          ...
          if (keep_it_simple) {
            if (!sh1.is_composite()) return geo_shape1;
            else                      return geo_shape2;
          }
          ...
          if (shape_name == "subtraction_3d") {
            ...
            DT_THROW_IF(true, std::logic_error, "Composite shape '" << shape_name
                        << "' not yet implemented !");
          }
          ...
          const geomtools::box & mbox =
            dynamic_cast<const geomtools::box &>(shape_3d_);

          shape = new TGeoBBox(mbox.get_x()/2.,
                               mbox.get_y()/2.,
                               mbox.get_z()/2.);
          ...
          shape = new TGeoTube(0.0,
                               mcylinder.get_radius(),
                               mcylinder.get_z());
        } else if (shape_name == "sphere") {
          const geomtools::sphere & msphere =
            dynamic_cast<const geomtools::sphere &>(shape_3d_);

          shape = new TGeoSphere(0.0,
                                 msphere.get_radius());
        } else if (shape_name == "polycone") {
          const geomtools::polycone & mpolycone =
            dynamic_cast<const geomtools::polycone &>(shape_3d_);

          TGeoPcon * polycone = new TGeoPcon(0, 360, mpolycone.points().size());

          size_t i_section = 0;
          for (geomtools::polycone::rz_col_type::const_iterator
                 it_section = mpolycone.points().begin();
               it_section != mpolycone.points().end();
