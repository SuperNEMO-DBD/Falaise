...
namespace snemo {
  ...
  namespace visualization {
    ...
    namespace view {
      ...
      // ctor:
      visual_track_renderer::visual_track_renderer() :
        base_renderer()
      {
        return;
      }
      ...
      void visual_track_renderer::push_reconstructed_tracks()
      {
        ...
        if (pt_data.has_non_associated_calorimeters()) {
          ...
          for (snemo::datamodel::calibrated_calorimeter_hit::collection_type::const_iterator
                 icalo = calos.begin(); icalo != calos.end(); ++icalo) {
        ...
        for (snemo::datamodel::particle_track_data::particle_collection_type::const_iterator
               iparticle = particles.begin(); iparticle != particles.end(); ++iparticle) {
          ...
          if (a_particle.get_auxiliaries().has_key(browser_tracks::CHECKED_FLAG) &&
              !a_particle.get_auxiliaries().has_flag(browser_tracks::CHECKED_FLAG)) continue;
