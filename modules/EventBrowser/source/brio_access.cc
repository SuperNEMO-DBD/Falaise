...
std::string brio_access::get_file_type_as_string() const { return "BRIO"; }
...
bool brio_access::is_valid(const std::vector<std::string>& filenames_) const {
  ...
  filenames_.size() > 1 ? message << " (and others) are brio archives"
                        : message << " is a brio archive";
  DT_LOG_INFORMATION(view::options_manager::get_instance().get_logging_priority(), message.str());
  return true;
}
...
bool brio_access::is_readable() {
  ...
  for (std::list<std::string>::const_iterator imode = modes.begin(); imode != modes.end();
       ++imode) {
...
bool brio_access::build_list() {
  ...
  for (size_t i = _number_of_entries_;
       i < _number_of_entries_ + _reader_->get_number_of_entries(_mode_); ++i) {
    _entry_list_[i] = std::make_pair(_current_file_number_, i - _number_of_entries_);
  }
...
bool brio_access::retrieve_event(event_record& event_, const size_t event_number_) {
