...
int main(int argc_, char ** argv_)
{
  ...
  try {
    ...
    snemo::visualization::view::options_manager::get_instance().set_logging_priority(datatools::logger::PRIO_TRACE);

    // boost_serial access instance
    svi::boost_access my_boost_access;

    // Validate file
    DT_THROW_IF(!my_boost_access.is_valid(filenames),
                std::logic_error,
                "Files are not boost archive !");

    // After validating, open file and check readibility
    DT_THROW_IF(!my_boost_access.open(filenames),
                std::logic_error,
