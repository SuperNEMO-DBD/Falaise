  ...
  try
    {
      clog << datatools::io::notice
           << appname_prefix
           << "Test program for analysis stuff!" << endl;
      ...
      my_event_server.tree_dump (clog,
                                 "Event server:",
                                 "NOTICE: ");

      DT_THROW_IF (!my_event_server.has_sequential_data () &&
                   my_event_server.get_number_of_events () == 0, std::logic_error,
                   "No events found in the given file!");
      ...
      while (my_event_server.next_event ())
        {
          ...
          if (++i_event % noevents_modulo == 0)
            {
              cout << datatools::io::notice
                   << appname_prefix
                   << "Event #" << i_event << endl;
            }

          if (noevents > 0 && i_event > noevents)
            break;
          ...
          // typedef from 'snvisualization/io/data_models.h':
          const svi::sim_data_bank & my_sim_data
            = my_event.get<svi::sim_data_bank> (svi::SD_LABEL);
          if (verbose)
            my_sim_data.tree_dump ();
          ...
          for (genbb::primary_event::particles_col_t::const_iterator
                 i_primary = my_primary_particles.begin ();
...

      {
        ...
        TH1I* primary_nature_histo = new TH1I ("primary_nature_histo","",
                                               last_particle_type + 2, 0,
                                               last_particle_type + 2);
        primary_nature_histo->SetTitle (";;# primary particles");
        primary_nature_histo->GetXaxis ()->SetBinLabel (genbb::primary_particle::GAMMA + 1,
                                                        "gamma");
        ...
        for (vector<int>::const_iterator i = primary_type.begin ();
             i != primary_type.end (); ++i)
          {
            primary_nature_histo->Fill (*i);
          }

        TCanvas* canvas =
          new TCanvas ("Test analysis macro","Test analysis macro", 700, 500);
...

      {
        ...
        TH1D* primary_energy_histo = new TH1D ("primary_energy_histo","",
                                               100, 0, 0);
        primary_energy_histo->SetTitle (";energy [MeV];# primary particles");
        for (vector<double>::const_iterator i = primary_energy.begin ();
             i != primary_energy.end (); ++i)
          {
            primary_energy_histo->Fill (*i);
          }

        TCanvas* canvas =
          new TCanvas ("Primary energy","Primary energy", 700, 500);
        primary_energy_histo->Draw ();
      }

      // Primary particle momentum
      for (map<string, vector<double> >::const_iterator
             i = primary_momentum.begin ();
           i != primary_momentum.end (); ++i)
        {
          ostringstream oss;
          oss << "Particle momentum " << i->first;
          TCanvas* canvas = new TCanvas (oss.str ().c_str (),
                                         oss.str ().c_str (),
                                         700, 500);
          oss.str ().clear ();
          oss << "histo_momentum_p" << i->first;
          TH1D* primary_momentum = new TH1D (oss.str ().c_str (), "",
                                             100, 0, 0);
      ...
      for (map<string, vector<double> >::const_iterator
           ...
           i != vertex_position.end (); ++i)
        {
          ...
          TH1D* vertex_position = new TH1D (oss.str ().c_str (), "",
                                            100, 0, 0);
