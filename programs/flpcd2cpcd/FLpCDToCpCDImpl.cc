...
namespace FLpCDToCpCD {
  ...
  void do_configure(int argc_, char * argv_[], FLpCDToCpCDApplication & flRecApp_)
  {
    FLpCDToCpCDParams & flRecParameters = flRecApp_.parameters;
    // - Default Config
    flRecParameters = FLpCDToCpCDParams::makeDefault();
    ...
    flRecParameters.pcdtocpcdConfig = clArgs.configScript;
    flRecParameters.numberOfRecords = clArgs.maxNumberOfRecords;
    flRecParameters.moduloRecords = clArgs.moduloRecords;
    ...
    //  flRecParameters.pcdtocpcdPipelineConfig = clArgs.configScript;
    DT_LOG_DEBUG(flRecParameters.logLevel, "FLpCDToCpCD parameters after command line parsing:");
    ...
    // First parsing pass of the FLpCDToCpCD configuration script:
    {
      ...
      if (! flRecParameters.pcdtocpcdConfig.empty()) {
        std::string flRecConfigScript = flRecParameters.pcdtocpcdConfig;
      ...
      if (flRecConfig.has_key_with_meta("flpcdtocpcd", "flpcdtocpcd::section")) {
        DT_LOG_DEBUG(flRecParameters.logLevel, "Parsing FLpCDToCpCD main configuration directives...");
        // pCDToCpCD basic subsystem:
        falaise::property_set basicSystem{flRecConfig.get_section("flpcdtocpcd")};
        flRecConfig.remove("flpcdtocpcd");
        
        // Fetch the pcdtocpcd setup URN:
        flRecParameters.pcdtocpcdSetupUrn =
          basicSystem.get<std::string>("pcdtocpcdSetupURN", flRecParameters.pcdtocpcdSetupUrn);
        if (flRecParameters.pcdtocpcdSetupUrn.empty()) {
          // Use the default one:
          flRecParameters.pcdtocpcdSetupUrn = // snemo::processing::
...

        {
          ...
          DT_THROW_IF(!dtkUrnQuery.check_urn_info(flRecParameters.pcdtocpcdSetupUrn, confCategory),
                      std::logic_error,
                      "Cannot query URN='" << flRecParameters.pcdtocpcdSetupUrn << "'!");
             // and extract the associated 'experimentalSetupUrn':
          const datatools::urn_info & recSetupUrnInfo = dtkUrnQuery.get_urn_info(flRecParameters.pcdtocpcdSetupUrn);
            ...
            if (flRecParameters.experimentalSetupUrn.empty()) {
              ...
              if (flRecParameters.experimentalSetupUrn != experimentalSetupUrn) {
                DT_THROW(std::logic_error,
                         ...
                         << "' from the pcdtocpcd setup '" << flRecParameters.pcdtocpcdSetupUrn
                         << "'!");
              } 
            }
          }
        ...
        flRecParameters.numberOfRecords =
          basicSystem.get<int>("numberOfRecords", flRecParameters.numberOfRecords);

        // Printing rate for events:
        flRecParameters.moduloRecords =
          basicSystem.get<int>("moduloRecords", flRecParameters.moduloRecords);
      ...
      if (flRecConfig.has_key_with_meta("flpcdtocpcd.variantService", "flpcdtocpcd::section")) {
        DT_LOG_DEBUG(flRecParameters.logLevel, "Parsing FLpCDToCpCD variant service's configuration directives...");
        falaise::property_set variantSubsystem{flRecConfig.get_section("flpcdtocpcd.variantService")};
    ...
    {
      DT_LOG_DEBUG(flRecParameters.logLevel, "Second parsing pass of the FLpCDToCpCD configuration script...");
     ...
     // Second parsing pass of the FLpCDToCpCD configuration script:
      ...
      if (flRecConfig.has_key_with_meta("flpcdtocpcd.plugins", "flpcdtocpcd::section")) {
        DT_LOG_DEBUG(flRecParameters.logLevel, "Parsing FLpCDToCpCD plugins' configuration directives...");
        try {
          falaise::property_set userFLPlugins{flRecConfig.get_section("flpcdtocpcd.plugins")};
          flRecConfig.remove("flpcdtocpcd.plugins");
      ...
      if (flRecConfig.has_key_with_meta("flpcdtocpcd.services", "flpcdtocpcd::section")) {
        DT_LOG_DEBUG(flRecParameters.logLevel, "Parsing FLpCDToCpCD services management directives...");
        falaise::property_set servicesSubsystem{flRecConfig.get_section("flpcdtocpcd.services")};
        flRecConfig.remove("flpcdtocpcd.services");
        ...
        DT_LOG_DEBUG(flRecParameters.logLevel,
                     "FLpCDToCpCD services management config URN = '" << flRecParameters.servicesSubsystemConfigUrn << "'");
        ...
        DT_LOG_DEBUG(flRecParameters.logLevel,
                     "FLpCDToCpCD services management config file = '" << flRecParameters.servicesSubsystemConfig << "'");
      ...
      if (flRecConfig.has_key_with_meta("flpcdtocpcd.pipeline", "flpcdtocpcd::section")) {
        DT_LOG_DEBUG(flRecParameters.logLevel, "Parsing FLpCDToCpCD pipeline's directives...");
        falaise::property_set pipelineSubsystem{flRecConfig.get_section("flpcdtocpcd.pipeline")};
        flRecConfig.remove("flpcdtocpcd.pipeline");

        if (datatools::logger::is_debug(flRecParameters.logLevel)) {
          std::cerr << "Pipeline subsystem: \n" << pipelineSubsystem.to_string() << std::endl;
        }

        flRecParameters.pcdtocpcdPipelineUrn =
          pipelineSubsystem.get<std::string>("configUrn", flRecParameters.pcdtocpcdPipelineUrn);
        DT_LOG_DEBUG(flRecParameters.logLevel,
                     "FLpCDToCpCD pipeline config URN = '" << flRecParameters.pcdtocpcdPipelineUrn << "'");

        flRecParameters.pcdtocpcdPipelineConfig =
          pipelineSubsystem.get<falaise::path>("config", flRecParameters.pcdtocpcdPipelineConfig);
        DT_LOG_DEBUG(flRecParameters.logLevel,
                     "FLpCDToCpCD pipeline config file = '" << flRecParameters.pcdtocpcdPipelineConfig << "'");

        DT_THROW_IF(! flRecParameters.pcdtocpcdPipelineUrn.empty() and ! flRecParameters.pcdtocpcdPipelineConfig.empty(),
                    FLConfigUserError,
                    "Explicit pipeline configuration URN and pipeline configuration file conflict!"); 

        // Name of the top level pipeline module:
        flRecParameters.pcdtocpcdPipelineModule =
          pipelineSubsystem.get<std::string>("module", flRecParameters.pcdtocpcdPipelineModule);
        DT_LOG_DEBUG(flRecParameters.logLevel,
                     "Explicit name of the top level pipeline module = '" << flRecParameters.pcdtocpcdPipelineModule << "'");
      }
  
      {
        // In principle all "flpcdtocpcd::section"(s) of interest should have
        // been processed now.
        // Clean the flRecConfig from unused sections of type "flpcdtocpcd::section":
        ...
        for (const auto& section_key : section_keys) {
          if (flRecConfig.has_key_with_meta(section_key, "flpcdtocpcd::section")) {
            DT_LOG_ERROR(flRecParameters.logLevel, "Found an unused flpcdtocpcd section named '"
        ...
        for (std::size_t i = 0; i < unused_section_keys.size(); i++) {
          ...
          DT_LOG_ERROR(flRecParameters.logLevel, "Unused flpcdtocpcd section named '"
  ...
  void do_configure_variant(FLpCDToCpCDApplication & recApplication_)
  {
    ...
    FLpCDToCpCDParams & flRecParameters_ = recApplication_.parameters;
  ...
  void do_postprocess(FLpCDToCpCDParams& flRecParameters_)
  {
    ...
    if (! flRecParameters_.pcdtocpcdPipelineUrn.empty()) {
      ...
      DT_THROW_IF(! dtkUrnQuery.resolve_urn_to_path(flRecParameters_.pcdtocpcdPipelineUrn,
                                                    conf_rec_category,
                                                    conf_rec_mime,
                                                    conf_rec_path),
                  std::logic_error,
                  "Cannot resolve URN='" << flRecParameters_.pcdtocpcdPipelineUrn << "'!");
      DT_LOG_DEBUG(flRecParameters_.logLevel, "Resolved pipeline definition file '" << conf_rec_path << "' from URN='"
                   << flRecParameters_.pcdtocpcdPipelineUrn << "'");
      flRecParameters_.pcdtocpcdPipelineConfig = conf_rec_path;
    }

    if (! flRecParameters_.pcdtocpcdPipelineConfig.empty()) {
      if (! flRecParameters_.modulesConfig.empty()) {
        DT_THROW(std::logic_error,
                 "Pipeline module definition file '"
                 << flRecParameters_.pcdtocpcdPipelineConfig << "' "
                 << "conflicts with pipeline inline configuration provided by the main configuration script!");
      }
      std::string pipelineConfigFilename = flRecParameters_.pcdtocpcdPipelineConfig;
  ...
  falaise::exit_code do_metadata(const FLpCDToCpCDParams & flRecParameters_,
                                 datatools::multi_properties & flRecMetadata_)
  {
...

    {
      ...
      datatools::properties & system_props =
        flRecMetadata_.add_section("flpcdtocpcd", "flpcdtocpcd::section");
      ...
      if (flRecParameters_.numberOfRecords > 0) {
        system_props.store_integer("numberOfRecords", flRecParameters_.numberOfRecords,
                                   "Number of pcdtocpcded events");
      }
      system_props.store_string("pcdtocpcdSetupUrn", flRecParameters_.pcdtocpcdSetupUrn,
                                "pcdtocpcd setup URN");
...

    {
      ...
      datatools::properties & variants_props =
        flRecMetadata_.add_section("flpcd2cpcd.variantService", "flpcd2cpcd::section");
...

    {
      ...
      datatools::properties & services_props =
        flRecMetadata_.add_section("flpcd2cpcd.services", "flpcd2cpcd::section");
    ...
    {
      // Pipeline section:
      datatools::properties& pcdtocpcd_props =
        flRecMetadata_.add_section("flpcd2cpcd.pipeline", "flpcd2cpcd::section");
      pcdtocpcd_props.set_description("pcd2cpcd setup parameters");

      if (!flRecParameters_.pcdtocpcdPipelineUrn.empty()) {
        pcdtocpcd_props.store_string("configUrn", flRecParameters_.pcdtocpcdPipelineUrn,
				     "pcd2cpcd setup URN");
      } else if (!flRecParameters_.pcdtocpcdPipelineConfig.empty()) {
        pcdtocpcd_props.store_path("config", flRecParameters_.pcdtocpcdPipelineConfig,
				   "pcd2cpcd setup main configuration file");
      }

      if (!flRecParameters_.pcdtocpcdPipelineModule.empty()) {
        pcdtocpcd_props.store_string("module", flRecParameters_.pcdtocpcdPipelineModule,
				     "pcd2cpcd pipeline top module");
      }
    }
