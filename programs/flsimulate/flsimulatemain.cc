...
namespace FLSimulate {
  ...
  falaise::exit_code do_flsimulate(int argc_, char * argv_[]);
...
int main(int argc_, char *argv_[])
{
  ...
  falaise::exit_code ret = FLSimulate::do_flsimulate(argc_, argv_);
...
namespace FLSimulate {
                                 ...
                                 datatools::multi_properties & flSimMetadata)
  {
    ...
    system_props.store_string("bayeux.version",
                              bayeux::version::get_version(),
                              "Bayeux version");

    system_props.store_string("falaise.version",
                              falaise::version::get_version(),
                              "Falaise version");
    
    system_props.store_string("falaise.version.commit",
                              falaise::version::get_commit(),
                              "Falaise commit");

    system_props.store_boolean("falaise.version.state",
                               falaise::version::is_dirty(),
                               "Falaise commit state");

    system_props.store_string("application",
                              "flsimulate",
    ...
    system_props.store_string("userProfile",
                              flSimParameters.userProfile,
                              "User profile");
    ...
    if (flSimParameters.doSimulation) {
      ...
      if (flSimParameters.saveRngSeeding and !flSimParameters.rngSeeding.empty()) {
    ...
    if (!flSimParameters.variantConfigUrn.empty()) {
      variants_props.store_string("configUrn",
                                  flSimParameters.variantConfigUrn,
                                  "Variants setup configuration URN");
    } else if (!flSimParameters.variantServiceConfig.config_filename.empty()) {
      variants_props.store_path("config",
                                flSimParameters.variantServiceConfig.config_filename,
                                "Variants setup configuration path");
    }

    if (!flSimParameters.variantProfileUrn.empty()) {
      variants_props.store_string("profileUrn",
                                  flSimParameters.variantProfileUrn,
                                  "Variants profile URN");
    } else if (!flSimParameters.variantServiceConfig.profile_load.empty()) {
      variants_props.store_path("profile",
                                flSimParameters.variantServiceConfig.profile_load,
                                "Variants profile path");
    }

    if (flSimParameters.saveVariantSettings and
        !flSimParameters.variantServiceConfig.settings.empty()) {
      // Saving effective list of variant settings:
      variants_props.store("settings",
                           flSimParameters.variantServiceConfig.settings,
    ...
    if (!flSimParameters.servicesSubsystemConfigUrn.empty()) {
      ...
      services_props.store_path("config",
  ...
  falaise::exit_code do_flsimulate(int argc_, char * argv_[])
  {
    ...
    try {
      do_configure(argc_, argv_, flSimParameters);
    ...
    if (!flSimParameters.variantServiceConfig.logging.empty()) {
      variantService.set_logging(datatools::logger::get_priority(flSimParameters.variantServiceConfig.logging));
    }
    try {
      if (flSimParameters.variantServiceConfig.is_active()) {
        variantService.configure(flSimParameters.variantServiceConfig);
        ...
        if (!flSimParameters.variantServiceConfig.settings.empty()) {
          ...
          flSimParameters.variantServiceConfig.settings = varRepExp.get_settings();
        }
      }
    ...
    try {
      ...
      DT_THROW_IF(services_config_file.empty(), std::logic_error, "Missing services config file!");
      ...
      if (flSimModule.is_initialized()) {
        ...
        DT_LOG_DEBUG(flSimParameters.logLevel,
                     "PRNG seeding = " << flSimParameters.rngSeeding);
      }
