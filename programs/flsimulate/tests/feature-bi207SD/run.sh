#!/bin/bash
#
# Author: F. Mauger
# Date:   2026-0D-02
#
# Test of the  feature
# ==========================
#
# This test must be run after Falaise installation and setup.
#
# Contents
# --------
#
#  * ``simu.conf`` : main configuration files for flsimulate
#  * ``simu.vprofile`` : variant profile
#  * ``run.sh`` : test script
#
# Run the test script
# -------------------
#
# flsimulate must be in the PATH.
# cd in the directory containing the script:
#
#  $ ./run.sh [--work-dir /tmp/${USER}/fltest-bi207SD] [--visu]
#

# pythonExe=$(which python3)
# if [ -z "${pythonExe}" ]; then
#     echo >&2 "[error] Cannot find Python executable! Abort!"
#     exit 1    
# fi

echo >&2 "[debug] PATH = '${PATH}'"

label="run-feature-bi207SD"
with_visu=0
work_dir=""
cfg_dir=""
step1=1

function parse_cl_opts()
{
    while [ -n "$1" ]; do
	local opt="$1"
	if [ "${opt}" == "--work-dir" ]; then
	    shift 1
	    work_dir="$1"
	elif [ "${opt}" == "--cfg-dir" ]; then
	    shift 1
	    cfg_dir="$1"
	elif [ "${opt}" == "--visu" ]; then
	    with_visu=1
	else
	    echo >&2 "[error] Invalid command line option '${opt}'! Abort!"
	    return 1
	fi
	shift 1
    done
    return 0
}

parse_cl_opts $@
if [ $? -ne 0 ]; then
    exit 1
fi

if [ -z "${work_dir}" ]; then
    work_dir="_tmp_work.d"
fi
if [ -z "${cfg_dir}" ]; then
    cfg_dir="."
fi

#########################################
export FLWORKDIR="${work_dir}/${label}"

echo >&2 "[info] cfg_dir   = '${cfg_dir}'"
echo >&2 "[info] work_dir  = '${work_dir}'"
echo >&2 "[info] FLWORKDIR = '${FLWORKDIR}'"

function my_exit()
{
    local error_code="$1"
    shift 1
    local error_msg="$@"
    if [ -n "${error_msg}" ]; then
	echo >&2 "[error] $@"
    fi
    if [ -d ${FLWORKDIR} ]; then
	echo >&2 "[info] remove work dir '${FLWORKDIR}'"
    	rm -fr ${FLWORKDIR}
    fi
    if [ -L _work.d ]; then
	echo >&2 "[info] remove symlink '_work.d'"
    	rm -f _work.d
    fi
    
    exit ${error_code}
}

which flsimulate > /dev/null 2>&1
if [ $? -ne 0 ]; then
    my_exit 1 "flsimulate is not available! Abort!"
fi

if [ ! -d ${FLWORKDIR} ]; then
    mkdir -p ${FLWORKDIR}
    ln -s ${FLWORKDIR} _work.d
fi

###############################################################################
if [ $step1 -eq 1 ]; then
    cat>&2<<EOF

    ==================================================================
    STEP 1 - Bi207 calibration sources with step hit processors
    ==================================================================
EOF
    cp ${cfg_dir}/simu.vprofile ${FLWORKDIR}/simu.vprofile
    if [ ! -f ${FLWORKDIR}/simu.vprofile ]; then
	my_exit 1 "Missing variant profile file! Abort!"
    fi

    echo >&2 "[info] Dump '${FLWORKDIR}/simu.vprofile' :"
    cat ${FLWORKDIR}/simu.vprofile
    echo >&2 ""

    echo >&2 "[info] Running flsimulate..."
    flsimulate -c ${cfg_dir}/simu.conf -o ${FLWORKDIR}/feature-bi207SD.brio > ${FLWORKDIR}/flsimu.log 2>&1
    if [ $? -ne 0 ]; then
	cat ${FLWORKDIR}/flsimu.log
	my_exit 1 "flsimulate failed! Abort!"	
    fi

    if [ ${with_visu} -eq 1 ]; then
	echo >&2 "[info] Running flvisualize..."
	flvisualize \
	    --variant-profile ${FLWORKDIR}/simu.vprofile \
	    --input-file ${FLWORKDIR}/feature-bi207SD.brio \
	    --focus-on-roi \
	    --show-simulated-vertex 1 \
	    --show-simulated-tracks 1 \
	    --show-simulated-hits 1
    fi
fi

###############################################################################
tree ${FLWORKDIR}/

my_exit 0

# end
