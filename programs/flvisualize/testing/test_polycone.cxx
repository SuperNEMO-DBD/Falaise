...
int main(int argc_, char** argv_) {
  ...
  try {
    clog << datatools::io::notice << appname_prefix << "Test program for class 'polycone_volume'!"
         << endl;
    ...
    while (iarg < argc_) {
      ...
      if (token[0] == '-') {
        ...
        if ((option == "-d") || (option == "--debug")) {
          ...
          clog << datatools::io::warning << appname_prefix << "Ignoring option '" << option << "'!"
               << endl;
        }
      } else {
        string argument = token;
        {
          clog << datatools::io::warning << appname_prefix << "Ignoring argument '" << argument
               << "'!" << endl;
        }
      }
      iarg++;
    ...
    for (double theta = dtheta; theta < 0.5 * M_PI - 0.1 * dtheta; theta += dtheta) {
      double z = b / sqrt(pow(tan(theta), 2) + pow(b / a, 2));
      double r = b * sqrt(1.0 - (z / a) * (z / a));
      my_polycone.add(z, r);
    }
    double a2 = pmt_back_a;
    for (double theta = 0.5 * M_PI; theta > 0.2 * M_PI; theta -= dtheta) {
    ...
    if (visu_mode) {
      ...
      TGeoVolume* gvolume = static_cast<TGeoVolume*>(my_polycone_volume->grab_volume());
