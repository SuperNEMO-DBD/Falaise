...
#include <EventBrowser/view/tiny_viewer.h>
...
// ctor:
tiny_viewer::tiny_viewer() : i_embedded_viewer() {
  _canvas_ = 0;
  _objects_ = 0;
  _text_objects_ = 0;
...
tiny_viewer::tiny_viewer(const std::string &name_, const unsigned int width_,
                         const unsigned int height_, const view_dim_type view_dim_)
...
// dtor:
tiny_viewer::~tiny_viewer() { return; }
  ...
  switch (_view_dim_type) {
    case VIEW_2D: {
      ...
      if (options_manager::get_instance().get_option_flag(FOCUS_ROI) && _objects_) {
        ...
        canvas->Range(-scale_factor / fudge_factor, -1. / fudge_factor, scale_factor / fudge_factor,
                      1. / fudge_factor);
      }
    } break;
    case VIEW_3D: {
...
view_type tiny_viewer::get_view_type() const { return _view_type_; }
...
TGFrame *tiny_viewer::get_frame() { return 0; }
    ...
    while (iter.Next()) {
      ...
      if (a_object->IsA() == TPolyLine3D::Class()) {
        ...
        for (int i = 0; i < pl3d->Size(); ++i) {
          ...
          min_roi_bound.SetXYZ(std::min(min_roi_bound.x(), x), std::min(min_roi_bound.y(), y),
                               std::min(min_roi_bound.z(), z));

          max_roi_bound.SetXYZ(std::max(max_roi_bound.x(), x), std::max(max_roi_bound.y(), y),
...
void tiny_viewer::optimize_range(const TVector3 &min_bound_, const TVector3 &max_bound_) {
  datatools::logger::priority local_priority = datatools::logger::PRIO_WARNING;
  DT_LOG_TRACE(local_priority, "Entering...");
  DT_LOG_TRACE(local_priority, "Min bound (x,y,z)=(" << min_bound_.x() << "," << min_bound_.y()
                                                     << "," << min_bound_.z() << ")");
  DT_LOG_TRACE(local_priority, "Max bound (x,y,z)=(" << max_bound_.x() << "," << max_bound_.y()
                                                     << "," << max_bound_.z() << ")");

  // Adapt pad range to show the most essential part of the
  // event
  double xmin, xmax, ymin, ymax;
  _convert_world_to_pad_coordinates_(min_bound_.x(), min_bound_.y(), min_bound_.z(), xmin, ymin);
  _convert_world_to_pad_coordinates_(max_bound_.x(), max_bound_.y(), max_bound_.z(), xmax, ymax);

  TCanvas *canvas = this->get_canvas();

  switch (_view_dim_type) {
    case VIEW_2D: {
      const double dw = static_cast<double>(canvas->GetWw()) / static_cast<double>(canvas->GetWh());
...
const TVector3 &tiny_viewer::_get_minimal_roi_bound_() const { return _min_roi_bound_; }

const TVector3 &tiny_viewer::_get_maximal_roi_bound_() const { return _max_roi_bound_; }

TVector3 &tiny_viewer::_grab_minimal_roi_bound_() { return _min_roi_bound_; }

TVector3 &tiny_viewer::_grab_maximal_roi_bound_() { return _max_roi_bound_; }

void tiny_viewer::_convert_pixel_to_pad_coordinates_(const int xpixel_, const int ypixel_,
                                                     double &xpad_, double &ypad_) {
...
void tiny_viewer::_convert_pixel_to_world_coordinates_(const int xpixel_, const int ypixel_,
...
void tiny_viewer::_convert_world_to_pad_coordinates_(const double xworld_, const double yworld_,
...
void tiny_viewer::_scale_text_() {
  ...
  while ((itext = (utils::root_utilities::TLatex3D *)iter.Next())) {
    ...
    _zoom_index_ >= 0 ? pixel_size *= (1.0 + _zoom_index_ / 10.0) : pixel_size *= 0.0;
