...
const std::string& style_manager::get_setup_label() const { return _setup_label_; }

size_t style_manager::get_startup_tab() const { return _startup_tab_; }

size_t style_manager::get_startup_2d_view() const { return _startup_2d_view_; }

void style_manager::set_background_color(const size_t color_) { _background_color_ = color_; }
...
bool style_manager::has_color_palette() const { return !_color_palette_.empty(); }
...
const style_manager::particle_properties_dict_type& style_manager::get_particles_properties()
    const {
  return _particles_properties_;
}
...
const std::string& style_manager::get_save_directory() const { return _save_directory_; }

const std::string& style_manager::get_save_extension() const { return _save_extension_; }
...
void style_manager::_read_style_file_(const std::string& style_filename_) {
  ...
  const datatools::properties& browser_config =
      configuration.get("browser_settings").get_properties();
  ...
  if (browser_config.has_key("background_color")) {
    _background_color_ = utils::root_utilities::get_color_value("background_color", browser_config);
  }
  ...
  const datatools::properties& misc_config = configuration.get("miscellaneous").get_properties();
...
void style_manager::_set_particle_settings_(const datatools::properties& config_) {
  for (std::map<std::string, std::string>::const_iterator i = _particle_name_dict_.begin();
    ...
    if (config_.has_key(key)) {
      _particles_properties_[particle_name]._visibility_ = config_.has_flag(key);
    } else {
...
void style_manager::_set_miscellaneous_(const datatools::properties& config_) {
  ...
  if (config_.has_key("digitized_data_color")) {
    _digitized_data_color_ =
        utils::root_utilities::get_color_value("digitized_data_color", config_);
  }
...
void style_manager::dump_into_file(const std::string& filename_) {
  ...
  for (auto it_volume = _volumes_properties_.begin(); it_volume != _volumes_properties_.end();
    ...
    index == _volumes_properties_.size() ? fout << std::endl : fout << " \\" << std::endl;
