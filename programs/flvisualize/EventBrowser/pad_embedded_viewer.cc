...
namespace snemo {
  ...
  namespace visualization {
	...
	for (const auto & objPtr : *_objects_) {
	  // const TObject* a_object = *iter;
	  const TObject* a_object = objPtr.get();
	...
	gStyle->SetPadBottomMargin(0.15);
      ...
      const TVector3& pad_embedded_viewer::_get_maximal_roi_bound_() const { return _max_roi_bound_; }

      TVector3& pad_embedded_viewer::_grab_minimal_roi_bound_() { return _min_roi_bound_; }

      TVector3& pad_embedded_viewer::_grab_maximal_roi_bound_() { return _max_roi_bound_; }

      void pad_embedded_viewer::_convert_pixel_to_pad_coordinates_(const int xpixel_, const int ypixel_,
	...
	for (auto & objPtr : *_text_objects_) {
	 
	  utils::root_utilities::TLatex3D* itext
	    = dynamic_cast<utils::root_utilities::TLatex3D*>(objPtr.get());
	  ...
	  utils::root_utilities::TLatex3D * found = nullptr;
	  // XXX
	  for (auto & objPtr2 : *_text_objects_) {
	    TObject * obj2 = objPtr2.get();
	    if (itext->IsEqual(obj2)) {
	      found = dynamic_cast<utils::root_utilities::TLatex3D*>(obj2);
	      FL_LOG_DEVEL("itext='" << itext->GetText() << "' == obj2='" << found->GetText() << "'");
	      break;
	    }
	  }
	  if (found != nullptr) {
	    FL_LOG_DEVEL("Found != nullptr");
	...
	if ((event_->fState & kKeyShiftMask) != 0u) {
	  ...
	  case kKey_Right:
	    ...
	    // french laptop
	  case kKey_Plus:
	    return this->_zoom(this->GetWidth() / 2, this->GetHeight() / 2);
	    break;
	  default:
	    break;
	  }
	  ...
	  case kKey_Plus:
	    ...
	    break;
	  case kKey_Minus:
	    return this->_unzoom(this->GetWidth() / 2, this->GetHeight() / 2);
	    break;
	  case kKey_Down:
	  case kKey_Up:
	  case kKey_Left:
	...
	DT_LOG_TRACE(datatools::logger::PRIO_WARNING,
		     event_->fCode << " " << event_->fType << " " << event_->fX << " " << event_->fY);
	...
	case kButton1Motion: {
	  ...
	  canvas->Range(+xdown - (x - xmin), +ydown - (y - ymin), +xup - (x - xmin), +yup - (y - ymin));
	  canvas->Resize();
	  canvas->Modified();
	  canvas->Update();
	  break;
