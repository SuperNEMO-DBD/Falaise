...
#
#  $ ./run.sh [--work-dir /tmp/${USER}/fltest-issue16] [--visu]
#

echo >&2 "[debug] PATH = '${PATH}'"

label="run-fix-issue16-validation"
with_visu=0
work_dir=""
cfg_dir=""
...
flsimulate-configure --no-gui \
    ...
    -o "${FLWORKDIR}/fix-issue16-validation.vprofile"
if [ $? -ne 0 ]; then
    my_exit 1 "flsimulate-configure failed! Abort!"
fi

echo >&2 "[info] Dump '${FLWORKDIR}/fix-issue16-validation.vprofile' :"
cat ${FLWORKDIR}/fix-issue16-validation.vprofile
echo >&2 ""

echo >&2 "[info] Running flsimulate..."
flsimulate -c ${cfg_dir}/simu.conf -o ${FLWORKDIR}/fix-issue16-validation.xml
if [ $? -ne 0 ]; then
    my_exit 1 "flsimulate failed! Abort!"
fi
grep --max-count=1 "__source_SD" ${FLWORKDIR}/fix-issue16-validation.xml
if [ $? -ne 0 ]; then
...
if [ ${with_visu} -eq 1 ]; then
    ...
    flvisualize \
	-i ${FLWORKDIR}/fix-issue16-validation.xml \
	--focus-on-roi \
