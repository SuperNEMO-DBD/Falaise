...
TEST_CASE("PropertyReader Scalars Work", "") {
  ...
  SECTION("String extraction") {
    ...
    REQUIRE( flp::getValueOrDefault<std::string>(test,"ICouldBePresentButAmNot", "adefault") == "adefault" );
  }
...
TEST_CASE("PropertyReader Vector extraction", "") {
  datatools::properties test;
  ...
  SECTION("Integer Vector extraction") {
    ...
    auto result = flp::getValueOrDefault<std::vector<int> >(test, "ICouldBePresentButAmNot", {4,5,6});
    REQUIRE( result == def );
  }
