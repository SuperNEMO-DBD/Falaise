// -*- mode: c++ ; -*-
// test_xcalo_locator_1.cxx
/*
 * Copyright 2011-2012 F. Mauger
 *
...
void test1 (geomtools::manager & mgr_)
{
  ...
  try
    {
      snemo::geometry::xcalo_locator badCL (mgr_, 666);
    }
  catch (exception & x)
    {
      cerr << "ERROR: test1: As expected, there is no way to use a xcalo_locator for module #666 !"  << endl;
    }
  return;
}
...
void test2 (geomtools::manager & a_mgr, size_t a_nhits, bool a_file)
{
  ...
  snemo::geometry::xcalo_locator CL;
  ...
  for (int i = 0; i < a_nhits; i++)
    {
      ...
      CL.find_geom_id (hit_pos, 1232, gid);
      CT.pause ();
      if (a_file)
        {
          if ((i % 100) == 0)
  ...
  CT.stop ();
  CT.tree_dump (clog);
  clog << "Counts (xcalo_locator) = " << counts << endl;
  return;
}
  ...
  for (int i = 0; i < a_nhits; i++)
    {
      ...
      if ((i % 10000) == 0) clog << "#i = " << i << endl;
      CT.resume ();
      geomtools::geom_id gid = xcalo_locator.get_geom_id (hit_pos, calo_block_type);
      CT.pause ();
      if (a_file)
        {
          if ((i % 100) == 0)
...
void test4 (geomtools::manager & a_mgr)
{
  ...
  clog << "Number of sides = "
       << CL.get_number_of_sides () << endl;

  clog << "Number of walls = "
       << CL.get_number_of_walls () << endl;

  clog << "Number of columns = "
       << CL.get_number_of_columns (0, 0) << endl;

  clog << "Number of rows = "
       << CL.get_number_of_rows (0, 0) << endl;

  clog << "Number of neighbours [0,0,0,0] = "
       << CL.get_number_of_neighbours (0, 0, 0, 0) << endl;

  clog << "Number of neighbours [0,0,0,12] = "
       << CL.get_number_of_neighbours (0, 0, 0, 12) << endl;

  clog << "Number of neighbours [0,0,0,5] = "
       << CL.get_number_of_neighbours (0, 0, 0, 5) << endl;

  clog << "Number of neighbours [0,0,1,15] = "
       << CL.get_number_of_neighbours (0, 0, 1, 15) << endl;
...
void test5 (geomtools::manager & a_mgr)
{
  ...
  for (uint32_t side = 0;  side < 2; side++)
    {
      {
        CL.get_neighbours_ids (side, 0, 0, 0, ids, snemo::geometry::xcalo_locator::NEIGHBOUR_FIRST);
        clog << "Neighbour blocks for block [" << side << ",0,0,0] (with first)=" << ids.size () << endl;
      ...
      {
        CL.get_neighbours_ids (side, 0, 0, 0, ids, snemo::geometry::xcalo_locator::NEIGHBOUR_DIAG);
        clog << "Neighbour blocks for block [" << side << ",0,0,0] (with diagonal)=" << ids.size () << endl;
        for (int i = 0; i < ids.size (); i++)
          {
            clog << ids[i] << ' ';
          }
      ...
      {
        CL.get_neighbours_ids (side, 0, 1, 0, ids, snemo::geometry::xcalo_locator::NEIGHBOUR_FIRST);
        clog << "Neighbour blocks for block [" << side << ",0,1,0] (with first)=" << ids.size () << endl;
        for (int i = 0; i < ids.size (); i++)
          {
            clog << ids[i] << ' ';
          }
      ...
      {
        CL.get_neighbours_ids (side, 0, 1, 4, ids, snemo::geometry::xcalo_locator::NEIGHBOUR_FIRST);
        clog << "Neighbour blocks for block [" << side << ",0,1,4] (with first)=" << ids.size () << endl;
        for (int i = 0; i < ids.size (); i++)
          {
            clog << ids[i] << ' ';
          }
      ...
      {
        CL.get_neighbours_ids (side, 0, 0, 12, ids, snemo::geometry::xcalo_locator::NEIGHBOUR_FIRST);
        clog << "Neighbour blocks for block [" << side << ",0,0,12] (with first)=" << ids.size () << endl;
        for (int i = 0; i < ids.size (); i++)
          {
            clog << ids[i] << ' ';
          }
      ...
      {
        CL.get_neighbours_ids (side, 1, 1, 12, ids, snemo::geometry::xcalo_locator::NEIGHBOUR_FIRST);
        clog << "Neighbour blocks for block [" << side << "1,1,12] (with first)=" << ids.size () << endl;
        for (int i = 0; i < ids.size (); i++)
          {
            clog << ids[i] << ' ';
          }
...

      {
        try
          {
            CL.get_neighbours_ids (side, 3, 0, 14, ids,snemo::geometry::xcalo_locator::NEIGHBOUR_FIRST);
          }
        catch (exception & x)
          {
            cerr << "ERROR: test5: As expected, block [" << side << "3,0,14] is not valid !"  << endl << endl;
          }
      }
    }
...
void test6 (geomtools::manager & a_mgr, bool draw_)
{
  ...
  geomtools::vector_3d vertex (-40.0*CLHEP::cm, -4.0*CLHEP::m, -2.0*CLHEP::m);
  ...
  geomtools::vector_3d propagation_dir (0.01, 0.2, 0.1);
                  ...
                  if (spurious)
                    {
                      ...
                      int neighbours_mask = snemo::geometry::xcalo_locator::NEIGHBOUR_DIAG;
                      if (hit_id.get (1) == 0)
                        {
                          neighbours_mask = snemo::geometry::xcalo_locator::NEIGHBOUR_SIDE;
                        }

                      CL.get_neighbours_ids (hit_id,
                                             neighbour_ids,
                                             neighbours_mask);
                      for (int in = 0; in < neighbour_ids.size (); ++in)
                        {
  ...
  if (draw_)
    {
...

      {
        ...
        gid1.set_address (my_module_number, 0, 0, 1, 3);
        ...
        gid2.set_address (my_module_number, 1, 1, 1, 9);
        if ( CL.is_block_partitioned ())
          {
...
int main (int argc_, char ** argv_)
{
  ...
  try
    {
      clog << "Test program for class 'snemo::geometry::xcalo_locator' !" << endl;
...
// end of test_xcalo_locator_1.cxx
