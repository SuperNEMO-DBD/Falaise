...
void test1 (geomtools::manager & mgr_)
{
  ...
  try
    ...
    {
      cerr << "ERROR: test1: As expected, there is no way to use a gveto_locator for module #666 !"  << endl;
    }
  return;
}
...
void test4 (geomtools::manager & a_mgr)
{
  ...
  clog << "Number of sides = "
       << GL.get_number_of_sides () << endl;

  clog << "Number of walls = "
       << GL.get_number_of_walls () << endl;

  clog << "Number of columns = "
       << GL.get_number_of_columns (0, 0) << endl;

  clog << "Number of neighbours [0,0,0,0] = "
       << GL.get_number_of_neighbours (0, 0, 0, 0) << endl;

  clog << "Number of neighbours [0,0,0,12] = "
       << GL.get_number_of_neighbours (0, 0, 0, 12) << endl;

  clog << "Number of neighbours [0,0,0,5] = "
       << GL.get_number_of_neighbours (0, 0, 0, 5) << endl;

  clog << "Number of neighbours [0,0,1,15] = "
       << GL.get_number_of_neighbours (0, 0, 1, 15) << endl;
...
void test5 (geomtools::manager & a_mgr)
{
  ...
  for (uint32_t side = 0;  side < 2; side++)
    {
      {
        GL.get_neighbours_ids (side, 0, 0, ids, snemo::geometry::gveto_locator::NEIGHBOUR_FIRST);
      ...
      {
        GL.get_neighbours_ids (side, 0, 0, ids, snemo::geometry::gveto_locator::NEIGHBOUR_DIAG);
      ...
      {
        GL.get_neighbours_ids (side, 0, 1, ids, snemo::geometry::gveto_locator::NEIGHBOUR_FIRST);
      ...
      {
        GL.get_neighbours_ids (side, 1, 4, ids, snemo::geometry::gveto_locator::NEIGHBOUR_FIRST);
      ...
      {
        GL.get_neighbours_ids (side, 0, 12, ids, snemo::geometry::gveto_locator::NEIGHBOUR_FIRST);
      ...
      {
        GL.get_neighbours_ids (side, 1, 12, ids, snemo::geometry::gveto_locator::NEIGHBOUR_FIRST);
...

      {
        try
          {
            GL.get_neighbours_ids (side, 3, 14, ids,snemo::geometry::gveto_locator::NEIGHBOUR_FIRST);
          }
        catch (exception & x)
          {
                  ...
                  if (spurious)
                    {
                      ...
                      int neighbours_mask = snemo::geometry::gveto_locator::NEIGHBOUR_DIAG;
                      if (hit_id.get (1) == 0)
                        {
                          neighbours_mask = snemo::geometry::gveto_locator::NEIGHBOUR_SIDE;
                        }

                      GL.get_neighbours_ids (hit_id,
                                             neighbour_ids,
                                             neighbours_mask);
...
// end of test_gveto_locator_1.cxx
