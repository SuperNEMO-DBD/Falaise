...
void test1 (geomtools::manager & mgr_)
{
  ...
  try {
    ...
    cerr << "ERROR: test1: As expected, there is no way to use a calo_locator for module #666 !"  << endl;
  }
  return;
}
...
void test4 (geomtools::manager & a_mgr)
{
  ...
  clog << "Number of columns = "
       << CL.get_number_of_columns (0) << endl;

  clog << "Number of rows = "
       << CL.get_number_of_rows (0) << endl;

  clog << "Number of neighbours [0,0,0] = "
       << CL.get_number_of_neighbours (0, 0, 0) << endl;

  clog << "Number of neighbours [0,0,19] = "
       << CL.get_number_of_neighbours (0, 0, 19) << endl;

  clog << "Number of neighbours [0,0,5] = "
       << CL.get_number_of_neighbours (0, 0, 5) << endl;

  clog << "Number of neighbours [0,6,154] = "
       << CL.get_number_of_neighbours (0, 6, 154) << endl;
...
void test5 (geomtools::manager & a_mgr)
{
  ...
  for (uint32_t side = 0;  side < 2; side++) {
    {
      CL.get_neighbours_ids (side, 0, 0, ids, snemo::geometry::calo_locator::NEIGHBOUR_FIRST);
    ...
    {
      CL.get_neighbours_ids (side, 0, 0, ids, snemo::geometry::calo_locator::NEIGHBOUR_DIAG);
      clog << "Neighbour blocks for block [" << side << ",0,0] (with diagonal)=" << ids.size () << endl;
      for (int i = 0; i < ids.size (); i++) {
        clog << ids[i] << ' ';
      }
      clog << endl << endl;
    ...
    {
      CL.get_neighbours_ids (side, 19, 0, ids, snemo::geometry::calo_locator::NEIGHBOUR_FIRST);
      clog << "Neighbour blocks for block [" << side << ",19,0] (with first)=" << ids.size () << endl;
      for (int i = 0; i < ids.size (); i++) {
        clog << ids[i] << ' ';
      }
      clog << endl << endl;
    ...
    {
      CL.get_neighbours_ids (side, 7, 0, ids, snemo::geometry::calo_locator::NEIGHBOUR_FIRST);
    ...
    {
      CL.get_neighbours_ids (side, 7, 4, ids, snemo::geometry::calo_locator::NEIGHBOUR_FIRST);
    ...
    {
      CL.get_neighbours_ids (side, 0, 12, ids, snemo::geometry::calo_locator::NEIGHBOUR_FIRST);
    ...
    {
      CL.get_neighbours_ids (side, 19, 12, ids, snemo::geometry::calo_locator::NEIGHBOUR_FIRST);
      clog << "Neighbour blocks for block [" << side << ",19,12] (with first)=" << ids.size () << endl;
      for (int i = 0; i < ids.size (); i++) {
        clog << ids[i] << ' ';
      }
      clog << endl << endl;
    ...
    {
      try {
        CL.get_neighbours_ids (side, 0, 14, ids,snemo::geometry::calo_locator::NEIGHBOUR_FIRST);
      }
      catch (exception & x) {
        cerr << "ERROR: test5: As expected, block [" << side << ",0,14] is not valid !"  << endl << endl;
      }
    }

    {
      CL.get_neighbours_ids (side, 19, 6, ids, snemo::geometry::calo_locator::NEIGHBOUR_FIRST);
      clog << "Neighbour blocks for block [" << side << ",19,6] (with first)=" << ids.size () << endl;
      for (int i = 0; i < ids.size (); i++) {
        clog << ids[i] << ' ';
      }
      clog << endl << endl;
    ...
    {
      CL.get_neighbours_ids (side, 4, 12, ids, snemo::geometry::calo_locator::NEIGHBOUR_FIRST);
      clog << "Neighbour blocks for block [" << side << ",4,12] (with first)=" << ids.size () << endl;
      for (int i = 0; i < ids.size (); i++) {
        clog << ids[i] << ' ';
      }
      clog << endl << endl;
    ...
    {
      CL.get_neighbours_ids (side, 0, 4, ids, snemo::geometry::calo_locator::NEIGHBOUR_FIRST);
                  ...
                  if (spurious)
                    {
                      ...
                      int neighbours_mask = snemo::geometry::calo_locator::NEIGHBOUR_DIAG;
                      if (hit_id.get (1) == 0)
                        {
                          neighbours_mask = snemo::geometry::calo_locator::NEIGHBOUR_SIDE;
                        }
...
// end of test_calo_locator_1.cxx
