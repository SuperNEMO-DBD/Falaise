...
namespace FLReconstruct {
  ...
  void do_configure(int argc, char *argv[], FLReconstructParams& flRecParameters)
  {
    ...
    if (flRecConfig.has_section("flreconstruct")) {
    ...
    if (flRecConfig.has_section("flreconstruct.variantService")) {
    ...
    if (flRecConfig.has_section("flreconstruct.plugins")) {
      try {
    ...
    if (flRecConfig.has_section("flreconstruct.services")) {
  ...
  void do_postprocess(FLReconstructParams & flRecParameters)
  {
    ...
    if (!flRecParameters.inputMetadata.empty()) {
      ...
      if (flRecParameters.inputMetadata.has_key("System")) {
        const datatools::properties & inputDataSystemSection
          = flRecParameters.inputMetadata.get_section("System");
      ...
      // Extract informations from the metadata:

      // Extract 'experimentalSetupUrn' if any:
      // If input metadata is provided by the simulation system (flsimulate):
      if (flRecParameters.inputMetadata.has_key("SimulationSubsystem")) {
        ...
        if (inputDataSimuSubsystemSection.has_key("experimentalSetupUrn")) {
          inputDataExperimentalSetupUrn = inputDataSimuSubsystemSection.fetch_string("experimentalSetupUrn");
        } else {
    ...
    if (flRecParameters.experimentalSetupUrn.empty()) {
      ...
      if (!flRecParameters.variantSubsystemParams.config_filename.empty()) {
        DT_LOG_NOTICE(flRecParameters.logLevel, "Using a manually set variants service configuration file.");
      }
                                 ...
                                 datatools::multi_properties & flRecMetadata)
  {
    ...
    datatools::properties & system_props
      = flRecMetadata.add_section("System", "flreconstruct::subsystem");
    ...
    datatools::properties & reconstruction_props
      = flRecMetadata.add_section("ReconstructionSubsystem", "flreconstruct::subsystem");
    reconstruction_props.set_description("Reconstruction setup parameters");

    if (!flRecParameters.experimentalSetupUrn.empty()) {
      reconstruction_props.store_string("experimentalSetupUrn",
                                        flRecParameters.experimentalSetupUrn,
                                        "Experimental setup URN");
    }

    if (flRecParameters.numberOfEvents > 0) {
      reconstruction_props.store_integer("numberOfEvents",
                                         flRecParameters.numberOfEvents,
    ...
    datatools::properties & variants_props
      = flRecMetadata.add_section("VariantSubsystem", "flreconstruct::subsystem");
    ...
    datatools::properties & services_props
      = flRecMetadata.add_section("ServicesSubsystem", "flreconstruct::subsystem");
