// -*- mode: c++ ; -*-
/* gg_step_hit_processor.cc
 */

// Ourselves:
#include <falaise/gg_step_hit_processor.h>
...
namespace snemo {
  ...
  namespace mc {
    ...
    MCTOOLS_STEP_HIT_PROCESSOR_REGISTRATION_IMPLEMENT (gg_step_hit_processor,
                                                       "snemo::mc::gg_step_hit_processor");
                                              ...
                                              const mctools::base_step_hit & step_hit_) const
    {
      ...
      //     .has_flag (snemo::mc::gg_track::missing_geiger_hit_key()))
      //   {
      //     return true;
      //   }
        ...
        */
        ...
        if (matching_gg == 0) {
          ...
          if (datatools::is_valid (ionization_time)) {
            ...
            if (get_logging_priority () >= datatools::logger::PRIO_TRACE) {
              DT_LOG_TRACE (get_logging_priority (), "Add '" << gg_track::missing_geiger_hit_flag()  << "'!");
              missing_hits_count++;
            }
            current_gg_hit->set_time_start (minimum_approach_time);
            current_gg_hit->grab_auxiliaries ()
              .store_flag (gg_track::missing_geiger_hit_flag());
          }

          if (_compute_minimum_approach_position_
              && datatools::is_valid (minimum_approach_world_distance)) {
            current_gg_hit->grab_auxiliaries ()
              .store (gg_track::minimum_approach_distance_key(),
                      minimum_approach_world_distance);
            ...
            current_gg_hit->grab_auxiliaries ()
              .store (gg_track::minimum_approach_position_key(),
          ...
          if (current_gg_hit->get_auxiliaries ()
              .has_flag(gg_track::missing_geiger_hit_flag())) {
            if (datatools::is_valid (ionization_time)) {
              if (get_logging_priority () >= datatools::logger::PRIO_TRACE) {
                DT_LOG_TRACE (get_logging_priority (), "Found ionization ! Remove '" << gg_track::missing_geiger_hit_flag() << "' flag !");
                missing_hits_count--;
              }
              update = true;
              current_gg_hit->grab_auxiliaries ()
                .erase (gg_track::missing_geiger_hit_flag());
            } else {
              // nothing here
            }
          } else {
          ...
          if (_compute_minimum_approach_position_
              && datatools::is_valid (minimum_approach_world_distance)) {
            if (! current_gg_hit->get_auxiliaries ().has_key (gg_track::minimum_approach_distance_key())) {
              current_gg_hit->grab_auxiliaries ().store (gg_track::minimum_approach_distance_key(),
                                                         minimum_approach_world_distance);
              std::vector<double> minimum_approach_world_pos_vec;
              minimum_approach_world_pos_vec.reserve (3);
              for (int i = 0; i < 3; i++) {
              ...
              current_gg_hit->grab_auxiliaries ().store (gg_track::minimum_approach_position_key(),
                                                         minimum_approach_world_pos_vec);
            } else {
              const double current_minimum_approach_distance
                = current_gg_hit->get_auxiliaries ()
                .fetch_real (gg_track::minimum_approach_distance_key());
              if (minimum_approach_world_distance < current_minimum_approach_distance) {
                current_gg_hit->grab_auxiliaries ()
                  .update (gg_track::minimum_approach_distance_key(),
                           minimum_approach_world_distance);
                std::vector<double> minimum_approach_world_pos_vec;
                minimum_approach_world_pos_vec.reserve (3);
                for (int i = 0; i < 3; i++) {
                ...
                current_gg_hit->grab_auxiliaries ()
                  .update (gg_track::minimum_approach_position_key(),
                           minimum_approach_world_pos_vec);
              }
            }
          }
          ...
          for (mctools::simulated_data::hit_handle_collection_type::iterator i = gg_hits_->begin ();
            ...
            if (the_hit_handle.has_data ()) {
              ...
              if (the_hit.grab_auxiliaries ()
                  .has_flag (gg_track::missing_geiger_hit_flag())) {
                DT_LOG_TRACE (get_logging_priority (), "Found a handle hit with '" << gg_track::missing_geiger_hit_flag() << "' !");
...
// end of gg_step_hit_processor.cc
