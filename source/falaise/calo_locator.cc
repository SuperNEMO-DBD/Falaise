...
namespace snemo {
  ...
  namespace geometry {
      ...
      if (side_ == (uint32_t) BACK_SIDE )
        {
          ...
          DT_THROW_IF (row_ >= _back_block_z_.size (),
                       std::logic_error,
                       "Invalid row number (" << row_ << ">" << _back_block_z_.size () - 1 << ")!");
          position_.set (_block_x_[BACK_SIDE], _back_block_y_[column_], _back_block_z_[row_]);
        }
      else
        {
          DT_THROW_IF (column_ >= _front_block_y_.size (),
          ...
          DT_THROW_IF (row_ >= _front_block_z_.size (),
                       std::logic_error,
                       "Invalid row number (" << row_ << ">" << _front_block_z_.size () - 1 << ")!");
          position_.set (_block_x_[FRONT_SIDE], _front_block_y_[column_], _front_block_z_[row_]);
        }
      return;
    }
    ...
    void calo_locator::_construct ()
    {
      ...
      vcy[BACK_SIDE]  = &_back_block_y_;
      vcy[FRONT_SIDE] = &_front_block_y_;
      ...
      std::vector<double> * vrz[NSIDES];
      vrz[BACK_SIDE] = &_back_block_z_;
      vrz[FRONT_SIDE] = &_front_block_z_;
      for (unsigned int side = 0; side < NSIDES; side++)
        {
          size_t i_row = 0;
          vrz[side]->reserve (13);
