...
int main (int argc_, char ** argv_)
{
  ...
  try
    {
      clog << "Test program for class 'geomtools::manager' from sngeometry !" << endl;

      // Force loading of the sngeometry DLL :
      DATATOOLS_DLL_FORCE_LOAD("sngeometry","sngeometry::testing::test_retrieve_info");

      /*
      datatools::library_loader LL(datatools::library_loader::allow_unregistered);
      int load_status = LL.load("sngeometry");
      if (load_status != EXIT_SUCCESS) {
        std::ostringstream message;
        message << "sngeometry::test_retrieve_info: "
      ...
      */
      ...
      if (manager_config_file.empty ())
        {
          manager_config_file = "${SNGEOMETRY_DATA_DIR}/resources/setups/snemo/config_3.0/manager.conf";
        }
