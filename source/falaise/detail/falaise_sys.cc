...
namespace falaise {
  ...
  namespace detail {
    ...
    falaise_sys::falaise_sys()
    {
      ...
      if (_logging_ == ::datatools::logger::PRIO_UNDEFINED) {
        DT_LOG_WARNING(::datatools::logger::PRIO_WARNING,
                       "Ignoring invalid FALAISE_SYS_LOGGING=\"" << getenv("FALAISE_SYS_LOGGING") << "\" environment!");
      }
      DT_LOG_TRACE_ENTERING(_logging_);
      DT_THROW_IF(falaise_sys::_instance_ != nullptr,
                  std::logic_error,
    ...
    datatools::logger::priority falaise_sys::get_logging() const
    {
      return _logging_;
    }

    bool falaise_sys::is_initialized() const
    {
    ...
    {
      DT_LOG_TRACE_ENTERING(_logging_);
      DT_THROW_IF(is_initialized(), std::logic_error, "Falaise system singleton is already initialized!");
    ...
    datatools::service_manager & falaise_sys::grab_services()
    {
      return _services_;
    }

    const datatools::service_manager & falaise_sys::get_services() const
    {
      return _services_;
    }

    // static
    bool falaise_sys::is_instantiated()
    {
      return _instance_ != nullptr;
    }

    // static
    falaise_sys & falaise_sys::instance()
    {
      return *_instance_;
    }

    // static
    const falaise_sys & falaise_sys::const_instance()
    {
      return *_instance_;
    }
      ...
      if (datatools::kernel::is_instantiated()) {
        ...
        if (krnl.has_library_info_register()) {
          ...
          if (lib_info_reg.has("falaise")) {
            DT_LOG_TRACE(_logging_, "Deregistration of the Falaise library from the Bayeux/datatools' kernel...");
            lib_info_reg.unregistration("falaise");
            DT_LOG_TRACE(_logging_, "Falaise library has been deregistered from the Bayeux/datatools' kernel.");
          }
        }
      }
    ...
    void falaise_sys::_initialize_urn_services_()
    {
      ...
      // Activate an URN info DB service:
      {
        datatools::urn_db_service & urnSetupDb =
          dynamic_cast<datatools::urn_db_service &>(_services_.load_no_init(fl_setup_db_name(),
      ...
      // Activate an URN resolver service:
      {
        datatools::urn_to_path_resolver_service & urnResourceResolver =
          dynamic_cast<datatools::urn_to_path_resolver_service &>(_services_.load_no_init(fl_resource_resolver_name(),
