// -*- mode: c++ ; -*-
/** \file falaise/snemo/visualization/toy_display.cc
 */

// Ourselves:
#include <falaise/snemo/visualization/toy_display.h>
...
namespace snemo {
  ...
  namespace visualization {
    ...
    bool toy_display_processor::is_initialized () const
    {
      return initialized_;
    }
    ...
    bool toy_display_processor::is_batch () const
    {
    ...
    void toy_display_processor::set_geom_manager (geomtools::manager & geom_manager_)
    {
      DT_THROW_IF (is_initialized (), std::logic_error, "Already initialized !");
      gmgr = &geom_manager_;
      return;
    ...
    void toy_display_processor::set_default_view_parameters ()
    {
      view_mode = "xy";
    ...
    void toy_display_processor::initialize ()
    {
      DT_THROW_IF (is_initialized (), std::logic_error, "Already initialized !");
      ...
      if (setup_label == "snemo" || setup_label == "snemo::tracker_commissioning" )
        {
          {
            // Tracker volume:
    ...
    void toy_display_processor::reset ()
    {
      DT_THROW_IF (! is_initialized (), std::logic_error, "Not initialized !");

      if (temp_draw_file != 0)
        {
          delete temp_draw_file;
          temp_draw_file = 0;
        }

      if (temp_geom_file != 0)
        {
          delete temp_geom_file;
          temp_geom_file = 0;
        }
    ...
    // Ctor:
    toy_display_processor::toy_display_processor (datatools::logger::priority priority_)
    {
      initialized_ = false;
      interactive_ = false;
    ...
    // Ctor:
    toy_display_processor::toy_display_processor (geomtools::manager & geom_manager_,
                                                  int module_number_,
    ...
    // Dtor:
    toy_display_processor::~toy_display_processor ()
    {
      if (is_initialized ())
        {
          reset ();
    ...
    void toy_display_processor::prepare_draw_module_ ()
    {
      DT_LOG_DEBUG (logging_priority, "Entering...");
      // 2012-11-11 FM : Cannot use /tmp/${USER} yet, needs ensure the directory exists(create it on the fly):
      //std::string tmp_geom_file_dir = "/tmp/${USER}";
      const std::string tmp_geom_file_dir = "/tmp";
      const std::string tmp_geom_file_prefix = "dsd_g.";
      temp_geom_file = new datatools::temp_file (tmp_geom_file_dir,
    ...
    datatools::things & toy_display_processor::get_current_event_record_ ()
    {
      return *current_event_record_;
    }

    const datatools::things & toy_display_processor::get_current_event_record_ () const
    {
      return *current_event_record_;
    }

    void toy_display_processor::generate_data_files_ ()
    {
      DT_LOG_DEBUG (logging_priority, "Entering...");
      ...
      temp_draw_file = new datatools::temp_file (tmp_draw_file_dir,
      ...
      // Simulated data :
      if (display_SD && get_current_event_record_ ().has (snemo::datamodel::data_info::SIMULATED_DATA_LABEL))
        {
          DT_LOG_DEBUG (logging_priority, "has SIMULATED_DATA_LABEL !");
          DT_THROW_IF (! get_current_event_record_ ().is_a<mctools::simulated_data> (snemo::datamodel::data_info::SIMULATED_DATA_LABEL),
                       std::logic_error,
                       "Data model has an item named '" << snemo::datamodel::data_info::SIMULATED_DATA_LABEL
                       << "' but which is not a 'simulated_data' instance !");
          mctools::simulated_data & the_sd = get_current_event_record_ ().grab<mctools::simulated_data> (snemo::datamodel::data_info::SIMULATED_DATA_LABEL);
          DT_LOG_DEBUG (logging_priority, "SIMULATED_DATA grabbed !");

          // draw the vertex:
          {
            DT_LOG_DEBUG (logging_priority, "draw the vertex !");
            temp_draw_file->out () << "# vertex: " << std::endl;
          ...
          if (setup_label == "snemo")
            {
              ...
              if (the_sd.has_step_hits (hit_category) && the_sd.get_step_hits (hit_category).size ())
                {
                  temp_draw_file->out () << "# '" << hit_category << "' step hits @ index " << index << ": " << std::endl;
                  ...
                  for (mctools::simulated_data::hit_handle_collection_type::const_iterator i
                       ...
                       i++, count++)
                    {
                      ...
                      geomtools::gnuplot_draw::draw_box (temp_draw_file->out (), pos, rot, cluster_box);
                      scin_hits[gid] = true;
                    }
                  if (BSHC.size ())
                    {
          ...
          if (scin_hits.size () > 0)
            {
              ...
              for (std::map<geomtools::geom_id, bool>::const_iterator i = scin_hits.begin ();
                   ...
                   i++)
                {
                  ...
                  const geomtools::geom_info & ginfo = mapping->get_geom_info (gid);
                  ...
                  if (shape.has_user_draw ())
                    {
                      ...
                      (*user_draw_function) (temp_draw_file->out (),
                    ...
                    {
                      geomtools::gnuplot_draw::draw (temp_draw_file->out (), world_plcmt, shape);
                      valid = true;
                    }
                }
              if (valid)
            ...
            if (the_sd.has_step_hits (hit_category) && the_sd.get_step_hits (hit_category).size ())
              {
                ...
                for (mctools::simulated_data::hit_handle_collection_type::const_iterator i
                     ...
                     i++, count++)
                  {
                    ...
                    geomtools::gnuplot_draw::basic_draw_point (temp_draw_file->out (), BSH.get_position_start ());
                    geomtools::gnuplot_draw::basic_draw_point (temp_draw_file->out (), BSH.get_position_stop ());
                    ...
                    geomtools::gnuplot_draw::draw_circle (temp_draw_file->out (),
            ...
            if (the_sd.has_step_hits (hit_category) && the_sd.get_step_hits (hit_category).size ())
              {
                    ...
                    if (BSH.get_auxiliaries ().has_key (snemo::datamodel::gg_track::minimum_approach_position_key()))
                      {
                        ...
                        geomtools::gnuplot_draw::basic_draw_point (temp_draw_file->out (), the_MAP);
                        count++;
                      }
                    else
                      {
                ...
                if (count > 0)
                  {
                    temp_draw_file->out () << "# '" << hit_category << ".MAP' step hits @ index " << index << ": " << std::endl;
      ...
      // Calibrated data :
      if (display_CD && get_current_event_record_ ().has (snemo::datamodel::data_info::CALIBRATED_DATA_LABEL))
        {
          DT_LOG_DEBUG (logging_priority, "has CALIBRATED_DATA_LABEL !");
          DT_THROW_IF (! get_current_event_record_ ().is_a<snemo::datamodel::calibrated_data> (snemo::datamodel::data_info::CALIBRATED_DATA_LABEL),
                       std::logic_error,
                       "Data model has an item named '" << snemo::datamodel::data_info::CALIBRATED_DATA_LABEL
                       << "' but which is not a 'simulated_data' instance !");
          snemo::datamodel::calibrated_data & the_cd = get_current_event_record_ ().grab<snemo::datamodel::calibrated_data> (snemo::datamodel::data_info::CALIBRATED_DATA_LABEL);
                  ...
                  for (snemo::datamodel::calibrated_data::tracker_hit_collection_type::const_iterator i
                       ...
                       i++)
                    {
                      ...
                      // draw the longitudinal error :
                      {
                        ...
                        geomtools::gnuplot_draw::basic_draw_point (temp_draw_file->out (), pos1);
                        geomtools::gnuplot_draw::basic_draw_point (temp_draw_file->out (), pos2);
                        temp_draw_file->out () << std::endl;
                      }
                  ...
                  for (snemo::datamodel::calibrated_data::tracker_hit_collection_type::const_iterator i
                       ...
                       i++)
                    {
                      ...
                      // draw a square:
                      {
                        ...
                        pos.setZ (CTH.get_z ());
                        const double rmax = 22. * CLHEP::mm;
                        geomtools::gnuplot_draw::draw_rectangle (temp_draw_file->out (),
                                                                 pos,
                                                                 drift_cell_world_plcmt.get_rotation (),
                                                                 2 * rmax, 2 * rmax);
                      }
            ...
            if (CCHC.size ())
              {
                ...
                for (snemo::datamodel::calibrated_data::calorimeter_hit_collection_type::const_iterator i
                     ...
                     i++)
                  {
                    ...
                    if (scin_block_gid.get_type () == calorimeter_block_type)
                      {
      ...
      if (display_TCD && get_current_event_record_ ().has (snemo::datamodel::data_info::TRACKER_CLUSTERING_DATA_LABEL))
        {
          DT_LOG_DEBUG (logging_priority, "has TRACKER_CLUSTERING_DATA_LABEL !");
          DT_THROW_IF (! get_current_event_record_ ().is_a<snemo::datamodel::tracker_clustering_data> (snemo::datamodel::data_info::TRACKER_CLUSTERING_DATA_LABEL),
                       std::logic_error,
          ...
          if (the_tcd.has_default_solution ())
            {
              ...
              */
              ...
              /*
                if (the_uhits.size ())
                {
                data["TCD.unclustered_hits"] = index++;
                temp_draw_file->out () << std::endl << std::endl;
                }
              */
            }
    ...
    void toy_display_processor::draw (datatools::things & a_event_record)
    {
      DT_LOG_DEBUG (logging_priority, "Entering...");
    ...
    void toy_display_processor::reset_display_geom_params_ ()
    {
      zoom_factor_2d = 1.0;
      zoom_center_x = 0.0;
      zoom_center_y = 0.0;
    ...
    void toy_display_processor::build_draw_command_ ()
    {
      DT_LOG_DEBUG (logging_priority, "Entering...");
      ...
      bool use_legend = true;
      if (use_legend)
        {
          cmdstr << "set key on outside right top vertical box;"; // set key graph 1.1, graph 1.1; ";
        }
      else
        {
          cmdstr << "set key off;";
              ...
              if (data.find ("SD.vertex") != data.end ())
                {
                  ...
                  cmdstr << "'" << temp_draw_file->get_filename ()  << "' index "
                         ...
                         << " title \"vertex\"" << ' ' << "with point lt 6 pt 4 ps 1";
                  count++;
                }
            } // if (display_SD_vertex)
              ...
              if (data.find ("SD.__visu.tracks.gamma") != data.end ())
                {
                  ...
                  cmdstr << "'" << temp_draw_file->get_filename ()  << "' index "
                         ...
                         << uopt << " title \"gamma true tracks\" with lines lt 2";
                  if (with_points)
                    {
                      cmdstr << ", '" << temp_draw_file->get_filename ()  << "' index "
                             << data.find ("SD.__visu.tracks.gamma")->second << " "
                             << uopt << " notitle with point lt 2 ps 1.0 pt 6";
                    }
                  count++;
                }
                  ...
                  if (with_points)
                    {
                      cmdstr << ", '" << temp_draw_file->get_filename ()  << "' index "
                             << data.find ("SD.__visu.tracks.alpha")->second << " "
                             << uopt << " notitle with point lt 4 ps 1.0 pt 6";
                    }
                  count++;
                }
              if (data.find ("SD.__visu.tracks.delta_ray_from_alpha") != data.end ())
                {
                  ...
                  cmdstr << "'" << temp_draw_file->get_filename ()  << "' index "
                         ...
                         << uopt << " title \"alpha delta ray true tracks\" with lines lt 4 lw 1";
                  if (with_points)
                    {
                      cmdstr << ", '" << temp_draw_file->get_filename ()  << "' index "
                             << data.find ("SD.__visu.tracks.delta_ray_from_alpha")->second << " "
              ...
              if (data.find ("SD.__visu.tracks.e-") != data.end ())
                {
                  ...
                  cmdstr << "'" << temp_draw_file->get_filename ()  << "' index "
                         ...
                         << uopt << " title \"e- true tracks\" with lines lt 3";
                  if (with_points)
                    {
                      cmdstr << ", '" << temp_draw_file->get_filename ()  << "' index "
                             << data.find ("SD.__visu.tracks.e-")->second << " "
                             << uopt << " notitle with point lt 3 ps 1.0 pt 6";
                    }
                  count++;
                }
              if (data.find ("SD.__visu.tracks.e+") != data.end ())
                {
                  ...
                  cmdstr << "'" << temp_draw_file->get_filename ()  << "' index "
                         << data.find ("SD.__visu.tracks.e+")->second << " "
                         << uopt << " title \"e+ true tracks\" with lines lt 1";
                  if (with_points)
                    {
                      cmdstr << ", '" << temp_draw_file->get_filename ()  << "' index "
                             << data.find ("SD.__visu.tracks.e+")->second << " "
                             << uopt << " notitle with point lt 1 ps 1.0 pt 6";
                    }
                  count++;
                }
              if (data.find ("SD.__visu.tracks.neutron") != data.end ())
                {
                  ...
                  cmdstr << "'" << temp_draw_file->get_filename ()  << "' index "
                         ...
                         << uopt << " title \"neutron true tracks\" with lines lt 1";
                  if (with_points)
                    {
                      cmdstr << ", '" << temp_draw_file->get_filename ()  << "' index "
                             << data.find ("SD.__visu.tracks.neutron")->second << " "
              ...
              if (data.find ("SD.__visu.tracks.mu+") != data.end ())
                {
                  ...
                  cmdstr << "'" << temp_draw_file->get_filename ()  << "' index "
                         ...
                         << uopt << " title \"mu+ true tracks\" with lines lt 1";
                  if (with_points)
                    {
                      cmdstr << ", '" << temp_draw_file->get_filename ()  << "' index "
                             << data.find ("SD.__visu.tracks.mu+")->second << " "
              ...
              if (data.find ("SD.__visu.tracks.__other__") != data.end ())
                {
                  ...
                  cmdstr << "'" << temp_draw_file->get_filename ()  << "' index "
                         ...
                         << uopt << " title \"other true tracks\" with lines lt 8";
                  if (with_points)
                    {
                      cmdstr << ", '" << temp_draw_file->get_filename ()  << "' index "
                             << data.find ("SD.__visu.tracks.__other__")->second << " "
                             << uopt << " notitle with point lt 8 ps 1.0 pt 6";
                    }
                  count++;
                }
            } // if (display_SD_visu_track)
              ...
              if (data.find ("SD.gg") != data.end ())
                {
                  ...
                  cmdstr << "'" << temp_draw_file->get_filename ()  << "' index "
                         ...
                         << uopt << " title \"Geiger hits\" with lines lt 8 lw 1";
                  count++;
                }
            } // if (display_SD_hits)
              ...
              if (data.find ("SD.gg.MAP") != data.end ())
                {
                  ...
                  cmdstr << "'" << temp_draw_file->get_filename ()  << "' index "
                         ...
                         << uopt << " title \"Geiger MAP\" with point lt 4 pt 4 ps 0.5";
                  count++;
                }
            } // if (display_SD_gg_MAP)
        }
          ...
          if (display_CD_tracker_prompt && data.find ("CD.tracker") != data.end ())
            {
              ...
              cmdstr << "'" << temp_draw_file->get_filename ()  << "' index "
                     ...
                     << uopt << " notitle with lines lt 1 lw 1";
              count++;
            }
          ...
          if (display_CD_tracker_noisy && data.find ("CD.tracker_noisy") != data.end ())
            {
              ...
              cmdstr << "'" << temp_draw_file->get_filename ()  << "' index "
                     ...
                     << uopt << " notitle with lines lt 7 lw 1";
              count++;
            }
          ...
          if (data.find ("CD.calorimeter") != data.end ())
            {
              ...
              cmdstr << "'" << temp_draw_file->get_filename ()  << "' index "
                     ...
                     << uopt << " notitle with lines lt 3 lw 1";
              count++;
            }
    ...
    int toy_display_processor::interpret_command_line ()
    {
      ...
      else if (cmd == "debug") logging_priority = datatools::logger::PRIO_DEBUG;
      else if (cmd == "nodebug") logging_priority = datatools::logger::PRIO_WARNING;
      else if (cmd == "dump")
        {
    ...
    Gnuplot & toy_display_processor::get_gnuplot_driver ()
    {
      if (! gnuplot_driver_)
        {
    ...
    void toy_display_processor::display_ ()
    {
      bool print_it = false;
      if (! print_file.empty ())
        {
      ...
      get_gnuplot_driver ().set_title ("SuperNEMO event display (Gnuplot version)");
      ...
      if (print_it)
      ...
      else
        {
          if (wxt)
            ...
            {
              full_cmd << "set terminal x11 title \"SuperNEMO Toy Display\"; ";
            }
        }
    ...
    void toy_display_processor::print_menu (std::ostream & out_) const
    {
      if (logging_priority >= datatools::logger::PRIO_DEBUG)
        {
      ...
      out_ << "  SD                      switch on/off the display of simulated data" << std::endl;
      out_ << "    SD.VX                 switch on/off the display of simulated vertex" << std::endl;
      out_ << "                          if any (SD on only)" << std::endl;
      out_ << "    SD.H                  switch on/off the display of hits if any (SD on only)" << std::endl;
      out_ << "    SD.GG.MAP             switch on/off the display of MAP for Geiger hits" << std::endl;
      out_ << "                          if any (SD on only)" << std::endl;
      out_ << "    SD.VT                 switch on/off the display of 'visu tracks'" << std::endl;
      out_ << "                          if any (SD on only)" << std::endl;
      out_ << "  CD                      switch on/off the display of calibrated data" << std::endl;
      out_ << "  2D modes only :" << std::endl;
      out_ << "    zoom +                to zoom in by a relative factor 2" << std::endl;
      out_ << "    zoom ++               to zoom in by a relative factor 10" << std::endl;
      out_ << "    zoom -                to zoom out by a relative factor 2" << std::endl;
      out_ << "    zoom --               to zoom out by a relative factor 10" << std::endl;
      out_ << "    zoom <value>          to zoom at zooming factor <value>" << std::endl;
      out_ << "    nozoom                no zooming factor" << std::endl;
      out_ << "    move <x> <y> [unit]   to move at position <x> <y> (XY view)" << std::endl;
      out_ << "    move <y> <z> [unit]   to move at position <y> <z> (YZ view)" << std::endl;
      out_ << "    move <x> <z> [unit]   to move at position <x> <z> (XZ view)" << std::endl;
      out_ << "    nomove                no move " << std::endl;
      out_ << "    skip <dx> <dz> [unit] to skip position by <dx> <dy> (XY view)" << std::endl;
      out_ << "    skip <dy> <dz> [unit] to skip position by <dy> <dz> (YZ view)" << std::endl;
      out_ << "    skip <dx> <dz> [unit] to skip position by <dx> <dz> (XZ view)" << std::endl;
    ...
    void toy_display_processor::cmd_dump (datatools::things & a_data)
    {
...
// end of falaise/snemo/visualization/toy_display.cc
