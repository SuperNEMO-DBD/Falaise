...
bool manager::has_geometry_manager() const { return _geometry_manager_ != nullptr; }

void manager::set_geometry_manager(const geomtools::manager& geo_mgr_) {
  DT_THROW_IF(is_initialized(), std::logic_error, "Electronics manager is already initialized ! ");
  DT_THROW_IF(!geo_mgr_.is_initialized(), std::logic_error,
...
const geomtools::manager& manager::get_geometry_manager() const {
  DT_THROW_IF(_geometry_manager_ == nullptr, std::logic_error, "Geometry manager is not set!");
...
bool manager::is_initialized() const { return _initialized_; }
...
void manager::reset() {
  DT_THROW_IF(!is_initialized(), std::logic_error, "Electronics manager is not initialized ! ");
  _initialized_ = false;
  _reset_cabling_();
...
void manager::tree_dump(std::ostream& a_out, const std::string& /*a_title*/,
