...
void particle_track::tree_dump(std::ostream& out_, const std::string& title_,
  ...
  out_ << indent << datatools::i_tree_dumpable::tag << "Track ID : " << get_track_id() << std::endl;

  out_ << indent << datatools::i_tree_dumpable::tag << "Trajectory : ";
  if (has_trajectory()) {
  ...
  out_ << indent << datatools::i_tree_dumpable::tag << "Particle charge : ";
  ...
  out_ << indent << datatools::i_tree_dumpable::tag << "Vertices : ";
  if (has_vertices()) {
  ...
  out_ << std::endl;
  for (vertex_collection_type::const_iterator i = _vertices_.begin(); i != _vertices_.end(); i++) {
  ...
  out_ << std::endl;
  for (calibrated_calorimeter_hit::collection_type::const_iterator i =
           _associated_calorimeter_hits_.begin();
       i != _associated_calorimeter_hits_.end(); i++) {
