...
DATATOOLS_SERIALIZATION_SERIAL_TAG_IMPLEMENTATION(raw_calorimeter_hit,
                                                  "snemo::datamodel::raw_calorimeter_hit")

bool raw_calorimeter_hit::is_valid() const { return _data_description_ != DATA_DESC_UNDEFINED; }
...
uint16_t raw_calorimeter_hit::get_trigger_id() const { return _trigger_id_; }
...
uint16_t raw_calorimeter_hit::get_channel_address() const { return _channel_address_; }
...
uint64_t raw_calorimeter_hit::get_time_info() const { return _time_info_; }

raw_calorimeter_hit::sampling_frequency_type raw_calorimeter_hit::get_sampling_frequency() const {
  return SAMPLING_FREQUENCY_UNDEF;
}

uint16_t raw_calorimeter_hit::get_posttrig() const { return TDC_UNDEFINED; }
...
uint16_t raw_calorimeter_hit::get_data_description() const { return _data_description_; }
...
uint16_t raw_calorimeter_hit::get_waveform_first_sample() const { return _waveform_first_sample_; }
...
uint16_t raw_calorimeter_hit::get_waveform_last_sample() const { return _waveform_last_sample_; }

const raw_calorimeter_hit::waveform_type& raw_calorimeter_hit::get_waveform() const {
  return _waveform_;
}

raw_calorimeter_hit::waveform_type& raw_calorimeter_hit::grab_waveform() { return _waveform_; }
...
uint16_t raw_calorimeter_hit::get_aux_baseline() const { return _aux_baseline_; }
...
uint16_t raw_calorimeter_hit::get_aux_peak_charge() const { return _aux_peak_charge_; }
...
uint16_t raw_calorimeter_hit::get_aux_peak_amplitude() const { return _aux_peak_amplitude_; }
...
uint16_t raw_calorimeter_hit::get_aux_underflow_tot() const { return _aux_underflow_tot_; }
...
uint16_t raw_calorimeter_hit::get_aux_time_cfd_interp() const { return _aux_time_cfd_interp_; }
...
raw_calorimeter_hit::~raw_calorimeter_hit() { return; }
...
void raw_calorimeter_hit::tree_dump(std::ostream& out_, const std::string& title_,
  ...
  out_ << indent << datatools::i_tree_dumpable::tag << "Channel address  : " << _channel_address_
       << std::endl;
  out_ << indent << datatools::i_tree_dumpable::tag << "Time information :  " << std::hex
       << _time_info_ << std::dec << std::endl;
  out_ << indent << datatools::i_tree_dumpable::tag << "Data description : " << _data_description_
       << std::endl;
  if (has_waveform()) {
    out_ << indent << datatools::i_tree_dumpable::tag << "Waveform : " << std::endl;
  ...
  if (has_aux_baseline()) {
    out_ << indent << datatools::i_tree_dumpable::tag << "Baseline : " << _aux_baseline_
         << std::endl;
  }
  if (has_aux_peak_charge()) {
    out_ << indent << datatools::i_tree_dumpable::tag << "Peak charge : " << _aux_peak_charge_
         << std::endl;
  }
  if (has_aux_peak_data()) {
    out_ << indent << datatools::i_tree_dumpable::tag << "Peak amplitude : " << _aux_peak_amplitude_
         << std::endl;
    out_ << indent << datatools::i_tree_dumpable::tag
         << "Peak time @ max amplitude : " << _aux_peak_time_max_amplitude_ << std::endl;
    out_ << indent << datatools::i_tree_dumpable::tag << "Underflow TOT : " << _aux_underflow_tot_
         << std::endl;
  }
