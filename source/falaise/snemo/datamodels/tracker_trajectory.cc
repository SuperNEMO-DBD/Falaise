...
tracker_trajectory::tracker_trajectory() { return; }
...
tracker_trajectory::handle_cluster& tracker_trajectory::grab_cluster_handle() { return _cluster_; }
...
tracker_cluster& tracker_trajectory::grab_cluster() { return _cluster_.grab(); }

const tracker_cluster& tracker_trajectory::get_cluster() const { return _cluster_.get(); }

bool tracker_trajectory::has_pattern() const { return _pattern_.has_data(); }

bool tracker_trajectory::has_orphans() const { return _orphans_.size() > 0; }
...
tracker_trajectory::handle_pattern& tracker_trajectory::grab_pattern_handle() { return _pattern_; }

const tracker_trajectory::handle_pattern& tracker_trajectory::get_pattern_handle() const {
  return _pattern_;
}

base_trajectory_pattern& tracker_trajectory::grab_pattern() { return _pattern_.grab(); }

const base_trajectory_pattern& tracker_trajectory::get_pattern() const { return _pattern_.get(); }
...
void tracker_trajectory::tree_dump(std::ostream& out_, const std::string& title_,
  ...
  out_ << indent << datatools::i_tree_dumpable::tag << "Cluster : ";
  if (has_cluster()) {
  ...
  out_ << indent << datatools::i_tree_dumpable::tag << "Orphan hits : ";
  if (has_orphans()) {
    out_ << _orphans_.size();
  } else {
    out_ << "<none>";
  ...
  out_ << indent << datatools::i_tree_dumpable::tag << "Pattern : ";
  if (!has_pattern()) {
    out_ << "<none>";
  } else {
