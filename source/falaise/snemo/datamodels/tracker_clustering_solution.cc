  ...
  namespace datamodel {
    ...
    */
                                                 ...
                                                 const boost::property_tree::ptree & options_) const
    {
      ...
      bool list_unclustered_hits_opt = options_.get("list_unclustered_hits", false);
      ...
      out_ << popts.indent << datatools::i_tree_dumpable::tag << "Solution ID  : " << id_ << std::endl;

      out_ << popts.indent << datatools::i_tree_dumpable::tag << "Cluster(s)   : " << get_clusters().size()
           << std::endl;
	...
	for (size_t i = 0; i < get_clusters().size(); i++) {
      ...
      out_ << popts.indent << inherit_tag(popts.inherit) << "Auxiliary properties : " << auxiliaries_.size() << std::endl;
      if (list_properties_opt) {
	boost::property_tree::ptree auxOpts;
        auxOpts.put("indent", popts.indent + tags::item(not popts.inherit, true));
        auxiliaries_.print_tree(out_, auxOpts);
      }
