...
namespace snemo {
  ...
  namespace datamodel {
    ...
    public:
      ...
      /// Return the sigma anodic time associated to the hit
      double get_sigma_anodic_time() const;

      /// Set the sigma anodic time associated to the hit
      void set_sigma_anodic_time(double);

      /// Check if the sigma anodic time is valid
      bool has_sigma_anodic_time() const;

      /// Reset the sigma anodic time
    ...
    private:
      double _anodic_time_{datatools::invalid_real()};                      //!< Anodic time of the cell
      double _sigma_anodic_time_{datatools::invalid_real()};                //!< Error on anodic time of the cell
