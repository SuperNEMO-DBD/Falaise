...
class raw_trigger_infos : public datatools::i_serializable, public datatools::i_tree_dumpable {
 public:
 ...
 private:
  ...
  // Trigger primitives:
  std::bitset<NUMBER_OF_GVETO_ZONES>
      _VTZW_[NUMBER_OF_SIDES];  //!< Gamma-veto trigger zoning words per side
  std::bitset<NUMBER_OF_CALO_ZONES>
      _CTZW_[NUMBER_OF_SIDES];  //!< Calorimeter trigger zoning words per side
  std::bitset<NUMBER_OF_LAYERS>
      _TTP_[NUMBER_OF_SIDES][NUMBER_OF_ROWS];  //!< Tracker trigger primitives per side and per row
