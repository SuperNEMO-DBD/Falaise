...
class xcalo_locator : public geomtools::base_locator, public datatools::i_tree_dumpable {
 public:
  ...
  bool find_block_geom_id(const geomtools::vector_3d& world_position_, geomtools::geom_id& gid_,
                          double tolerance_ = GEOMTOOLS_PROPER_TOLERANCE) const;
 ...
 public:
  ...
  /**! @return the Y-position of the entrance window of a wall for specific side and wall (in module
   * coordinate system).
   */
   ...
   */
  void compute_block_position(uint32_t side_, uint32_t wall_, uint32_t column_, uint32_t row_,
   ...
   */
  void compute_block_window_position(uint32_t side_, uint32_t wall_, uint32_t column_,
                                     uint32_t row_, geomtools::vector_3d& module_position_) const;

  /**! @return the position of a block for specific side, wall, column and row (in module coordinate
   * system).
   */
  geomtools::vector_3d get_block_position(uint32_t side_, uint32_t wall_, uint32_t column_,
                                          uint32_t row_) const;

  /**! @return the position of the center of the entrance window of a block
   *   for specific side, wall, column and row (in module coordinate system).
   */
  geomtools::vector_3d get_block_window_position(uint32_t side_, uint32_t wall_, uint32_t column_,
                                                 uint32_t row_) const;
   ...
   * bounding envelope).
   */
  bool is_in_block(const geomtools::vector_3d& module_position_, uint32_t side_, uint32_t wall_,
                   uint32_t column_, uint32_t row_,
                   double tolerance_ = GEOMTOOLS_PROPER_TOLERANCE) const;

  /** Check if a world coordinate system position is in a specific block virtual volume (its
   * bounding envelope).
   */
  bool is_world_position_in_block(const geomtools::vector_3d& world_position_, uint32_t side_,
   ...
   * blocks.
   */
  size_t get_number_of_neighbours(uint32_t side_, uint32_t wall_, uint32_t column_, uint32_t row_,
   ...
   */
  void get_neighbours_ids(uint32_t side_, uint32_t wall_, uint32_t column_, uint32_t row_,
  ...
  /** Given a block with a specific side, wall, column and row, compute its position in the module
   * coordinate system.
   */
  void get_block_position(uint32_t side_, uint32_t wall_, uint32_t column_, uint32_t row_,
 ...
 private:
  ...
  double _block_y_[2][NWALLS_PER_SIDE];
  double _block_window_y_[2][NWALLS_PER_SIDE];
