...
namespace snemo {
	...
	namespace geometry {
		...
		direction_t gg_locator::relativeDirection(const geomtools::geom_id & from_gid_,
																							const geomtools::geom_id & to_gid_) const
		{
			direction_t dir = direction_t::INVALID;
			auto sideF = getSideAddress(from_gid_);
			auto sideT = getSideAddress(to_gid_);
			if (sideF != sideT) {
				return dir;
			}
			auto layerF = getLayerAddress(from_gid_);
			auto layerT = getLayerAddress(to_gid_);
			auto rowF = getRowAddress(from_gid_);
			auto rowT = getRowAddress(to_gid_);
			if (layerF == layerT) {
				if (rowT > rowF) {
			...
			if (rowF == rowT) {
				if (layerT > layerF) {
					if (sideF == 0) {
						dir = direction_t::BACK;
					} else {
						dir = direction_t::FRONT;
					}
				} else if (layerT < layerF) {
					if (sideF == 0) {
						dir = direction_t::FRONT;
					} else {
						dir = direction_t::BACK;
		...
		bool gg_locator::matchGeigerCell(const geomtools::geom_id& gid) const
		{
			if (gid.get_type() != cellGIDType_) return false;
			unsigned int moduleId = gid.get(0);
			if (moduleId != moduleNumber_) return false;
			unsigned int sideId = gid.get(1);
			if (sideId >= numberOfSides() and not gid.is_any(1)) return false;
			if (not gid.is_any(1)) {
				if (gid.get(2) >= numberOfLayers(sideId) and not gid.is_any(2)) return false; 
				if (gid.get(3) >= numberOfRows(sideId) and not gid.is_any(3)) return false;
			} else {
				if (not gid.is_any(2)) {
					if (gid.get(2) >= 9) return false;
				}
				if (not gid.is_any(3)) {
					if (gid.get(3) >= 113) return false;
				}
			}
			return true;
		}

		size_t gg_locator::buildGeigerCells(const geomtools::geom_id & gid_pattern_,
																				std::set<geomtools::geom_id> & gids_) const
		{
			if (not matchGeigerCell(gid_pattern_)) return 0u;
			uint32_t sideMin = 0;
			uint32_t sideMax = numberOfSides() - 1;
			if (not gid_pattern_.is_any(1)) {
				sideMin = gid_pattern_.get(1);
				sideMax = gid_pattern_.get(1);
			}
			DT_LOG_DEBUG(get_logging_priority(), "Side min = " << sideMin);
			DT_LOG_DEBUG(get_logging_priority(), "Side max = " << sideMax);
			size_t nbGids = 0u;
			for (uint32_t sideId = sideMin; sideId <= sideMax; sideId++) {
				DT_LOG_DEBUG(get_logging_priority(), "Scanning side #" << sideId);
				uint32_t layerMin = 0;
				uint32_t layerMax = numberOfLayers(sideId) - 1;
				uint32_t rowMin = 0;
				uint32_t rowMax = numberOfRows(sideId) - 1;
				if (not gid_pattern_.is_any(2)) {
					layerMin = gid_pattern_.get(2);
					layerMax = gid_pattern_.get(2);
				}
				if (not gid_pattern_.is_any(3)) {
					rowMin = gid_pattern_.get(3);
					rowMax = gid_pattern_.get(3);
				}
				DT_LOG_DEBUG(get_logging_priority(), "Layer min = " << layerMin);
				DT_LOG_DEBUG(get_logging_priority(), "Layer max = " << layerMax);
				DT_LOG_DEBUG(get_logging_priority(), "Row min = " << rowMin);
				DT_LOG_DEBUG(get_logging_priority(), "Row max = " << rowMax);
				for (uint32_t layerId = layerMin; layerId <= layerMax; layerId++) {
					DT_LOG_DEBUG(get_logging_priority(), "Scanning layer #" << layerId);
					for (uint32_t rowId = rowMin; rowId <= rowMax; rowId++) {
						DT_LOG_DEBUG(get_logging_priority(), "Scanning row #" << rowId);
						geomtools::geom_id tgid(gid_pattern_.get_type(),
																		gid_pattern_.get(0), sideId, layerId, rowId);				
						DT_LOG_DEBUG(get_logging_priority(), "Checking cell GID = " << tgid << " valid=" << std::boolalpha << isGeigerCell(tgid));
						if (isGeigerCell(tgid) and not gids_.count(tgid)) {
							gids_.insert(tgid);
							nbGids++;
						}
					}
