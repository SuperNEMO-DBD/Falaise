...
/* Copyright (C) 2011-2025 Francois Mauger <mauger@lpccaen.in2p3.fr>
 *
...
namespace snemo {
  ...
  namespace geometry {
    ...
    void gveto_locator::construct_() {
      ...
      for (unsigned int iside = 0; iside < utils::NSIDES; iside++) {
        ...
        for (unsigned int wall = 0; wall < NWALLS_PER_SIDE; wall++) {
          ...
          while (true) {
            ...
            DT_LOG_DEBUG(verbosity, "G-veto block ID=" << caloBlockGID << "  ->  POS=" << translation);
      ...
      DT_LOG_DEBUG(verbosity, "========================== Final construct  ==========================");
      for (unsigned int iside = 0; iside < utils::NSIDES; iside++) {
        for (unsigned int iwall = 0; iwall < NWALLS_PER_SIDE; iwall++) {
          for (unsigned int icol = 0; icol < 16u; icol++) {
            geomtools::geom_id caloBlockGID;
            caloBlockGID.set_type(caloBlockGIDType_);
            caloBlockGID.set(moduleAddressIndex_, moduleNumber_);
            caloBlockGID.set(sideAddressIndex_, iside);
            caloBlockGID.set(wallAddressIndex_, iwall);
            caloBlockGID.set(columnAddressIndex_, icol);
            const auto & pos = getBlockPosition(caloBlockGID);
            DT_LOG_DEBUG(verbosity, "G-veto block ID=" << caloBlockGID << "  ->  POS=" << pos);
          }
        }
      }
