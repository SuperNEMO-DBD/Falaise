...
namespace snemo {
	...
	namespace geometry {
		...
		public:
			...
			/** @arg gid the GID to be checked.
			 *  @return true if the GID corresponds to a Geiger cell's drift volume.
			 */
			bool isGeigerCell(const geomtools::geom_id& gid) const;

			/** @arg gid_pattern the GID pattern to be checked (ex: [1204:0.*.3.*], [1204:0.1.3.13], [1204:*.*.*.112])
			 *  @return true if the GID corresponds to a valid pattern to identify Geiger cell's drift volumes.
			 */
			bool matchGeigerCell(const geomtools::geom_id& gid_pattern_) const;

			/** @arg gid_pattern the GID pattern to be checked (ex: [1204:0.*.3.*], [1204:0.1.3.13], [1204:*.*.*.112])
			 *  @arg gids the set to be updated with the GIDs compatible with the pattern (the set can be non-empty)
			 *  @return the number of added GIDs (ignoring the ones that were already in the set)
			 */
			size_t buildGeigerCells(const geomtools::geom_id & gid_pattern_,
			...
			/** Given a cell with a specific geometry IDs, return the relative direction in the XY PLANE
			 *  Only support first rank neighbour cells:
			 *
			 *     [R]
			 *  [B][.][F]
			 *     [L]
			 */
			direction_t relativeDirection(const geomtools::geom_id & from_gid_,
