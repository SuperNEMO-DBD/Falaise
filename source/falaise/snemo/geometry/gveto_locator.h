...
class gveto_locator : public geomtools::base_locator, public datatools::i_tree_dumpable {
 public:
  ...
  /// Constructor:
  gveto_locator(const ::geomtools::manager& mgr_, uint32_t module_number_);
 ...
 public:
   ...
   */
  void compute_block_position(uint32_t side_, uint32_t wall_, uint32_t column_,
   ...
   *   for specific side, wall and column (in module coordinate system).
   */
  geomtools::vector_3d get_block_window_position(uint32_t side_, uint32_t wall_,
  ...
  bool is_in_block(const geomtools::vector_3d& module_position_, uint32_t side_, uint32_t wall_,
                   uint32_t column_, double tolerance_ = GEOMTOOLS_PROPER_TOLERANCE) const;
  ...
  bool is_world_position_in_block(const geomtools::vector_3d& world_position_, uint32_t side_,
                                  uint32_t wall_, uint32_t column_,
                                  double tolerance_ = GEOMTOOLS_PROPER_TOLERANCE) const;

  /** Given a block at specific side, wall and column, returns the number of neighbouring blocks.
   */
  size_t get_number_of_neighbours(uint32_t side_, uint32_t wall_, uint32_t column_,
   ...
   */
  void get_neighbours_ids(uint32_t side_, uint32_t wall_, uint32_t column_,
  ...
  /** Given a block with a specific side, wall and column, compute its position in the module
   * coordinate system.
   */
  void get_block_position(uint32_t side_, uint32_t wall_, uint32_t column_,
