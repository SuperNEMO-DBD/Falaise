...
namespace snemo {
  ...
  namespace geometry {
    ...
    bool locator_plugin::has_calo_locator() const
    {
      return _calo_locator_;
    }

    bool locator_plugin::has_xcalo_locator() const
    {
      return _xcalo_locator_;
    }

    bool locator_plugin::has_gveto_locator() const
    {
    ...
    void locator_plugin::_build_locators(const datatools::properties & config_)
    {
      ...
      if (get_geo_manager().get_setup_label() != "snemo::demonstrator") {
        DT_THROW_IF(true, std::logic_error, "Locator plugin does not support setup '"
