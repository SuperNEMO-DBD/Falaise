...
int main(int argc_, char** argv_) {
  ...
  try {
    std::clog << "Test program for the 'snemo::datamodel::tracker_clustering_solution' class."
              << std::endl;
    ...
    if (draw) {
      ...
      cmd_oss << "plot ";
      cmd_oss << "'"
              << "test_tracker_clustering_solution_vis.data"
              << "' index 0 title 'hits' with lines ";
      cmd_oss << ", '"
              << "test_tracker_clustering_solution_vis.data"
              << "' index 1 title 'cluster #0' with lines";
      cmd_oss << ", '"
              << "test_tracker_clustering_solution_vis.data"
              << "' index 2 title 'cluster #1' with lines";
      cmd_oss << ", '"
              << "test_tracker_clustering_solution_vis.data"
              << "' index 3 title 'solution #0/clusters' with lines";
      cmd_oss << ", '"
              << "test_tracker_clustering_solution_vis.data"
              << "' index 4 title 'solution #0/unclustered' with lines";
      g1.cmd(cmd_oss.str());
      wait_for_key();
    }
