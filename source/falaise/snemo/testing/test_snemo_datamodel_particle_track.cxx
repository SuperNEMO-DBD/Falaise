  ...
  try
    {
      ...
      {
        const size_t nvtx = PT0.fetch_vertices(vertices, sdm::particle_track::VERTEX_ON_MAIN_CALORIMETER);
        std::clog << "Number of vertices on the main calorimeter = " << nvtx << std::endl;
        std::clog << "Total number of vertices = " << vertices.size() << std::endl;
      }
      {
        const size_t nvtx = PT0.fetch_vertices(vertices, sdm::particle_track::VERTEX_ON_MAIN_CALORIMETER | sdm::particle_track::VERTEX_ON_SOURCE_FOIL, true);
