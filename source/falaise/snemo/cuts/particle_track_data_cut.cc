  ...
  if (_mode_ == MODE_UNDEFINED) {
    ...
    if (is_mode_range_non_associated_calorimeter_hits()) {
      ...
      DT_THROW_IF(count == 0, std::logic_error,
                  "Missing 'range_non_associated_calorimeters.min' or "
                  "'range_non_associated_calorimeter_hits.max' property !");
      if (count == 2 && _non_associated_calorimeter_hits_range_min_ >= 0 &&
          _non_associated_calorimeter_hits_range_max_ >= 0) {
        DT_THROW_IF(_non_associated_calorimeter_hits_range_min_ >
                        _non_associated_calorimeter_hits_range_max_,
                    std::logic_error,
                    "Invalid 'range_non_associated_calorimeter_hits.min' > "
                    "'range_non_associated_calorimeter_hits.max' values !");
      }
    ...
    if (is_mode_range_particles()) {
      ...
      if (configuration_.has_key("range_particles.min")) {
        ...
        DT_THROW_IF(nmin < 0, std::range_error,
                    "Invalid min number of particles (" << nmin << ") !");
      ...
      if (configuration_.has_key("range_particles.max")) {
        ...
        DT_THROW_IF(nmax < 0, std::range_error,
                    "Invalid max number of particles (" << nmax << ") !");
...
int particle_track_data_cut::_accept() {
  ...
  if (is_mode_range_non_associated_calorimeter_hits()) {
    ...
    if (!PTD.has_non_associated_calorimeters()) {
      DT_LOG_DEBUG(get_logging_priority(),
                   "The event does not have non associated calorimeter hits !");
      return cuts::SELECTION_INAPPLICABLE;
    }
    const size_t n_unasso_calorimeters = PTD.get_non_associated_calorimeters().size();
    DT_LOG_DEBUG(get_logging_priority(),
                 "Number of non associated calorimeters = "
                     << n_unasso_calorimeters
                     << " (min = " << _non_associated_calorimeter_hits_range_min_
                     << " , max = " << _non_associated_calorimeter_hits_range_max_ << ")");

    bool check = true;
    if (_non_associated_calorimeter_hits_range_min_ >= 0 &&
        n_unasso_calorimeters < (size_t)_non_associated_calorimeter_hits_range_min_) {
      check = false;
    }
    if (_non_associated_calorimeter_hits_range_max_ >= 0 &&
        n_unasso_calorimeters > (size_t)_non_associated_calorimeter_hits_range_max_) {
      check = false;
    }
    if (!check) {
      DT_LOG_DEBUG(get_logging_priority(),
                   "Event rejected by RANGE_NON_ASSOCIATED_CALORIMETER_HITS cut!");
  ...
  if (is_mode_range_particles()) {
    ...
    DT_LOG_DEBUG(get_logging_priority(),
                 "Number of particle track = " << nparticles << " (min = " << _particles_range_min_
                                               << " , max = " << _particles_range_max_ << ")");
    bool check = true;
  ...
  if (check_flag && check_non_associated_calorimeter_hits &&
      check_range_non_associated_calorimeter_hits && check_has_particles && check_range_particles) {
...
DOCD_CLASS_IMPLEMENT_LOAD_BEGIN(snemo::cut::particle_track_data_cut, ocd_) {
  ...
  ocd_.set_class_description(
      "Cut based on criteria applied to the Particle Track data bank stored in the event record");
...

  {
    ...
    cpd.set_name_pattern("mode.range_non_associated_calorimeter_hits")
        .set_terse_description(
            "Mode with a special requested range of non associated calorimeter hits")
        .set_traits(datatools::TYPE_BOOLEAN)
        .add_example(
            "Activate the mode::                                           \n"
            "                                                              \n"
...

  {
    ...
    cpd.set_name_pattern("range_non_associated_calorimeter_hits.min")
        ...
        .add_example(
            "Set a specific minimum number of non associated calorimeter hits:: \n"
            "                                                                   \n"
...

  {
    ...
    cpd.set_name_pattern("range_non_associated_calorimeter_hits.max")
        ...
        .set_traits(datatools::TYPE_INTEGER)
        .add_example(
            "Set a specific maximum number of non associated calorimeter hits:: \n"
            "                                                                   \n"
...

  {
    ...
    cpd.set_name_pattern("range_particles.min")
        ...
        .set_traits(datatools::TYPE_INTEGER)
        .add_example(
            "Set a specific minimum number of particle tracks:: \n"
            "                                                   \n"
...

  {
    ...
    cpd.set_name_pattern("range_particles.max")
        ...
        .set_traits(datatools::TYPE_INTEGER)
        .add_example(
            "Set a specific maximum number of particle tracks:: \n"
            "                                                   \n"
...
DOCD_CLASS_SYSTEM_REGISTRATION(snemo::cut::particle_track_data_cut,
                               "snemo::cut::particle_track_data_cut")
