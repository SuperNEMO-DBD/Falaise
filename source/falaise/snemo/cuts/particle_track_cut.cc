...
void particle_track_cut::initialize(const datatools::properties& configuration_,
  ...
  if (_mode_ == MODE_UNDEFINED) {
    ...
    if (is_mode_has_charge()) {
      ...
      DT_THROW_IF(_charge_type_ != "negative" && _charge_type_ != "positive" &&
                      _charge_type_ != "undefined" && _charge_type_ != "neutral",
    ...
    if (is_mode_range_associated_calorimeter_hits()) {
      ...
      if (configuration_.has_key("range_associated_calorimeter_hits.category")) {
        _calorimeter_hits_range_category_ =
            configuration_.fetch_string("range_associated_calorimeter_hits.category");
      ...
      DT_THROW_IF(count == 0, std::logic_error,
                  "Missing 'range_associated_calorimeters.min' or "
                  "'range_associated_calorimeter_hits.max' property !");
      if (count == 2 && _calorimeter_hits_range_min_ >= 0 && _calorimeter_hits_range_max_ >= 0) {
        DT_THROW_IF(_calorimeter_hits_range_min_ > _calorimeter_hits_range_max_, std::logic_error,
                    "Invalid 'range_associated_calorimeter_hits.min' > "
                    "'range_associated_calorimeter_hits.max' values !");
      }
    ...
    if (is_mode_has_vertex()) {
      ...
      DT_THROW_IF(_vertex_type_ != "wire" && _vertex_type_ != "foil" && _vertex_type_ != "calo" &&
                      _vertex_type_ != "xcalo" && _vertex_type_ != "gveto",
...
int particle_track_cut::_accept() {
  ...
  if (is_mode_range_associated_calorimeter_hits()) {
    ...
    if (_calorimeter_hits_range_category_.empty()) {
      ...
      DT_LOG_DEBUG(get_logging_priority(),
                   "Number of associated calorimeters = "
                       << ncalorimeters << " (min = " << _calorimeter_hits_range_min_
    ...
    if (!check) {
      DT_LOG_DEBUG(get_logging_priority(),
                   "Event rejected by PARTICLE_RANGE_ASSOCIATED_CALORIMETER_HITS cut!");
  ...
  if (is_mode_has_vertex()) {
    ...
    if (!a_particle.has_vertices()) {
      ...
      if (!has_vertex) {
        DT_LOG_DEBUG(get_logging_priority(),
                     "A particle has no vertex on the '" << _vertex_type_ << "' !");
        check_vertex = false;
      }
    }
  ...
  if (check_flag && check_charge && check_calorimeter_association && check_range_calorimeter_hits &&
      check_vertex && check_delayed_cluster) {
...
DOCD_CLASS_IMPLEMENT_LOAD_BEGIN(snemo::cut::particle_track_cut, ocd_) {
  ...
  ocd_.set_class_description(
      "Cut based on criteria applied to a particle track object stored in the particle track data "
      "bank");
...

  {
    ...
    cpd.set_name_pattern("mode.has_charge")
        ...
        .set_traits(datatools::TYPE_BOOLEAN)
        .add_example(
            "Activate the requested charge mode:: \n"
            "                                     \n"
...

  {
    ...
    cpd.set_name_pattern("mode.has_vertex")
        ...
        .set_traits(datatools::TYPE_BOOLEAN)
        .add_example(
            "Activate the requested vertex mode:: \n"
            "                                     \n"
...

  {
    ...
    cpd.set_name_pattern("mode.has_delayed_cluster")
        ...
        .set_traits(datatools::TYPE_BOOLEAN)
        .add_example(
            "Activate the requested delayed cluster mode:: \n"
            "                                              \n"
...

  {
    ...
    cpd.set_name_pattern("mode.has_associated_calorimeter_hits")
        ...
        .set_traits(datatools::TYPE_BOOLEAN)
        .add_example(
            "Activate the requested associated calorimeter hits mode:: \n"
            "                                                          \n"
...

  {
    ...
    cpd.set_name_pattern("has_charge.type")
        ...
        .set_long_description(
            ...
            "                      \n")
        .add_example(
            "Set a specific charge type to be selected:: \n"
            "                                            \n"
...

  {
    ...
    cpd.set_name_pattern("has_vertex.type")
        ...
        .set_long_description(
            ...
            "             \n")
        .add_example(
            "Set a specific vertex type to be selected:: \n"
            "                                            \n"
...

  {
    ...
    cpd.set_name_pattern("range_associated_calorimeter_hits.category")
        ...
        .add_example(
            "Set a specific calorimeter category to be selected::              \n"
            "                                                                  \n"
