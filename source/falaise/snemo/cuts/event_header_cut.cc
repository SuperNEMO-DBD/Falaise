    ...
    if (!line.empty()) {
      ...
      if (word.length() > 0 && word[0] != '#') {
        ...
        if (the_event_id.is_valid()) {
          ...
          DT_LOG_WARNING(get_logging_priority(), "Invalid value for event ID ('"
                                                     << word << "')  while reading file '"
                                                     << filename << "' (list of event IDs) !");
        }
      }
    }
...
int event_header_cut::_accept() {
  ...
  if (is_mode_run_number()) {
    ...
    if (check_run_number) {
      DT_LOG_DEBUG(get_logging_priority(), "RUN_NUMBER mode : (" << _run_number_min_ << " <= " << rn
                                                                 << " <= " << _run_number_max_
                                                                 << ") ACCEPTED !");
    } else {
  ...
  if (is_mode_event_number()) {
    ...
    if (check_event_number) {
      DT_LOG_DEBUG(get_logging_priority(), "EVENT_NUMBER mode : ("
                                               << _event_number_min_ << " <= " << en
                                               << " <= " << _event_number_max_ << ") ACCEPTED !");
    } else {
      DT_LOG_DEBUG(get_logging_priority(),
  ...
  if (is_mode_list_of_event_ids()) {
    ...
    if (check_list_of_events) {
      ...
      DT_LOG_DEBUG(get_logging_priority(),
                   "LIST_OF_EVENT_IDS mode : EID=" << EH.get_id()
