  ...
  if (_mode_ == MODE_UNDEFINED) {
    ...
    if (is_mode_range_chi2ndf()) {
      ...
      if (configuration_.has_key("range_chi2ndf.min")) {
        ...
        DT_THROW_IF(min < 0, std::range_error, "Invalid min value of chi2/ndf (" << min << ") !");
        _chi2ndf_range_min_ = min;
        count++;
      }
      if (configuration_.has_key("range_chi2ndf.max")) {
        double max = configuration_.fetch_real("range_chi2ndf.max");
        DT_THROW_IF(max < 0, std::range_error, "Invalid max value of chi2/ndf (" << max << ") !");
...
int tracker_trajectory_data_cut::_accept() {
  ...
  if (is_mode_range_chi2ndf() || is_mode_range_pvalue()) {
    ...
    for (const auto& the_trajectorie : the_trajectories) {
      ...
      if (is_mode_range_pvalue()) {
        ...
        if (datatools::is_valid(_pvalue_range_min_) && datatools::is_valid(_pvalue_range_max_)) {
      ...
      if (is_mode_range_chi2ndf()) {
        ...
        if (datatools::is_valid(_chi2ndf_range_min_) && datatools::is_valid(_chi2ndf_range_max_)) {
  ...
  if (check_flag && check_has_solution &&
...
DOCD_CLASS_IMPLEMENT_LOAD_BEGIN(snemo::cut::tracker_trajectory_data_cut, ocd_) {
  ...
  ocd_.set_class_description(
      "Cut based on criteria applied to the Tracker Trajectory data bank stored in the event "
      "record");
...

  {
    ...
    cpd.set_name_pattern("range_chi2ndf.min")
        ...
        .add_example(
            "Set a specific minimum value of chi2/ndf:: \n"
            "                                           \n"
...

  {
    ...
    cpd.set_name_pattern("range_chi2ndf.max")
        ...
        .set_explicit_unit(false)
        .add_example(
            "Set a specific maximum value of chi2/ndf:: \n"
            "                                           \n"
...

  {
    ...
    cpd.set_name_pattern("range_pvalue.min")
        ...
        .set_unit_symbol("%")
        .add_example(
            "Set a specific minimum value of p-value::    \n"
            "                                             \n"
...

  {
    ...
    cpd.set_name_pattern("range_pvalue.max")
        ...
        .set_unit_symbol("%")
        .add_example(
            "Set a specific maximum value of p-value::    \n"
            "                                             \n"
...
DOCD_CLASS_SYSTEM_REGISTRATION(snemo::cut::tracker_trajectory_data_cut,
                               "snemo::cut::tracker_trajectory_data_cut")
