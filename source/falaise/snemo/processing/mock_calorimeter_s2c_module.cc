...
namespace snemo {
  ...
  namespace processing {
    ...
    uint32_t mock_calorimeter_s2c_module::energy_correction_parse(const std::string & label_)
    {
      if (not label_.empty()) {
	if (label_ == "bcu") {
	  return mock_calorimeter_s2c_module::ECORR_NON_UNIFORMITY | mock_calorimeter_s2c_module::ECORR_BIRK_CERENKOV;
	} else if (label_ == "u") {
	  return mock_calorimeter_s2c_module::ECORR_NON_UNIFORMITY;
	} else if (label_ == "bc") {
	  return mock_calorimeter_s2c_module::ECORR_BIRK_CERENKOV;
	} else if (label_.empty()) {
	  return mock_calorimeter_s2c_module::ECORR_NONE;
	}
	DT_THROW(std::logic_error, "Invalid label '" << std::quoted(label_) << "' for energy correction");
      }
      return mock_calorimeter_s2c_module::ECORR_NONE;
    }

    std::string mock_calorimeter_s2c_module::energy_correction_repr(const uint32_t flags_)
    {
      std::ostringstream repr;
      if (flags_ & mock_calorimeter_s2c_module::ECORR_BIRK_CERENKOV) repr << "bc";
      if (flags_ & mock_calorimeter_s2c_module::ECORR_NON_UNIFORMITY) repr << "u";
      return repr.str();
    }
	...
	pol3d_parameters_gveto_path = ps.fetch_string("pol3d_parameters_gveto_path");
      ...
      /// Save the energy with specific energy correction factors:
      /// Possible values are:
      ///
      ///   - "" (empty) : no correction
	...
	uint32_t ecorrBitset = energy_correction_parse(ecorrRepr);
	_energy_correction_flags_ = ecorrBitset;
      }
    ...
    void mock_calorimeter_s2c_module::digitizeHits(const mctools::simulated_data& simdata,
	...
	// Default final energy and sigma:
	...
	if (_energy_correction_flags_ == ECORR_U) {
	  // Apply non uniformity energy correction:
	  theCaloHit->set_energy(measuredEnergy);
	  theCaloHit->set_sigma_energy(measuredEnergySigma);
	}
	...
	if (_energy_correction_flags_ == ECORR_BC) {
	  // Apply Birk-Cerenkov energy correction:
	  theCaloHit->set_energy(measuredEnergy);
	  theCaloHit->set_sigma_energy(measuredEnergySigma);
	}
	...
	if (_energy_correction_flags_ == ECORR_BCU) {
	  // Apply non uniformity and Birk-Cerenkov energy corrections:
	  theCaloHit->set_energy(measuredEnergy);
	  theCaloHit->set_sigma_energy(measuredEnergySigma);
	}
    ...
    void mock_calorimeter_s2c_module::process_impl(const mctools::simulated_data& simdata,
						   snemo::datamodel::CalorimeterHitHdlCollection& calohits)
    {
...
DOCD_CLASS_IMPLEMENT_LOAD_BEGIN(snemo::processing::mock_calorimeter_s2c_module, ocd_) {
  ...
  {
    // Description of the 'energy_corrections' configuration property :
    datatools::configuration_property_description& cpd = ocd_.add_property_info();
    cpd.set_name_pattern("energy_corrections")
      .set_terse_description("Label to use specific energy corrections")
      .set_traits(datatools::TYPE_STRING)
      .set_mandatory(false)
      // .set_long_description("Default value: ``0`` ")
      .set_default_value_string("")
      .add_example(
		   "Use the 'Birk-Cerenkov+Non uniformity' corrections:: \n"
		   "                                         \n"
		   "  energy_corrections : string = \"bcu\"  \n"
		   "                                         \n");
  }
