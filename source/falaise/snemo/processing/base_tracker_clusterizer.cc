...
namespace snemo {
  ...
  namespace processing {
    ...
    void base_tracker_clusterizer::_initialize (const datatools::properties & setup_)
    {
      ...
      // Check :
      DT_THROW_IF (! _tpc_setup_data_.check (), std::logic_error,
                                          ...
                                          snemo::datamodel::tracker_clustering_data & clustering_)
    {
      ...
      if (_tpc_setup_data_.processing_prompt_hits) {
        ...
        for (int i = 0; i < (int) _prompt_time_clusters_.size (); i++) {
        ...
        if (_prompt_time_clusters_.size() == 1) {
          ...
          for (int isol = 0; isol < (int) prompt_cd.get_number_of_solutions(); isol++) {
          ...
          // of the source:
          for (int isol = 0; isol < (int) nb_sols; isol++) {
            sdm::tracker_clustering_solution::handle_type h_tc_sol(new sdm::tracker_clustering_solution);
            sdm::tracker_clustering_solution & tc_sol = h_tc_sol.grab();
            tc_sol.set_solution_id(isol);
            tc_sol.grab_auxiliaries().store_flag(sdm::tracker_clustering_data::prompt_key());
      ...
      if (_tpc_setup_data_.processing_delayed_hits) {
        ...
        for (size_t idelayed_clustering = 0;
             idelayed_clustering < delayed_work_clusterings.size();
             idelayed_clustering++) {
          sdm::tracker_clustering_data & delayed_cd = delayed_work_clusterings[idelayed_clustering];
          for (size_t idelayed_sol = 0;
               idelayed_sol < delayed_cd.get_number_of_solutions();
               idelayed_sol++) {
            ...
            if (delayed_sol.get_auxiliaries().has_key(sdm::tracker_clustering_data::clusterizer_id_key())) {
              tc_sol.grab_auxiliaries().store_string(
                sdm::tracker_clustering_data::clusterizer_id_key(),
