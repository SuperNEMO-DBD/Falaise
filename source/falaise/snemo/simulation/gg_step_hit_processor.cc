...
namespace snemo {
  ...
  namespace simulation {
    ...
    void gg_step_hit_processor::process(const ::mctools::base_step_hit_processor::step_hit_ptr_collection_type &baseStepHits,
    ...
    void gg_step_hit_processor::_process(const mctools::base_step_hit_processor::step_hit_ptr_collection_type &hitPtrCollection,
					 mctools::simulated_data::hit_handle_collection_type *handleHits,
					 mctools::simulated_data::hit_collection_type *plainHits)
    {
	...
	if (!perModuleFastGeigerLocators_.empty()) {
	  ...
	  DT_THROW_IF(perModuleFastGeigerLocators_.find(module_number) == perModuleFastGeigerLocators_.end(),
		      std::logic_error,
	...
	if (algo_discrete) {
	  ...
	  for (size_t ion = 0; ion < number_of_ionizations; ++ion) {
	    ...
	    if (not datatools::is_valid(min_drift_distance)) {
	...
	if (not datatools::is_valid(ionization_time) && not computeMinimumApproachPosition_) {
	  continue;
	}
	...
	if (matching_gg == nullptr) {
	  ...
	  if (datatools::is_valid(ionization_time)) {
	    ...
	    current_gg_hit->grab_auxiliaries().store_flag(snemo::datamodel::gg_track::missing_geiger_hit_flag());
	  }
	   ...
	   */
	  bool update = false;
	  if (current_gg_hit->get_auxiliaries().has_flag(snemo::datamodel::gg_track::missing_geiger_hit_flag())) {
	    if (datatools::is_valid(ionization_time)) {
	      update = true;
	      current_gg_hit->grab_auxiliaries().erase(snemo::datamodel::gg_track::missing_geiger_hit_flag());
	    } else {
	      // nothing here
	    }
	  } else {
	  ...
	  if (update) {
	    ...
	    const bool is_primary_particle =
	      the_step_hit.get_auxiliaries().has_flag(mctools::track_utils::PRIMARY_PARTICLE_FLAG);
	    if (is_primary_particle) {
	      current_gg_hit->grab_auxiliaries().update_flag(mctools::track_utils::PRIMARY_PARTICLE_FLAG);
	    }
	  }
	  if (computeMinimumApproachPosition_ && datatools::is_valid(minimum_approach_world_distance)) {
		...
		current_gg_hit->get_auxiliaries().fetch_real(snemo::datamodel::gg_track::minimum_approach_distance_key());
	      if (minimum_approach_world_distance < current_minimum_approach_distance) {
		current_gg_hit->grab_auxiliaries().update(snemo::datamodel::gg_track::minimum_approach_distance_key(),
							  minimum_approach_world_distance);
		std::vector<double> minimum_approach_world_pos_vec;
		minimum_approach_world_pos_vec.reserve(3);
		for (size_t i = 0; i < 3; i++) {
		...
		current_gg_hit->grab_auxiliaries().update(snemo::datamodel::gg_track::minimum_approach_position_key(),
							  minimum_approach_world_pos_vec);
	      }
	    }
	  }
    ...
    void gg_step_hit_processor::_purge_gg_hits(mctools::simulated_data::hit_handle_collection_type *handleHits,
					       mctools::simulated_data::hit_collection_type *plainHits)
    {
      // Check the type of output collection (handles or plain hits) :
      bool useHandles = false;
