// test_drift_time_calibration.cxx
...
int main(int argc_, char ** argv_)
{
  ...
  try {
    ...
    for(int i = 0; i < 6; i++) {
      ...
      hit.set_y(1.25 * hit.get_x() + 4.5 * CLHEP::cm
                + random.flat(0.0, 3.0) * CLHEP::cm);
      ...
      dtc.drift_time_to_radius(hit.get_t(),
                               drift_radius,
                               sigma_drift_radius);
    ...
    if (draw) {
      ...
      }
      {
        ...
        cmd_oss << " cm=10.; sz=45.;"
                ...
                << " set grid; ";
        cmd_oss << "plot ";
        cmd_oss << " '" << ftmp.get_filename () << "' index 1 using 1:2 title \"Cells (X-Y view)\" with lines; pause 3; ";
        cmd_oss << "splot ";
        cmd_oss << " '" << ftmp.get_filename () << "' index 1 using 1:2:3 title \"Cells (3D view)\" with lines; ";
        g1.cmd (cmd_oss.str ());
        g1.showonscreen (); // window output
        geomtools::gnuplot_drawer::wait_for_key ();
      }
