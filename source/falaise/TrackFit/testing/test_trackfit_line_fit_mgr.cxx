...
geomtools::line_3d generate_aligned_geiger_hits ( mygsl::rng & random_,
...
int main (int argc_, char ** argv_)
{
  ...
  try {
    std::clog << "Test program for class 'line_fit_mgr'!" << std::endl;
    ...
    geomtools::line_3d trajectory
      = generate_aligned_geiger_hits(random,
                                     hits,
                                     nhits,
                                     dtc.rmax,
                                     dtc,
    ...
    ftmp.create("/tmp", "test_line_fit_mgr_");
    ...
    std::vector<TrackFit::line_fit_params> guess(TrackFit::line_fit_mgr::guess_utils::NUMBER_OF_GUESS);
    ...
    if (do_fit) {
      ...
      for (int iguess = 0; iguess < max_guess; iguess++) {
        ...
        if (LFM.get_solution ().ok) {
          ...
          std::clog << "NOTICE:   Chi     = "
                    << LFM.get_solution ().chi << std::endl;
          std::clog << "NOTICE:   N(dof)  = "
                    << LFM.get_solution ().ndof << std::endl;
          std::clog << "NOTICE:   Prob(P) = "
                    << LFM.get_solution ().probability_p () << std::endl;
          std::clog << "NOTICE:   Prob(Q) = "
                    << LFM.get_solution ().probability_q () << std::endl;
    ...
    TrackFit::line_fit_mgr::convert_solution (hits_ref,
                                              best_solution,
                                              working_ref,
                                              best_track);
...
std::string gp_macro(const std::string & filename_)
{
  ...
  out << "set ylabel 'y (mm)'; ";
  out << "set zlabel 'z (mm)'; ";
  out << "set title 'trackfit::line_fit_mgr: World coordinates system (X-Y view)'; ";
