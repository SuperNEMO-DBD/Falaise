...
const geomtools::intersection_3d &calo_tapered_scin_box_model::get_solid() const { return _solid_; }
...
calo_tapered_scin_box_model::~calo_tapered_scin_box_model() { return; }
...
void calo_tapered_scin_box_model::_at_construct(const std::string &name_,
  ...
  DT_THROW_IF(!config_.has_key("tapered_angle"), std::logic_error,
              "Missing 'tapered_angle' property !");
  ...
  if (config_.has_key("optical_glue_thickness")) {
    ...
    DT_LOG_TRACE(get_logging_priority(),
                 "optical glue thickness = '" << optical_glue_thickness << "'");
  } else {
  ...
  DT_THROW_IF(
      optical_glue_thickness < 0.0, std::underflow_error,
  ...
  _extrusion_.set(_r_);
  DT_THROW_IF(!_extrusion_.is_valid(), std::logic_error,
              "Invalid dimension(s) for the extrusion sphere !");
  _extrusion_.lock();

  DT_THROW_IF((_h_ < 0.0) || (_h_ > 2 * _z_), std::range_error, "Invalid extrusion position !");

  const double ze = 0.5 * _z_ - _r_ - _h_;
  const double angle_e = acos(std::abs(ze + 0.5 * _z_) / _r_);
  const double re = _r_ * std::sin(angle_e);
  DT_THROW_IF(re > 0.5 * _w_, std::logic_error,
              "Radius of extrusion is too large ("
                  << re / CLHEP::mm << " mm) compared to the dimension of the block ("
                  << 0.5 * _w_ / CLHEP::mm << " mm) !");

  const double rt = _tapered_r_;
  DT_THROW_IF(rt < re, std::logic_error,
              "Tapered radius (" << rt / CLHEP::mm << " mm) is too small (should be >= "
                                 << re / CLHEP::mm << " mm) !");

  const double angle_t = _tapered_angle_;
  DT_THROW_IF((angle_t < 0. * CLHEP::degree) && (angle_t > 90. * CLHEP::degree), std::range_error,
  ...
  // check:
  {
    ...
    DT_THROW_IF(z2 > z1, std::logic_error,
  ...
  _removed_cone_.add(0.5 * _z_ + eps, (0.5 * _z_ - zt) * tan(angle_t), true);
  _removed_cone_.lock();
...
void calo_tapered_scin_box_model::tree_dump(std::ostream &out_, const std::string &title_,
                                            const std::string &indent_, bool inherit_) const {
  std::string indent;
  if (!indent_.empty()) indent = indent_;
  i_model::tree_dump(out_, title_, indent, true);

  out_ << indent << datatools::i_tree_dumpable::tag << "Material : '" << _material_ << "'"
       << std::endl;

  {
    // Mother scintillator box:
    out_ << indent << datatools::i_tree_dumpable::tag << "Mother box : " << std::endl;
    {
  ...
  {
    // Spherical extrusion:
    out_ << indent << datatools::i_tree_dumpable::tag << "Spherical extrusion : " << std::endl;
    {
      std::ostringstream indent_oss;
      indent_oss << indent;
      indent_oss << datatools::i_tree_dumpable::skip_tag;
  ...
  {
    // Solid:
    out_ << indent << datatools::i_tree_dumpable::inherit_tag(inherit_) << "Solid : " << std::endl;
    {
      std::ostringstream indent_oss;
      indent_oss << indent;
...
calo_tapered_scin_box_model::wires_drawer::~wires_drawer() { return; }

void calo_tapered_scin_box_model::wires_drawer::generate_wires_self(geomtools::wires_type &wires_,
                                                                    uint32_t options_) const {
  datatools::logger::priority local_priority = get().get_logging_priority();
  DT_LOG_TRACE(local_priority, "Entering...");
  const geomtools::intersection_3d &solid = get().get_solid();
  ...
  DT_LOG_TRACE(local_priority,
               "Extruded sphere shape @ address " << &extrusion_sphere << " is ok !");

  const double w = get()._w_;
  const double z = get()._z_;
  const double h = get()._h_;
  const double zt = get()._zt_;
  const double rt = get()._tapered_r_;
  const double angle_t = get()._tapered_angle_;
  // const double optical_glue_thickness = _model_->_optical_glue_thickness_;

  double zlim1 = datatools::invalid_real();
  double zlim2 = datatools::invalid_real();

  // Extract specific rendering options for this model:
  const bool draw_scin_block = !(options_ & WR_CTSBM_NO_SCIN_BLOCK);
  const bool draw_extrusion = !(options_ & WR_CTSBM_NO_EXTRUSION);
  ...
  if (draw_scin_block) {
    ...
    mother_world_placement.set_translation(s1.get_placement().get_translation());
    mother_world_placement.set_orientation(s1.get_placement().get_rotation());

    geomtools::placement world_item_placement;
    mother_world_placement.child_to_mother(s1.get_placement(), world_item_placement);
    ...
    top_face_pl.generate_wires(wires_, sh1_pos, sh1_rot, options);
    ...
    for (size_t i = 0; i < 4; i++) {
      ...
      side_pl.generate_wires(wires_, sh1_pos, sh1_rot, options);
    }
    ...
    for (size_t i = 0; i < 4; i++) {
      ...
      cut_pl.generate_wires(wires_, sh1_pos, sh1_rot, options);
    }
  }
  ...
  if (draw_extrusion) {
    ...
    const geomtools::vector_3d &position = s1.get_placement().get_translation();
    const geomtools::rotation_3d &rotation = s1.get_placement().get_rotation();
    geomtools::placement mother_world_placement;
    mother_world_placement.set_translation(position);
    mother_world_placement.set_orientation(rotation);
    ...
    // surface extrusion circle:
    {
      ...
      geomtools::placement world_item_placement;
      mother_world_placement.child_to_mother(c1_plcmt, world_item_placement);
      ...
      geomtools::circle circle1(r1);
      circle1.generate_wires(wires_, sh2_pos, sh2_rot, options);
      geomtools::circle circle2(rt);
      circle2.generate_wires(wires_, sh2_pos, sh2_rot, options);
    }
    ...
    // extrusion arcs:
    {
      ...
      arc1.generate_wires(wires_, position, rotation, options);
      arc2.generate_wires(wires_, position, rotation, options);
    }

    {
      ...
      const geomtools::vector_3d &sh2_pos = world_item_placement.get_translation();
      const geomtools::rotation_3d &sh2_rot = world_item_placement.get_rotation();
      geomtools::circle circle(r1);
      circle.generate_wires(wires_, sh2_pos, sh2_rot, options);
    }
