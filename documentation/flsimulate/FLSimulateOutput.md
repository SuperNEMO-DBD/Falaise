...
generated by Geant4 in the fiducial drift region of a given Geiger cell
 (sensitive volume)
and to randomize the creation of ion/electron ionization pairs
along the truth track. The created pair that is located at the nearest
position from the anod wire is considered as the one that triggers the cell, others
pairs are simple ignored.
...
in the output `mctools::base_step_hit` object (see [this section](@ref flsimulateoutput_mctoolsbasestephitclass)):
 ...
 - the time of creation of the ion/electron pair,
 - the optional position of the *minimal approach position* (MAP) of the truth track
