...
List the name of the available particle generators:
  ...
  From the Falaise source directory: ::
   ...
   $ bxgenbb_inspector \
      ...
      --configuration "@falaise:config/simulation/common/primary_events/1.0/manager.conf" \
      --action "list"
...
Generate 1000 primary events from the ``Bi214_Po214`` generator:
  ...
  From the Falaise source directory: ::
   ...
   $ bxgenbb_inspector \
      ...
      --modulo 100 \
      --histo-def "$(bayeux-config --datadir@genbb_help)/inspector/config/le_nuphy-1.0/inspector_histos_prompt.conf" \
      --histo-def "$(bayeux-config --datadir@genbb_help)/inspector/config/le_nuphy-1.0/inspector_histos_delayed.conf" \
      --prompt \
      --delayed \
      --prompt-time-limit 1 \
