...
int main(int argc_, char** argv_) {
  ...
  try {
    ...
    opts.add_options()("help,h", "produce help message")("debug,d", "debug mode")(
        "input,i", po::value<std::vector<std::string> >(&input_filenames)->multitoken(),
        "set a list of input files")("output,o", po::value<std::string>(&output_filename),
                                     "set the output filename")(
        "max-events,M", po::value<std::size_t>(&max_events)->default_value(10),
        "set the maximum number of events")(
        "calo-map,c", po::value<std::string>(&input_calo_mapping_file), "set the calorimeter map")(
        "tracker-map,t", po::value<std::string>(&input_tracker_mapping_file),
        "set the tracker map")("min-calo-hits,C",
                               po::value<std::size_t>(&min_calo_hits)->default_value(0),
                               "set the minimum number of calo hits in an event to record in a "
                               "brio file")("min-tracker-hits,T",
                                            po::value<std::size_t>(&min_tracker_hits)
                                                ->default_value(0),
                                            "set the minimum number of tracker hits in an event to "
                                            "record in a brio file");  // end of options description
    ...
    for (auto file = input_filenames.begin(); file != input_filenames.end(); file++) {
    ...
    DT_THROW_IF(output_filename.empty(), std::logic_error,
                "The output filename is empty, set a value for the -o option, for the text file "
                "(statistics) !");
    ...
    DT_THROW_IF(input_tracker_mapping_file.empty() || input_calo_mapping_file.empty(),
                std::logic_error, "Missing tracker or calo mapping file !");
    ...
    DT_LOG_INFORMATION(logging,
                       "Output filename for data in root format :" + output_data_root_filename);
    ...
    // std::vector<TGraph>   raw_waveform_data; // to add ???? <- big size 1024 * 16 int * X calo
    // hit std::vector<int> x_value_sample; for (auto isample = 0; isample <
    // fecom::calo_constants::MAX_NUMBER_OF_SAMPLES; isample++) x_value_sample.push_back(isample);
    ...
    // Branch of std::vector<TGraph> not working, error : The class requested (vector<TGraph>) for
    // the branch "vector_raw_waveform_data" refer to an stl collection and do not have a compiled
    // CollectionProxy.  Please generate the dictionary for this class (vector<TGraph>) We don't add
    // waveforms to the root tree for the moment.
    ...
    DT_THROW_IF(!cts.is_initialized(), std::logic_error,
      ...
      if (ER.has(HCRD_bank_label) && ER.is_a<fecom::commissioning_event>(HCRD_bank_label)) {
        ...
        if (CE.has_calo_hits()) {
          for (auto icalo = CE.get_calo_hit_collection().begin();
               icalo != CE.get_calo_hit_collection().end(); icalo++) {
            ...
            if (my_channel_mapping.is_calo_channel_in_map(calo_eid)) {
              ...
              // for (auto isample = 0; isample < fecom::calo_constants::MAX_NUMBER_OF_SAMPLES;
              // isample ++) {
        ...
        if (CE.has_tracker_hits()) {
          ...
          for (auto itrack = CE.get_tracker_hit_collection().begin();
               itrack != CE.get_tracker_hit_collection().end(); itrack++) {
            geomtools::geom_id the_actual_cell_gid = itrack->cell_geometric_id;

            if (itrack->cell_geometric_id.is_valid()) {
              unsigned int layer =
                  itrack->cell_geometric_id.get(fecom::tracker_constants::LAYER_INDEX);
              ...
              if (itrack->has_bot_cathodic_time())
                bot_cathodic_time_ns.push_back(itrack->bot_cathodic_time_ns);
              if (itrack->has_top_cathodic_time())
                top_cathodic_time_ns.push_back(itrack->top_cathodic_time_ns);
              tracker_hit_counter++;
            } else {
              // std::clog << "GID is not valid, it seems that the tracker hit was not in the
              // mapping" << std::endl;
              // Error not throw because it will be useful for debug purpose
              // In commissioning, we had some data from the FEB not mapped with a physical geiger
              // cell
            }
          }
          ...
          for (auto icalo = CE.get_calo_hit_collection().begin();
            ...
            if (my_channel_mapping.is_calo_channel_in_map(calo_eid)) {
              ...
              if (icalo->high_threshold) {
                ...
                cts.charge_picocoulomb_no_ht_calo_TH1F[row]->Fill(
                    std::abs(icalo->charge_picocoulomb));
              ...
              if (CE.get_calo_hit_collection().size() > 1) {
                if (icalo->high_threshold) {
                  double DT_calo_tref =
                      icalo->tdc_ns + icalo->falling_time_ns - event_time_start_ns;
                  if (DT_calo_tref != 0) cts.calo_delta_t_calo_tref_TH1F->Fill(DT_calo_tref);
                }
              }
            } else {
              std::string unmapped_channel =
                  "CATEGORY=CALO TRIGGER_ID=" + std::to_string(icalo->trigger_id) + " SLOT=" +
                  std::to_string(icalo->electronic_id.get(fecom::calo_constants::SLOT_INDEX)) +
                  " CHANNEL=" +
                  std::to_string(icalo->electronic_id.get(fecom::calo_constants::CHANNEL_INDEX));
              cts.unmapped_channels_in_raw_data.push_back(unmapped_channel);
            }

          }  // end of for icalo

          cts.mean_number_of_calo.first =
              (cts.mean_number_of_calo.first * cts.mean_number_of_calo.second +
               CE.get_number_of_calo()) /
              (cts.mean_number_of_calo.second + 1);
          cts.mean_number_of_calo.second++;
          cts.mean_number_of_calo_ht.first =
              (cts.mean_number_of_calo_ht.first * cts.mean_number_of_calo_ht.second +
               CE.get_number_of_calo_ht()) /
              (cts.mean_number_of_calo_ht.second + 1);
        ...
        if (CE.has_tracker_hits()) {
          ...
          for (auto itrack = CE.get_tracker_hit_collection().begin();
            ...
            if (itrack->cell_geometric_id.is_valid()) {
              ...
              if (itrack->has_anodic_t0() && itrack->has_bot_cathodic_time())
                cts.cell_registers[layer].bot_cathode_efficiency++;

            } else {
              // std::clog << "GID is not valid, it seems that the tracker hit was not in the
              // mapping" << std::endl; Error not throw because it will be useful for debug purpose
              // In commissioning, we had some data from the FEB not mapped with a physical geiger
              // cell
              std::string unmapped_channel =
                  "CATEGORY=TRACKER TRIGGER_ID=" + std::to_string(itrack->trigger_id);
              cts.unmapped_channels_in_raw_data.push_back(unmapped_channel);
            }
          }

          cts.mean_number_of_tracker_cells.first =
              (cts.mean_number_of_tracker_cells.first * cts.mean_number_of_tracker_cells.second +
               CE.get_number_of_tracker()) /
              (cts.mean_number_of_tracker_cells.second + 1);
        ...
        if (CE.is_only_tracker()) {
          ...
          cts.mean_number_of_tracker_cells_if_0_calo_ht.first =
              (cts.mean_number_of_tracker_cells_if_0_calo_ht.first *
                   cts.mean_number_of_tracker_cells_if_0_calo_ht.second +
               CE.get_number_of_tracker()) /
              (cts.mean_number_of_tracker_cells_if_0_calo_ht.second + 1);
          cts.mean_number_of_tracker_cells_if_0_calo_ht.second++;
        }
        ...
        if (CE.is_calo_tracker()) {
          ...
          for (auto icalo = CE.get_calo_hit_collection().begin();
            ...
            if (my_channel_mapping.is_calo_channel_in_map(calo_eid)) {
              ...
              if (icalo->high_threshold || icalo->low_threshold) {
                ...
                cts.calo_ht_number_tracker_number_distrib_TH2F->Fill(CE.get_number_of_tracker(),
                                                                     CE.get_number_of_calo_ht());

                double delta_t_calo_calo = icalo->tdc_ns - event_time_start_ns;
                if (delta_t_calo_calo != 0)
                  cts.calo_tracker_delta_t_calo_tref_TH1F->Fill(delta_t_calo_calo);
              }
            }
          }  // end of for icalo
          cts.mean_number_of_calo_ht_if_calo_tracker.first =
              (cts.mean_number_of_calo_ht_if_calo_tracker.first *
                   cts.mean_number_of_calo_ht_if_calo_tracker.second +
               CE.get_number_of_calo_ht()) /
              (cts.mean_number_of_calo_ht_if_calo_tracker.second + 1);
          cts.mean_number_of_calo_ht_if_calo_tracker.second++;
          ...
          for (auto itrack = CE.get_tracker_hit_collection().begin();
            ...
            if (itrack->has_anodic_t0()) {
              if (anode_min_time_ns == 0 || itrack->anodic_t0_ns < anode_min_time_ns)
                anode_min_time_ns = itrack->anodic_t0_ns;
            }
            if (itrack->has_bot_cathodic_time()) {
              if (cathode_min_time_ns == 0 || itrack->bot_cathodic_time_ns < cathode_min_time_ns)
                cathode_min_time_ns = itrack->bot_cathodic_time_ns;
            }
          }  // end of itrack
          cts.mean_number_of_tracker_cells_if_calo_tracker.first =
              (cts.mean_number_of_tracker_cells_if_calo_tracker.first *
                   cts.mean_number_of_tracker_cells_if_calo_tracker.second +
               CE.get_number_of_tracker()) /
              (cts.mean_number_of_tracker_cells_if_calo_tracker.second + 1);
          cts.mean_number_of_tracker_cells_if_calo_tracker.second++;

          if (CE.get_number_of_calo_ht() == 1) {
            cts.mean_number_of_tracker_cells_if_1_calo_ht.first =
                (cts.mean_number_of_tracker_cells_if_1_calo_ht.first *
                     cts.mean_number_of_tracker_cells_if_1_calo_ht.second +
                 CE.get_number_of_tracker()) /
                (cts.mean_number_of_tracker_cells_if_1_calo_ht.second + 1);
            cts.mean_number_of_tracker_cells_if_1_calo_ht.second++;
          } else if (CE.get_number_of_calo_ht() == 2) {
            cts.mean_number_of_tracker_cells_if_2_calo_ht.first =
                (cts.mean_number_of_tracker_cells_if_2_calo_ht.first *
                     cts.mean_number_of_tracker_cells_if_2_calo_ht.second +
                 CE.get_number_of_tracker()) /
                (cts.mean_number_of_tracker_cells_if_2_calo_ht.second + 1);
            cts.mean_number_of_tracker_cells_if_2_calo_ht.second++;
          } else {
            cts.mean_number_of_tracker_cells_if_3p_calo_ht.first =
                (cts.mean_number_of_tracker_cells_if_3p_calo_ht.first *
                     cts.mean_number_of_tracker_cells_if_3p_calo_ht.second +
                 CE.get_number_of_tracker()) /
                (cts.mean_number_of_tracker_cells_if_3p_calo_ht.second + 1);
            cts.mean_number_of_tracker_cells_if_3p_calo_ht.second++;
          }
          ...
          for (auto itrack = CE.get_tracker_hit_collection().begin();
            ...
            if (itrack->cell_geometric_id.is_valid()) {
              ...
              if (itrack->has_bot_cathodic_time())
                cts.cell_registers_if_calo_tracker[layer].Rbot_cath_count++;
              if (itrack->has_top_cathodic_time())
                cts.cell_registers_if_calo_tracker[layer].Rtop_cath_count++;
              if (itrack->has_anodic_t0() && itrack->has_bot_cathodic_time())
                cts.cell_registers_if_calo_tracker[layer].bot_cathode_efficiency++;
              ...
              if (itrack->has_anodic_t0()) {
                ...
                if (DT_anode_tref != 0)
                  cts.calo_tracker_delta_t_anode_tref_TH1F->Fill(DT_anode_tref);

                double DT_anode_anode = itrack->anodic_t0_ns - anode_min_time_ns;
                if (DT_anode_anode != 0)
                  cts.calo_tracker_delta_t_anode_anode_TH1F->Fill(DT_anode_anode);
              }
              ...
              if (itrack->has_bot_cathodic_time()) {
                ...
                if (itrack->has_anodic_t0()) {
                  double DT_cathode_anode_same_hit =
                      itrack->bot_cathodic_time_ns - itrack->anodic_t0_ns;
                  cts.calo_tracker_delta_t_anode_cathode_same_hit_TH1F->Fill(
                      DT_cathode_anode_same_hit);
                }
              }
            }  // end of gid is valid
