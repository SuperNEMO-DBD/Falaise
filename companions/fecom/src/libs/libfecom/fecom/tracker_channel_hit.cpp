...
tracker_channel_hit::tracker_channel_hit() : base_hit(SIG_TRACKER) { reset(); }
...
double tracker_channel_hit::get_timestamp() const { return timestamp_time_ns; }
...
void tracker_channel_hit::tree_dump(std::ostream& out_, const std::string& title_,
                                    const std::string& indent_, bool inherit_) const {
  this->base_hit::tree_dump(out_, title_, indent_, true);

  out_ << indent_ << io::tag() << "Timestamp type            : " << (std::string)timestamp_type
       << std::endl;

  out_ << indent_ << io::tag() << "Timestamp value           : " << (uint64_t)timestamp_value
       << std::endl;

  out_ << indent_ << io::tag() << "Timestamp time (in ns)    : " << timestamp_time_ns << std::endl;
