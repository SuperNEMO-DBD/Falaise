...
void mock_hc2cd_module::initialize(const datatools::properties& setup_,
  ...
  // void 	store_real_with_explicit_unit (const std::string &prop_key, double a_value, const
  // std::string &desc="", bool a_lock=false)
  ...
  gg_setup.store_real_with_explicit_unit("plasma_longitudinal_speed",
                                         20 * CLHEP::cm / CLHEP::microsecond);
  gg_setup.set_unit_symbol("plasma_longitudinal_speed", "cm/us");

  gg_setup.store_real_with_explicit_unit("sigma_plasma_longitudinal_speed",
                                         5 * CLHEP::cm / CLHEP::microsecond);
...
dpp::base_module::process_status mock_hc2cd_module::process(datatools::things& event_record_) {
  ...
  if (!event_record_.has(_HCRD_label_)) {
    DT_THROW_IF(abort_at_missing_input, std::logic_error,
                "Missing half commissioning raw data to be processed !");
...
void mock_hc2cd_module::_process(const fecom::commissioning_event& hc_raw_com_event_,
  ...
  _process_calo_calibration(hc_raw_com_event_, the_calibrated_data_.calibrated_calorimeter_hits());

  _process_tracker_calibration(hc_raw_com_event_, the_calibrated_data_.calibrated_tracker_hits());
 ...
 */
void mock_hc2cd_module::_process_calo_calibration(
    const fecom::commissioning_event& hc_raw_com_event_,
  ...
  for (auto icalo = hc_raw_com_event_.get_calo_hit_collection().begin();
       icalo != hc_raw_com_event_.get_calo_hit_collection().end(); icalo++) {
    ...
    if (_channel_mapping_->is_calo_channel_in_map(icalo->electronic_id)) {
      _channel_mapping_->get_geometric_id_from_electronic_id(icalo->electronic_id,
                                                             fecom_geometric_id);
    }
    // If no, skip the hit
    else
    ...
    convert_gid_to_falaise_gid(fecom_geometric_id, falaise_geometric_id);
    ...
    double time = (icalo->tdc_ns + icalo->falling_time_ns - hc_raw_com_event_.get_time_start_ns()) *
                  CLHEP::nanosecond;
    new_calibrated_calorimeter_hit.set_time(time);

    double energy = 0;
    double sigma_energy = 0;
    chb.calibrate_hit(falaise_geometric_id, icalo->peak_volt * CLHEP::volt, energy, sigma_energy);
 ...
 */
void mock_hc2cd_module::_process_tracker_calibration(
    const fecom::commissioning_event& hc_raw_com_event_,
  ...
  for (auto itracker = hc_raw_com_event_.get_tracker_hit_collection().begin();
       itracker != hc_raw_com_event_.get_tracker_hit_collection().end(); itracker++) {
    if (itracker->is_valid()) {
      ...
      snemo::datamodel::calibrated_data::tracker_hit_handle_type new_handle(
      ...
      if (itracker->has_anodic_t0()) {
        ...
        // 	    << "Mult               = " << itracker->get_anodic_t0() * _tracker_clock_tick_
        // << std::endl
        // 	    << "Event Time start   = " << hc_raw_com_event_.get_time_start_ns() <<
        // std::endl;
        ...
        anode_time =
            (itracker->anodic_t0_ns - hc_raw_com_event_.get_time_start_ns()) * CLHEP::nanosecond;
      ...
      new_calibrated_tracker_hit.set_xy(cell_module_pos.getX(), cell_module_pos.getY());
