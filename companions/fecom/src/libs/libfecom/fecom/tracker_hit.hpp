...
struct tracker_hit : public datatools::i_serializable {
  /// Default constructor
  tracker_hit();
  ...
  void set_anodic_timestamps(const uint64_t t0_, const uint64_t t1_, const uint64_t t2_,
                             const uint64_t t3_, const uint64_t t4_);

  /// Set all cathodic times
  void set_cathodic_timestamps(const uint64_t bot_time_, const uint64_t top_time_);
