...
namespace fecom {
  ...
  // static
  const std::size_t HEADER_NBLINES;
  ...
  bool calo_hit_reader::has_next_event() const
  {
    if (!_initialized_) {
  ...
  void calo_hit_reader::load_next_event(calo_hit & event_)
  {
    if (!is_initialized()) {
      throw std::logic_error("Reader is not initialized!");
    }
  ...
  int calo_hit_reader::get_logging() const
  {
    return _logging_;
  }

  void calo_hit_reader::set_logging(int logging_)
  {
    _logging_ = logging_;
    return;
  }
  ...
  void calo_hit_reader::_init_input_file()
  {
  ...
  void calo_hit_reader::_reset_input_file()
  {
    if (_fin_) {
      _fin_->close();
      _fin_.reset();
  ...
  void calo_hit_reader::_init_header()
  {
    _header_.reset(new run_header);
    std::string hlines[HEADER_NBLINES];
    for (std::size_t ih = 0; ih < HEADER_NBLINES; ih++) {
      std::getline(*_fin_, hlines[ih]);
      if (_logging_ > 1) std::cerr << "fecom::calo_hit_reader::_init_header: "
  ...
  void calo_hit_reader::_reset_header()
  {
    if (_header_) {
      _header_.reset();
    }
  ...
  void calo_hit_reader::_decode_header(const std::string & hline_, const int index_)
  {
    ...
    if (index_ == 0) {
      ...
      if (_logging_ > 1) {
        std::cerr << "fecom::calo_hit_reader::_decode_header: "
