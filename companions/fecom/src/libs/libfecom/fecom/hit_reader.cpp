...
// static
const std::size_t hit_reader::HEADER_NBLINES;

hit_reader::hit_reader() { return; }

void hit_reader::set_input_filename(const std::string& input_filename_) {
  DT_THROW_IF(_initialized_, std::logic_error, "Reader is already initialized!");
  _input_filename_ = input_filename_;
  return;
}

const std::string& hit_reader::get_input_filename() const { return _input_filename_; }

bool hit_reader::has_next_hit() const {
  DT_THROW_IF(!_initialized_, std::logic_error, "Reader is not initialized!");
  if (!*_fin_ || _fin_->eof()) return false;
  return true;
}

void hit_reader::load_run_header(run_header& header_) {
  DT_THROW_IF(!_initialized_, std::logic_error, "Reader is not initialized!");
  header_ = *_header_.get();
  return;
}

void hit_reader::load_next_hit(calo_hit& a_calo_hit_, tracker_channel_hit& a_tracker_channel_hit_) {
  DT_THROW_IF(!_initialized_, std::logic_error, "Reader is not initialized!");

  bool success = _hit_parser_.parse(*_fin_, a_calo_hit_, a_tracker_channel_hit_);
  if (!success) {
...
datatools::logger::priority hit_reader::get_logging() const { return _logging_; }
...
bool hit_reader::is_initialized() const { return _initialized_; }

void hit_reader::initialize() {
  DT_THROW_IF(_initialized_, std::logic_error, "Reader is already initialized!");
  _init_input_file_();
  _init_header_();
  *_fin_ >> std::ws;
...
void hit_reader::_init_input_file_() {
...
void hit_reader::_reset_input_file_() {
  if (_fin_) {
    _fin_->close();
    _fin_.reset();
  }
...
void hit_reader::_reset_header_() {
  if (_header_) {
    _header_.reset();
  }
  return;
...
void hit_reader::_decode_header_(const std::string& hline_, const int index_) {
  ...
  if (index_ == 0) {
    ...
    res = qi::phrase_parse(
        ...
        ("===" >> qi::lit("DATA FILE SAVED WITH SN CRATE SOFTWARE VERSION:") >>
         (+~qi::char_("=")) >> "==" >> qi::lit("DATE OF RUN:") >> qi::lit("UnixTime =")
         // >> (+~qi::char_("="))
         // >> "="
         >> qi::double_ >> qi::lit("date =") >> (+~qi::char_("t")) >> qi::lit("time =") >>
         (+~qi::char_("=")) >> "==="),
        //  End grammar
        qi::space, sw_version, unix_time, date, time);
