...
void calo_calibration::clear_pedestals() { calo_pedestals.clear(); }

void calo_calibration::tree_dump(std::ostream& out_, const std::string& title_,
                                 const std::string& indent_, bool inherit_) const {
  if (!title_.empty()) {
    out_ << indent_ << title_ << std::endl;
  }

  out_ << indent_ << io::tag() << "Calorimeter pedestals: ";
  ...
  out_ << indent_ << io::inherit_last_tag(inherit_) << "End." << std::endl;
  ...
  bool res = false;
  {
    ...
    res = qi::phrase_parse(
        ...
        (qi::lit("Pedestal Calibration values ( int values ADC Counts *16) from cell ") >>
         qi::uint_ >> qi::lit("to") >> qi::uint_ >> qi::lit("for channels") >> qi::uint_ >>
         qi::lit("to") >> qi::uint_ >> qi::lit(":")),
        //  End grammar
        qi::space, start_cell, stop_cell, start_channel, stop_channel);
...
void calo_calibration::_parse_pedestals_per_channel_(const std::string& offsets_line_,
  ...
  std::vector<int> offsets;
  {
    ...
    for (int16_t ioffset = parsing_data_.header.start_cell;
         ioffset <= parsing_data_.header.stop_cell; ioffset++) {
  ...
  try {
    {
      ...
      DT_LOG_DEBUG_SHORT(logging,
                         "|-- Start cell    = [" << parsing_data_.header.start_cell << "]");
      DT_LOG_DEBUG_SHORT(logging, "|-- Stop cell     = [" << parsing_data_.header.stop_cell << "]");
      DT_LOG_DEBUG_SHORT(logging,
                         "|-- Start channel = [" << parsing_data_.header.start_channel << "]");
      DT_LOG_DEBUG_SHORT(logging,
                         "`-- Stop channel  = [" << parsing_data_.header.stop_channel << "]");
    }

    {
      // Parsing pedestals data:
      for (int channel_id = parsing_data_.header.start_channel;
           channel_id <= parsing_data_.header.stop_channel; channel_id++) {
        // Channel header line:
        std::string pdline1;
        ...
        in_ >> std::ws;
        {
          ...
          bool res = false;
          {
            ...
            res = qi::phrase_parse(str_iter, end_iter,
                                   //  Begin grammar
                                   (qi::lit("Channel") >> qi::uint_),
                                   //  End grammar
                                   qi::space, parsed_channel_id);
            if (!res || str_iter != end_iter) {
              DT_THROW(std::logic_error, "Cannot parse channel ID from '" << pdline1 << "'");
            }
          }
          DT_THROW_IF(
              parsed_channel_id != channel_id, std::logic_error,
              "Unexpected channel ID [" << parsed_channel_id << " != " << channel_id << "]!");
        }
