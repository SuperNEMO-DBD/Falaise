...
tracker_hit::tracker_hit() { _reset_(); }

tracker_hit::~tracker_hit() {}

bool tracker_hit::is_valid() const {
  if (!has_geom_id() ||
      (!has_anodic_t0() && !has_anodic_t1() && !has_anodic_t2() && !has_anodic_t3() &&
       !has_anodic_t4() && !has_bot_cathodic_time() && !has_top_cathodic_time()))
    return false;
  return true;
}
...
void tracker_hit::add_anodic_tracker_channel(const fecom::tracker_channel_hit& a_tracker_channel_) {
  DT_THROW_IF(a_tracker_channel_.electronic_id.get_type() != tracker_constants::ANODIC_CHANNEL_TYPE,
              std::logic_error, "Invalid type of tracker channel !");
...
void tracker_hit::add_bot_cathodic_tracker_channel(
    const fecom::tracker_channel_hit& a_tracker_channel_) {
  DT_THROW_IF(
      a_tracker_channel_.electronic_id.get_type() != tracker_constants::CATHODIC_CHANNEL_TYPE,
      std::logic_error, "Invalid type of tracker channel !");
...
void tracker_hit::add_top_cathodic_tracker_channel(
    const fecom::tracker_channel_hit& a_tracker_channel_) {
  DT_THROW_IF(
      a_tracker_channel_.electronic_id.get_type() != tracker_constants::CATHODIC_CHANNEL_TYPE,
      std::logic_error, "Invalid type of tracker channel !");
...
bool tracker_hit::has_geom_id() const { return cell_geometric_id.is_valid(); }

bool tracker_hit::has_anodic_t0() const {
  bool valid = false;
  if (anodic_timestamp_t0 != tracker_constants::INVALID_TIME && datatools::is_valid(anodic_t0_ns))
    valid = true;
  return valid;
}
...
bool tracker_hit::has_anodic_t1() const {
  bool valid = false;
  if (anodic_timestamp_t1 != tracker_constants::INVALID_TIME && datatools::is_valid(anodic_t1_ns))
    valid = true;
  return valid;
}
...
bool tracker_hit::has_anodic_t2() const {
  bool valid = false;
  if (anodic_timestamp_t2 != tracker_constants::INVALID_TIME && datatools::is_valid(anodic_t2_ns))
    valid = true;
  return valid;
}
...
bool tracker_hit::has_anodic_t3() const {
  bool valid = false;
  if (anodic_timestamp_t3 != tracker_constants::INVALID_TIME && datatools::is_valid(anodic_t3_ns))
    valid = true;
  return valid;
}
...
bool tracker_hit::has_anodic_t4() const {
  bool valid = false;
  if (anodic_timestamp_t4 != tracker_constants::INVALID_TIME && datatools::is_valid(anodic_t4_ns))
    valid = true;
  return valid;
}
...
bool tracker_hit::has_bot_cathodic_time() const {
  ...
  if (bot_cathodic_timestamp != tracker_constants::INVALID_TIME &&
      datatools::is_valid(bot_cathodic_time_ns))
    valid = true;
  return valid;
}
...
void tracker_hit::set_bot_cathodic_timestamp(const uint64_t value_) {
  ...
  bot_cathodic_time_ns =
      bot_cathodic_timestamp * tracker_constants::tracker_clock_tick() / CLHEP::nanosecond;
  return;
}
...
bool tracker_hit::has_top_cathodic_time() const {
  ...
  if (top_cathodic_timestamp != tracker_constants::INVALID_TIME &&
      datatools::is_valid(top_cathodic_time_ns))
    valid = true;
  return valid;
}
...
void tracker_hit::set_top_cathodic_timestamp(const uint64_t value_) {
  ...
  top_cathodic_time_ns =
      top_cathodic_timestamp * tracker_constants::tracker_clock_tick() / CLHEP::nanosecond;
  return;
}
...
void tracker_hit::set_anodic_timestamps(const uint64_t t0_, const uint64_t t1_, const uint64_t t2_,
                                        const uint64_t t3_, const uint64_t t4_) {
...
void tracker_hit::set_cathodic_timestamps(const uint64_t bot_time_, const uint64_t top_time_) {
...
void tracker_hit::tree_dump(std::ostream& out_, const std::string& title_,
                            const std::string& indent_, bool inherit_) const {
  if (!title_.empty()) {
    out_ << indent_ << title_ << std::endl;
  }

  out_ << indent_ << io::tag() << "Trigger ID                : " << trigger_id << std::endl;

  out_ << indent_ << io::tag()
       << "Cell geometric ID         : " << (geomtools::geom_id)cell_geometric_id << std::endl;

  out_ << indent_ << io::tag() << "Anodic timestamp 0        : " << anodic_timestamp_t0
       << std::endl;

  out_ << indent_ << io::tag() << "Anodic time 0 (ns)        : " << anodic_t0_ns << std::endl;

  out_ << indent_ << io::tag() << "Anodic timestamp 1        : " << anodic_timestamp_t1
       << std::endl;

  out_ << indent_ << io::tag() << "Anodic time 1 (ns)        : " << anodic_t1_ns << std::endl;

  out_ << indent_ << io::tag() << "Anodic timestamp 2        : " << anodic_timestamp_t2
       << std::endl;

  out_ << indent_ << io::tag() << "Anodic time 2 (ns)        : " << anodic_t2_ns << std::endl;

  out_ << indent_ << io::tag() << "Anodic timestamp 3        : " << anodic_timestamp_t3
       << std::endl;

  out_ << indent_ << io::tag() << "Anodic time 3 (ns)        : " << anodic_t3_ns << std::endl;

  out_ << indent_ << io::tag() << "Anodic timestamp 4        : " << anodic_timestamp_t4
       << std::endl;

  out_ << indent_ << io::tag() << "Anodic time 4 (ns)        : " << anodic_t4_ns << std::endl;

  out_ << indent_ << io::tag() << "Bot cathodic timestamp    : " << bot_cathodic_timestamp
       << std::endl;

  out_ << indent_ << io::tag() << "Bot cathodic time (ns)    : " << bot_cathodic_time_ns
       << std::endl;

  out_ << indent_ << io::tag() << "Top cathodic timestamp    : " << top_cathodic_timestamp
       << std::endl;

  out_ << indent_ << io::tag() << "Top cathodic time (ns)    : " << top_cathodic_time_ns
       << std::endl;
