...
bool hit_parser::parse(std::istream& in_, calo_hit& a_calo_hit_,
  ...
  try {
    ...
    DT_LOG_DEBUG(logging,
                 "Internal hit mode type after hit header parsing = " << _hit_mode_type_ << "'");
    ...
    if (_hit_mode_type_ == base_hit::SIG_CALORIMETER) {
      ...
      DT_THROW(std::logic_error, "Unknow hit type, check your file !");
    in_ >> std::ws;
    success = true;
...
void hit_parser::_parse_hit_header_(const std::string& header_line_, const int index_) {
  DT_LOG_TRACE_ENTERING(logging);
  ...
  if (index_ == 0) {
    ...
    res = qi::phrase_parse(str_iter, end_iter,
                           //  Begin grammar
                           (qi::lit("= HIT") >> qi::ulong_long >> "=" >> (+~qi::char_("=")) >>
                            "=" >> qi::lit("TRIG_ID") >> qi::ulong_long >> "="),
                           //  End grammar
                           qi::space, _hit_id_, hit_type, _trigger_id_);
    ...
    DT_THROW_IF(hit_type != "CALO" && hit_type != "TRACKER", std::logic_error,
                "Invalid hit type label '" << hit_type << "'!");

    if (hit_type == "CALO")
      _hit_mode_type_ = base_hit::SIG_CALORIMETER;
    else if (hit_type == "TRACKER")
      _hit_mode_type_ = base_hit::SIG_TRACKER;
  }
