...
channel_mapping::channel_mapping() { reset(); }
...
bool channel_mapping::is_initialized() const { return initialized; }
  ...
  if (file_stream) {
    ...
    while (std::getline(file_stream, line)) {
      if (line_counter != 0)  // ignore the header
      {
        ...
        // Electronic ID
        geomtools::geom_id electronic_id(calo_constants::CALO_CHANNEL_TYPE, slot_number,
                                         channel_number);

        // Geometric ID
        geomtools::geom_id geometric_id(calo_constants::GEOMETRIC_CALO_TYPE, column_number,
                                        row_number);
    ...
    DT_THROW(std::runtime_error, "Cannot open input file '" << filename_ << "'");
  ...
  if (file_stream) {
    ...
    while (std::getline(file_stream, line)) {
      if (line_counter != 0)  // ignore the header
      {
        ...
        ss >> slot_number >> layer_number >> row_number >> anode_number >> bot_cathode_number >>
            top_cathode_number;
        ...
        // Geometric ID
        geomtools::geom_id cell_id_anodic(tracker_constants::GEOMETRIC_CELL_TYPE, layer_number,
                                          row_number, tracker_constants::ANODIC_PORT);

        geomtools::geom_id cell_id_bot_cathode(tracker_constants::GEOMETRIC_CELL_TYPE, layer_number,
                                               row_number, tracker_constants::BOT_CATHODE_PORT);

        geomtools::geom_id cell_id_top_cathode(tracker_constants::GEOMETRIC_CELL_TYPE, layer_number,
                                               row_number, tracker_constants::TOP_CATHODE_PORT);

        // Electronic ID
        geomtools::geom_id anode_channel_id(tracker_constants::ANODIC_CHANNEL_TYPE, slot_number,
                                            feast_anode, channel_anode);

        geomtools::geom_id bottom_cathode_channel_id(tracker_constants::CATHODIC_CHANNEL_TYPE,
                                                     slot_number, feast_bot_cathode,
                                                     channel_bot_cathode);

        geomtools::geom_id top_cathode_channel_id(tracker_constants::CATHODIC_CHANNEL_TYPE,
                                                  slot_number, feast_top_cathode,
                                                  channel_top_cathode);
...
void channel_mapping::get_cell_layer_row_for_a_channel(const geomtools::geom_id& electronic_id_,
  ...
  if (it_map != tracker_bimap.right.end()) {
    ...
    DT_THROW(std::logic_error, "The input channel with EID : "
                                   << electronic_id_ << " is not in the bimap GID <-> EID !");
  return;
}

void channel_mapping::get_electronics_id_for_a_cell_layer_row(
    const uint16_t& layer_number_, const uint16_t& row_number_,
    geomtools::geom_id& electronic_anodic_id_, geomtools::geom_id& electronic_bot_cathodic_id_,
    geomtools::geom_id& electronic_top_cathodic_id_) const {
  geomtools::geom_id geometric_anodic_cell_id(tracker_constants::GEOMETRIC_CELL_TYPE, layer_number_,
                                              row_number_, tracker_constants::ANODIC_PORT);

  get_electronic_id_from_geometric_id(geometric_anodic_cell_id, electronic_anodic_id_);

  geomtools::geom_id geometric_bot_cathodic_cell_id(tracker_constants::GEOMETRIC_CELL_TYPE,
                                                    layer_number_, row_number_,
                                                    tracker_constants::BOT_CATHODE_PORT);

  get_electronic_id_from_geometric_id(geometric_bot_cathodic_cell_id, electronic_bot_cathodic_id_);

  geomtools::geom_id geometric_top_cathodic_cell_id(tracker_constants::GEOMETRIC_CELL_TYPE,
                                                    layer_number_, row_number_,
                                                    tracker_constants::TOP_CATHODE_PORT);

  get_electronic_id_from_geometric_id(geometric_top_cathodic_cell_id, electronic_top_cathodic_id_);

  return;
}

void channel_mapping::get_electronic_id_from_geometric_id(
    const geomtools::geom_id& geometric_id_, geomtools::geom_id& electronic_id_) const {
...
void channel_mapping::get_associated_electronics_id(
    const geomtools::geom_id& input_electronic_id_, geomtools::geom_id& electronic_anodic_id_,
    geomtools::geom_id& electronic_bot_cathodic_id_,
    geomtools::geom_id& electronic_top_cathodic_id_) const {
  geomtools::geom_id geometric_cell_id;
  get_geometric_id_from_electronic_id(input_electronic_id_, geometric_cell_id);
  ...
  get_electronics_id_for_a_cell_layer_row(layer, row, electronic_anodic_id_,
                                          electronic_bot_cathodic_id_, electronic_top_cathodic_id_);
...
bool channel_mapping::is_tracker_channel_in_map(
    const geomtools::geom_id& input_electronic_id_) const {
  DT_THROW_IF(!is_initialized(), std::logic_error, "Channel mapping is not initizalied !");
  bool validity = false;
...
bool channel_mapping::is_bottom_cathodic_channel(
    const geomtools::geom_id& input_electronic_id_) const {
  DT_THROW_IF(!is_initialized(), std::logic_error, "Channel mapping is not initizalied !");
  geomtools::geom_id associated_geometric_id;

  get_geometric_id_from_electronic_id(input_electronic_id_, associated_geometric_id);
  bool is_bot_cathodic = false;
  if (associated_geometric_id.get(tracker_constants::PORT_INDEX) ==
      tracker_constants::BOT_CATHODE_PORT)
    is_bot_cathodic = true;
  return is_bot_cathodic;
}

bool channel_mapping::is_top_cathodic_channel(
    const geomtools::geom_id& input_electronic_id_) const {
  DT_THROW_IF(!is_initialized(), std::logic_error, "Channel mapping is not initizalied !");
  geomtools::geom_id associated_geometric_id;
  ...
  if (associated_geometric_id.get(tracker_constants::PORT_INDEX) ==
      tracker_constants::TOP_CATHODE_PORT)
    is_top_cathodic = true;
  return is_top_cathodic;
}

void channel_mapping::tree_dump(std::ostream& out_, const std::string& title_,
                                const std::string& indent_, bool /*inherit_*/) const {
  if (!title_.empty()) {
    out_ << indent_ << title_ << std::endl;
  }

  out_ << indent_ << io::tag() << "Initialized : " << is_initialized() << std::endl;
