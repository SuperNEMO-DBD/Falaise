...
int main(int /*argc_*/, char** /*argv_*/) {
  ...
  try {
    ...
    // std::string
    // input_filename("${FECOM_RESOURCES_DIR}/data/samples/fake_run/calo_fake_tracker_hits_1.data.bz2");
    // // std::string input_filename =
    // "${FECOM_RESOURCES_DIR}/output_test/commissioning_event_10_events.data.bz2";
    ...
    // std::clog << "Size of deserialized commissioning event data = [" <<
    // deserialize_commissioning_event_collection.get_commissioning_event_collection().size() << "]"
    // << std::endl;
    ...
    // for (std::set<fecom::commissioning_event>::iterator it_event =
    // deserialize_commissioning_event_collection.grab_commissioning_event_collection().begin();
    // 	 it_event !=
    // deserialize_commissioning_event_collection.grab_commissioning_event_collection().end();
    // 	 it_event++)
    //   {
    // 	std::clog << "DEBUG : TEST EVENT BUILDER : EVENT #" << event_counter << std::endl;
    // 	std::clog << "Size of tracker channel hit in the event : " <<
    // it_event->get_tracker_channel_hit_collection().size() << std::endl;

    // 	const_cast<fecom::commissioning_event&>(*it_event).set_channel_mapping(my_channel_mapping);

    // 	// std::size_t tchit_counter = 0;
    // 	// for (auto it_tchit = it_event->get_tracker_channel_hit_collection().begin(); it_tchit !=
    // it_event->get_tracker_channel_hit_collection().end(); it_tchit++)
    // 	//   {
    // 	//     it_tchit -> tree_dump(std::clog, "Tracker Channel hit #" +
    // std::to_string(tchit_counter));
    // 	//     tchit_counter++;
    // 	//   };

    // 	const_cast<fecom::commissioning_event&>(*it_event).build_tracker_hit_from_channels();

    // 	std::clog << "Size of tracker hit in the event : " <<
    // it_event->get_tracker_hit_collection().size() << std::endl; 	std::size_t thit_counter =
    // 0; 	for (auto it_thit = it_event->get_tracker_hit_collection().begin(); it_thit !=
    // it_event->get_tracker_hit_collection().end(); it_thit++)
    // 	  {
